...
1. **Define variables to process and push video data**
    ...
    In `Assets/Agora-RTC-Plugin/Agora-Unity-RTC-SDK/Code/JoinChannelVideo`, add the following declarations to the `JoinChannelVideo` class:
...
1. **Setup external video source**
   ...
   Call `SetExternalVideoSource` to setup an external video source in your <Vpl k= "CLIENT"/>. Add the followong method to `JoinChannelVideo` class:
   ...
   ```
   To call this method before joining a channel, in the `JoinChannelVideo` class, add the following line at the end of the `Start` method:
...
1. **Enable external video device**
   ...
   To get video frames from the external video source, get the list of available video WebCams, select the first video source from the list, set the selected source texture, play the selected source to start receiving local video frames. To implement this logic, add the following to `JoinChannelVideo` class:
...
1. **Publish custom video track**
   ...
   You use `ChannelMediaOptions` to stop publish the local video track and start publishing the custom video track. Create an instance of `ChannelMediaOptions` and set `publishVideoTrack` to `false` and `customVideoTrack` to `true`, then update the channel media options in your channel by calling `updateChannelMediaOptions`. To impelement this logic, add the following at the end of the `Join` method:
...
1. **Diplay the custom video track locally**
   ...
   To display the custom video track in th local view of your <Vpl k = "CLIENT"/>, in the `JoinChannelVideo` class, locate the `Join` method and replace `LocalView.SetForUser(0, "", VIDEO_SOURCE_TYPE.VIDEO_SOURCE_CAMERA);` with the following:
...
1. **Add the required libraries**
   ...
   In the `JoinChannelVideo` class, add the following to namespace declarations:
...
1. **Implement audio frame pushing logic**
    ...
    When a user successfully joins a channel, you setup an instance of `AudioFrame`, set its audio parameter, and push the created audio frame to the channel by calling `PushAudioFrame`. To implement this workflow, add the following method to `JoinChannelVideo` class:
    ```csharp
...
1. **Add multi-threading to audio frame pushing logic to avoid congestion**
   ...
   Call `PushAudioFrameThread` in a thread after you join a channel to avoid congestion problem in your <Vpl k= "CLIENT"/>. To implement this logic, add the following to `JoinChannelVideo` class: 
   ```csharp   
...
2. **Update channel media options and start the audio frame pushing task**
   ...
   Use the `ChannelMediaOptions` object you created in the `SetupVideoSDK` method to publish the custom audio track.  Call `StartPushAudioFrame` after you Join a channel to start pushing the local audio frames. To implement this workflow, in `JoinChannelVideo` class, add the following at the end of the `Join` method:
