...
This section shows how to use Chat <Vg k="UIK" /> to rapidly implement peer-to-peer messaging in your app.

### Initialize <Vg k="UIK" />

Follow the steps to initialize the <Vg k="UIK" />.
...
2. Initialize the <Vg k="UIK" /> by calling the `initWithAgoraChatOptions` method. In `SceneDelegate.m`, replace the `willConnectToSession` method with the following code:
