...
<ParameterList title="BODY" required={true}>
  ...
  Real-time subtitle configuration. After a user's voice is converted to text, the information is pushed to the channel as subtitles to match the UI real-time display.
    ...
    <Parameter name="subBotUid" type="string" required={false}>
      The ID of the bot that subscribes to the audio stream. **This is always identical to the value of the pubBotUid below.**
    </Parameter>
  ...
  <Tabs groupId="code-examples">
    ...
    const options = {
      ...
      body: JSON.stringify({
        ...
        name: 'agora-test',
        maxIdleTime: 50,
        rtcConfig: {channelName: 'agora-test', subBotUid: '47091', pubBotUid: '88222'},
        translateConfig: {
