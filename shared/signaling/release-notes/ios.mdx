...
- Added a `['RtmKit']` subspec version to resolve integration issues when co-integrating with the <Vg k="VSDK" />. For details, see [Handle issues when integrating the Signaling SDK and Video SDK simultaneously](/help/integration-issues/rtm2_rtc_integration_issue).
...
In IoT scenarios, devices may be restricted by Internet Service Providers (ISPs). To address this limitation, this version adds the `ispPolicyEnabled` field to [`AgoraRtmClientConfig`](/signaling/reference/api?platform=ios#configrtmpropsag_platform). When you set this field to `true`, the SDK connects only to servers whose domain names or IP addresses have been registered with the ISP and are included in the allowlist.

#### Improvements

**New reconnection timeout configuration**

This version adds the `reconnectTimeout` field to [`AgoraRtmClientConfig`](/signaling/reference/api?platform=ios#configrtmpropsag_platform). Use this field to configure the timeout period in seconds for the SDK to return a timeout error during login or reconnection. When the login or reconnection exceeds the configured duration, the SDK reports the current connection state and reason through a callback.
...
- In some scenarios, when the iOS system exited the app, the RTM SDK crashed.
...
In IoT scenarios, devices may be restricted by Internet Service Providers (ISPs). To address this limitation, this version adds the `ispPolicyEnabled` field to [`AgoraRtmClientConfig`](/signaling/reference/api?platform=macos#configrtmpropsag_platform). When you set this field to `true`, the SDK connects only to servers whose domain names or IP addresses have been registered with the ISP and are included in the allowlist.
...
- New link state change reason
    ...
    This release adds the reason <code>{enumv.linkstatereasonlogintoofrequent['swift']}</code> (37) to <code>{enumv.linkstatereason['swift']}</code>, indicating that the current login operation is too frequent.

- New renew token timeout error code

    To better reflect the result of the renew token operation, this release adds the error code <code>{code.rtmrenewtokentimeout['swift']}</code> (10026).

- Presence event notification optimization

    When channel data fails to sync properly, the SDK retriggers the <code>{enumv.presencetypesnap['swift']}</code> event. Upon receiving this event, you can update the app's local cache.
...
**Initial setup**
    ...
    - <code>{config.create['swift']}</code>: Create an instance of the Signaling client and perform the initial configuration as follows:
    ...
    - <code>{config.login['swift']}</code>: Log into the Signaling service.
    - <code>{config.logout['swift']}</code>: Log out of the Signaling service.
    - <code>{config.release['swift']}</code>: Destroy the Signaling client.
...
**Channel**
    ...
    - Message channel:
      - <code>{channel.subscribe['swift']}</code>: Subscribe to the specified message channel and receive messages and event notifications in the channel.
      - <code>{channel.unsubscribe['swift']}</code>: Unsubscribe from the specified message channel and stop receiving messages and event notifications in the channel.
    - Stream channel:
      - <code>{channel.create['swift']}</code>: Create a stream channel instance and then call relevant APIs.
      - <code>{channel.join['swift']}</code>: Join the stream channel and receive messages and event notifications in the channel.
      - <code>{channel.leave['swift']}</code>: Leave the stream channel and stop receiving messages and event notifications in the channel.
      - <code>{channel.release['swift']}</code>: Destroy the stream channel.

    By calling different methods, the SDK triggers different event notifications.

    - The <code>{channel.subscribe['swift']}</code>, <code>{channel.unsubscribe['swift']}</code>, <code>{channel.join['swift']}</code>, and <code>{channel.leave['swift']}</code> methods trigger the `onPresenceEvent` event notification. Other users in the channel receive the corresponding `Join` and `Leave` event notifications.
    - When calling <code>{channel.subscribe['swift']}</code> and <code>{channel.join['swift']}</code> to subscribe or join a channel, you can choose whether to configure` withMessage`, `withPresence`, `withMetadata`, `withLock`, and other parameters to enable or disable the monitoring of the corresponding event. To enable the monitoring, you also need to register the corresponding event listener to receive corresponding event notifications.
...
**Message**
    ...
    Call <code>{message.publish['swift']}</code> to send a message to the specified message channel. Calling <code>{message.publish['swift']}</code> triggers the `onMessageEvent` event notification. If you want to receive messages in the channel, set `withMessage` to `true` when calling <code>{channel.subscribe['swift']}</code> and register the event handler for the `onMessageEvent` event.
...
**Topic**
    ...
    - <code>{topic.join['swift']}</code>: Register as the publisher of this topic to gain the ability to send messages.
    - <code>{topic.publish['swift']}</code>: Send a message to the topic in the stream channel.
    - <code>{topic.leave['swift']}</code>: Unregister as the message publisher of this topic.
    - <code>{topic.subscribe['swift']}</code>: Subscribe to one or more message publishers of the topic in this channel.
    - <code>{topic.unsubscribe['swift']}</code>: Unsubscribe from this topic or unsubscribe from one or more message publishers in this topic.

    Register (<code>{topic.join['swift']}</code>) and unregister (<code>{topic.leave['swift']}</code>) as a message publisher trigger the `onTopicEvent` event notification that is sent to other users in the channel.

    <Admonition type="info" title="Info">Topics exist only in stream channels, not in message channels.</Admonition>
...
**Presence**
    ...
    - <code>{storage.setuser['swift']}</code>: Set the user metadata or a user metadata item for a specified user.
    - <code>{storage.getuser['swift']}</code>: Get the user metadata or a user metadata item for a specified user.
    - <code>{presence.setstate['swift']}</code>: Set the temporary status of a user in a specified channel.
    - <code>{presence.getstate['swift']}</code>: Get the temporary status of a user in a specified channel.
    - <code>{presence.removestate['swift']}</code>: Remove the temporary status of a user in a specific channel.
...
1. Channel metadata
    ...
    - <code>{storage.setchannel['swift']}</code>: Set the channel metadata or channel metadata item for a specified channel.
    - <code>{storage.getchannel['swift']}</code>: Get the channel metadata and channel metadata item for a specified channel.
    - <code>{storage.removechannel['swift']}</code>: Remove the channel metadata or channel metadata item for a specified channel.
    - <code>{storage.updatechannel['swift']}</code>: Update the existing channel metadata or channel metadata item for a specified channel.
...
1. User Metadata
    ...
    - <code>{storage.setuser['swift']}</code>: Set the user metadata or a user metadata item for a specified user.
    - <code>{storage.getuser['swift']}</code>: Get the user metadata or a user metadata item for a specified user.
    - <code>{storage.removeuser['swift']}</code>: Remove the user metadata or a user metadata item for a specified user.
    - <code>{storage.updateuser['swift']}</code>: Update the existing user metadata or a user metadata item for a specified user.
    - <code>{storage.subscribeuser['swift']}</code>: Subscribe to event notifications for changes in user metadata or a user metadata item for a specified user.
    - <code>{storage.unsubscribeuser['swift']}</code>: Unsubscribe from event notifications for changes in user metadata or a user metadata item for a specified user.
...
1. Compare And Set (CAS) control
    ...
    - Enable version number verification for the entire set of channel metadata by setting the `majorRevision` property through the <code>setMajorRevision</code> method in the <code>{storage.metadata['swift']}</code> data type.
    - Enable version number verification for a single metadata item by setting the `revision` property in the <code>{storage.metadataitem['swift']}</code> class.
...
1. Lock
    ...
    - <code>{lock.set['swift']}</code>: Set a lock for a specified channel.
    - <code>{lock.acquire['swift']}</code>: Acquire a specified lock in a specified channel.
    - <code>{lock.release['swift']}</code>: Release a specified lock in a specified channel.
    - <code>{lock.revoke['swift']}</code>: Revoke the ownership of a specific user for a lock in a specified channel to release the lock.
    - <code>{lock.get['swift']}</code>: Get the details of all locks in a specified channel.
    - <code>{lock.remove['swift']}</code>: Remove a specified lock in a specified channel.
...
1. SDK connection state change reason
    ...
    This release adds the [<code>{enumv.linkstatereason[props.ag_platform]}</code>](../reference/api#enumvlinkstatereasonpropsag_platform) enumeration class to the `LinkStateEvent` of the SDK connection state, to report the reason for the connection state change.
...
- Adds error codes: `AgoraRtmErrorInvalidChannelType`, `AgoraRtmErrorInvalidEncryptionParameter`, and `AgoraRtmErrorOperationRateExceedLimitation`. For error code descriptions and solutions, see [Error Codes](/signaling/reference/error-codes).
