...
1. Call [`createProcessor`](#createprocessor) to create a `SpatialAudioProcessor` instance.
...
To realize the spatial effect of audio files such as background sound, accompaniment, and NPC voice locally, take the following steps:

1. Call the `createProcessor` method to create a `SpatialAudioProcessor` instance.

1. To inject a plugin into the audio track of a local audio file, call the `pipe` method and specify the `processorDestination` properties.
...
1. Update the spatial position of the local audio player.
