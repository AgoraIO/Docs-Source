...
```shell
curl -L -X POST 'https://{host}/app-id/{app_id}' \
...
```shell
curl -L -X GET 'https://{host}/app-id/{app_id}/{username}' \
-H 'Authorization: Bearer {YourAppToken}' \
-H 'Content-Type: application/json'
```
...
```shell
curl -L -X GET 'https://{host}/app-id/{app_id}?pageNum=1&pageSize=10' \
-H 'Authorization: Bearer {YourAppToken}' \
-H 'Content-Type: application/json'
```
