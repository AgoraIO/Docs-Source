...
1. Double-click **BasicVideoCallScene** and click **Blueprints > Open Level Blueprint** above the editor to open the level blueprint.
    ...
    ![](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/quickstart-implementation-blueprint-1.png)
</ProductWrapper>
...
<ProductWrapper notAllowed="voice-calling">
![](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/quickstart-implementation-blueprint-2.png)
</ProductWrapper>
<ProductWrapper product="voice-calling">
![](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/voice-call-level-blueprint.png)
</ProductWrapper>
...
|:--|:-----|:-----|:------------|
| 1 | **Set Show Mouse Cursor** | Native* | (Optional) Set whether to display the mouse cursor. Check to display it. <Admonition type="info" title="Information"><ul><li>This node is available on Windows and macOS only.</li><li>If the node is not retrieved at creation time, uncheck **Context Sensitive**.</li></ul> ![](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/quickstart-implementation-blueprint-3.png) </Admonition> |
...
1. In the **Load Agora Config** function, add the **Sequence** node, and then connect **Set Token**, **Set Channel Id**, and **Set App Id** respectively. Fill in the token, channel name, and app ID values obtained from <Vg k="CONSOLE" />.
    ...
    ![](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/quickstart-implementation-blueprint-4.png)
...
1. If your target platform is Android, check whether system permissions have been granted before initializing the RTC engine. Refer to the following figure to create nodes for adding permissions to access the microphone and camera in the **CheckPermission** function.
    ...
    ![](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/quickstart-implementation-blueprint-5.png)
...
1. To initialize the RTC engine, in the **InitRtcEngine** function, create and connect nodes as shown in the following figure:
    <ProductWrapper notAllowed="voice-calling">
    ![](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/quickstart-implementation-blueprint-6.png)
    </ProductWrapper>
    <ProductWrapper product="voice-calling">
    ![](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/quickstart-implementation-blueprint-6-voice-sdk.png)
    </ProductWrapper>
  1. Create `IRtcEngine` and `IRtcEngineEventHandler`.
    ...
    1.  Bind `IRtcEngineEventHandler` class-related callback functions.
        ...
        1. Create a **Bind Event** function. In this function, add a **Sequence** node, and then bind the `onJoinChannelSuccess`, `onLeaveChannel`, `onUserJoined`, and `onUserOffline` callback events.
            ...
            ![](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/quickstart-implementation-blueprint-7.png)
...
1. Create and implement the `OnJoinChannelClicked` event callback.
    <ProductWrapper notAllowed="voice-calling">
    ![Join Channel Clicked](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/quickstart-implementation-blueprint-8.png)
    ...
    <ProductWrapper product="voice-calling">
    ![Join Channel Clicked](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/quickstart-implementation-blueprint-8-voice-sdk.png)
...
1. Create and implement the `OnLeaveChannelClicked` event callback. When the event is triggered, call **Leave Channel** to leave the channel.
    ...
    ![Leave Channel Clicked](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/quickstart-implementation-blueprint-9.png)
...
1. In the **Bind UIEvent** function, refer to the figure below to bind the `OnJoinChannelClicked` and `OnLeaveChannelClicked` callback functions to the **Join Channel** and **Leave Channel** buttons respectively. When the button is clicked, the corresponding event callback is triggered.
    <ProductWrapper notAllowed="voice-calling">
    ![Bind Join Leave Callbacks](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/quickstart-implementation-blueprint-10.png)
    </ProductWrapper>
    <ProductWrapper product="voice-calling">
    ![Bind Join Leave Callbacks](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/quickstart-implementation-blueprint-10-voice-sdk.png)
    </ProductWrapper>
...
1. Create and implement the **MakeVideoView** function to load the view when local or remote users join the channel:
    ...
    ![Implement Make Video View](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/quickstart-implementation-blueprint-11.png)
...
2. Create and implement the **ReleaseVideoView** function to release the view when a local or remote user leaves the channel.
    ...
    ![Implement Release Video View](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/quickstart-implementation-blueprint-12.png)
...
1. After the local user successfully joins the channel, the `onJoinChannelSuccess` callback is triggered and the local view is created. The `uid` is set to `0` by default, but it is assigned a random value by the SDK. The video source type is `VIDEO_SOURCE_CAMERA_PRIMARY` (first camera):
    <ProductWrapper notAllowed="voice-calling">
    ![Join Channel Success Callback](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/quickstart-implementation-blueprint-13.png)
    </ProductWrapper>
    <ProductWrapper product="voice-calling">
    ![Join Channel Success Callback](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/quickstart-implementation-blueprint-13-voice-sdk.png)
    </ProductWrapper>
2. After the local user leaves the channel, the `onLeaveChannel` callback is triggered which releases the local view:
    <ProductWrapper notAllowed="voice-calling">
    ![Leave Channel Callback](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/quickstart-implementation-blueprint-14.png)
    </ProductWrapper>
    <ProductWrapper product="voice-calling">
    ![Leave Channel Callback](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/quickstart-implementation-blueprint-14-voice-sdk.png)
    </ProductWrapper>
...
3. When a remote user joins the channel, the `onUserJoined` callback is triggeredd to create a remote view. `uid` is the uid sent from the remote end, and the video source type is `VIDEO_SOURCE_REMOTE` (remote video obtained from the network):
    <ProductWrapper notAllowed="voice-calling">
    ![Remote User Join Channel Callback](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/quickstart-implementation-blueprint-15.png)
    </ProductWrapper>
    <ProductWrapper product="voice-calling">
    ![Remote User Join Channel Callback](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/quickstart-implementation-blueprint-15-voice-sdk.png)
    </ProductWrapper>
4. When a remote user leaves the channel, the `onUserOffline` callback is triggered to release the remote view:
    <ProductWrapper notAllowed="voice-calling">
    ![Remote User Leave Channel Callback](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/quickstart-implementation-blueprint-16.png)
    </ProductWrapper>
    <ProductWrapper product="voice-calling">
    ![Remote User Leave Channel Callback](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/quickstart-implementation-blueprint-16-voice-sdk.png)
    </ProductWrapper>
...
![Leave Channel and Release Resources](https://agora-doc.s3.us-east-1.amazonaws.com/images/video-sdk/quickstart-implementation-blueprint-17.png)
