...
Starting from v2.2.0, the RTM SDK and RTC SDK (version 4.3.0 or later) both include the `libaosl.so` library. If you use both the RTM and RTC SDKs, you need to perform the following operations to avoid conflicts:
- If you manually integrate the SDK through CDN, please manually delete the lower version `libaosl.so` library;
- If you integrate the SDK using a dependency management tool (such as Maven or CocoaPods), please ensure that the higher version `libaosl.so` library is included in your project.

For more information, see [How do I handle conflicts when integrating the RTM SDK and RTC SDK?](/faq/integration-issues/rtm2-rtc-integration-issue)

- v2.2.2 RTM SDK `libaosl.so` library version: 1.0.11.
- v2.2.1 RTM SDK `libaosl.so` library version: 1.0.11.
- v2.2.0 RTM SDK `libaosl.so` library version: 1.0.0.17.

### v2.2.2

v2.2.2 was released on November XX, 2024.
