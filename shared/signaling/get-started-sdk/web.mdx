...
To create the environment necessary to add <Vg k="MESS" /> into your <Vpl k="CLIENT" />.

1. **Create a web project**

    1. Create a new directory named `Signaling_quickstart`:

    1. In `Signaling_quickstart`, create a `package.json` file:
    ...
    ``` text
    Signaling_quickstart
    ├─ index.html
    ├─ index.js
    └─ package.json
    ```

2. **Integrate <Vg k="MESS_SDK" />**

    To add <Vg k="MESS_SDK" /> for Web into your project through npm:
...
`<script src="./dist/bundle.js"></script>` refers to
the `bundle.js` file packaged by webpack. A sample webpack configuration is shown later.
...
1. Replace the contents of  `index.js` with the following code:
    ...
    To avoid browser-compatibility issues, this sample uses the import command to import  <Vg k="MESS_SDK" />  and webpack to package
    the JS file.


1. Update the authentication information:

      - Update the value of `<Your App ID>` with the App ID of your project.

      - Update the value of `<Your token>` with the authentication token you obtained.
...
-  Sample project
    ...
    Agora provides an open-source [sample project](https://github.com/AgoraIO/RTM/tree/master/Agora-RTM-Tutorial-Web) on
