...
* **Filler phrases**
    ...
    Adds a `properties.filler_words` field to insert pre-set or LLM-generated filler phrases during conversations to fill periods of silence while waiting for LLM output. This feature smooths dialogue flow, reduces user anxiety, and makes agent speech sound more natural.
...
* **Turn detection configuration optimization**
    ...
    **Start of Speech (SoS) detection**
    ...
    The `config.start_of_speech` parameter supports three detection modes:
    ...
    **End of Speech (EoS) detection**
    ...
    The `config.end_of_speech` parameter supports two detection modes:
...
- Changes to [**Start a conversational AI agent**](/conversational-ai/rest-api/agent/join)
    ...
    * **AIVAD activation**
        ...
        The `advanced_features.enable_aivad` field is now deprecated; To configure AIVAD activation use `turn_detection.config.end_of_speech.mode= "sematic"`.

    * **New parameters added**

        - `advanced_features.enable_tools`
        - `llm.mcp_servers`
        - `properties.filler_words`
        - `turn_detection.mode`
