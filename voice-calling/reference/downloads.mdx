...
- **Using a download**:
    ...
    1. Extract the files from: https://download.agora.io/sdk/release/Agora_Web_SDK_v4_13_0_FULL.zip to a local directory.
...
- **Using CocoaPods**:
    ...
    3.  Open `Podfile`, and add <Vpd k="SDK" /> pod.

        ``` bash
        // For version, fill in a specific SDK version number. For example, 4.0.0-beta.1
        pod 'AgoraRtcEngine_iOS_Beta', 'version'
        ```
    4. Replace `Your App` with the target name of your project and replace `version` with the version of the SDK that you want to integrate.
