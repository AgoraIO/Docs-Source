...
RtcEngineProxy->adjustPlaybackSignalVolume(volume);
// Set the local playback volume for a specific remote user. For example, user 123456
RtcEngineProxy->adjustUserPlaybackSignalVolume(uid, volume)
```
...
int vol = 50;
RtcEngineProxy->adjustRecordingSignalVolume(vol);
```
...
To mute or unmute a remote user, call `muteRemoteAudioStream` with the `uid` of the remote user:
    ...
    RtcEngineProxy->muteRemoteAudioStream(remoteUid, true);

    // Resume subscribing to the audio stream of the remote user
    RtcEngineProxy->muteRemoteAudioStream(remoteUid, false);
    ```
