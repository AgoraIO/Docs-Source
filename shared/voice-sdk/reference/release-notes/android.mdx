...
This release adds the `enableInstantMediaRendering` method to enable instant rendering mode for audio and video frames,
which can speed up the first video or audio frame rendering after the user joins the channel.
...
- When the playback of a local music finished, the `onAudioMixingFinished` callback was not properly triggered.
