...
### Add `pthread` library to your project
    ...
    1. Integrate the `pthread` library into your project. Open  `<extracted-folder>/agora_rtsa_sdk/example/hello_rtsa/CMakeLists.txt` and add the following line after `target_link_libraries(hello_rtsa_multi agora-rtc-sdk file_parser ${LIBS})`:
    
        ```bash
        target_link_libraries(hello_rtsa_multi -lpthread)
        ```

    1. Include the header in your code. Open `<extracted-folder>/agora_rtsa_sdk/example/hello_rtsa/hello_rtsa_multi.c` and add the following to the list of `#include` statements:
...
Pushing multiple audio and video streams to a single channel is already implemented in sample project `<extracted-folder>/agora_rtsa_sdk/example/hello_rtsa/hello_rtsa_multi.c`.
You can [test](#test-pushing-multiple-streams-to-a-single-channel) the same.

The below section presents code snippets from the sample project that you can reuse in your own <Vpl k="CLIENT"/> to integrate <Vpd k="SDK"/>.

Open the file `<extracted-folder>/agora_rtsa_sdk/example/hello_rtsa/hello_rtsa_multi.c` and examine the code. Reuse the following code blocks from this project to implement the desired functionality in your own <Vpl k="CLIENT"/>:

1. **Initialize the SDK, verify license, and set connection count**

    The following code initializes <Vg k="COMPANY"/> engine event handler, configures options, and set connections count, the number of streams you push to a single channel:
...
1. **Create multiple connections**
    ...
    When the <Vpl k="CLIENT"/> starts, you create multiple connections that you use to send audio or video data to a channel:
...
1. **Handle Agora engine events**
    ...
    <Vpd k="SDK"/> notifies you of important events, such as users joining or leaving a channel and receipt of audio or video data, through callbacks in the event handler. The following code adds some important methods to handle <Vpd k="SDK"/> events and initializes the event handler.
...
1. **Stream audio and video data**
    ...
    You create tasks (Asynchronous threads) for sending video and audio frames.
...
1.  **Declare the variables you need**
    ...
    To manage other connections and join additional channels, add the following variable declarations to the `main()` in `<extracted-folder>/agora_rtsa_sdk/example/hello_rtsa/hello_rtsa_multi.c`:
...
1. **Set channel name & token for each connection**
    ...
    Sample project is compatible to join 10 additional channels, but to test the functionality, you set channel names & tokens for 3 connections. Add below code before the loop of joining channel `agora_rtc_join_channel`:
...
3. **Join multiple channels**
    ...
    To send audio and video streams to multiple channels, join these different channels with different tokens, uesr Ids and connection Ids. Replace `agora_rtc_join_channel` api in the loop of joining channel:
