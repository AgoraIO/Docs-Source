...
This callback indicates that the local user has received the audio metadata sent by the remote user using the [`send_audio_metadata`](#send_audio_metadata) method.
...
|---------|-----------|
| `agora_local_user` | The `LocalUserObserver` class |
| `user_id` | The ID of the user who sends the audio metadata. |
