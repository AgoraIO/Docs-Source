...
Modify sent messages or custom messages on the server side. The call frequency for this action is limited to 100 times per second.

<Admonition type="info" title="Hint">
If you want to use this feature, you need to contact [support@agora.io](mailto:support@agora.io) to enable it.
</Admonition>
...
| Parameter        | Type    | Is it necessary | Describe|                    
|:---------------| :------ | :------- |:------------------|
| `user`         | String  | no | The user who modified the message.|
| `new_msg`          | JSON    | yes | Revised message.|
| `new_msg.type` | String | yes | Modified message types: <ul><li>`txt`: Text message</li><li>`custom`: Custom message</li></ul> |
| `new_msg.msg`      | String  | yes | Modified message content. **This field only takes effect for text messages**.|
| `new_msg.customEvent` | String | no| User-defined event type. The value of this parameter must satisfy the regular expression `[a-zA-Z0-9-_/\.]{1,32}` and be `1-32` characters in length. **This field only takes effect for custom messages**. |
| `new_msg.customExts` | JSON   | no | User-defined event attributes must be of type` Map<String,String>` and can contain up to 16 elements. **This field only takes effect for custom messages**.|
| `new_ext`        | JSON    | no  | Modified message extension information. |
| `is_combine_ext`   | Boolean | no  | Whether the modified message extension information is merged or replaced with the original extension information.  <ul><li>(default) `true`: merge;</li>  <li>`false`: replace.</li></ul>|
...
|------------------|---------|--------------------------------------------------------------------------------------------------------------|
| `path`             | String  | The request path is part of the request URL and developers do not need to pay attention.                     |
...
| `application`      | String  | The unique identifier of the application within the system. This identifier is generated by the system and developers do not need to care. |
| `action`           | String  | Request method.                                                                                              |
| `data`             | String  | A value of `success`, indicates that the message was successfully modified.                                    |
| `duration`         | Int     | The length of time from sending the HTTP request to the response, in milliseconds.                           |
| `applicationName`  | String  | The application name you filled in when creating the application in the HuanXin Instant Messaging Cloud Console is `app_name` the same as the request parameter. |

If the returned HTTP status code is non-positive `200`, it means that the request failed. Common exception types are as shown in the following table.

| Exception type                       | HTTP status code | Error message                                                        | Wrong description                                                                                    |
|--------------------------------------|------------------|----------------------------------------------------------------------|------------------------------------------------------------------------------------------------------|
| `UnsupportedMessageTypeException`     | 400              | The message is of a type that is currently not supported for modification. | The modified message type is not supported. Currently only modified text messages and customized messages are supported. |
| `InvalidMessageIdException`            | 400              | The provided message ID is not a valid number.                     | Message ID must be numeric.                                                                          |
| `RewriteMessageNotAuthorizedException`| 401              | You are not authorized to edit this message.                       | The modified message ID does not belong to the current app.                                           |
| `EditLimitExceededException`          | 403              | The message has reached its edit limit and cannot be modified further. | The current number of message modifications has reached the upper limit. Currently, each message can be modified up to 10 times. |
| `EditFeatureNotEnabledException`       | 403              | The edit message feature is not enabled for this user or system.    | The message modification function is not enabled. Before using this function, you need to contact Huanxin Business to activate it. |
| `MessageUnavailableException`          | 404              | The message is unavailable or has expired.                          | The modified message does not exist or has expired.                                                  |
| `RewriteMessageInternalErrorException`| 500              | An unknown error occurred while processing the request.             | Internal service exception, message modification failed.                                             |
...
- Modify the text message sent successfully:
...
- Modify the custom message sent successfully:
    
    ```shell
    # Replace <YourAppToken> with the App Token generated on the server side
