...
This is the first release of the Signaling SDK for C++ 2.x, which brings innovation in functional coverage, performance improvement, and experience optimization.
...
- `Future<(RtmStatus, RtmClient)> RTM(...)`: Create an instance of the Signaling client, and config as follows:
  ...
  - `useStringUserId`: Set the data type of the user ID, which can be of type `String` or type `Int`.
