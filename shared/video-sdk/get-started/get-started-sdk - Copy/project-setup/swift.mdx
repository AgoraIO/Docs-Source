...
- **Lite SDK**: Provides basic audio and video capabilities. For the list of supported APIs, see the [API Reference](https://api-ref.agora.io/en/lite-sdk/ios/4.x/documentation/agorartckit). 
</PlatformWrapper>
...
<Tabs>
  <TabItem value="tab1" label="Swift Package Manager" default>
    ...
    1. In the search bar, paste the package URL:

        <PlatformWrapper platform='ios'>

        | SDK |  |
        |-----|--|
        | Full SDK | `https://github.com/AgoraIO/AgoraRtcEngine_iOS.git` |
        | Voice SDK | `https://github.com/AgoraIO/AgoraAudio_iOS.git` |
        | Lite SDK | `https://github.com/AgoraIO/AgoraLite_iOS.git` |
  ...
  <TabItem value="tab2" label="CocoaPods">
    ...
    1. Open `Podfile` and modify the content as follows. Replace `Your App` with your target name and the pod name with the appropriate package.
        ...
        If your use case requires the Voice SDK or the Lite SDK, use the following table to update the dependency:

        | SDK | Pod name |
        |-----|----------|
        | Full SDK | `'AgoraRtcEngine_iOS', 'x.y.z'` |
        | Voice SDK | `'AgoraAudio_iOS', 'x.y.z'` |
        | Lite SDK | `'AgoraLite_iOS', 'x.y.z'` |
