...
When a user opens the app, you instantiate and initialize a `ChatClient`. When the user taps a button, the app logs in or out of the chat server. When a user presses Send, the typed message is sent to the chat server. When the <Vpl k="CLIENT"/> receives a message from the chat server, the message is displayed in the message list. This simple workflow enables you to rapidly build a functional chat client. 
