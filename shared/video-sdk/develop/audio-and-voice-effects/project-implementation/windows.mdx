...
1. **Enable the user to start and stop audio mixing**
    ...
    When the user presses **Mix Audio**, the <Vg k = "VSDK"/> fetches an audio file using `audioFilePath` and mixed it with the local audio stream. The remote and local users listen the audio file playing with the audio stream. To enable audio mixing in your <Vpl k = "CLIENT"/>, in **Dialog Editor**, double-click **Mix Audio**. **Dialog Editor** automatically creates and opens an event listener for you. Add the following code to the event listener you just created:
