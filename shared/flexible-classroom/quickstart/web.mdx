...
The source code for Flexible Classroom is located in the GitHub [CloudClass-Desktop](https://github.com/AgoraIO-Community/flexible-classroom-desktop.git) repository. To download the source code, take the following steps:
...
1. Agora recommends switching the branch to the latest release. To switch the branch version, run the following command. Remember to replace `{VERSION}` with a specific version number:
   ...
   For example, if you want to switch to the branch for version `2.8.0`, run the following command:
