...
- In the `preload.js` file, replace `<-- Insert App Id -->`, `<-- Insert Token -->`, and `<-- Insert Channel Name -->` with your App ID, the temporary token from the console, and the channel name you used to generate the token.
