...
1. **Update the network status indication in the UI**
    ...
    To show the network quality result visually to the user, in `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelAudio.cs`, add the following to the `JoinChannelAudio` class:
...
1. **Enable the user to test the network**
    ...
    In `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelAudio.cs`, add the following method to the `JoinChannelAudio` class:
...
1. **Implement best practice for app initiation**
    ...
    * _Start the network probe test_: A quick test at startup to gauge network quality.
    ...
    To implement these features, in `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelAudio.cs`, add the following code to `setupVoiceSDKEngine` after `RtcEngine.SetLogFileSize(2048);`:
...
1. **Listen to <Vg k="ENGINE" /> events to receive state change notifications and quality statistics**
    ...
    * `OnRtcStats`: Receives the <Vg k="ENGINE" /> stats.
    ...
    In `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelAudio.cs`, add the following callback methods to the `UserEventHandler` class:
