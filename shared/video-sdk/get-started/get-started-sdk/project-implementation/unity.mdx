...
1. Add the join and leave buttons
    ...
    4. Use the same procedure to create a button called **Join** with a **Text** sub-item where its **Text** says "Leave".
...
2. Import the <Vpl k="NAME" /> libraries
    ...
    1. Click `JoinChannelVideo` on the **Canvas** and `Open` in the **Inspector** to open the script in your default text editor.
...
4. Bind your script to the **Canvas**
    ...
    In `Assets/Agora-RTC-Plugin/Agora-Unity-RTC-SDK/Code`, drag and drop `JoinChannelVideo.cs` on **Canvas** to the **Hierarchy**.

    Click **Inspector**, you see that `JoinChannelVideo.cs` is added to **Canvas**.
