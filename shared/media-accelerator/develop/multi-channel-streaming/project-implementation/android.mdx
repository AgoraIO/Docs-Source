...
1. **Set joined status**
    ...
    When your call to `agoraRtcService.joinChannel` succeeds, you receive a notification through the `onJoinChannelSuccess` callback. You use connection Id to identify the connection and set the corresponding `isJoined` variable to true. To do this, **replace** the `onJoinChannelSuccess` method under `agoraRtcEvents` with the following: 
...
1. **Stream video over the first connection**
    ...
    In this example, you start a video stream over the first connection and a second video stream over the second connection to the same channel. To start or stop the first stream when a user taps the first button, add the following method to the `MainActivity` class:
...
1. **Stream video over the second connection**
    ...
    To push another video stream to the same channel when a user taps the second button, add the following method to the `MainActivity` class:
    ...
    public void stream2(View view) {
        ...
        videoThread2 = new VideoSendThread(getApplicationContext(), agoraRtcService,
...
1. **Leave all connections to a channel**
    ...
    To leave a channel on the second connection when a user taps **Leave**, add the following lines to `leaveChannel(View view)` after `int ret = agoraRtcService.leaveChannel(connectionId);`:
