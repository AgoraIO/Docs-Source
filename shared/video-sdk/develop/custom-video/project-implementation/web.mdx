...
function createAndDrawCanvas() {
    ...
    let textPosition = -200; // Initialize text position
    ...
    function drawCanvas() {
        ...
        textPosition += 1; 
        if (textPosition > canvas.width) {
            textPosition = -200; // Reset position when it moves off-screen
        }

        context.fillStyle = "white";
        context.font = "30px Arial";
        context.fillText("Agora Custom Track", textPosition, 100);

        requestAnimationFrame(drawCanvas); // Keep refreshing the canvas
    }
    drawCanvas();
