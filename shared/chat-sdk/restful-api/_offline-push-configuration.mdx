...
For each App ID, the total call frequency limit of this method and the method to set the display style is 100 per second.
...
```bash
curl -X PUT -H 'Content-Type: application/json' -H 'Accept: application/json' -H 'Authorization: Bearer <YourAppToken>' -d '{ "nickname": "testuser" }' 'http://XXXX/app-id/XXXX/users/user1'
```
...
For each App ID, the total call frequency limit of this method and the method to set the display name is 100 per second.
...
```bash
curl -X PUT -H "Authorization: Bearer <YourAppToken>" -i https://XXXX/app-id/XXXX/users/a -d '{"notification_display_style": "1"}'
```
