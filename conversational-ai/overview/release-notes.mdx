...
* **MCP integration**
    ...
    Adds a `llm.mcp_servers` field to the [Start a conversational AI agent](/conversational-ai/rest-api/agent/join) API to connect the LLM to an MCP (Model Context Protocol) server. Set `advanced_features.enable_tools` to `true`to enable tool calls. This allows the agent to call tools provided by external services to extend functionality.

* **Filler phrases**

    Adds a `llm.filler_words` field to insert pre-set or LLM-generated filler phrases during conversations to fill periods of silence while waiting for LLM output. This feature smooths dialogue flow, reduces user anxiety, and makes agent speech sound more natural.
...
* **Turn detection configuration optimization**
    ...
    Turn detection configuration now gives you more control over when the agent starts and stops listening. Configure Start of Speech (SoS) and End of Speech (EoS) detection separately with multiple modes (VAD, keyword, semantic) and adjustable thresholds. This helps you create more natural conversations and reduce unintended interruptions based on your specific use case.
...
- Changes to [**Start a conversational AI agent**](/conversational-ai/rest-api/agent/join)
    ...
    * **`turn_detection` has a revamped structure**

        All previous fields under `turn_detection` have been deprecated and replaced with a new data structure. 

    * **AIVAD activation**

        The `advanced_features.enable_aivad` field is now obsolete; To configure AIVAD activation use `turn_detection.config.start_of_speech.mode`.
...
{/*
* **Agent metrics enhancement**

    Notification event type [`111 agent_metrics`](/conversational-ai/develop/event-types#111-agent-metrics) payload adds three new fields to help you monitor and optimize the agent response performance: 

    - `aivad_delay`: Graceful interruption (AIVAD) module latency in milliseconds
    - `bhvs_delay`: Background voice suppression (BHVS) module latency in milliseconds  
    - `total_delay`: Total end-to-end latency in milliseconds
*/}
