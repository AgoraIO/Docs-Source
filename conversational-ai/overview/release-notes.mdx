...
* **Control LLM response interruption in custom LLM scenarios**
    ...
    You can now use the `metadata.interruptable` field in the first chunk of the LLM response to control whether the agent's TTS speech can be interrupted by user speech.

* **RTC media content encryption**

    The [Start a conversational AI agent](/conversational-ai/rest-api/agent/join) API now includes an `rtc` parameter for configuring RTC encryption.
...
* **Stop agent API optimization**

    The [Stop a conversational AI agent](/conversational-ai/rest-api/agent/leave) API now returns asynchronously, responding immediately after request parameters are validated.

* **Eleven Labs TTS Compatibility Optimization**

    Supports SSML parsing and forwarding to ensure Eleven Labs TTS correctly renders intended speech effects such as pauses, emphasis, and pronunciation.

* **Background Voice Suppression Enhancement**

    Updates the AINS component within the 3A algorithm (AEC, AGC, AINS) to significantly improve suppression of background human voices.
...
* **New TTS providers**
    ...
    - [PlayHT (Beta)](/conversational-ai/models/tts/playht)
