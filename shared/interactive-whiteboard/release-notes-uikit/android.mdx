...
* **Register third-party window apps**
  ...
  This version adds the [`registerApp`](../reference/uikit-sdk#registerapp) method to the `FastRoom` class, allowing you to register third-party window apps in the whiteboard room. This method enables the registration of external applications that can run as independent windows on the whiteboard, and supports two registration methods:

  - **Local script registration**: Uses local JavaScript code (recommended for its high reliability)
  - **Remote package registration**: Uses a remote URL (may fail due to network issues)

  Additionally, the [`FastRegisterAppParams`](../reference/uikit-sdk#fastregisterappparams) class is introduced to configure registration parameters, including options such as app name, script code, and URL.
...
- **Set stroke thickness range**
  ...
  This version introduces the [`setStrokeRange`](../reference/uikit-sdk#setstrokerange) method in the `FastUiSettings` class, which allows you to set the range of the line width slider in the toolbox. This method configures the minimum and maximum values of the line width slider in the toolbox extension panel. Users can adjust the line width within this range when using drawing tools like pencil, rectangle, circle, etc. The default range is 1 to 24.

#### Bug fixes

- The [`setToolboxEdgeMargin`](../reference/uikit-sdk#settoolboxedgemargin) method did not work when Toolbox Gravity was not specified.
- UI updates did not take effect on the first call of [`setResource`](../reference/uikit-sdk#setresource).
- The brush color was not in the custom color list when first entering the room after setting it with [`setToolsColors`](../reference/uikit-sdk#settoolscolors), which was not expected.

#### API changes

- Added

  - [`registerApp`](../reference/uikit-sdk#registerapp)
  - [`FastRegisterAppParams`](../reference/uikit-sdk#fastregisterappparams)
  - [`setStrokeRange`](../reference/uikit-sdk#setstrokerange)


## v1.6.2

Released on January XX, 2025.

#### New features

* **Manual loading of whiteboard view**

  This version adds the [`preloadWhiteboardView`](../reference/uikit-sdk#preloadwhiteboardview) method, which is used to manually perform a whiteboard view preload to improve the speed of joining the room next time. Before using this feature, ensure that the `autoPreload` in [`FastboardConfig`](../reference/uikit-sdk?platform=android#fastboardconfig) is set to `false`.
...
- [`preloadWhiteboardView`](../reference/uikit-sdk#preloadwhiteboardview)
- [`FastboardConfig`](../reference/uikit-sdk?platform=android#fastboardconfig) adds `autoPreload`
- [`insertStaticDoc`](../reference/uikit-sdk?platform=android#insertstaticdoc)
- [`insertPptx`](../reference/uikit-sdk?platform=android#insertpptx)
- [`DocPage`](../reference/uikit-sdk?platform=android#docpage)
- [`setConfig`](../reference/uikit-sdk?platform=android#setconfig)
- [`FastboardConfig`](../reference/uikit-sdk?platform=android#fastboardconfig)
- [`FastAppliance`](../reference/uikit-sdk?platform=android#fastappliance) adds:
  - `PENCIL_ERASER`
  - `HAND`
- [`FastLogger`](../reference/uikit-sdk?platform=android#fastlogger-class)
  - `setLogger`
  - `Logger`
  - `warn`
  - `debug`
...
v1.6.0 was released on February xx, 2024.
