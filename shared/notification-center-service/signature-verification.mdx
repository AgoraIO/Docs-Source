To communicate securely between <Vg k="NCS_LONG" /> and your webhook,  <Vg k="AGORA_BACKEND" />  uses signatures for identity verification.

- When you configure <Vg k="NCS_LONG" /> in <Vg k="CONSOLE" />,  <Vg k="AGORA_BACKEND" />  generates the secret you use for verification.
- When sending a notification,  <Vg k="NCS_LONG" /> generates two signature values from the secret using `HMAC/SHA1` and `HMAC/SHA256` algorithms. These signatures are added as `Agora-Signature` and `Agora-Signature-V2` to the `HTTPS` request header.
...
2. Add the following code to `Handlers.java` after `String query = br.lines()...;` to verify the data in the <Vg k="NCS_LONG" /> callback:
...
2. To verify data in the <Vg k="NCS_LONG" /> callback, add the following code to your <Vg k="NCS_LONG" /> notification handler:
