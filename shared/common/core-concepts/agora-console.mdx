...
To use <Vg k="COMPANY" /> <Vpd k="NAME" />, create a project in the <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link> first.
</ProductWrapper>

<ProductWrapper notAllowed="interactive-whiteboard">
![Create project in Agora Console](https://agora-doc.s3.us-east-1.amazonaws.com/images/common/create-project.svg)
</ProductWrapper>

<Vg k="CONSOLE" /> provides an intuitive interface for developers to query and manage their <Vg k="COMPANY" /> account. After registering an Agora account, you use the <Vg k="CONSOLE" /> to perform the following tasks:
