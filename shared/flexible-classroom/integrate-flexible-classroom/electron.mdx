...
4. Import the Agora Classroom SDK in the HTML file and launch a classroom.
    ...
    <head>
        ...
        <!-- Introduce the SDK, 2.9.10 is the sample version number, you can check the latest version number through the release notes or GitHub warehouse branch -->
        <script src="https://download.agora.io/edu-apaas/release/edu_sdk@2.9.10.bundle.js"></script>
        <!-- Introducing Widget, 2.9.10 is the sample version number. You can check the latest version number through the release notes or GitHub warehouse branch. (Widgets before 2.9.10 are packaged together with the SDK, and there is no need to introduce this library) -->
        <script src="https://download.agora.io/edu-apaas/release/edu_widget@2.9.10.bundle.js"></script>
    </head>
    <body>
        <style>
            #root {
    ...
    The sample code requires passing in `rtmToken`. You can refer to [Generate a <Vg k="MESS" /> token](../reference/manage-agora-account#generate-an-rtm-token) to learn what is a <Vg k="MESS" /> token, how to get a temporary <Vg k="MESS" /> token for testing purpose, and how to generate a <Vg k="MESS" /> token from the server. The generated token passed in `userId` must be consistent with the `userUuid` parameters passed in the `launch` method; otherwise, the generated token will be invalid.
...
1. Switch to the latest branch.
    ...
    For example, if you want to switch to the branch of v2.9.10, just run the following command:

    ```bash
    git checkout release/2.9.10
    ```
