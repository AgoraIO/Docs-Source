...
**Initial setup**
    ...
    - <code>{config.create[props.ag_platform]}</code>: Create an instance of the Signaling client and perform the initial configuration as follows:
...
**Channel**
    ...
    - The <code>{channel.subscribe[props.ag_platform]}</code>, <code>{channel.unsubscribe[props.ag_platform]}</code>, <code>{channel.join[props.ag_platform]}</code>, and <code>{channel.leave[props.ag_platform]}</code> methods trigger the `onPresenceEvent` event notification. Other users in the channel receive the corresponding `Join` and `Leave` event notifications.
...
**Message**
    ...
    Call <code>{message.publish[props.ag_platform]}</code> to send a message to the specified message channel. Calling <code>{message.publish[props.ag_platform]}</code> triggers the `onMessageEvent` event notification. If you want to receive messages in the channel, set `withMessage` to `true` when calling <code>{channel.subscribe[props.ag_platform]}</code> and register the event handler for the `onMessageEvent` event.
...
**Topic**
    ...
    Register (<code>{topic.join[props.ag_platform]}</code>) and unregister (<code>{topic.leave[props.ag_platform]}</code>) as a message publisher trigger the `onTopicEvent` event notification that is sent to other users in the channel.
