...
1. **Update the UI after the effect finishes playing**
    ...
    When <Vpd k="SDK" /> has finished playing the sound effect, the `onAudioEffectFinished` even is fired. To handle this event, in the `MainActivity` class, add the following after `private final IRtcEngineEventHandler mRtcEventHandler = new IRtcEngineEventHandler() {`
