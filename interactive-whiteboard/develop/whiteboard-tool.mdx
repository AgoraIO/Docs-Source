...
After successfully joining an Agora Interactive Whiteboard room, you can use the default tool to write and draw on the whiteboard. The Agora Interactive Whiteboard SDK provides rich basic editing tools, such as pencil, arrow, straight line, eraser, and various shapes. In addition, the whiteboard SDK supports modifying the font size, stroke width, and stroke color.

<PlatformWrapper notAllowed="web">
This doc is for Web only.
</PlatformWrapper>
...
- `currentApplianceName`: The name of the whiteboard tool currently in use, which can be set as one of the following values:
  ...
  - `pencil`: Pencil
  - `rectangle`: Rectangle
  - `selector`: Selector. If you set the `floatBar` property in [`JoinRoomParams`](https://docs.agora.io/en/whiteboard/API%20Reference/whiteboard_web/globals.html#joinroomparams) as `true`, when the user uses the `selector` tool, a floating bar that provides more text editing options displays.
...
<div class="alert note">The presentation of these tools is related to the interactive design and visual style of a specific web application. Considering this, the Interactive Whiteboard SDK does not provide the user interface (UI) of these tools. You need to implement the UI yourself.</div>

## Prerequisites

To follow the procedure on this page, ensure that you have integrated the Agora Interactive Whiteboard SDK into your project and implemented joining a room. For details, see [Join a Whiteboard Room](/interactive-whiteboard/get-started/get-started-sdk).

## Set the whiteboard tool in the code

You can directly set the whiteboard tool used in the current room in the program. This section uses the [Join a Whiteboard Room](/interactive-whiteboard/get-started/get-started-sdk#join-the--room-from-your-app-client) sample code as an example.
...
This section extends the [Join a Whiteboard Room](/interactive-whiteboard/get-started/get-started-sdk#join-the--room-from-your-app-client) sample code to show the implementation of a simple toolbar.
...
Save the changes, and refresh the `index.html` page. The following toolbar displays on the lower left corner of the page. You can click any button and use the corresponding tool to write and draw on the whiteboard.
...
In addition to the basic editing tools listed in the <a href="#tech">Understand the tech</a> section, the Agora Interactive Whiteboard SDK provides additional editing functions through the following methods:

| Methods                                                      | Description                                                  |
| :----------------------------------------------------------- | :----------------------------------------------------------- |
| [copy](https://docs.agora.io/en/whiteboard/API%20Reference/whiteboard_web/interfaces/room.html#copy) | Copies the selected content.                                 |
| [paste](https://docs.agora.io/en/whiteboard/API%20Reference/whiteboard_web/interfaces/room.html#paste) | Pastes the copied content.                                   |
| [duplicate](https://docs.agora.io/en/whiteboard/API%20Reference/whiteboard_web/interfaces/room.html#duplicate) | Duplicates the selected content.                             |
| [delete](https://docs.agora.io/en/whiteboard/API%20Reference/whiteboard_web/interfaces/room.html#delete) | Deletes the selected content.                                |
| [redo](https://docs.agora.io/en/whiteboard/API%20Reference/whiteboard_web/interfaces/room.html#redo) | Redoes an undone action.                                     |
| [undo](https://docs.agora.io/en/whiteboard/API%20Reference/whiteboard_web/interfaces/room.html#undo) | Undoes an action.                                            |
| [disableEraseImage](https://docs.agora.io/en/whiteboard/API%20Reference/whiteboard_web/interfaces/room.html#disableeraseimage) | Disables the eraser from erasing images on the whiteboard.   |
| [disableDeviceInputs](https://docs.agora.io/en/whiteboard/API%20Reference/whiteboard_web/interfaces/room.html#disabledeviceinputs) | Disables the whiteboard from responding to users' operations. |
