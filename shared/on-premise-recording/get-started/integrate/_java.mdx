...
1. [Download](/en/Recording/downloads?platform=All%20Platforms) the Agora On-premise Recording SDK for Linux package. The package structure is listed as follows:

   | Folder | Description |
   | --- | --- |
   | bin | The directory where AgoraCoreService is located. |
   | include | <li>base: Required header files for developing the recording application.</li>
...
5. If your firewall restricts access to the Agora services, use [cloud proxy service](https://docs.agora.io/en/Recording/cloudproxy_recording?platform=Linux).
...
Now you can use the demo to start recording by command line, see [Record a Call](./recording_cmd_java?platform=Java). If you want to call APIs, you need to import the SDK files to your project.
...
The Agora On-premise Recording SDK is integrated. You can start recording by calling APIs, see [Agora Recording Java API Reference](./API%20Reference/recording_java/index.html).
