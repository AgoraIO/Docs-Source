...
Starting from v2.2.1, Signaling `2.x` supports User Channels for peer-to-peer messaging. Use the `publish()` method with `channelType` set to `USER` to send messages to a specific user. Handle incoming messages in the `message` event handler. For more information, see [User channels](/signaling/core-functionality/user-channel).
