...
1. **Query task status**
    ...
    document.getElementById("realTimeTranscriptionQuery").onclick = async function ()
    {
        ...
        .then(res => {
            ...
            console.log("real-time transcription status: " + status + "");
            document.getElementById("realTimeTranscription").innerHTML = "Start real-time transcription-to-text";
            }
        })
        .catch(error => {
            console.error(error);
...
1. **Stop the task**
    ...
    function stopRealTimeTranscription()
    {
      ...
      .catch(error => {
        console.error("Error while stopping the real-time transcription-to-text task: " + error)
      });  
    }
    ```
...
1. **Receive and display the text stream**
    ...
    When you set one of the `destinations` in the starting configuration as `AgoraRTCDataStream`, the real-time transcription-to-text conversion output stream is pushed to your channel.
