...
The functional components are located in the `packages/agora-classroom-sdk/src/ui-kit` (Agora Classroom Education Scenario) and `packages/agora-proctor-sdk/src/ui-kit` (Agora Classroom Proctor Scenario) directories. The technologies used are `react`, `ts`, and `storybook`. Each functional component folder contains the following files:
