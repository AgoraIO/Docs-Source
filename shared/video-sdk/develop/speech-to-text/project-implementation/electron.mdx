...
1. **Query task status**
    ...
    To notify the user of any change to the speech-to-text task you [Query the task status](#query-the-status-of-speech-to-text) using a `GET` request containing the `taskId` and `builderToken`.
