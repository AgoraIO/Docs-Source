...
| :------------ | :----- | :----------------------------------------------- |
| `combineLevel` | Int | The number of nesting levels to merge messages. |
| `file_length` | Int | The size of the combined message attachments, in bytes. |
| `filename` | String | The name of the attachment to the merged message. |
| `msg` | String | The compatible text of the merged message. When an SDK that supports merged messages sends a message to an earlier SDK that does not support merged messages, the earlier SDK will parse this attribute as the message content of the text message. |
| `secret` | String | The access key for merging message attachments. This field is present if file access restrictions are set when uploading a file. |
| `subType` | String | Message type. Combined messages are `sub_combine`. |
| `summary` | String | A summary of the merged message. |
| `title` | String | The title of the merged message. |
| `url` | String | The URL address of the attachment to be merged into the message. You can visit this URL to download the attachment. |
| `ext` | String | Extended information for the merged message. |
| `from` | String | The user ID of the sender of the merged message. |
