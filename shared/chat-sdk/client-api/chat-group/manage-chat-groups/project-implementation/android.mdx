...
- Users can call `getAllGroups` method to load the local group list. To ensure the accuracy of the data, it is necessary to first obtain the list of groups they have joined and created from the server. 

```java
List<ChatGroup> grouplist = ChatClient.getInstance().groupManager().getAllGroups();
```
- Users can also paginate to obtain the public group list:
...
From version 4.2.1 onwards, users have the ability to determine the number of groups they have joined by utilizing the `asyncGetJoinedGroupsCountFromServer` method provided by the `ChatGroupManager`. This functionality enables users to retrieve this count directly from the server. The limit on the number of groups a user can join is determined by their subscribed instant messaging package. For more details, please refer to the [product pricing](/agora-chat/reference/pricing-plan-details#group).

```java
ChatClient.getInstance().groupManager().asyncGetJoinedGroupsCountFromServer(new ChatValueCallBack<Integer>() {
    @Override
