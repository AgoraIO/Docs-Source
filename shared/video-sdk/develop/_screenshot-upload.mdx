...
Agora <Vg k="VSDK" /> supports taking screenshots of the video stream in a channel and uploading images to a third-party cloud storage you specify.

## Understand the tech

When you call the API to enable the screenshot upload function, <Vg k="VSDK" /> takes screenshots of videos sent by local users and uploads images to a third-party cloud storage according to the frequency you set. After taking a screenshot, the Agora cloud server sends a callback notification to your server in the form of an `HTTPS` request, and uploads the screenshot to the third-party cloud storage you specify.
...
Before implementing the video screenshot upload function, ensure that you have:

* Integrated <Vg k="VSDK" /> and the video screenshot upload dynamic library in your project to join a channel and publish a video stream. For details, see the [SDK quickstart](../get-started/get-started-sdk/).

* Configured a third-party cloud storage account for storing screenshots.

* Implemented a webhook to receive callback notifications from the Agora cloud server.

* You have submitted a request to support@agora.io for enabling and configuring the <Vg k="COMPANY" /> video screenshot upload service.
