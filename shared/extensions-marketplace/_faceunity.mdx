...
To implement <Vg k="RTEE_FU" /> features in your <Vpl k="CLIENT" />, open the <Link to="../../../video-calling/get-started/get-started-sdk"><Vg k="GET_STARTED"/> for <Vg k="VIDEO"/></Link> project you created previously.
...
1. [Contact <Vg k="COMPANY" />](mailto:support@agora.io) for activation and provide your package name. For example,
  `yourCompany.yourProject.faceunity`. You receive the `authpack` certificate file from <Vg k="COMPANY" /> that corresponds to your package name for subsequent integration.
