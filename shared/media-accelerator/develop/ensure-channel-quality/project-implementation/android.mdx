...
1. **Define variables to access checkboxes**
...
1. **Manage audio and video streaming status**
    ...
    checkMuteLocalAudio.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
        ...
        public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
            if (isJoined) {
                agoraRtcService.muteLocalAudio(connectionId, checkMuteLocalAudio.isChecked());
            }
        }
    });
    ...
    checkMuteLocalVideo.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
        ...
        public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
            if (isJoined) {
                agoraRtcService.muteLocalVideo(connectionId, checkMuteLocalVideo.isChecked());
            }
        }
    });
    ```
    To pause and resume playing remote audio or video streams call `agoraRtcService.muteRemoteAudio(connectionId, remoteUid, isMuted)` or `agoraRtcService.muteRemoteVideo(connectionId, remoteUid, isMuted)`.
