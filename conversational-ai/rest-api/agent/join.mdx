...
<ParameterList title="BODY" required={true}>
  ...
  <Parameter name="pipeline_id" type="string" required={false}>
    The unique ID of a published agent in [<Vpd k="STUDIO" />](/conversational-ai/studio/overview). When provided, the agent uses the pipeline's saved configuration as its base. Any fields specified in `properties` override the corresponding pipeline settings. When you specify a pipeline ID, the `asr`, `tts`, and `llm` fields in `properties` are optional.
  </Parameter>
