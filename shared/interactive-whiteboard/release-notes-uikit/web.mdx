...
* **Manage window apps in main view**

  The [`FastboardOptions`](../reference/uikit-sdk#fastboardoptions) now includes a configuration option `enableAppInMainViewPlugin` to enable the [`@netless/app-in-mainview-plugin`](https://www.npmjs.com/package/@netless/app-in-mainview-plugin) plugin. This supports managing window apps in the main view of the whiteboard. The plugin is based on the multi-window mode of [`@netless/window-manager`](https://www.npmjs.com/package/@netless/window-manager), integrating app windows (such as courseware and plugins) into the main window. App windows can hide or display as the main whiteboard pages switch. More details can be found in [`appInMainView`](https://github.com/netless-io/appInMainView).

* **Laser pen**

  With the `appliance-plugin` enabled, the Fastboard UI introduces a new mark pen tool called `markPen`.
...
v0.3.10 was released on February xx, 2024.
