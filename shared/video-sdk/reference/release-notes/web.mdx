...
Chrome plans to deprecate and remove the Plan B dialect in WebRTC. For details, see the [Chrome feature description](https://www.chromestatus.com/feature/5823036655665152). This change affects the following Agora <Vpd k="NAME"/> versions:
...
<td><p>v2.5.0-2.9.0</p>
<p>v3.0.0-3.6.1</p></td>
<td><p>After a stream is published, the method call of <code>switchDevice</code> causes video freezes or no sound issues.</p></td>
</tr>
<tr>
<td><p>v4.0.0-4.0.1</p></td>
<td><p>The <code>publish</code> and <code>subscribe</code> methods do not take effect.</p></td>
</tr>
</tbody>
</table>
</ProductWrapper>
...
**Default mode of Cloud Proxy**

As of v4.15.0, the `mode` parameter in `startProxyServer` defaults to `3`, which represents the Force UDP Cloud Proxy mode. In this mode, the SDK always transmits data over UDP.

#### New features

**Audio stream filter (Beta)**

v4.15.0 introduces filtering audio streams based on volume. Once this function is enabled, the Agora server ranks all subscribed audio streams by volume and transports the three audio streams with the highest volumes to the receivers by default. The number of audio streams to be transported can be adjusted; contact [support@agora.io](mailto:support@agora.io) to adjust this number according to your scenarios.

Agora also supports publishers in choosing whether the audio streams being published are to be filtered based on volume.
Streams that are not filtered bypass this filter mechanism and are transported directly to the receivers. In scenarios with a large number of publishers,
enabling this function helps reduce the bandwidth and device system pressure for the receivers.

To enable this function, contact [support@agora.io](mailto:support@agora.io).

**VP9 codec (Beta)**

As of v4.15.0, the web SDK adds support for the VP9 codec in the following browsers:

|         | Supported browsers                                           |
| ------- | ------------------------------------------------------------ |
| Desktop | <li>Chrome 65 or later</li><li>Firefox 69 or later</li><li>Safari 14 or later. VP9 is an experimental feature on Safari; be sure to enable it first </li><li>Edge 79 or later</li> |
| Android | <li>Chrome 68 or later</li><li>WeChat v8.0.6 or later </li>  |
| iOS     | <li>Safari 14 or later. VP9 is an experimental feature on Safari; be sure to enable it first </li><li>All apps that use WKWebView, such as Chrome: WebView v55.0.28883.91 or later</li> |

To start a call using the VP9 codec with an app that integrates with the native SDK (including the SDK for Android, iOS, Windows, and macOS), ensure that the native SDK version is v4.0.0 or later.

**CSP violation callback**

v4.15.0 adds the `AgoraRTC.onSecurityPolicyViolation` callback. When Agora fails to load a resource or send a request due to CSP (Content Security Policy) violations, the
SDK triggers this callback. After receiving this callback, modify your CSP configuration to ensure that you can access Agora-related services.

#### Improvements

The following improvements have been implemented:

- Improved accuracy of network quality reported by `AgoraRTCClient.on("network-quality")` in certain scenarios.
- Faster resumption of audio or video streams when the user switches between Wi-Fi networks.
...
v4.14.1 fixed the following issues:
...
#### Fixed issues
v4.14.0 fixed the following issues:
...
| AI Noise Suppression Extension (Beta)        | Reduces many types of sudden noise and reduces vocal distortion when multiple people speak at the same time.      | [AI Noise Suppression](../develop/ai-denoiser) |
