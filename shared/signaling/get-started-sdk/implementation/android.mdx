...
You need to log in to actually connect to the RTM server and access RTM's network resources, such as sending messages, subscribing to channels, etc.

When performing a login operation, the client will try to establish a long connection with the RTM server. Once successfully established, the client will transmit heartbeat information to the RTM server at fixed intervals to keep the client active until the client actively logs out or is disconnected. The connection will be interrupted when the timeout occurs. During this period, users can freely access RTM's network resources within their own permissions and usage restrictions.
...
You can use the `login` return value or listen to the `onConnectionStateChanged` event notification to confirm that the login is successful or obtain the error code and cause of the login failure. When performing a login operation, the client's network connection status is `CONNECTING`; after a successful login, the status is updated to `CONNECTED`. In order to continuously monitor the network connection status of the client, best practice is to continue to listen for `onConnectionStateChanged` event notifications throughout the life cycle of the application. For details, see [Event Listeners](../reference/api?platform=android#event-listeners) .
