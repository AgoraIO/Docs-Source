...
See [beta documentation](https://docs-beta.agora.io/en/realtime-communication/overview/release-notes) for details.
...
Cloud Proxy default mode has changed. In Video SDK for Web v4.15.0, the `mode` parameter in `startProxyServer` defaults to `3`. This represents the Force UDP cloud proxy mode. In this mode, Video SDK always transmits data over UDP. For details, see [Restricted network connection with Cloud Proxy](/realtime-communication/advanced-features/cloud-proxy).
