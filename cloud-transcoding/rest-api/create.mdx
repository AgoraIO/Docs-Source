...
<Admonition type="info" title="Output destination constraints">

- A single transcoding task cannot push to both a CDN address and an RTC channel simultaneously. To push to both destinations, you must create two separate tasks.
- If the audio or video configurations differ between the two destinations, you must also create two separate tasks.
- To push to multiple CDN addresses, include all destination addresses within a single `outputs` object.
- Each transcoding task can only push to one RTC channel. To push to two different RTC channels, you must create two separate tasks.
...
<Section title="Request example">
  ...
  <Tabs groupId="code-examples">
    ...
    const url = "https://api.sd-rtn.com/v1/projects/:appId/rtsc/cloud-transcoder/tasks?builderToken=<actual_token_value>";
