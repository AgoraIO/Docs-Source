...
v2.8.80 was released on November xx, 2023.

#### New features

This release added the following features:

- Real-time indication of the network state.
- The `onMediaErrorOccurred` callback.
...
- Minimized the time it takes to join a classroom.

#### Issues fixed

This release fixed the following issues:

- In scenarios where the number of publishers exceeded the upper limit, if a student did not stop publishing streams after getting off the stage, the student that got on the stage could not publish streams.
- After whiteboard was disabled, the billing system continued to calculate the usage.
- The user presence state was incorrectly displayed.

#### API

This release added the following APIs:

- Added `onAudioRawDataBeforeMixed` and `onAudioRawDataMixed`.
- Added enumerators `8: beforeMixed` and `4: mixed` in `FcrAudioRawDataPosition`.
...
As of v1.1.5, Flexible Classroom adds the [Media Context API](/en/agora-class/edu_context_api_ref_ios_media) to allow developers to implement the pre-class test module, with which students can check whether their camera and microphone are functioning correctly and preview how they look and sound before they join a class.
