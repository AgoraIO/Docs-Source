...
1. **Start a Real-Time Transcription task**
    ...
    To [start a speech-to-text task](#start-a-Real-Time Transcription-task), you:
...
1. **Query task status**
    ...
    You [query the status](#query-the-status-of-speech-to-text) of a Real-Time Transcription task using a `GET` request containing the `taskId` and `builderToken` to notify the user of any change. To do this, add the following method to the `ViewController` class:
