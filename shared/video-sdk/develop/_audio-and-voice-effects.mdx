...
* **Audio route**
    ...
    When audio is played back on a device, it can be routed to one of several hardware components such as the earpiece, headphones, speakerphone, or Bluetooth devices. By default <Vpd k="SDK" /> routes the <Vpd k="NAME"/> audio to the speakerphone. The audio route is changed by the device user when they add or remove an external device. Using <Vpd k="SDK" />, you can change the default audio route or temporarily set an audio route.
