...
1. **Enable external video device**
   ...
   To get video frames from the external video source, get the list of available video WebCams, select the firt video source from the list, set the selected source texture, play the selected source to start recieving local video frames. To implement this logic, add the following to `JoinChannelVideo` class:
