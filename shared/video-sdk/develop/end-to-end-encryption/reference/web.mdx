...
If the SDK is disconnected and then reconnected, the `RTCRtpTransceiver` instance corresponding to the current track may change. You can obtain the new `RTCRtpTransceiver` object through the following callbacks:

- **Local track**: `transceiver-updated`
- **Remote track**: `transceiver-updated`

After enabling the end-to-end encryption function, due to browser restrictions, even if the local track created through the Web SDK does not require encryption, you still need to create an empty encryption pipeline. Otherwise, the media data of the track cannot be sent. Refer to the following code to create an empty encryption pipeline:
