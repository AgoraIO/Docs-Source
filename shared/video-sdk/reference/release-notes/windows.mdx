...
- **Adaptive video publishing (Beta)**  
 ...
 Added support for sending multiple video streams with different resolutions from the same video source, including multi-channel scenarios. Use `setSimulcastConfig` with `SimulcastConfig` to configure up to four layers (one high-resolution main stream and up to three lower-resolution streams) and control publishing options such as resolution, bitrate, and whether to automatically disable multiple streams when uplink network or device performance is poor. When layered streaming fallback is enabled, the SDK automatically falls back to a lower-resolution stream in poor network conditions to ensure a smooth viewing experience. Subscribers can select the desired stream via `setRemoteVideoStreamType`. This feature improves viewing experiences in conferences, large classes, and interactive live streaming, especially under poor network conditions.
