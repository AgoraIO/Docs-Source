...
To ensure secure communication, your <Vpl k="CLIENT" /> uses an SSL [key](https://en.wikipedia.org/wiki/Key_authentication) and [salt](https://en.wikipedia.org/wiki/Salt_(cryptography)) to encrypt and decrypt data in the channel. You use the key and salt to create an encryption configuration. <Vg k="AGORA_BACKEND" /> uses the encryption configuration to encrypt a stream and sends it to remote users. When a remote user receives an encrypted media stream, the remote <Vpl k="CLIENT" /> decrypts it using the same salt and key.
