...
**Path parameters**
   ...
   <PathParameter name="id" type="number" possibleValues="na,eu,ap,cn" required={true}>
...
<ParameterList title="HEADER" required={true}>
  <Parameter name="X-Request-ID" type="UUID" required={true}>
    The Agora Channel Management RESTful APIs only support HTTPS. Before sending HTTP requests, you must generate a Base64-encoded credential with the **Customer ID** and **Customer Secret** provided by Agora, and pass the credential to the `Authorization` field in the HTTP request header.
  </Parameter>
