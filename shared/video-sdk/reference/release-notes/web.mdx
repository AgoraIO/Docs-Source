...
v4.23.0 was released on December XX, 2024.

#### Compatibility changes

This release modified the calculation of [`freezeRate`](/api-ref/rtc/javascript/interfaces/remotevideotrackstats.html#freezerate) <Link to="{{Global.API_REF_WEB_ROOT}}/interfaces/iagorartc.html#createscreenvideotrack">`freezeRate`</Link> in `RemoteVideoTrackStats`. The calculation is changed from the cumulative average freeze rate to the freeze rate in the last second. You can use this parameter to determine whether the video is experiencing a momentary freeze. To calculate the cumulative average freeze rate for the current session, use <Link to="{{Global.API_REF_WEB_ROOT}}/interfaces/iagorartc.html#createscreenvideotrack">`totalFreezeTime`</Link> [`totalFreezeTime`](/api-ref/rtc/javascript/interfaces/remotevideotrackstats.html#totalfreezetime) or [`totalDuration`](/api-ref/rtc/javascript/interfaces/remotevideotrackstats.html#totalduration). <Link to="{{Global.API_REF_WEB_ROOT}}/interfaces/iagorartc.html#createscreenvideotrack">`totalDuration`</Link>

#### New features

  1. **Remote user connection state callback**

    To improve the interoperability experience under poor network conditions, this release adds the [`peer-connection-state-change`](/api-ref/rtc/javascript/interfaces/iagorartcclient.html#event_peer_connection_state_change) <Link to="{{Global.API_REF_WEB_ROOT}}/interfaces/iagorartc.html#createscreenvideotrack">`peer-connection-state-change`</Link> event, which is used to obtain the media connection state of the remote user.

  1. **Sharing the current tab by default**

    To improve the ease of use of screen sharing, this release adds the [`preferCurrentTab`](/api-ref/rtc/javascript/interfaces/screenvideotrackinitconfig.html#prefercurrenttab) <Link to="{{Global.API_REF_WEB_ROOT}}/interfaces/iagorartc.html#createscreenvideotrack">`preferCurrentTab`</Link> parameter in `ScreenVideoTrackInitConfig`. This parameter supports sharing the current tab as the default when creating a screen sharing track.
...
- The issue where Safari failed to retrieve `stats` in certain scenarios.
- The issue with inaccurate bitrate settings under poor network conditions in certain scenarios.
