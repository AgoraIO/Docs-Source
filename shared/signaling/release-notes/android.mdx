...
- The class `StateItem` is removed and the type of the following parameters is changed from `ArrayList<StateItem>` to `HashMap<String, String>`:
...
- Added the `CERTIFICATION_VERIFY_FAILURE(22)` enumeration in `RtmConnectionChangeReason` and changed the values of `STREAM_CHANNEL_NOT_AVAILABLE` and `INCONSISTENT_APPID` enumerations to `23` and `24`.

#### New features

1. Added privatization deployment capabilities

    This version adds a `privateConfig` parameter to `RtmConfig` to implement the private deployment function. For details, see Private Configuration .
