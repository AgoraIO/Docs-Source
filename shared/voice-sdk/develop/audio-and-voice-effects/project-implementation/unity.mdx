...
1. **Set up access to the UI elements**
    ...
    In `Assets/Agora-RTC-Plugin/Agora-Unity-RTC-SDK/Code/JoinChannelAudio.cs`, add the following at the end of `SetupUI();`
...
1. **Update the UI after the effect finishes playing**
    ...
    When <Vg k="SDK" /> has finished playing the sound effect, the `OnAudioEffectFinished` event is fired. To handle this event, in `Assets/Agora-RTC-Plugin/Agora-Unity-RTC-SDK/Code/JoinChannelAudio.cs`, add the following to the `UserEventHandler` class:
