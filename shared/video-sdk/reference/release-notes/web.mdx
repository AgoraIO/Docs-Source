...
- **First-frame rendering**
  To reduce the time users wait for the first video frame of a host to render, this version adds a new options parameter to the join method to configure the Channel media options. By setting autoSubscribe to true in IJoinOptions, you can automatically subscribe to the host’s media stream when joining the channel.

In real-time interactive scenarios such as one-on-one or one-to-many, this feature effectively reduces the time to first-frame rendering and enhances the interactive experience.

If you enable automatic subscription to the host’s media stream, the Agora backend will start delivering the media stream and billing as soon as you join the channel.
Improvements
New media connection strategy
This version introduces a new media connection strategy that improves the SDK’s reconnection and media login success rates under enterprise and restricted network environments.

More accurate network quality evaluation
This version enhances the algorithm for evaluating NetworkQuality by incorporating available bandwidth as a reference. This allows the SDK to more accurately reflect the impact of bandwidth-limited network conditions.

This improvement requires enabling a private parameter. If needed, please contact support@agora.io.

Other improvements
Optimizes the default video bitrate under different video encoding standards to allocate uplink bandwidth more efficiently and reduce stuttering in poor network conditions.
Improves the fallback mechanism for devices that do not support AV1 decoding.
Bug Fixes
Fixes a potential black screen issue on Firefox 138+ caused by browser version rollback.
Fixes an encoding failure issue that occurred when the resolution was too low.
Fixes an occasional black screen issue in scenarios using Plan-B.
