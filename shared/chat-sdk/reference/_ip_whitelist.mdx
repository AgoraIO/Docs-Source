...
For added security, <Vg k="COMPANY"/> <Vg k="CHAT"/> provides the IP whitelist function. If only certain IP addresses are allowed to call the RESTful APIs to send messages, you can add these IP addresses to the IP whitelist on the <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link>. IP addresses outside the IP whitelist cannot call the RESTful API to send messages.

### Add an IP address to the IP whitelist

1. Log in to <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link>.

2. In the left navigation bar of the <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link>, choose **Chat** > **Features** > **IP Whitelist** to open the **Security Setting** page.
...
4. Click **OK** to complete the configuration.
    ...
    ![IP_whitelist_add_IP](https://agora-doc.s3.us-east-1.amazonaws.com/images/chat/IP_whitelist_add_IP.png)
...
3. In the IP Whitelist list, click **Delete** to the right of an IP address.
    ...
    ![IP_whitelist_delete_IP](https://agora-doc.s3.us-east-1.amazonaws.com/images/chat/IP_whitelist_delete_IP.png)
