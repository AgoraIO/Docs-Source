...
<Tabs>
  ...
  - <Vg k="SIG" /> `1.x` integration:
    ```javascript 
    // Replace x.y.z with the specific SDK version number, such as 1.5.1
...
The package name for the <Vg k="SIG" /> `2.x` SDK is `agora-rtm`, whereas the package name for the `1.x` SDK is `agora-rtm-sdk`.
</Admonition>
