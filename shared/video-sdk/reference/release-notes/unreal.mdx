...
This version was released on August 16, 2023.

#### Compatibility changes

Please review the compatibility changes below and update your implementation after upgrading the SDK.

**1. Video capture (Windows, iOS)**

This release optimizes the APIs for camera and screen capture functions. As of v4.2.1, use the alternative methods listed in the table below and specify the video source by setting the `sourceType` parameter.
...
- The `onCaptureVideoFrame` and `onPreEncodeVideoFrame` callbacks have been added with a new parameter called `sourceType`, which is used to indicate the specific video source type.

- The following callbacks have been deleted. Get the video source type through the `sourceType` parameter in the `onPreEncodeVideoFrame` and `onCaptureVideoFrame` callbacks. (Windows)
...
- The `VideoInputStreams` in `LocalTranscoderConfiguration` has changed to `videoInputStreams`.
- The `MediaSourceType` in `TranscodingVideoStream` has changed to `VideoSourceType`.

**5. Video encoder algorithm**

As of this release, the SDK optimizes the video encoder algorithm and upgrades the default video encoding resolution from 640 × 360 to 960 × 540 to accommodate improvements in device performance and network bandwidth. This provides users with an end-to-end HD experience across a variety of audio and video scenarios
...
NOTE: The upgraded default resolution settings may result in an increased aggregate resolution for your implementation. This may have have an impact on your monthly bill. Please review your default resolution settings after upgrading the SDK to prevent additional charges. See [Pricing](../reference/pricing).

**6. Miscellaneous**

- `onApiCallExecuted` has been deleted. Agora recommends getting the results of the API implementation through relevant channels and media callbacks.
- The `IAudioFrameObserver` class has been renamed to `IAudioPcmFrameSink`. The following methods have been updated accordingly:
    - `onFrame`
    - `registerAudioFrameObserver` and `unregisterAudioFrameObserver` in `MediaPlayer`
- `startChannelMediaRelay`, `updateChannelMediaRelay`, `startChannelMediaRelayEx` and `updateChannelMediaRelayEx` have been deprecated. Use `startOrUpdateChannelMediaRelay` and `startOrUpdateChannelMediaRelayEx` instead.

#### New features

**1. AI Noise Suppression**

This release introduces AI Noise Suppression. Once enabled, the SDK automatically detects and reduces background noises. Whether in bustling public venues or real-time competitive arenas that demand lightning-fast responsiveness, this function guarantees optimal audio clarity, providing users with a superior audio experience. You can enable this feature through the newly-introduced `setAINSMode` method. Then, set the noise suppression mode as  balance, aggressive or low latency according to your needs.
...
This release introduces `setVideoScenario` for setting the video application scene. The SDK will automatically enable the best practice strategy based on different scenes, adjusting key performance indicators to optimize video quality and improve the user experience. Whether it is a formal business meeting or a casual online gathering, this feature ensures that the video quality meets your requirements.

Currently, this feature provides targeted optimizations for real-time video conferencing scenarios, including:

- Automatically activate multiple network optimization technologies to enhance the capability and performance of low-quality video streams in meeting scenarios where high bitrates are required. This ensures a smooth experience when the receiving end is subscribed to multiple streams.
- Monitor the number of subscribers for the high-quality and low-quality video streams in real time by dynamically adjusting the configuration of the high-quality stream and enabling or disabling the low-quality stream as required to optimize uplink bandwidth and consumption.

**4. Local video mixing (macOS, iOS, Android)**

This release adds the local video mixing feature. You can use the `startLocalVideoTranscoder` method to mix and render multiple video streams locally, such as camera-captured video, screen sharing streams, video files, images, etc. This allows you to achieve custom layouts and effects, making it easy to create personalized video display effects to meet various scenario requirements, such as remote meetings, live streaming, and online education, while also supporting functions like portrait-in-picture effects.

Additionally, the SDK provides the `updateLocalTranscoderConfiguration` method and the `onLocalVideoTranscoderError` callback. After enabling local video mixing, you can use the `updateLocalTranscoderConfiguration` method to update the video mixing configuration. When an error occurs while starting the local video mixing or updating the configuration, you can get the reason for the failure through the `onLocalVideoTranscoderError` callback.
...
In real-time collaborative singing scenarios, network issues can cause inconsistencies in the downlinks of different client devices. To address this, this release introduces `getNtpWallTimeInMs` to obtain the current Network Time Protocol (NTP) time. By using this method to synchronize lyrics and music across multiple client devices, users can experience seamless collaborative singing.

**7. Instant frame rendering**

This release adds the `enableInstantMediaRendering` method to enable instant rendering mode for audio and video frames, which can speed up the rendering of the first video or audio frame after a user joins the channel.
...
As of this release, the SDK optimizes the `onRenderVideoFrame` callback. The returned value will differ in meaning depending on the video processing mode:

- When the video processing mode is `processModeReadOnly`, the returned value is reserved for future use.
- When the video processing mode is `processModeReadWrite`, the SDK receives the video frame when the return value is `true`. The video frame is discarded when the returned value is `false`.
...
- Occasional crashes occur on Android devices when users join or leave a channel. (Android)
- When the host frequently switches the user role between broadcaster and audience in a short period of time, the audience members cannot hear the audio of the host.
- Occasional failures when enabling in-ear monitoring. (Android)
...
- When a user left a channel, a request to leave was not sent to the server and the leaving action was incorrectly determined by the server as timed out.
