...
To add the UI framework to your <Vpl k="NAME"/> project, open `<project-name>` > `MainWindow.xaml` from the Solution Explorer, and replace the file contents with the following:
...
1. **Import the relevant namespaces**
    ...
    In **Solution Explorer**, select `<project-name>` > `MainWindow.xaml`, and double-click it to open the `MainWindow.xaml.cs` file. Add the following lines to the namespaces list in the file:
...
1.  **Log events and show status updates to your users**
    ...
    In the `MainWindow` class, add the following method:
