...
- `appId`: The App ID must be the same as that of the Agora RTC SDK. For details, see [Getting an App ID](https://docs.agora.io/en/Agora%20Platform/get_appid_token?platform=All%20Platforms#get-the-app-id).
...
- `mixedVideoAudio`：When `isMixingEnabled` is 1, you can set this parameter to mix the audio and video in an MP4 file in real time. 
  ...
  - `7`: Mixes the audio and video in real time into an MP4 file and a TS file. The SDK deletes the TS file after recording. See [FAQ](https://docs.agora.io/en/faq/recording_player) for the supported players.

- `layoutMode`：Sets the video mixing layout. See [Set Video Layout](../develop/video-profile) for details.
...
- `mixResolution`: When `isMixingEnabled` is set as 1, you can use this parameter to set the video profile, including the width, height, frame rate, and bitrate. The default setting is 360 x 640, 15 fps, and 500 Kbps. We recommend you set the video profile according to the [Video Profile Table](../develop/video-profile#video-profile-table).
