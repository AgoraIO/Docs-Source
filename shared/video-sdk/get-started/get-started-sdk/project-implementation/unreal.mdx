...
2. **Create a blueprint for the user widget class**
    ...
    1. From the search results, select `MyUserWidget` and press **Select**.
...
1. **Add the widget blueprint to viewport**
    ...
    Once you have created and laid out your widget blueprint, in order to be display it in your game, you need to call it by using the **Create Widget** and **Add to Viewport** nodes inside **Level Blueprint**. To implement this workflow, take the following steps:
    ...
    1. Right-click in the **Event Graph** window and input `create widget` in the **Search** field. From the search results, choose **Create Widget**.
        ...
        A node appears in **Level Blueprint**.

    1.  **Add to Viewport** node to **Level Blueprint** 

            1. Drag right pins of the **Construct NONE** to level editor and then right click at the dragged pin end and search for **Addto Viewport** . From the search results, choose **Addto Viewport**. It actually put **Addto Viewport** in level editor whose left pin is connected with the right pin of **Construct NONE**. 
            
            1. Set `Return Value` pin of **Construct NONE** to `Target` pin of **Addto Viewport**.
...
To check that you successfully added `NewBlueprint` to the view port, in **Content Browser**, navigate to the `Content` folder and double-click `Untitled`. You see the new game level opens. Click **Play**, you see the following UI:
...
Import the C++ libraries, setup your <Vpl k="CLIENT" /> to run on Android, and request permissions for the camera and microphone.
...
3. **Manage Android permissions**
    ...
    2. Setup a function to check permissions are granted.
...
1. **Setup <Vg k="ENGINE"/>**
    ...
    To setup an <Vg k="ENGINE"/> instance, take the following steps:
    ...
    2. Setup the <Vg k="ENGINE" /> instance. 
...
1. **Handle and respond to <Vg k="ENGINE"/> events**
    ...
    1. In `MyUserWidget.h`, modify `UMyUserWidget` function interface  `class AGORAIMPLEMENTATION_API UMyUserWidget : public UUserWidget` as below :
...
1. **Join a channel to start <Vpd k="PRODUCT" />**
    ...
    When the user clicks **Join**, a call for `OnJoinButtonClick()` is executed. This method securely connects the local user to a channel using the authentication token. In `MyUserWidget.cpp`, add the following before function definition of `setupVideoSDKEngine()` :
    <ProductWrapper product="video-calling">
    ``` cpp
...
1. **View the remote user who joins the channel**
    ...
    When a remote user joins the channel, <Vg k="COMPANY"/> fires `OnUserJoined` callback event. To catch this callback and start remote video, in `MyUserWidget.cpp`, add the following code after the function definition of `OnJoinChannelSuccess()`:
    <ProductWrapper product="video-calling">
    ``` cpp
...
8. **Stop the remote video when a remote user leaves the channel**
    ...
    When a remote user leaves the channel or drops offline, <Vg k="COMPANY"/>  fires `OnUserOffline` callback event. To catch this callback and stop remote video, in `MyUserWidget.cpp`, add the following callback method after the function definition of `OnJoinChannelSuccess()`:
    <ProductWrapper product="video-calling">
    ``` cpp
...
1. **Check that the <Vpl k="CLIENT" /> has the correct permissions to start**
    ...
    3. After checking the permissions, need to call `setupVideoSDKEngine()` to create an engine instance.
...
3. **Clean up the resources used by your <Vpl k="CLIENT" />**
    ...
    2. Add the resource clean up logic to `NativeDestruct()`.
