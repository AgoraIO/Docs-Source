...
SQLCipher is used to encrypt the database that stores local messages. The <Vg k="CHAT_SDK"/> uses `IChatManager` and `IConversationManager` to manage local messages. Followings are the core methods:
