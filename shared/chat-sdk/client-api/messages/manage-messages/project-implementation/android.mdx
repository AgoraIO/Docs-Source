...
You can call the `EMChatManager#searchMsgFromDB(java.lang.String, long, int, java.lang.String, com.hyphenate.chat.EMConversation.EMSearchDirection, com.hyphenate.chat.EMConversation.EMMessageSearchScope)` method. In addition to setting the keywords, message timestamps, number of messages, sender, and search direction to search for messages in all conversations, you can also select the search scope, such as only searching for the message content, only the message extension information, or searching both the message content and the extension.
To use this feature, you need to upgrade the SDK to V4.4.0 or above.
...
You can call `com.hyphenate.chat.EMConversation#searchMsgFromDB(java.lang.String, long, int, java.lang.String, com.hyphenate.chat.EMConversation.EMSearchDirection, com.hyphenate.chat.EMConversation.EMMessageSearchScope)` method to search for messages in the current session in addition to setting the keyword, message timestamp, number of messages, sender, and search direction. In addition to setting keywords, message timestamps, number of messages, sender, direction, etc., you can also select a search scope, such as message content only, message extensions only, and message content and extensions at the same time.
To use this feature, you need to upgrade the SDK to V4.4.0 or above.
