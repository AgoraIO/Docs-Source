...
In a real-word application, you provide several buttons to enable a user to open, play, pause and stop playing files in the media player. In this simple example, you use a single button to demonstrate the basic media player functions. 
...
1. The `MediaPlayerButton` component displays a suitable caption on a button depending on the state of the media player. To define this component, add the following code to the `App` component:
...
1.  **Display media player preview locally**
    ...
    To show the media player preview locally, you create an `RtcSurfaceView` component, set its `sourceType` to `VideoSourceType.VideoSourceMediaPlayer` and specify the media player ID. To enable switching between displaying media player output and the camera stream, do the following:
