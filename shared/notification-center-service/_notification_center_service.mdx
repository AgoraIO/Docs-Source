...
- [Enable <Vg k="NCS" /> events](#enable-ncs)
...
- [Handle redundand notifications and abnormal user activity](#handle-redundand-notifications-and-abnormal-user-activity)
...
Download [`json-simple-1.1.1.jar`](https://mvnrepository.com/artifact/com.googlecode.json-simple/json-simple/1.1.1) and add it to your JAVA project.
...
public class Handlers {
	...
	public static class NcsHandler implements HttpHandler {
		...
		public void handle(HttpExchange he) throws IOException {
			...
			System.out.println("Verified: " + Signature.verify(query, agoraSignature));
			...
			try {
				Object obj = new JSONParser().parse(query);
				...
				Signature.verify(query, noticeId);
...
public class UserRegistry {
  ...
  // Holds the list of users in each channel
  class Channel {
  ...
  public void HandleNcsEvent(String channelName, int uid, int eventType, long clientSeq) {
    ...
    // Check the eventType to only process events that notify of users joining or
    // leaving
    // a channel or a change in the user role
