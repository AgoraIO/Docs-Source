...
<Admonition type="info" title="Note">To ensure a consistent user experience across different platforms while enabling native access on each platform, the <Vg k="WHITE_SDK" /> for Android is implemented by bridging the <Vg k="WHITE_SDK" /> for Web, rather than being a completely native SDK. Therefore, the version update content of the Android SDK will by default include the specified Web SDK version and its previous update content. You can find the bridging instructions in each Android release note and then refer to the Web SDK release notes to learn the updates.</Admonition>

## v2.16.86

v2.16.86 was released on June XX, 2024.
...
<Admonition type="info" title="Note">The bridged Web SDK has been upgraded to v2.16.37.</Admonition>
