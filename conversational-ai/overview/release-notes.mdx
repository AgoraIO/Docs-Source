...
This version was released on March xx, 2025.

#### New features

The [Start a conversational agent](../rest-api/join) API adds the `enable_rtm`and `agent_rtm_uid` parameters to enable Signaling integration with <Vpd k="AGENT" />. When this feature is enabled, the agent can leverage the Signaling SDK to obtain a users's custom context information such as speaking status, selected text, signature, and score, and pass this data to the agent to generate more relevant content. For details, see [Pass Custom Information](../develop/custom-information).
