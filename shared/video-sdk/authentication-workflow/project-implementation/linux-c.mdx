...
1.  **Add an HTTPS library to your project**
    ...
    1. To send requests to your token server, you add an HTTPS library to your project. In this example, you use the [`libcurl`](https://curl.se/libcurl/) file transfer library. [Download and install](https://curl.se/download.html#Linux) a suitable `libcurl` package for your hardware and Linux distribution. On Ubuntu Linux, use the following commands to install the package:
    ...
    1. To integrate the `libcurl` library into your project : Open the file `<extracted-folder>/agora_rtsa_sdk/example/hello_rtsa/CMakeLists.txt` and add the following line after `target_link_libraries(hello_rtsa agora-rtc-sdk file_parser ${LIBS})`:
    
        ```bash
        target_link_libraries(hello_rtsa -lcurl)
        ```

    1. To include the header in your code, open the file `<extracted-folder>/agora_rtsa_sdk/example/hello_rtsa/hello_rtsa.c` and add the following to the list of `#include` statements:
...
1.  **Add variables for your connection to the token server**
    ...
    char* server_url = "<Token Server URL>"; // The base URL to your token server
    // For example, "https://agora-token-service-production-xxxx.up.railway.app".
...
1.  **Handle the event triggered by <Vg k="AGORA_BACKEND" /> when the token is about to expire**
    ...
    1. Declare a global boolean variable to indicate that the current token is about to expire. To do this, add the following code before function `static void app_signal_handler(int sig)` in `hello_rtsa.c`: 

        ```c 
        bool token_privilege_will_expire = false;
        ```

    1. To receive the `on_token_privilege_will_expire` callback, add the following line to `app_init_event_handler` after `event_handler->on_license_validation_failure = __on_license_failed;`:

        ```c 
        event_handler->on_token_privilege_will_expire = __on_token_privilege_will_expire;
        ```

    1. To handle the `on_token_privilege_will_expire` callback, add the following code before the `app_init_event_handler` function:
