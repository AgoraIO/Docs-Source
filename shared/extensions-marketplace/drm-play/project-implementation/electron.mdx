...
1. **Declare the variables you need**
    ...
    To instantiate `IAgoraMusicContentCenter`, play music, and handle its events, in `preload.js`, add the following to variable declarations:
...
1. **Set up the music content center**
    ...
    1. To configure the music content, you call `getMusicContentCenter` to get an instance of `IAgoraMusicContentCenter`. To initialize the created instance, call `initialize` and pass your `appId`, `token`, and `mccUid`. To receive preloading and search results, setup an event handler and attach it to the music content center. To implement this logic, in `preload.js`, add the following code before `window.onload = () =>`:
        ...
        const musicEventHandler =
        {
            ...
            {
                console.log("Music chart items count: " + list.length);
                if (list.length > 0) 
                {
