...
### Real-Time Transcription pricing

[Real-Time Transcription](../enable-features/real-time-transcription) takes the audio content of a host's media stream
and transcribes it into written words in real time. Agora charges for the time that Real-Time Transcription is enabled in a channel. Transcription is available for up to 3 hosts speaking at the same time.
If several hosts speak simultaneously, Agora also charges for the time of speaking of each additional host using
the same pricing.
Enabling Real-Time Transcription creates an audience member in the cloud to subscribe to the audio in the channel for transcription. This increases audio usage for the duration that Real-Time Transcription is enabled.
...
Contact sales@agora.io for volume discounts.

## Sample calculation for video streaming cost
