...
3.  Create and publish the audio track:
  ...
  2.  Publish the audio track by calling `publish`.
...
4.  When a remote user joins the channel, this <Vpl k="CLIENT"/>:
  ...
  2.  Retrieves the remote track using the `IAgoraRTCRemoteUser` object.
  3.  Plays the retrieved tracks by calling `play`.
