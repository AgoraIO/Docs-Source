...
4.  **Handle and respond to  <Vg k="MESS" /> events**
    ...
    The following event listeners handle notification events for <Vg k="MESS" /> features:
  ...
  status change
    ...
    The following code sets up event handlers for <Vg k="MESS" /> events:
...
1.  **Login to <Vg k="MESS" />**
      ...
      In <Vg k="MESS" />, each token is specific to a user ID. If `uid` and `token` do not match, your user cannot initiate <Vg k="MESS_ENGINE" />.
...
1.  **Monitor users in channel using presence**
      ...
      In <Vg k="MESS" />, you can use `presence` to get information about users in the channel. To get a list of active users, call `presence.getOnlineUsers()`:
