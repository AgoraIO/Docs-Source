...
1. **Add variables for your connection to the token server**
    ...
    Declare the variables you need to specify the user id, token-server URL and the token expire time. Add the following declarations to `AgoraImplementation.cs` before `private VideoSurface LocalView;`:

    ``` csharp
    private string serverUrl = ""; // The base URL to your token server. For example, https://app-name.herokuapp.com"
...
1. **Retrieve a token from the server**
    ...
    Use a GET request to retrieve an authentication token for a specific channel from the token server. In `Assets/AgoraEngine/Scripts/AgoraImplementation.cs`, add the following code before `Start`:
...
1. **Add a class to extract the token from a JSON object**

    In `Assets/AgoraEngine/Scripts/AgoraImplementation.cs`, add the following class before `AgoraImplementation : MonoBehaviour {`:
...
1. **Handle the event triggered by <Vg k="AGORA_BACKEND" /> when the token is about to expire**
    ...
    i. In `Assets/AgoraEngine/Scripts/AgoraImplementation.cs`, add the following code before `Join`:
    ...
    ii. In `SetupVideoSDKEngine`, add the following code before `mRtcEngine.OnUserOffline = OnUserOffline;`:
...
1. **Update the `Join` method to fetch a token**
    ...
    Use `FetchToken` to retrieve a fresh token from server before joining a channel. Add the following code before `mRtcEngine.EnableVideo();`:
