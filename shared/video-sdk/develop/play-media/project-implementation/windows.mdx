...
1.  **Open, play and pause media files**
    ...
    To open a media file and configure the playback scenario, call <Link to="{{Global.API_REF_CPP_ROOT}}/class_imediaplayer.html#api_imediaplayer_openwithmediasource">openWithMediaSource</Link>. Use <Link to="{{Global.API_REF_CPP_ROOT}}/class_irtcengine.html#api_Imediaplayer_selectmultiaudiotrack">selectMultiAudioTrack</Link> to select the audio tracks that you want to play on your local device and publish to the channel respectively.
