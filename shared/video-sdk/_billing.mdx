...
Agora calculates service minutes **by the number of users**, which is easier and more straightforward. Note that service minutes are calculated for aggregate video resolution. See [Calculate aggregate video resolution](pricing#calculate-aggregate-video-resolution).
