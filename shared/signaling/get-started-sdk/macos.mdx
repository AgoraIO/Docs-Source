...
-   When you leave a channel and do not want to join it again, you can call the <Link to="{{Global.SIGNALING_API_REF_OC_ROOT}}/Classes/AgoraRtmKit.html#//api/name/destroyChannelWithId:">destroyChannelWithId</Link> method to release all resources used by the channel instance.
...
Agora also provides an open-source [sample project](https://github.com/AgoraIO/RTM/tree/master/Agora-RTM-Tutorial-macOS-Objective-C) for Objective-C and [sample project](https://github.com/AgoraIO/RTM/tree/master/Agora-RTM-Tutorial-macOS) for Swift on GitHub for your reference.
...
5.  Run the\`pod install\`command to install the Agora SDK. Once you successfully install the SDK, it shows\`Pod installation complete!\` in Terminal, and you can see an **workspace** file in the project folder.

6.  Open the generated **workspace** file in **Xcode**.

**Method 2: Manually add the SDK files**

**v1.4.4 or higher**

1.  Download the latest version of the [RTM macOS SDK](../reference/downloads.html), and extract the files.

2.  Copy `AgoraRtmKit.framework` from the SDK to your project folder.
