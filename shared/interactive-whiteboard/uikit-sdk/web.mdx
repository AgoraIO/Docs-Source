...
Create a playback `FastboardPlayer` instance.
...
- `options`: Object. Replay configuration options. See [FastboardReplayOptions](#fastboardreplayoptions)

**Returns**

The method returns a `FastboardPlayer` object when the call is successful.
...
<Admonition type="info">Agora does not recommend this method since v0.3.6. Use <a href="#createui"><code>createUI</code></a> instead.</Admonition>
...
Check if the room is operable.

Determine whether the current room is in interactive mode (`writable`) and whether the room connection status is connected (`connected`).
...
<Admonition type="info">This method is deprecated since v0.3.6. Use `fastboard.manager.addApp({ kind: 'Monaco' })<` instead.</Admonition>
...
<Admonition type="info">This method is deprecated since v0.3.6. Use `fastboard.manager.addApp({ kind: 'Countdown' })` instead.</Admonition>
...
<Admonition type="info">This method is deprecated since v0.3.6. Use `fastboard.manager.addApp({ kind: 'GeoGebra' })` instead.</Admonition>
...
- `joinRoom`: (Required) Object. Parameters for joining a room:
  ...
  - `callbacks`: (Optional) Event callbacks for the whiteboard room. See <Link to="{{Global.API_REF_WEB_WHITE}}/globals.html#roomcallbacks">RoomCallbacks</Link> for details.
...
The `FastboardReplayOptions` interface includes the following properties:

* `sdkConfig`: (Required) Object. `WhiteWebSdk` instance configuration. The following parameters must be passed:
   * `appIdentifier`: String. The App Identifier for the interactive whiteboard project. 
   * `region`: String. Data center, which must be consistent with the data center set when the room was created. Possible values are:
      * `us-sv`: Silicon Valley, USA, with service areas covering North and South America.
      * `cn-hz`: Hangzhou, China, with service areas not covered by other data centers.
      * `sg`: Singapore, with service areas covering Singapore, East Asia, and Southeast Asia.
      * `in-mum`: Mumbai, India, with service areas covering India.
      * `eu`: Frankfurt, Europe, with service areas covering Europe.
* `replayRoom`: (Required) Object. Room parameter settings. The following parameters must be passed:
   * `room`: String. The room's UUID, the room's unique identifier. See the `uuid` parameter value in the response body after a successful room creation request for details.
   * `roomToken`: String. Room Token used for user authentication when joining a room. It can be obtained in the following ways:
      * Invoke the API to generate a Room Token.
      * Generate tokens using code on the app server. 
   * `beginTimestamp`: Number. Playback start time in milliseconds.
   * `duration`: Number. Playback duration in milliseconds.
* `managerConfig`: (Optional) Object. WindowManager configuration options.
* `netlessApps`: (Optional) Object. Official built-in or custom Fastboard plugins. 
* `enableAppliancePlugin`: (Optional) Object. Enable the `appliance-plugin`. See `AppliancePluginOptions` for details.
* `enableAppInMainViewPlugin`: (Optional) Boolean or Object. Enable the `@netless/app-in-mainview-plugin` to support managing window applications in the whiteboard main view:
   * `true`: Enable the plugin using default settings.
   * `AppInMainViewOptions`: Enable the plugin using custom configuration. See `AppInMainViewOptions` for details.
...
UI configuration options include the following properties:

* `app`: `FastboardApp` object.
* `theme`: String. Theme setting. Supported values are:
   * `dark`: Dark theme.
   * `light`: (Default) Light theme.
* `language`: String. Language setting. Supported values are:
   * `en`: (Default) English.
   * `zh-CN`: Chinese.
* `containerRef`: Function. Callback triggered when joining a whiteboard room and successfully mounting the whiteboard application to a webpage element:
   * `container`: Webpage element that contains the whiteboard application.
* `config`: Object. Component configuration. See `FastboardUIConfig` for details.
