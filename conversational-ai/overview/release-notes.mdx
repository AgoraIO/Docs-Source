...
* **Stop agent API is now asynchronous**
    ...
    The [Stop a conversational AI agent](/conversational-ai/rest-api/agent/leave) API now returns asynchronously, responding immediately after request parameters are validated without waiting for the agent to fully leave the channel.
