...
1. **Add the widget blueprint to viewport**
    ...
    1.  Add the **Add to Viewport** node to **Level Blueprint**:
            ...
            1. Drag the right pin of the **Construct NONE** node to the level editor, then right-click the dragged pin end and search for **Add to Viewport**. From the search results, choose **Add to Viewport**.
...
1. **Handle and respond to <Vg k="ENGINE"/> events**
    ...
    1. In `MyUserWidget.h`, modify `UMyUserWidget` function interface  `class AGORAIMPLEMENTATION_API UMyUserWidget : public UUserWidget` as specified below :
