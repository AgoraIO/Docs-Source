...
conn.getSelfIdsOnOtherPlatform().then((res) => {
    console.log(res, 'Successfully obtained the login ID list of other login devices of the current user');
    ...
    onMultiDeviceEvent: (event) => {
        switch (event.operation) {
            ...
            case "chatThreadNameUpdate":
                ...
                break;
            case "deleteRoaming":
                //The current user deletes the server session on other devices.
                break;
            case "memberAttributesUpdate":
                //The current user has updated the group member attributes on other devices.
                break;
            case "deleteRoaming":
                //The current user has deleted historical messages on other devices.
                break;
