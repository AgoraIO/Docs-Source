...
1. **Implement best practice for app initiation**
    ...
    1. In your script file, add the following methods before `Start()`:
        ...
        ```
        For demonstration, we only added two dropdowns in the user interface. However, you may add one more dropdown to show the available audio playback devices. For selecting and setting a playback device, you may use `EnumeratePlaybackDevices` and `SetPlaybackDevice`. For more details, see our [API Examples](https://github.com/AgoraIO-Extensions/Agora-Unity-Quickstart/blob/main/API-Example-Unity/Assets/API-Example/Examples/Advanced/DeviceManager/DeviceManager.cs).
1. **Enable the user to test the selected audio and video devices**

    When the user presses **Start device testing**, your <Vpl k = "CLIENT"/> select an audio and video device based on user input and set them using the corresponding device IDs in the `_audioDeviceManager` and `_videoDeviceManager`. To test the audio recording device, your <Vpl k = "CLIENT"/> starts an audio device loopback test with a duration of 500 milliseconds, which is used to verify that the selected audio device is functioning properly. To test the video device, your <Vpl k = "CLIENT"/> creates a window using the `Win32` API and then it calls `StartDeviceTest` and pass the window handle to display the video from the camera. To implement this workflow, do the following:
