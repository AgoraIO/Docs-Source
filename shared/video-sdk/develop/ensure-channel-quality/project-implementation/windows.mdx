...
1. **Add a network status indicator to the user interface**
    ...
    To enable <Vpl k="CLIENT" /> users to see the network status, add `Static Text` to the user interface. To do this, take the following steps`:
        ...
        1. From **ToolBox**, drag **Static Text** and drop it onto the surface of the **Dialog Editor**.
...
1. **Add a button to start and stop the echo test**
    ...
    1. To change the button label, in **Properties**, update the **Caption** field with `Echo Test`.

1. **Add an Edit Control to display statistics**

    To display call and quality statistics, you use **Edit Control**. From **ToolBox**, drag **Edit Control** and drop it onto the surface of the **Dialog Editor**.

1. **Add a button to switch the remote video quality**

    1. From **ToolBox**, drag **Button** and drop it onto the surface of the **Dialog Editor**

    1. To change the button label, in **Properties**, update the **Caption** field with `Video Quality`.
...
1. **Implement best practice for app initiation**
    ...
    * _Start the network probe test_: A quick test at startup to gauge network quality.
    ...
    videoConfig.compressionPreference = agora::rtc::PREFER_LOW_LATENCY;    // Apply the configuration
