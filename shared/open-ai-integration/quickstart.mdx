...
2. Execute the following command to run the demo server:

   ```bash
   python -m realtime_agent.agent --channel_name=<channel_name> --uid=<agent_uid>
   ```

   Replace `your_channel_name` with the desired channel name and `your_user_id` with a unique user ID.
