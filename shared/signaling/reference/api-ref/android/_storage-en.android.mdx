...
| `channelName` | String | Required | - | Channel name. |
| `channelType` | <code>{enumv.channeltype[props.ag_platform]}</code> | Required | - | Channel types. See <a href="../enumv#channeltype"><code>{enumv.channeltype[props.ag_platform]}</code></a>. |
| `resultCallback` | `ResultCallback<Metadata>` | Required | - | Callback of invocation result:<li>Success: Executes the `onSuccess` method and return the <a href={`#storagemetadatapropsag_platform`}><code>{storage.metadata[props.ag_platform]}</code></a> query result.</li><li>Failure: Executes the `onFailure` method.</li> |
