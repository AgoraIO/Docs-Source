...
1. **Show network status and switch video quality**

    To enable <Vpl k="CLIENT" /> users to see the network status, you add a `_networkStatus` widget to the user interface. To switch between high and low quality video, you add a button. To update the UI, open `/lib/main.dart` and add the following lines after `ListView(...children: [`:
    ...
    Expanded(
        ...
        child: isHighQuality ? const Text("Swith to low quality"): const Text("Swith to high quality"),
        onPressed: () => {changeVideoQuality()},
        ),
    ),
    ```

2. **Define the network status widget**
...
1. **Declare variables to manage test state and workflow**
    ...
    In `/lib/main.dart`, add the following declaration to `_MyAppState` class:

    ```dart
    int networkQuality = 0; // Quality of the network connection
...
1. **Run a network quality test**
    ...
    In `_MyAppState` class, add the following method to run a last mile probe test:
...
1. **Listen to <Vg k="ENGINE" /> events to receive state change notifications and quality statistics**
    ...
    * `onRemoteVideoStats`: Receives stats about remote videos.
    ...
    The `onRemoteVideoStats` callback  reports statistics for each remote user and the host. 
...
1. **Switch stream quality when the user taps the remote video**
    ...
    if (isHighQuality) {
        agoraEngine.setRemoteVideoStreamType(uid: remoteUid, streamType: VideoStreamType.videoStreamHigh);
        showMessage("Switching to high-quality video");
    } else {
        agoraEngine.setRemoteVideoStreamType(uid: remoteUid, streamType: VideoStreamType.videoStreamLow);
