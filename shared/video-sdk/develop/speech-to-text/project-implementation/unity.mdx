...
To create a simple interface for testing speech to text functionality, you add two buttons to the UI.
    ...
    2. In **Inspector**, rename **Button** to `speechToText`, and then change the following coordinates:

        * **Pos X** - -140
        * **Pos Y** - -172
        * **Width** - 250

    3. Select the **Text(TMP)** sub-item of `speechToText`, and in **Inspector**, change **Text** to *Start Speech-to-text*.

    4. Use the same procedure to create a button called `querySpeechToText` with a **Text(TMP)** sub-item where its **Text** says *Query Speech-to-text*.

    5. To adjust the position and size of `querySpeechToText`, in **Inspector**, change the following coordinates:
...
1. **Add the required namespaces**
    ...
    To start, stop, and query a speech-to-text task, in your script file, add the following to the list of namespaces:
...
1. **Define variables to manage a speech-to-text task**
    ...
    private bool isSpeechToTextRunning = false;
    int Uid = 1;
    ```
...
1. **Start a speech-to-text task**
    ...
    public void startSpeechToText()
    {
        ...
        using (var streamWriter = new StreamWriter(request.GetRequestStream()))
        {
            // Setup a JSON string to specify a speech-to-text configuration.
            string json = @"
            {
                ""audio"": 
                {
        ...
        using (Stream dataStream = response.GetResponseStream())
        {
            // Read response and learn about the status of the speech-to-text task.
            StreamReader reader = new StreamReader(dataStream);
            string responseFromServer = reader.ReadToEnd();
            Debug.Log(responseFromServer);
            if(((HttpWebResponse)response).StatusDescription != "OK")
            ...
            if(status == "STARTED"|| status == "IN_PROGRESS")
            {
                ...
                String[] list2 = list1[2].Split(":");
                speechToTextTaskId = list2[1];
                speechToTextTaskId = speechToTextTaskId.Replace("\"", "");
...
1. **Query task status**
    ...
    You [query the status](#query-the-status-of-speech-to-text) of a speech-to-text task using a `GET` request containing the `taskId` and `builderToken` to notify the user of any change. To do this, in your script file, add the following method to `NewBehaviourScript`:
    ...
    public void querySpeechToText() 
    {
        ...
        {  
            ...
            if(status == "STARTED"|| status == "IN_PROGRESS")
            {
                Debug.Log("Speech-to-text service is running");
                isSpeechToTextRunning = true;
            }
            else
            {
                isSpeechToTextRunning = false;
                speechToTextTaskId = "";
                Debug.Log("Speech to text service is not running: " + status + "");
            }
        }
        response.Close();
    }
...
1. **Stop the task**
    ...
    To [stop speech to text](#stop-speech-to-text), you send a `DELETE` request with the, `appId`, `taskId` and `builderToken` in the URL. In your script file, add the following method to `NewBehaviourScript`:
...
1. **Enable a user to start, query and stop a task**
    ...
    In this example, you use two buttons to manage a speech-to-text task. When a user presses the speech-to-text button, the <Vpl k="CLIENT" /> calls `startSpeechToText` and starts a speech-to-text task using the `builderToken`. If a task is already running, the <Vpl k = "CLIENT"/> stops the task. When the user presses the query button, the <Vpl k="CLIENT"/> calls `querySpeechToText` and informs you of the current status of the speech-to-text task. To implement this logic, in your script file, add the following at the end of `SetupUI`:

    ```csharp
    go = GameObject.Find("speechToText");
    go.GetComponent<Button>().onClick.AddListener(startSpeechToText);
    go = GameObject.Find("querySpeechToText");
    go.GetComponent<Button>().onClick.AddListener(querySpeechToText);
    ```
</PlatformWrapper>
