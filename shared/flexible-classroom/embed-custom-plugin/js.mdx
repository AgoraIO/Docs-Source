...
<Vg k="FC"/> provides a plugin mechanism to help developers expand classroom capabilities under various scenarios. This mechanism also reduces the degree of code coupling between the custom application and the classroom, thereby reducing the difficulty of subsequent upgrading of the integrated source code. See [Plugin technology principles](../reference/plugin-technology-principles).
