...
- **Choose an audio codec to optimize bit rate and quality**
  ...
  In audio and video streaming, the choice of an encoding algorithm affects both quality and bit rate. Your goal is to lower the required bit rate while maintaining the quality at the desired level. <Vpd k="SDK" /> offers the following built-in audio codecs to choose from:
  ...
  You specify an audio codec when you join a channel. You also set the sampling rate and the number of channels at this time, depending on your audio quality requirements. If your <Vpl k="CLIENT"/> needs a custom encoding algorithm, you disable use of a built-in codec and implement your own. 

- **Adjust target bit rate in real time**

  In order to optimize data transmission and avoid network congestion, <Vpd k="SDK"/> suggests adjusting the sending bit rate in real-time according to changes in network conditions.
...
- **Change audio and video streaming status**
  ...
  Network traffic can be reduced by suspending data transmission when the audio or video feed is not being rendered by the receiver. After a user successfully connects to a channel and starts audio and video streaming, they can suspend sending streams to a specific connection or to all connections to flexibly manage the transmission status of audio and video streams. Similarly, the user may suspend receiving specific or all streams as required.

  When a user changes the transmission status of the local audio or video stream, the <Vpd k="SDK"/> triggers a corresponding callback to prompt the remote user to suspend or resume sending the audio or video stream.
...
![Ensure Channel Quality](/images/media-accelerator/media-accelerator-channel-quality.svg)
