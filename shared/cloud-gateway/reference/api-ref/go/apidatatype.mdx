...
|`RenderTimeMs`|Timestamp of the external audio frame. This parameter is required. You can use this timestamp to restore the order of audio frames; in scenarios with video (including those using external video sources), this parameter helps achieve audio-video synchronization.|
...
Server access regions. This function is an advanced setting suitable for scenarios with access security restrictions. The area code supports bitwise operations. After specifying the access region, the SDK will connect to the Agora servers within the specified region.
