...
- On Safari 16.x, screen sharing automatically changes the channel resolution to 1920 * 1080, even when the
  video profile is set to 1080 * 720. As 1920 * 1080 belongs to HD+, this may affect the billing calculation. Best
  practice is to check the [User Agent](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/User-Agent). If
  this returns version 16.x, suggest the user changes to a different browser.
