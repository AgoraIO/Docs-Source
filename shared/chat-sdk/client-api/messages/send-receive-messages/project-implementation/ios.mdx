...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    if ((currentTimestamp - _previousChangedTimeStamp) > 5) {
        // Sends a CMD message of start typing.
...
- (void)startReceiveCountDown
{
    if (_receiveTypingCountDownNum == 0) {
        [self stopReceiveTypingTimer];
        // After 10s, sends a CMD message of end typing.
        [self _sendEndTyping]
        return;
    }
...
- (void)_sendEndTyping
{
    AgoraChatCmdMessageBody *body = [[AgoraChatCmdMessageBody alloc] initWithAction:MSG_TYPING_END];
    body.isDeliverOnlineOnly = YES;
    AgoraChatMessage *message = [[AgoraChatMessage alloc] initWithConversationID:conversationId body:body ext:nil];
    message.chatType = AgoraChatTypeChat;
    [[AgoraChatClient sharedClient].chatManager sendMessage:message progress:nil completion:nil];
...
The following code example shows how to accept and parse a CMD message of the input status.
...
- (void)cmdMessagesDidReceive:(NSArray *)aCmdMessages
{
    ...
    for (AgoraChatMessage *message in aCmdMessages) {
        ...
        // Receives a CMD message of ending typing.
        if ([body.action isEqualToString:MSG_TYPING_END]) {
            // Updates the UI.
        }
    }
}
