...
1. **Set encryption parameters**
    ...
    You specify the media stream encryption method and the related parameters in JSON format by calling `setParams`. Depending on your choice of an encryption method, add **one** of the following pieces of code to `joinChannel` before `agoraEngine.joinChannel(...)`:
