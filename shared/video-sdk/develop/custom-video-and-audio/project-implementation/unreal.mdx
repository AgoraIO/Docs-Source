...
1. **Add a raw video file to the project**
    ...
    In this example, you use a video file as the source of your custom video data. To add the video file to your Unreal project, create a folder called `Video` in the `<Your_project_folder>/Content` directory and add a sample video file in `*.mp4` format to this folder. Update the value of the `VIDEO_FILE` variable to show the audio file name.
...
1. **Setup a method to push video frames**
    ...
    4. Creates an external video frame and pass the video file data to the frame buffer.
    5. Updates the channel media options and publishes the custom video track.
    5. Calls `pushVideoFrame` and push the external video frame in the channel.

    To implement logic, do the following:
...
1. **Add a raw audio file to the project**
    ...
    In this example, you use an audio file as the source of your custom audio data. To add the audio file to your Unreal project, create a folder called `Audio` in the `<Your_project_folder>/Content` directory and add a [sample audio file](\files\applause.wav) in `*.wav` or `*.raw` format to this folder. Update the value of the `AUDIO_FILE` variable to show the audio file name.
