...
Data insights in <Vpd k="NAME" /> offer periodic analysis of call usage and quality data. It helps you understand the overall usage and quality trends, map the distribution across different dimensions, and provides detailed data for each day, hour, and minute.

The regular version of Data Studio currently supports query analysis of time series graphs and one-dimensional queries. In comparison, Data Insights Plus offers additional capabilities, including multi-dimensional cross analysis, sampling analysis, comparative analysis, and extended querying of RTE service indicators. For a detailed comparison of the Standard Edition and the Plus Edition, refer to [pricing](../../reference/pricing).
...
1. Enable <Vpd k= "NAME" />  
    To see the available packages visit the [pricing](https://analytics-lab.agora.io/pricing) page, choose a package and press **Subscribe**.

1. To open the Data Insights Plus page, log in to <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link>, click **<Vpd k= "NAME" />** in the left menu bar, and then select **Data Insights** in the left menu of the page.

1. Select metrics to analyze

    The indicator selection area is located on the left side of the Data Insights page. You can click the button in the upper left corner of the page to expand the indicator selection area. Indicator selection is divided into Core indicators and All indicators:

    * The seven [Core indicators](#core-indicators), commonly used in business, are displayed under **Core indicators**. To the right of the indicator is the thumbnail time series diagram of the corresponding indicator. The default time range is 'last week'. Hovering the mouse over any point on the thumbnail displays the corresponding time point and indicator value.

    * All indicators that can be analyzed are listed under **All Indicators**. You can choose according to the product and your requirements. For an explanation of indicators, other than the core indicators, see [Other indicators](#other-indicators).
...
1. Start the analysis
    ...
    After selecting the indicators to be analyzed and setting the global filter conditions, you can view the indicator data in the analysis area on the right and start the analysis. You can click the collapse button in the upper right corner to collapse the indicator selection area so that you can focus on the analysis.
...
* **Line chart interaction**
    ...
    You can interact with the line chart in the following ways:
...
You can find the comparative analysis button in the upper right corner of the analysis area, click and select the desired analysis method to start the comparative analysis. Comparative analysis supports the following methods: 
...
Time comparison is utilized to compare the data performance and change trend of the same indicator across different time periods. The time comparison method supports the following selection modes:
