...
1. **Update the UI after the effect finishes playing**
    ...
    When <Vg k="SDK" /> has finished playing the sound effect, the `OnAudioEffectFinished` event is fired. To handle this event, in your script file, add the following to `UserEventHandler`:
