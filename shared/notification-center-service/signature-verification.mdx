...
For demo, this sample code uses `crypto/sha1`. 

To add signature verification to your server, take the following steps:

1. To verify signature in the <Vg k="NCS_LONG" /> callback, in the `main.go` file, replace your imports with with the following:
...
1. To verify the signature, in `the main.go`, add the following code after the list of imports:
...
1. In the `main.go` file, add the following code after `fmt.Println("Request Body:", string(body))`:
...
1. Once you receive an event from the console, and if the signature matches, the event details will be displayed in your browser.
