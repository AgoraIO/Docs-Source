...
1.  Open the project you created in [Get started](../get-started/get-started-sdk).
...
4.  Test your server
    ...
    1.  Ping the server
      ...
      2.  Retrieve a token
            ...
            ```
To see how to create a token generator inside your IAM system, see [Integrate a token generator](../develop/integrate-token-generation.xml).
...
2.  In your browser, navigate to the <Vg k="DEMO-PAGE-LINK" /> and update *App ID*, *Channel*, and *Token* with the values for your temporary token, then click **Join**.
<ProjectTest />

Your <Vpl k="PLATFORM" /> <Vpl k="CLIENT" /> magically connects to the same channel you used in <Vg k="DEMO-PAGE-LINK" />. You don’t have to hardcode a token in your app; each channel is secured with a specific token, and each token is refreshed automatically. That’s pretty Cool!
