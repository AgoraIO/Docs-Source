...
In the <Vg k="COMPANY" /> <Vg k="VSDK" />, a user ID identifies a user in the channel. The user ID is a 32-bit signed integer, with a value range from -2<sup>31</sup> to 2<sup>31</sup>-1, that you can specify yourself. If you specify `0` for the user ID when joining a channel, the SDK generates a random number and returns the value in the join channel success callback. 

In the <Vg k="COMPANY" /> <Vg k="SIG" /> SDK, a user ID identifiers a user in <Vg k="SIG" />. 

The user ID in the <Vg k="COMPANY" /> <Vg k="VSDK" /> and the <Vg k="COMPANY" /> <Vg k="SIG" /> SDK are independent of each other.
...
An SDK developed by <Vg k="COMPANY" /> to enable developers to add real-time audio interaction to their projects.
