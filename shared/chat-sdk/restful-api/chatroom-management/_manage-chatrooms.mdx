...
| `members` | JSONArray | The array of user IDs of regular <Vg k="CHAT_ROOM"/> members and administrators, excluding the <Vg k="CHAT_ROOM"/> owner. If you specify this parameter, remember to pass in at least one user ID. The number of user IDs in the array cannot exceed the value of `maxusers`. Note that these users always stay in the chat room if they have never logged in to Chat.| No |
