...
Starting from version 1.3.0, you can use the `fetchJoinedGroupCount` method to retrieve the number of groups the current user has joined from the server. The maximum number of groups a single user can join depends on the instant messaging package subscribed to, as detailed in the [product pricing](/agora-chat/reference/pricing-plan-details#group).
