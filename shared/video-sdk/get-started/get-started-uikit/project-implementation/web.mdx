...
Open the file `<app-name>/src/App.ts` and add `AgoraUIKit` after the other imports:
...
To set the variables for this logic, in `<app-name>/src/App.ts`, replace the code in `App()` the <Vg k="UIK" /> variables:
...
Add the callbacks to `<app-name>/src/App.ts` after the `rtcProps` object:
