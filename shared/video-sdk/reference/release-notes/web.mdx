...
#### Green screen when modifying the video resolution

After using `createScreenVideoTrack` to create a screen track on macOS Ventura 13.x (Safari 16.x), calling `setEncoderConfiguration` to modify the video resolution of this track might result in a green screen. This issue is caused by a functional regression in WebKit. See [Bug 254652](https://bugs.webkit.org/show_bug.cgi?id=254652) for details. The solution is to upgrade to Ventura 13.3 (Safari 16.4).

### v4.17.1

v4.17.1 was released on April 12, 2023.
...
- You can now specify the resolution and frame rate when calling `AgoraRTC.createCustomVideoTrack` to create a custom video track.
- After creating a screen track or a custom video track, you can dynamically adjust the video profile using `LocalVideoTrack.setEncoderConfiguration`. Please note that there is a known issue with calling this method on a screen track in certain versions of macOS Safari. See [known issues](#known-issues).
...
- When you have two local `AgoraRTCClient` objects, you can choose to block some callbacks from one `AgoraRTCClient` to another, including callbacks triggered when joining or leaving a channel, and publishing or unpublishing streams. Take screen sharing as an example: If Client A is used for publishing the screen track and Client B is used for publishing the camera track, then Client A and B will not receive the following callbacks from each other:
