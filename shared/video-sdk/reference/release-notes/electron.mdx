...
This release has changed the SDK package name from `electron-agora-rtc-ng` to `agora-electron-sdk`, and optimized the implementation of some features, resulting in incompatibility with v3.7.x. The following are the main features with compatibility changes:
