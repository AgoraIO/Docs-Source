...
and transcribes it into written words in real time. Agora charges for the time that Real-Time Transcription is enabled in a channel,
which includes transcription for one host. Transcription is available for up to 3 hosts speaking at the same time.
If several hosts speak simultaneously, Agora also charges for the time of speaking of each additional host using
the same pricing. Also note that when you enable RTT, the SD-RTN creates a cloud audience member who subscribes to the audio in the channel.
...
Contract sales@agora.io to get a discount.
