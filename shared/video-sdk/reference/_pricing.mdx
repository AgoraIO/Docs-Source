...
Typically, about 5% to 10% of the audio and video traffic may require [<Vg k="CP" />](../core-functionality/cloud-proxy). For <Vg k="CP" />, automatic mode is free of charge. Agora offers Force UDP and Force TCP cloud proxy modes with tiered capacity.
...
Media relay allows streaming from one source channel to multiple target channels at the same time. For details, see [Cross channel media relay](../core-functionality/cross-channel-media-relay).
