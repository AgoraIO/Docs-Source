...
As of v4.5.0, both Video SDK and Signaling SDK (v2.2.0 and above) include the `aosl.dll` library. If you manually integrate Video SDK via CDN and also use Signaling SDK, delete the earlier version of the `aosl.dll` library to avoid conflicts. The `aosl.dll` library version in Video SDK v4.5.1 is xxx. You can check the version by viewing the `aosl.dll` file properties.
</Admonition>
