...
public class HelloWorld : MonoBehaviour
{
    ...
    public async void Initialize()
    {
        ...
        try
        {
            ...
            ShowMessage("RTM Client Initialized Successfully");
            // Set unnecessary components as inactive
        ...
        if (rtmClient != null)
        {
            ...
            var (status,response) = await rtmClient.LoginAsync(appId);
            if (status.Error)
            {
            ...
            var (status2,response2) = await rtmClient.SubscribeAsync(msChannelName, options);
            if (status2.Error)
            {
    ...
    private void OnPresenceEvent(PresenceEvent eve)
    {
        ...
        switch (eventType)
        {
            ...
            case RTM_PRESENCE_EVENT_TYPE.REMOTE_JOIN:
                ...
                break;
            case RTM_PRESENCE_EVENT_TYPE.REMOTE_LEAVEL:
    ...
    public async void PublishBinaryMessage()
    {
        ...
        if (streamChannel != null)
        {
            ...
            options.sendTs = 0;
            var (status,response) = await streamChannel.PublishTopicMessageAsync(topicName, byteMsg, options);
            if (status.Error)
            {
                ShowMessage(string.Format("{0} is failed", status.Operation));
                ShowMessage(string.Format("The error code is {0}, due to: {1}",status.ErrorCode, status.Reason));
    ...
    public async void JoinTopic()
    {
        ...
        if (streamChannel != null)
        {
            ...
            var (status,response) = await streamChannel.JoinTopicAsync(topicName, options);
            if (status.Error)
            {
                ShowMessage(string.Format("{0} is failed", status.Operation));
                ShowMessage(string.Format("The error code is {0}, due to: {1}",status.ErrorCode, status.Reason));
    ...
    public async void LeaveTopic()
    {
        ...
        if (streamChannel != null)
        {
            var (status,response) = await streamChannel.LeaveTopicAsync(topicName);
            if (status.Error)
            {
                ShowMessage(string.Format("{0} is failed", status.Operation));
                ShowMessage(string.Format("The error code is {0}, due to: {1}",status.ErrorCode, status.Reason));
    ...
    private async void OnDestroy()
    {
        ...
        if (rtmClient != null)
        {
            var (status2,response2) = await rtmClient.UnsubscribeAsync(msChannelName);
            if (status2.Error)
            {
...
if (rtmClient != null)
{
    var (status3,response3) = await rtmClient.UnsubscribeAsync(msChannelName);
    if (status3.Error)
    {
        Debug.Log(string.Format("{0} failed", status3.Operation));
