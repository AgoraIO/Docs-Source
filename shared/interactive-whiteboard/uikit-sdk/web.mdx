...
After successfully launching a file conversion task and calling the [Query file conversion progress (GET) API](https://docs.agora.io/en/whiteboard/whiteboard_file_conversion#query-file-conversion-progress-get) to get the file conversion result, you can call this method and pass in the obtained file conversion result. After a successful call, the SDK automatically creates a sub-window to insert and display the converted document per page.

**Parameters**

- `title`: The title of the sub-window.
- `response`: The response body returned when the call of the [Query file conversion progress (GET) API](https://docs.agora.io/en/whiteboard/whiteboard_file_conversion#query-file-conversion-progress-get) succeeds.
...
After successfully [launching a file conversion task](https://docs.agora.io/en/whiteboard/whiteboard_file_conversion#start-file-conversion-post), you can call this method and pass in the parameters of the converted file. After a successful call, the SDK automatically creates a sub-window to insert and display the converted file per page.
...
- `sdkConfig`: (Required) Object. Configurations for the [`WhiteWebSdk`](https://docs.agora.io/en/whiteboard/API%20Reference/whiteboard_web/globals.html#whitewebsdkconfiguration) object, in which you must pass the following parameters:
  ...
  - `region`: The data center, which must be the same as the data center you chose when [creating the whiteboard room](../reference/whiteboard-api/room-management#create-a-room-post). The supported values are as follows:
...
- `scenes`: SceneDefinition[]. The content to be displayed on the whiteboard scene, that is, the converted file. The `SceneDefinition[]` consists of multiple `SceneDefinition` objects. Each `SceneDefinition` object represents a page of file and must contain the following parameters:
  ...
  - `src`: String. The URL address of each page of file, which you can get from the field in the response body when the [Query file conversion progress (GET)](https://docs.agora.io/en/whiteboard/whiteboard_file_conversion#query-file-conversion-progress-get) API call succeeds.
...
- `taskId`: String. The UUID of the file conversion task. You can get `uuid` from the response body when the [Start file conversion (POST) API](https://docs.agora.io/en/whiteboard/whiteboard_file_conversion#start-file-conversion-post) call succeeds.
- `title`: (Optional) String. The title of the sub-window.
