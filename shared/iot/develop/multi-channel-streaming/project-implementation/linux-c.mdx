...
Now this section presents code snippets from the sample project that you can reuse in your own project to integrate <Vpd k="SDK"/>.

Open the file `<extracted-folder>/agora_rtsa_sdk/example/hello_rtsa/hello_rtsa_multi.c` in a code editor and examine the code. Reuse the following code blocks from this project to implement the desired functionality in your own project:

1. **Initialize the SDK, verify license, and stream(connection) count**
...
1. **Create multiple connections**
    ...
    When the <Vpl k="CLIENT"/> starts, you create multiple connections:
...
1. **Stream  audio and video data**
    ...
    You create tasks(thread) for sending video and audio frames
