...
* **Message notifications that are redundant**. You receive multiple notifications because the Agora NCS server can send more than one notification callback for each channel event to ensure reliability of the service.
* **Message notifications that arrive out of order**. You receive notifications out of order as the callbacks may not arrive at your server in the same order as event occurrence due to network issues.

To accurately maintain the online status of users, your server needs to be able to deal with redundant notifications and handle received notifications in the same order as events occur. This page shows you how to use channel event callbacks to accomplish this.
