...
| 100    | `INVALID_APP_KEY`                 | The App Key is invalid. For how to get the App Key, see [Get the information of the <Vg k="COMPANY"/> <Vg k="CHAT"/> project](https://docs.agora.io/en/agora-chat/get-started/enable?platform=android#get-chat-project-information).  |
...
| 201    | `USER_NOT_LOGIN`                  | The user hasn't logged in. For example, the login session is either invalid or has expired when a user requests to send a message or perform operations to a <Vg k="CHAT_GROUP_LOWER"/>. |
...
| 217    | `USER_KICKED_BY_OTHER_DEVICE`     | In a multi-device login use-case, the user gets kicked off from a device in on the <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link> or by calling an API on another device. |
...
| 507    | `MESSAGE_ILLEGAL_WHITELIST`       | The user is not on the allow list of a group or <Vg k="CHAT_ROOM_LOWER"/>. This error occurs when a user that is not on the allow list attempts to send a message in a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/> if all members of the group or <Vg k="CHAT_ROOM_LOWER"/> are muted. |
...
| 600    | `GROUP_INVALID_ID`                |  Invalid group ID. For a group-related API, the <Vg k="CHAT_GROUP_LOWER"/> ID is an empty string or invalid.       |
...
| 602    | `GROUP_NOT_JOINED`                | The user hasn't joined this <Vg k="CHAT_GROUP_LOWER"/>. This error occurs when a user attempts to send a message or performs operations in a <Vg k="CHAT_GROUP_LOWER"/> that he or she is not in.  |
| 603    | `GROUP_PERMISSION_DENIED`         | The user does not have the permission to perform the operation to a <Vg k="CHAT_GROUP_LOWER"/>. For example, a <Vg k="CHAT_GROUP_LOWER"/> member does not have permission to add or remove a <Vg k="CHAT_GROUP_LOWER"/> admin.  |
| 604    | `GROUP_MEMBERS_FULL`              | The number of members in the <Vg k="CHAT_GROUP_LOWER"/> has reached the upper limit specified during group creation.  |
| 605    | `GROUP_SHARED_FILE_INVALIDID`     |  The ID of the shared file in the <Vg k="CHAT_GROUP_LOWER"/> is invalid.               |
| 606    | `GROUP_NOT_EXIST`                 |  The <Vg k="CHAT_GROUP_LOWER"/> to which the user is requesting to perform operations does not exist.  |
| 607    | `GROUP_DISABLED`       | The <Vg k="CHAT_GROUP_LOWER"/> is disabled.       |
| 608    | `GROUP_NAME_VIOLATION`        | The <Vg k="CHAT_GROUP_LOWER"/> name is invalid. |
| 609    | `GROUP_MEMBER_ATTRIBUTES_REACH_LIMIT`   | The number of custom attributes has reached the upper limit for a <Vg k="CHAT_GROUP_LOWER"/> member. |
| 610    | `GROUP_MEMBER_ATTRIBUTES_UPDATE_FAILED` | Fails to set custom attributes for a <Vg k="CHAT_GROUP_LOWER"/> member.     |
| 611    | `GROUP_MEMBER_ATTRIBUTES_KEY_REACH_LIMIT`   | The key of a custom attribute for a <Vg k="CHAT_GROUP_LOWER"/> member exceeds the limit of 16 bytes. |
...
| 700    | `CHATROOM_INVALID_ID`             | Invalid <Vg k="CHAT_ROOM_LOWER"/> ID. This error occurs when the <Vg k="CHAT_ROOM_LOWER"/> ID is an empty string for the <Vg k="CHAT_ROOM_LOWER"/>-related method that is used.        |
| 701    | `CHATROOM_ALREADY_JOINED`         | The user is already in the <Vg k="CHAT_ROOM_LOWER"/>. This error occurs when a user attempts to join a <Vg k="CHAT_ROOM_LOWER"/> that he or she is already in. |
| 702    | `CHATROOM_NOT_JOINED`             | The user hasn't joined this <Vg k="CHAT_ROOM_LOWER"/>. This error occurs when a user attempts to send a message or perform operations in a <Vg k="CHAT_ROOM_LOWER"/> that he or she is not in.  |
| 703    | `CHATROOM_PERMISSION_DENIED`      | The user does not have the permission to perform the operation to a <Vg k="CHAT_ROOM_LOWER"/>. For example, a <Vg k="CHAT_ROOM_LOWER"/> member does not have permission to add or remove a <Vg k="CHAT_ROOM_LOWER"/> admin.   |
...
| 705    | `CHATROOM_NOT_EXIST`              | The <Vg k="CHAT_ROOM_LOWER"/> to which the user is requesting to perform operations does not exist. |
...
| 1302   | `REACTION_OPERATION_IS_ILLEGAL`         | The user does not have permission to operate on this Reaction. For example, a user that does not add this Reaction attempts to delete it or a user that is neither the message sender nor recipient tries to add a Reaction in a <Vg k="CHAT_ONE_LOWER"/> chat.  |
