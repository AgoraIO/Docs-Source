...
<Admonition>
Tp integrate <Vpd k="SDK" /> version 2.2.0 or above, and Video SDK version 4.3.0 or above at the same time, refer to the [FAQ](/help/integration-issues/rtm2_rtc_integration_issue) to handle integration issues.
</Admonition>
...
- Input boxes for user name, channel name, and message
...
{`<resources>
    ...
    <string name="app_id">add_your_appid_here</string>
    <string name="token">add_your_token_here</string>
</resources>`}
</CodeBlock>

</details>

<Admonition title="Note" type="caution">Remember to replace the values for `app_id` and `token` in `strings.xml` with your App ID and token from <Vg k="CONSOLE" />. </Admonition>
