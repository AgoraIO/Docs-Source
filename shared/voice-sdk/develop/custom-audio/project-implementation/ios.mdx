...
2.  **Add a raw audio file to the project**
...
3.  **Enable publishing the custom audio track**
...
4.  **Open the audio file**
    ...
    Suppose you want the audio to play when the User joins the channel. When the User clicks join, you open the audio file and push the data to the SDK. To do this, add the following lines after `joined = true` in the `joinChannel()` method:
