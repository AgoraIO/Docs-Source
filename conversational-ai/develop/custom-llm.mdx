...
In conversational AI interaction scenarios, your particular use-case may require to use a custom large language model (Custom LLM). This document explains how to integrate a custom LLM into Agora's Conversational AI Engine.

## Understand the tech

Agora's <Vpd k="NAME" /> interacts with LLM services using the OpenAI API protocol. The core of integrating a custom LLM is to provide an HTTP service compatible with the OpenAI API, capable of receiving and returning requests and responses in the OpenAI API format.
...
- **Multi-modal Capabilities**: Enables the model to generate output in both text and audio formats.
...
- Access to a custom LLM service.
- If using Retrieval-Augmented Generation (RAG), prepared a vector database or retrieval system.

## Implementation

### Create an OpenAI API-compatible service

To successfully integrate with Agora's <Vpd k="NAME" />, your custom LLM service must provide an interface compatible with the OpenAI Chat Completions API. Key requirements include:

- **Endpoint**: Provide a request-handling endpoint, such as https://your-custom-llm-service/chat/completions.

- **Request format**: Accept request parameters that adhere to the OpenAI API protocol.

- **Response format**: Return responses that are both OpenAI API-compatible and follow the Server-Sent Events (SSE) standard for streaming.
...
When calling POST to create the conversational AI agent, point the LLM configuration to your custom service:

<Admonition type="info">
If accessing your custom large model service requires identity verification, enter your authentication information in the `api_key` field.
</Admonition>
...
### Implementing search-enhanced generation

To improve the accuracy and relevance of the agent's responses, use the Retrieval-Augmented Generation (RAG) feature. This features allows your custom LLM to retrieve information from a specific knowledge base and provide the retrieved results as context for generating responses.

The following example simulates the process of retrieving and returning content from a knowledge base and creates the `/rag/chat/completions` endpoint to use RAG retrieval results for generating responses with the LLM.
...
When calling POST to create the conversational agent, simply point the LLM URL to your RAG interface:
...
Agora's conversational AI engine supports LLMs in generating outputs in multi-modal forms (text and audio). You can create dedicated multi-modal interfaces to meet more personalized requirements.

The following example demonstrates the process of reading text and audio files and sending them to an LLM to generate an audio response.
...
When calling POST to create a conversational agent, refer to the following configuration:
...
Agora provides an [open source sample project](https://github.com/Shengwang-Community/Conversational-AI-Server-Sample) on GitHub for your reference. Download the project or view the source code for a more detailed example.

### Interface standards

Custom LLM services must be compatible with the OpenAI Chat Completions API interface standard:

- **Request format**: contains parameters such as model, message, tool call configuration, etc.
...
- [Agora Conversational AI Engine API Documentation](../rest-api/join)