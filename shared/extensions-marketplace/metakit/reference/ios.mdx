...
To implement the capabilities of the `<Vg k="RTEE_MK"/>` extension, use the <Link to="{{Global.API_REF_IOS_ROOT_RTC_KIT}}/setextensionpropertywithvendor(_:extension:extensioninfo:key:value:)">setExtensionPropertyWithVendor</Link> method provided by Agora SDK v4.x. Pass in the specified `key` and `value` as follows:
...
The <Vg k="RTEE_MK"/> extension allows you to switch the image, viewpoint, face, and outfit of the avatar. To experience the avatar-related functions, you need to set `avatarMode` to `0` when [adding a scene view](#Adding a scene view).
...
The <Vg k="RTEE_MK"/> extension allows you to switch the image of Animoji. To experience Animoji related functions, you need to set `avatarMode` to `1` when [adding a scene view](#Adding a scene view).
...
The <Vg k="RTEE_MK"/> extension provides lighting effects such as 3D lighting, screen ripples, aurora effects, and portrait edge flames, and supports fine-tuning the color, intensity, range and other parameters of the lighting effects. To experience the lighting effect related functions, you need to set `backgroundEffect` to `true` when [adding a scene view](#adding a scene view).
...
This article introduces the virtual human face-pinching resources provided by the <Vg k="RTEE_MK"/> extension.
