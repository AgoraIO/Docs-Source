...
1. **Switch stream(video) quality when the user taps the remote video**
    ...
    To automatically switch to lower-quality video or audio-only mode when network conditions are poor, call <Link to="{{global.API_REF_CPP_ROOT}}/class_irtcengine.html#api_irtcengine_setremotesubscribefallbackoption">`setRemoteSubscribeFallbackOption`</Link> to set the desired fallback. The SDK triggers the <Link to="{{global.API_REF_CPP_ROOT}}/class_irtcengineeventhandler.html#callback_irtcengineeventhandler_onremotesubscribefallbacktoaudioonly">`onRemoteSubscribeFallbackToAudioOnly`</Link> callback when switching back and forth to audio-only mode due to network conditions.
...
1. **Listen to <Vg k="ENGINE" /> events to receive state change notifications and quality statistics**
    ...
    * `onRemoteVideoStats`: Receives stats about the remote videos.
    ...
    1. In `call_quality.cpp`, define these events actions:
        ...
        ```cpp
        LRESULT CallQuality::OnEIDRemoteVideoStateChanged(WPARAM Uid, 
                                                          LPARAM NewState) {
        ...
        LRESULT CallQuality::OnEIDRemoteVideoStats(WPARAM uid, 
                                                   LPARAM receivedBitrate) {
            // Display the callback result.
            wchar_t a[256];
        ...
        LRESULT CallQuality::OnEIDLastMileQuality(WPARAM quality, 
                                                  LPARAM lParam) {
            // Display the callback result.
        ...
        LRESULT CallQuality::OnEIDNetworkQuality(WPARAM txQuality, 
                                                 LPARAM rxQuality) {
            // Display the callback result.
        ...
        LRESULT CallQuality::OnEIDRtcStats(WPARAM userCount, 
                                           LPARAM packetLoss) {
            // Display the callback result.
            wchar_t a[256];
        ...
        LRESULT CallQuality::OnEIDLastMileProbeResult(WPARAM jitter, 
                                                      LPARAM lParam) {
            // Display the callback result.
            wchar_t a[256];
