...
<div class="alert note">By default, if a user logs in and out several times in one second, the <Vg k="CHAT"/> server sends the presence status change notification to the SDK only after the last login or logout of the user.</div>
