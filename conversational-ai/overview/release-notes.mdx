...
* **Custom ASR provider support**
    ...
    **Billing update:**  
    In earlier versions, the service fee included the cost of the Agora ASR provider. Starting in v1.6, the pricing is restructured as follows:

    - If you use **Agora ASR**, the total price remains unchanged:  
        _**Total cost = Conversational AI Engine Audio Basic Task + Agora ASR fee**_
...
Agora now offers a set of modular client component to help you quickly build conversational agent apps. These components are available for **iOS**, **Android**, and **Web**, and include APIs for common scenarios. Call these APIs to combine the capabilities of the Agora Video SDK and Signaling SDK to achieve the following functions:
...
* **Voice interruption mode**
    ...
    This release adds the `parameters.interruptable` parameter to the [Start a conversational AI agent](../rest-api/join) API, allowing you to control how the agent handles human voice interruptions. The following modes are supported:
