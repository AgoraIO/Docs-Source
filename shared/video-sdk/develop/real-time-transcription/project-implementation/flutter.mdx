...
1. **Integrate a `HTTP` client library into your project**
    ...
    To call `HTTP` methods that start, stop, and query a real-time transcription task, you add a `HTTP` client library to your project. Open `pubspec.yaml` and add the following line under `dependencies:`
...
1. **Query task status**
    ...
    You [query the status](#query-the-status-of-real-time-transcription) of a real-time transcription task using a `GET` request to notify the user of any change. The request URL contains the `taskId` and `builderToken`. To do this, add the following method to the `_MyAppState`class:
...
1. **Stop the task**
    ...
    To [stop real-time transcription](#stop-real-time-transcription), you send a `HTTP DELETE` request with the, `appId`, `taskId` and `builderToken` in the URL. In the `_MyAppState` class, add the following method:
    ...
    Future<void> stopRealTimeTranscription() async {
        ...
        if (response.statusCode == 200) {
            ...
            Map<String, dynamic> json = jsonDecode(response.body);
            String message = json["message"];
            showMessage("Exception stopping real-time transcription-to-text task: $message");
        }
    }
    ```
...
1. **Receive and display the text stream**
      ...
      When you set one of the `destinations` in the starting configuration as `AgoraRTCDataStream`, the real-time transcription-to-text conversion output stream is pushed to your channel  and you receive it using `onStreamMessage`.
...
1. **Enable a user to start, query and stop a task**
    ...
    In this example, you use buttons to manage a real-time transcription-to-text task.

        1. When a user presses the first button, the <Vpl k="CLIENT" /> requests a `builderToken`. If the request is successful, the <Vpl k="CLIENT" /> starts a real-time transcription-to-text task. If a task is already running, you stop the task. To implement this functionality, add the following method to the `_MyAppState` class.
