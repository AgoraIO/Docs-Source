...
sidebar_position: 10
description: >
  Configure a Twilio Elastic SIP Trunk for inbound and outbound calling with Agora Studio.
---

This guide walks you through configuring Twilio Elastic SIP Trunk to work with Agora Studio for both inbound calls (customers call your AI agent) and outbound calls (your AI agent calls customers).
...
Studio supports both inbound and outbound calls through Twilio Elastic SIP Trunk, but each direction requires different configuration in both Twilio and Studio. The table below summarizes the key differences.
...
The transport setting in Agora Studio must match Twilio’s Secure Trunking setting. A mismatch causes calls to fail silently.
</Admonition>
...
1. **Configure the Termination URI**
    ...
    1. This domain is required later in Agora Studio as the **SIP Domain**.
...
1. **Import the number in Agora Studio**
