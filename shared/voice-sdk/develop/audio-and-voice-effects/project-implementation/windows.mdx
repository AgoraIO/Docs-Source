...
To enable the user to play sound and voice effects and modify the audio route, add the following elements to the user interface:
    ...
    * A check box to turn the speakerphone on or off.
...
1. **Set up access to the speakerphone switch**
    ...
    ```cpp
    speakerphoneSwitch = (CButton*)GetDlgItem(IDC_CHECK1);
...
1. **Enable the user to start and stop audio mixing**
    ...
    When the user presses **Mix Audio**, the <Vg k = "SDK"/> fetches an audio file using `audioFilePath` and mixed it with the local audio stream. The remote and local users listen the audio file playing with the audio stream. To enable audio mixing in your <Vpl k = "CLIENT"/>, in **Dialog Editor**, double-click **Mix Audio**. **Dialog Editor** automatically creates and opens an event listener for you. Add the following code to the event listener you just created:
...
1. **Pre-load sound effects**
    ...
    To set up playing voice effects, call `preloadEffect` to pre-load the sound effects. In `setupVoiceSDKEngine`, add the following lines after `agoraEngine->initialize(context);`:
...
1. **Play, pause, or resume playing the sound effect**
    ...
    When a user presses **Audio Effect**, the sound effect is played. When they press the button again, the effect is paused and resumed alternately. To implement this workflow, in **Dialog Editor**, double-click **Audio Effect**. **Dialog Editor** automatically creates and opens an event listener for you. Add the following code to the event listener you just created:
