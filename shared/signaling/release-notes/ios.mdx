...
Starting from v2.2.0, the RTM SDK and RTC SDK (version 4.3.0 or later) both include the `aosl.xframework` library. If you use both the RTM and RTC SDKs, you need to perform the following operations to avoid conflicts:

- If you manually integrate the SDK through CDN, please manually delete the lower version `aosl.xframework` library;
- If you integrate the SDK using a dependency management tool (such as Maven or CocoaPods), please ensure that the higher version `aosl.xframework` library is included in your project.

For more information on integration, see [How to handle issues when integrating the RTM SDK and RTC SDK?](/faq/integration-issues/rtm2-rtc-integration-issue)

- 2.2.2 RTM SDK `aosl.xframework` library version: 1.0.11.
- 2.2.1 RTM SDK `aosl.xframework` library version: 1.0.11.
- 2.2.0 RTM SDK `aosl.xframework` library version: 1.0.0.17.
