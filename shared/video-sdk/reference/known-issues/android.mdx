...
Starting from v4.5.0, both <Vg k="VSDK" /> and <Vg k="SIG" /> SDK (v2.2.0 and above) include the `libaosl.so` library. If you manually integrate <Vpd k="SDK" /> via CDN and also use the Signaling SDK, delete the earlier version of the `libaosl.so` library to avoid conflicts. 
</Admonition>
