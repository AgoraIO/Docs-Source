...
1. **Log in to the <Vg k="COMPANY" /> <Vg k="CHAT" />** 
    ...
    When a user clicks **Join**, your <Vpl k="CLIENT"/> logs in to the <Vg k="COMPANY" /> <Vg k="CHAT" /> and when clicks **Leave**, the <Vpl k="CLIENT"/> logs out from the <Vg k="COMPANY" /> <Vg k="CHAT" />. To implement this logic, add the following method after `showLog()` in the `MainWindow` class:
