...
For the `appId` and `token` variables, replace the placeholders with the values you obtained from <Vg k="CONSOLE"/>. Ensure you enter the same `channelName` you used when generating the temporary token.
</Admonition>
