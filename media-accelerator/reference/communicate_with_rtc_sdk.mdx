...
If your business scenario requires not only high-quality audio and video transmission, but also easy integration, low memory usage, and low power consumption, such as smart cameras and smart doorbells, Agora recommends that you use the <Vg k="VSDK"/> (v4.x) with the <Vpd k="SDK" />.
...
The following sections describe how to set the audio and video data format in the SDKs.
...
## Set up <Vg k="VSDK"/> for Native/third-party frameworks (v4.x)
