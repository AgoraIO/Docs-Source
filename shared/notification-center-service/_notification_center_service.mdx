...
For <Vg k="NCS" />, a webhook is an endpoint on an HTTP server that handles these requests. In a production environment you write this in your web infrastructure, for development purposes best practice is to create a simple local server and use a service such as [ngrok](https://ngrok.com/download) to supply a public URL that you register with  <Vg k="AGORA_BACKEND" /> when you enable <Vg k="NCS" />. To do this, take the following steps:

1. **Setup a Java project for your server**

    In your IDE, create a new Java project and add the following code to `Main.java`:
      ...
      static class SimpleHttpServer {
        ...
        public void Start(int port) {
          try {
            ...
            System.out.println("NCS server started at port " + port);
...
1.  **Add a JSON library to your project**
    ...
    [Download](https://mvnrepository.com/artifact/com.googlecode.json-simple/json-simple/1.1.1) the `json-simple-1.1.1.jar` library and add it to your project. 

1. **Handle <Vg k="NCS" /> callbacks** 

    Create a new file named `Handlers.java` in your project folder containing the following code:
    ...
    public class Handlers {
      ...
      public static class NcsHandler implements HttpHandler {
        ...
        public void handle(HttpExchange he) throws IOException {
          ...
          long productId, clientSeq;
          int uid=0, eventType;

          // Parse JSON data
          try {
            JSONObject jsonObj = (JSONObject) new JSONParser().parse(query);
...
1. Create a new file in you project folder named `UserRegistry.java` containing the following code.
...
2. To update the user registry when you receive a <Vg k="NCS" /> callback, add the following code to the `handle` method in the `NcsHandler` class after `os.close();`:
