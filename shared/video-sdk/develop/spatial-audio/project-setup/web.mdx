...
1.	**Integrate the spatial audio plugin to your project** 

    To do this:
    
    1.	Install the spatial audio plugin by running the following command:

        ```bash
        npm install agora-extension-spatial-audio
        ```

    1.	Import the extension module. Add the following code to your `.js` file:
...
1. **Register the spatial audio effect plugin**
    ...
    Pass an `SpatialAudioExtension` instance to `AgoraRTC.registerExtensions`.
