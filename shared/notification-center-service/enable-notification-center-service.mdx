...
1. In **Notification Center Service Configuration**, fill in the following information, and click **Save**:
    ...
    * **Secret**: The secret used for signature verification. For instructions on how to use the secret for signature verification, see [Add signature verification](#add-signature-verification).
    
    * **Allowed IP List**: If your server is behind a firewall, ensure that you have called the [IP address query API](#ip-address-query-api) to get the IP addresses of the Agora NCS server and added them to the firewall's allowed IP list, and check the box here.
...
1. Agora performs a health test for your configuration as follows:
    ...
    1. When the NCS health test succeeds, read the prompt and press **Save NCS Configuration**. After your configuration is approved, the Status of Notification Center Service shows **Enabled**.
