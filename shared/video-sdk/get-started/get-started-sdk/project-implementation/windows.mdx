...
1. **Setup Agora Engine**
   ...
   When the user launches this application, you call `setupVideoSDKEngine` to create and initialize an instance of Video SDK. To setup an engine instance of Video SDK, do the following:
...
1. **View the remote user when they join a channel**
   ...
   When a remote user joins the channel, <Vg k = "VSDK"/> triggers `onUserJoined`. You catch this callback and retrieve the remote user ID and display the remote video. To Implement this workflow, in `AgoraImplementationDlg.cpp`, take the following steps:
    ...
    1. To retrieve the remote user `uid` and display the remote video, send a notification to the parent window. In `AgoraImplementationDlg.cpp`, add the following before `OnInitDialog`:
...
4. **Join the channel to start <Vpd k = "PRODUCT"/>**
   ...
   <ProductWrapper product = "video-calling">
    ...
    // Pass the local view window handle to canvas to render the local video.
    canvas.view = localView;
    // Select the source of local video.
    canvas.sourceType = VIDEO_SOURCE_CAMERA;
    // Start rendering the local video.
    agoraEngine->setupLocalVideo(canvas);
    // Preview the local video.
    agoraEngine->startPreview();
    // Disable local video capturer if the user role is Audience.
...
8. **Manage the user role**
    ...
    1.  When the user selects `Host`, you call `setClientRole` and `muteRemoteVideoStream` to switch the user role to `Host` and stop viewing the remote user stream. To implement this work flow, in **Dialog Editor**, double-click  **Host**, **Dialog Editor** automatically setup an event listener for `Host`. Add the following code to the listener you just created:
