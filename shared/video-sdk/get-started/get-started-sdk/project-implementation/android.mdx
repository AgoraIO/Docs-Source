...
To ensure that you receive all events from the moment the engine starts, set the `mEventHandler` property of the `RtcEngineConfig` to your event handler instance when [initializing the engine](#initialize-the-engine).
</Admonition>
...
<ProductWrapper notAllowed="voice-calling">
Set up two `FrameLayout` elements in your activity to display local and remote video streams. 
