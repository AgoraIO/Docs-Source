...
1.  **Define a variable to manage video processing**
        ...
        From **Solution Explorer**, open `AgoraImplementationDlg.cpp` and add the following variable after the list of header includes:
...
2.  **Start and stop video processing**
    ...
    When the user presses the **Zoom In** button, the current state of the `isZoomed` variable is updated. This variable controls the behavior of the `onCaptureVideoFrame` callback, either enabling or disabling frame cropping. To implement this workflow, double-click on the **Zoom In** button in the **Dialog Editor**, which creates an event listener for you. Add the following code to the event listener:
    ```cpp
...
<Vg k = "COMPANY"/> provides `IVideoFrameObserver` and `IAudioFrameObserver` interfaces for real-time video and audio processing. These interfaces allow developers to capture and process raw video and audio data before it's sent over the network for <Vpd k = "PRODUCT"/>. To implement video and audio frame observer interfaces `IVideoFrameObserver` and `IAudioFrameObserver`, do the following:

1.  **Implement the video frame observer**
    
    `IVideoFrameObserver` provides access to each local video frame after it is captured, as well as to each remote video frame before it is played back. In this example, the `onCaptureVideoFrame` callback is used to modify the captured video frame buffer for cropping and scaling, resulting in a modified version of the frame that is played back. To implement this workflow, in `AgoraImplementation.cpp`, add the following class:
    ```cpp
    ...
    ```
    Note that the function `getVideoFrameProcessMode()` needs to be overridden with `PROCESS_MODE_READ_WRITE` to ensure that any modifications made to the raw data take effect.
2. **Implement audio frame observer callbacks**
            
    `IAudioFrameObserver` provides access to each captured or played back audio frame, allowing you to process the audio frames according to your needs. To override `IAudioFrameObserver` callbacks, add the following to `AgoraImplementationDlg.cpp`:  
    ```cpp
...
2.  **Register the video and audio frame observers**
    ...
    1. To register the observers at startup, in `AgoraImplementationDlg.cpp`, add the following at the of `setupVideoSDKEngine()`:
...
3. **Unregister the video and audio frame observers when you close the <Vpl k="CLIENT"/>**
    ...
    When you quit the <Vpl k="CLIENT"/>, unregister the frame observers by disabling the `EnableAudioVideoCapture()` method with a `false` argument. To implement this workflow, in `AgoraImplementationDlg.cpp`, add the following to `OnClose` before `	agoraEngine->release();`:
