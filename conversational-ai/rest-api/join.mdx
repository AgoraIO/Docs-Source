...
<ParameterList title="BODY" required={true}>
  ...
  <Parameter name="properties" type="object" required={true}>
    ...
    <Parameter name="silence_timeout" type="integer" defaultValue="0" possibleValues="0 to 60" required={false}>
      <Admonition type="info">This field is deprecated; use `parameters.silence_config` instead.</Admonition>
      Specifies the maximum silent time of the agent, in seconds.

      After the agent is successfully created and the user joins the channel, any period during which the agent is not listening, thinking, or speaking is considered silent time. When the silent time reaches the configured value, the agent broadcasts the silent prompt message specified in `llm.silence_message`, and then restarts the silent time countdown.

      - `0` : Disables the silent prompt feature.
      - `(0, 60]`: Enables the silent prompt feature. You must also configure `llm.silence_message` to play the prompt message; otherwise, this setting does not take effect.
    </Parameter>
  ...
  <Parameter name="llm" type="object" required={true}>
    ...
    <Parameter name="silence_message" type="string" defaultValue="" required={false}>
      <Admonition type="info">This field is deprecated; use `parameters.silence_config` instead.</Admonition>
      Specifies the silent reminder message. Default is an empty string.

      After the agent is successfully created and the user joins the channel, any period during which the agent is not listening, thinking, or speaking is considered silent time. When the silent time reaches the value set in `silence_timeout`, the agent broadcasts the specified silent reminder message.

      This parameter is ignored if `silence_timeout` is set to `0`.
    </Parameter>
