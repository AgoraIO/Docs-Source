...
1.  **Define a variable to manage video processing**
    ...
    In `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelVideo.cs`, add the following declaration to the `JoinChannelVideo` class:
...
1. **Access the UI elements programmatically**
    ...
    To setup the event listener for the **ZoomButton**, in `JoinChannelVideo`, add the following at the end of `SetupUI` method:
...
1.  **Set up the audio frame observer**
    ...
    `IAudioFrameObserver` gives you access to each audio frame after it is captured or access to each audio frame before it is played back. To setup the `IAudioFrameObserver`, add the following code to the `JoinChannelVideo` class:
    ...
    internal class AudioFrameObserver : IAudioFrameObserver
    {
        ...
        internal AudioFrameObserver(JoinChannelVideo videoSample)
        {
...
2.  **Set up the video frame observer**
    ...
    internal class VideoFrameObserver : IVideoFrameObserver
    {
        ...
        internal VideoFrameObserver(JoinChannelVideo videoSample)
        {
            _videoSample = videoSample;
        }
...
4.  **Unregister the video and audio observers when you leave a channel**
    ...
    When you leave a channel, you unregister the frame observers by calling the unregister frame observer method. To do this, add the following lines to the `Leave` method before `RtcEngine.LeaveChannel();`:
...
5.  **Start and stop video processing**
    ...
    When a user presses the button, enable or disable video processing. To do this, add the following method to the `JoinChannelVideo` class:
