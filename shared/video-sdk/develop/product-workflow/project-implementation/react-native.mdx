...
* A `SurfaceView` to render the screensharing.
...
1. **Add the required libraries**
    ...
    To switch between publishing screen and camera streams, import the corresponding Agora libraries. In `App.tsx`, add the following code before `createAgoraRtcEngine,`:
    ...
    Remove the `IRtcEngine,` line from the imports.

1. **Change the IRtcEngine instance to IRtcEngineEx instance**

    To enable screensharing, repalce `const agoraEngineRef = useRef<IRtcEngine>();` in `const App = () => {}` to:

    ```ts
    const agoraEngineRef = useRef<IRtcEngineEx>();
    ```

    Also, repalce `agoraEngineRef.current = createAgoraRtcEngine();` in `setupVideoSDKEngine` to:
...
1. **Add the required variables**
   ...
   To screenshare, mute and unmute the remote user audio stream, in `App.tsx`, add the following variables to the declarations:
   ...
   Add the following state variable after the declarations in `const App = () => {}`:
...
1. **Start or stop screen sharing**
    ...
    - If the `startScreenCaptureBool` state variable is `false`, calls `startScreenCapture()` and shares your screen.
    - If the `startScreenCaptureBool` state variable is `true`, calls `stopScreenSharing()` and stops sharing the screen.
...
1. **Register the necessary callbacks**
    ...
    Register the `onLocalVideoStateChanged` callback to manage the `startScreenCaptureBool` state variable. Add the following code in `agoraEngine.registerEventHandler({})`:
