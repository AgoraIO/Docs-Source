...
With the 4.x release, SDK versions 1.x–3.x have entered the retirement phase. For details, see the [Service Sunset Plans](/on-premise-recording/reference/sunset).
</Admonition>
...
* **Record real-time audio and video** for all participants in a channel.
* **Multi-channel support**: Record participants from multiple channels at the same time.
* **Audio-only recording** for one or more channels.
* **Encrypted channel recording** using Agora SDK’s built-in encryption scheme.
