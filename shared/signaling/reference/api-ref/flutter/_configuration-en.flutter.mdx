...
| Parameters | Type | Required | Default | Description |
| :--------------: | :--------------------: | :------: | :----: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `token` | String | Required | - | The token used for logging into the Signaling system.<li>If your project enables token authentication, you can provide either the [Signaling temporary token](/signaling/get-started/authentication-workflow) or the Signaling token generated by your token server. See [User authentication](/signaling/get-started/authentication-workflow) and [Deploy Signaling token generator](/doc/rtm2/flutter/user-guide/token/token-generation).</li><li>If your project does not enable token authentication, you can enter an empty string or the App ID of a project that enables Signaling services.</li> |
...
You can refer to the `errorCode` field in the <a href="/signaling/reference/error-codes">Error Codes</a> to understand the cause of the error and find the corresponding solution.