...
1. Integrate the iOS plugin via CocoaPods:

    1. Unzip the downloaded iOS package and import all `.framework` files into the project.
    1. Add dependencies to the `.podspec` file as follows:
        ...
        ![](/images/extensions-marketplace/faceunity-ios-integration.png)

    1. Add the following lines to the `Podfile` file :
...
1. Save the required model and prop `.bundle` files from the Resource package to the project folder as follows:
...
1. Add necessary dependencies to the `pubspec.yaml` file.
    ...
    Make sure your reference all the required supporting libraries and assets as shown in the example below:
