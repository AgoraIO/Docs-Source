...
Before using Chat, you need to enable and configure <Vg k="CHAT" /> at [Agora Console](https://console.agora.io/#onboarding).
...
3. In the **Features** section of the **Edit Project** page, click **Enable** next to **Chat** to enable <Vg k="CHAT" />.
    ...
    If this is the first project you enable <Vg k="CHAT" /> for, select and subscribe a chat package:
...
1. Select a data center closest to where most of your end users are located.
    ...
    Once <Vg k="CHAT" /> is enabled, you are redirected to the Chat config page.
...
Agora Console assigns the following information to each project that enables <Vg k="CHAT" />:
...
1. Find the project that has enabled <Vg k="CHAT" /> on the [Project management](https://console.agora.io/projects) page at Agora Console, and click **Config**.
...
4. On the **Application Information** page, disable **Chat Service Status** section.
   **Note:** When you disable <Vg k="CHAT" />, the Chat-related data are still stored in the Chat data center, and you are still charged for the plan activation fee.
