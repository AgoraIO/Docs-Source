...
The current version of <Vg k="VSDK" /> for Web does not support adding sound effects to the local audio track. However, you can mix an audio file to the local audio track and play it to the remote users. You can also change the audio route.
...
* A button to change the plaback device.
...
1. **Add the required variables**
    ...
    // A variabe to hold the audio file track.
    var audioFileTrack;
    ```
...
1. **Enable the user to start and stop audio mixing**
    ...
    if(isAudioMixing == false)
    {
        ...
        await agoraEngine.publish([channelParameters.localAudioTrack, audioFileTrack]);
