...
- Added support for sending and receiving GIF image messages. A GIF image message is a subtype of image message that is not compressed when sent. For details, see [Send and receive a GIF image message](/agora-chat/client-api/messages/send-receive-messages?platform=web#send-and-receive-a-gif-image-message).
- Added support for setting, updating, and retrieving chat group avatars. For details, see [Update the chat group avatar](/agora-chat/client-api/chat-group/manage-group-attributes?platform=web#update-the-chat-group-avatar).
- Added an `info` field to the `joinChatRoom` response, so that a user receives the following information when joining a chat room: the chat room creation time (`createtimestamp`), whether all members are muted (`isallmembersmuted`), whether the user is on the allow list (`isinallowlist`), the current member count (`membercount`), and the user's mute expiration time (`muteexpiretimestamp`). For details, see [Join and leave a chat room](/agora-chat/client-api/chat-room/manage-chatrooms?platform=web#join-and-leave-a-chat-room).
- Added the `muteTimestamp` and `userId` parameters to the `muteMember` event, so that the chat room mute event includes the muted member's user ID and mute expiration time. For details, see [Listen for chat room events](/agora-chat/client-api/chat-room/manage-chatrooms?platform=web#listen-for-chat-room-events).
- Added support for retrieving roaming messages sent by a specific group member from the server. For details, see [Retrieve message history of the specified conversation](/agora-chat/client-api/messages/retrieve-messages?platform=web#retrieve-message-history-of-the-specified-conversation).
- Added the `membersPresence` and `membersAbsence` events, which notify other members in batches when one or more members join or leave a group. Previously, the SDK triggered a separate event for each member. The original `memberPresence` and `memberAbsence` events remain valid. For details, see [Listen for chat group events](/agora-chat/client-api/chat-group/manage-chat-groups?platform=web#listen-for-chat-group-events).
...
- Added support for setting extension information when creating a group. Use `createGroupVNext` instead of the deprecated `createGroup`. For details, see [Create and destroy a chat group](/agora-chat/client-api/chat-group/manage-chat-groups?platform=web#create-and-destroy-a-chat-group).
...
- Added support for modifying more message types with `modifyMessage`:
  ...
  For details, see [Modify sent messages](/agora-chat/client-api/messages/send-receive-messages?platform=web#modify-sent-messages).
- Allowed group owners, chat room owners, and admins to recall messages sent by other members. For details, see [Recall a message](/agora-chat/client-api/messages/send-receive-messages?platform=web#recall-a-message).
- Changed the trigger threshold for the `onTokenWillExpire` event: the SDK now triggers the event when 80% of the token's validity period has elapsed, instead of 50%. For details, see [Authentication](/agora-chat/develop/authentication?platform=web).
- Added support for retrieving a group or chat room member list that includes each member's user ID, role, and join time:
  - Use `getGroupMembers` instead of the deprecated `listGroupMembers`. For details, see [Retrieve the member list of a chat group](/agora-chat/client-api/chat-group/manage-chat-groups?platform=web#retrieve-the-member-list-of-a-chat-group).
  - Use `getChatRoomMembers` instead of the deprecated `listChatRoomMembers`. For details, see [Retrieve the chat room member list](/agora-chat/client-api/chat-room/manage-chatroom-members?platform=web#retrieve-the-chat-room-member-list).
