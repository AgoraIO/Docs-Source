...
1.  **Enable extension and set extension properties**
        ...
        To accomplish these tasks, follow these steps: 
            ...
            1. In `AgoraImplementationDlg.h`, declare `setupMarketExtension()` before `setupVideoSDKEngine();``:
                ...
                ```
            1. In `AgoraImplementationDlg.cpp`, put following in `setupVideoSDKEngine` function just after initializing <Vg k="ENGINE" /> `agoraEngine->initialize(context);`:
...
1.  **Disable extension before closing the application**	
    ...
    Extension is needed to unload and disable before the application close. Since you have set `unload_after_use` as `true` in `loadExtensionProvider()` call, so  extension dynamic link library unloads automatically after it has been used. Now disabling the extension is required to prevent any potential memory leaks or unanticipated effects on your application's performance. To do this, put the following in `OnClose()` definition before `agoraEngine->release();`:
