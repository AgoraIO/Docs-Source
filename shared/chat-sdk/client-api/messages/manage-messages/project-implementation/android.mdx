...
To delete historical messages on the server side in one direction, you can call the `removeMessagesFromServer` method. Up to 50 messages can be deleted each time. Once deleted, the message is automatically removed from the user's device locally, and cannot be retrieved from the server. This operation does not affect other users.

Other devices logged in to the account will receive the `EMMultiDeviceListener` callback `onMessageRemoved`, and the deleted messages will be automatically removed from the device locally.

<Admonition type="info" title="Hint">
If you want to use this function, you need to upgrade the SDK to V3.9.8 or above and contact the business for activation.
</Admonition>
```java
