...
1. **Speech driven avatar (BETA)**
   ...
   The SDK introduces a speech driven extension that converts speech information into corresponding facial expressions to animate avatar. You can access the facial information through the newly added <Link to="{{Global.API_REF_IOS_ROOT}}/">setFaceInfoDelegate</Link> method and the <Link to="{{Global.API_REF_IOS_ROOT}}/">onFaceInfo</Link> callback. This facial information conforms to the ARKit standard for Blend Shapes, which you can further process using third-party 3D rendering engines.

   The speech driven avatar extension is a trimmable dynamic library. See [App size optimization](../reference/downloads) for details about the increase in the app size.

   To use speech driven avatar, please contact [technical support](mailto:support@agora.io).
