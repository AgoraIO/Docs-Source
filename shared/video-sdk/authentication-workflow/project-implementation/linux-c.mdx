...
1.  **Handle the event triggered by <Vg k="AGORA_BACKEND" /> when the token is about to expire**
    ...
    1. Register `on_token_privilege_will_expire` event to the event handler list. Add following line to `app_init_event_handler` after `event_handler->on_license_validation_failure = __on_license_failed;`:
