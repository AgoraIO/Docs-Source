...
Deployed on your server, the Agora Cloud Gateway communicates with clients integrated with the [Agora RTC SDK](../reference/glossary#agora-video-sdk). You can use the Cloud Gateway to transmit video and audio streams between the server and the client through [SD-RTN™](../reference/glossary#agora-sd-rtn).
...
![1658370187632](https://web-cdn.agora.io/docs-files/1658370187632)

<div class="alert note">When you use the <a href="../reference/glossary#agora-video-sdk">Agora RTC SDK</a> to communicate with the Cloud Gateway, set the channel scenario of the RTC SDK to <code>LIVE_BROADCASTING</code>.</div>

<div class="alert note">The Cloud Gateway does not support SIP transcoding.</div>
...
| :----------------------------- | :----------------------------------------------------------- |
| Sending and receiving audio and video data in various formats.	 | Supports sending and receiving audio and video data in various formats. See [Send and Receive Media streams](../develop/send-receive-media-streams-c). |
...
| Using user ID in string format to join a channel.              | Supports joining a channel with a user ID in string format. See [Using String User ID](../develop/stringuid-c). |
...
| Media encryption. | Supports media encryption. See [Media Encryption](../develop/media-stream-encryption-c).  |
| Cloud proxy. | Supports cloud proxy service. You need to add specified IPs and ports to your firewall allowlist to access Agora services from restricted networks. See [Cloud Proxy](../develop/cloud-proxy-c).|
| Network Geofencing | Supports network geofencing. After enabling geofencing, the Agora SDK only connects to Agora servers within the specified region. See [Network Geofencing](../develop/network-geofencing-c).|
