...
| `target`         | array        | The target languages for translation. You can configure up to 10 target languages per source language. See [Supported Languages](/real-time-stt/develop/translation#supported-languages) |

<Admonition type="info">
- A single transcription task supports translating up to 5 speakers simultaneously.
- The source and target languages must be different, otherwise an error is returned.
</Admonition>
...
| Chinese Xiang (Hunan dialect)  | `hsn-CN` |
| Chinese (Min Nan, Simplified) |`nan-CN`|
| Mandarin Chinese (Shaanxi dialect, Henan dialect), Southwest Mandarin(Hubei dialect), Jianghuai Mandarin (Anhui dialect)| `cmn-CN` |
