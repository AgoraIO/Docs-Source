...
- **Deprecation of direct CDN streaming APIs**  
  ...
  Deprecated the APIs related to direct CDN streaming, which will be removed in a future release. Agora recommends using [Media Push](/media-push/product-overview) instead.
...
- **Deprecation of virtual metronome APIs**  
  ...
  Deprecated the APIs for the virtual metronome feature, which will be removed in a future release.
...
- **Deprecation of watermark APIs**  
  ...
  Deprecated the old watermark APIs. Agora recommends using the new watermark APIs introduced in this version.
...
- **Adaptive video publishing (Beta)**  
  ...
  Added support for sending multiple video streams with different resolutions from the same video source, including in multi-channel scenarios. Use `setSimulcastConfig:` with `AgoraSimulcastConfig` to configure up to four layers (one high-resolution stream and up to three lower-resolution streams) and control stream publishing. Subscribers can select the desired stream by calling `setRemoteVideoStream:type:`. This feature improves viewing experiences in conferences, large classes, and interactive live streaming, especially under poor network conditions.

- **Multipath network transmission**  

  Introduced multipath transmission for devices with multiple network interfaces (such as 5G, Wi-Fi, and LAN). This feature improves stability in challenging network environments. Enable it by setting `enableMultipath` in `AgoraRtcChannelMediaOptions` to `YES`.

  - *Dynamic mode*: Dynamically selects the optimal path based on network conditions. Optionally set `preferMultipathType` to prioritize a path type.  
  - *Duplicate mode*: Sends data simultaneously over all available paths for maximum stability. This mode incurs additional costs.

  Configure uplink and downlink modes separately with `uplinkMultipathMode` and `downlinkMultipathMode`. Monitor performance with the `rtcEngine:didMultipathStatsChanged:` callback.
...
- **Other new features**  
  ...
  - Added `preloadEffectEx:soundId:` for preloading audio effects in a specific channel.  
...
- Incorrect relay state (`AgoraChannelMediaRelayStateFailure`) and error (`AgoraChannelMediaRelayErrorServerErrorResponse`) reported after a specific join/relay sequence using `joinChannelExByToken:connection:delegate:mediaOptions:joinSuccess:`.
