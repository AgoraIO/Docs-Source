...
This article introduces the new billing strategy for the <Vg k="COMPANY"/> <Vg k="VSDK"/> and the updated billing model in the <Vg k="COMPANY"/> Console.

<Admonition type="info" title="Information">
- Starting August xx, 2024, when you create a new general project in the <Vg k="COMPANY"/> Console, all existing general projects under your account will automatically switch to the new billing model. If the switch fails, contact [`support@agora.io`](mailto:support@agora.io).
...
<Vg k="COMPANY"/>'s real-time interaction services are billed monthly. At the end of each month, <Vg k="COMPANY"/> calculates usage fees for all projects under the [<Vg k="COMPANY"/> Console](https://console.agora.io).
...
<Vg k="COMPANY"/> settles the previous month’s charges on the **1st day of each month**. If you are on a postpaid billing cycle, deductions occur on the **6th day of the month**.
