...
In the scenarios above, the messages between the two users, in each <Vg k="CHAT_GROUP_LOWER"/>, and in each <Vg k="CHAT_ROOM_LOWER"/> are also known as a conversation.
