...
1. **Update local user metadata**
    ...
    When the local user clicks the button to switch their status, you update the user's metadata. To do this, add the following method after `document.getElementById("logout").onclick`:
