...
This version adds the [`preloadWhiteboardView`](../reference/uikit-sdk#setconfig) method, which is used to manually perform a whiteboard view preload to improve the speed of joining the room next time. Before using this feature, ensure that the `autoPreload` in `FastboardConfig` is set to `false`.
