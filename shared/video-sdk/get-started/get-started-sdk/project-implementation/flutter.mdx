...
* Camera: Set the `key` to `Privacy - Camera Usage Description` and the `value` to the purpose of use, such as `for video calls`.
</Admonition>
...
A complete code sample demonstrating the basic process of real-time interaction is provided for your reference. To use the sample code, copy the following code to replace the entire contents of the `/lib/main.dart` file in your project. 
