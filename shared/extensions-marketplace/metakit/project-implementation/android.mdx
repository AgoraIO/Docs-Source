...
Once the project configuration is complete, follow these steps to explore the various functional modules of the MetaKit plugin:
...
When calling `createInitialize` on `RtcEngine`, ensure the following configurations are performed in `RtcEngineConfig`:
    ...
    1. Call `addExtension` with `AgoraFaceCapturePlugin` (`agora_face_capture_extension`) and `MetaKitPlugin` (`agora_metakit_extension`). Then, implement the event callback interface `IMediaExtensionObserver` for plug-ins and register it for `onEvent` plug-in event callbacks.
        ...
        // Add Face Capture plugin
        config.addExtension("agora_face_capture_extension");

        // Add MetaKit plugin
        config.addExtension("agora_metakit_extension");

        // Create the event callback interface class for plugins and register callbacks for plugin events such as onEvent
    ...
    1. In the callback, specify `provider` as `agora_video_filters_metakit` and `extension` as `metakit` to filter events from the MetaKit plugin. The `onEvent` event transmits engine status events transparently, such as `unityLoadFinish` (Unity environment loading completed) and `loadSceneResp` (scene resource loading completed).
        ...
        public void onError(String provider, String ext, int key, String msg) {
            // Filter errors from the MetaKit plugin
            if (!provider.equals("agora_video_filters_metakit") || !ext.equals("metakit")) return;
        ...
        public void onStart(String provider, String ext) {
            // Filter start events from the MetaKit plugin
            if (!provider.equals("agora_video_filters_metakit") || !ext.equals("metakit")) return;
        ...
        public void onStop(String provider, String ext) {
            // Filter stop events from the MetaKit plugin
            if (!provider.equals("agora_video_filters_metakit") || !ext.equals("metakit")) return;
...
#### Enable the facial capture plugin

To enable the facial capture plugin, follow these steps:
    
    <Admonition type="alert" info="info">
    Before enabling the MetaKit plugin, ensure that both the Facial Capture plugin and the Virtual Background plugin are enabled.
    </Admonition>
    1. Call `registerExtension` and `enableExtension` with the provider name `agora_video_filters_face_capture` and the plugin name `face_capture`.

        ```java
        // Register the facial capture plugin
        ...
        ```
    1. Call `setExtensionProperty` to authenticate and authorize the facial capture plugin. Use `authentication_information` as the key, and a value containing the company name (`company_id`) and the facial capture certificate (`license`).
        ...
        ```
        <Admonition type="alert" info="info">
        To obtain the company name and interview certificate, please contact us at [extensions.marketplace@agora.io](mailto:extensions.marketplace@agora.io).
        </Admonition>

#### Enable the virtual background plugin

To enable the virtual background plugin, take the following steps:
...
To enable the MetaKit plugin, follow these steps:

    1. Call `registerExtension` with the service provider name `agora_video_filters_metakit` and the plugin name `metakit`.

        ```java
        mRtcEngine.registerExtension("agora_video_filters_metakit", "metakit", Constants.MediaSourceType.PRIMARY_CAMERA_SOURCE);
        ```
    1. Call `enableExtension` with the same service provider name and plugin name.
    ...
    1. To initialize the MetaKit plugin, call `setExtensionProperty` with the following parameters:
        ...
        ```
    1. When the `onEvent` callback captures the `unityLoadFinish` event, it indicates that the Unity environment has been loaded. At this point, you can call `setExtensionProperty` to load MetaKit scene resources. Use the following parameters:
    ...
    1. When the `onEvent` callback captures the `loadSceneResp` event, it indicates that the scene resources have been loaded. You can then follow these steps to experience the virtual human, Animoji, lighting effects, and 360 background function modules.
...
1. Call `setExtensionProperty` to request texture and render the virtual human scene. Set `key` to `requestTexture` and `value` to include the scene configuration information. To experience the virtual human feature, set `avatarMode` to `0` for the virtual human scene mode and specify the avatar as your desired virtual human image, such as girl or huamulan.
    ...
    In addition to the default avatars `girl` and `huamulan`, the Agora MetaKit plugin offers an open artistic ecosystem. It supports **one-click import** of virtual human models created according to Agora's art standards, providing users with more flexible creation and integration options. Contact Agora [Technical Support](mailto:extensions.marketplace@agora.io) to use this feature.
    </Admonition>
...
1. Call `setExtensionProperty` to perform face pinching operations on the virtual human. Set `key` to `updateFace` and value to support passing multiple sets of resource IDs for face pinching parts and their corresponding adjustment ranges. See [face pinching] for details.
    ...
    // highlight-end
    ```
1. Call `setExtensionProperty` to perform dressing operations on the virtual human. Set `key` to `updateDress` and value to support passing an array of integers containing multiple resource IDs for dressing parts. See [dressing resources](#) for details.
    ...
    // highlight-end
    ```
    Additionally, the MetaKit plugin supports switching the virtual human's image and perspective. See [Virtual Human Key-Value Description](#) for details.
...
In addition to the Animoji images already available in the three plug-ins (dog, girl, headarkit), the Agora MetaKit plug-in provides an open art ecosystem. It supports one-click import of Animoji images created according to Agora's art standards, offering users more flexible creation and integration options. Contact Agora [Technical Support](mailto:extensions.marketplace@agora.io) to use this feature.
</Admonition>
...
#### Apply lighting effect and 360 background
...
2. Experience lighting effects and 360 background.
    ...
    1. **Lighting Effects**:
        ...
        Call `setExtensionProperty` to set up lighting effects. The `key` is `setEffectVideo`, and the `value` contains a series of lighting materials and their corresponding parameter configurations. The MetaKit plugin provides lighting effects such as 3D lighting, screen ripples, aurora effects, and portrait edge flames, and supports fine-tuning of parameters such as color, intensity, and range. See the [Lighting Effects key-value documentation](../../reference/metakit-key-value-description#lighting-effects) for more details. The example code below demonstrates how to overlay advertising lights on a live video.
...
#### Release resources
    ...
    When you are done using the plugin, you can follow the sample code below to stop texture requests, unload scene resources, and destroy the engine.
