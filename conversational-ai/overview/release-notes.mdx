...
* **Support for Microsoft TTS**

    The <Vpd k="NAME" /> now supports Microsoft TTS. When calling the [Start a conversational AI agent](../rest-api/join) API, set `tts.vendor` to `microsoft` to use Microsoft TTS.

* **Voice interruption mode**

    This release adds the `parameters.interruptable` field to the [Start a conversational AI agent](../rest-api/join) API, allowing you to control how the agent handles human voice interruptions. The following modes are supported:
...
* **Metadata support for LLM requests**
    ...
    This release adds the `llm.vendor` field to the [Start a conversational AI agent](../rest-api/join) API. When set to `"custom"`, the agent includes additional metadata when calling the LLM, such as `turn_id` and `timestamp`.

#### Improvements

This release includes the following enhancements:

* **Real-time agent status**: Agent status can now be delivered to the RTC SDK through data stream callbacks during conversations.
