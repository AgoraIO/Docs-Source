...
## Authentication
The RESTful APIs require basic HTTP authentication. You need to set the `Authorization` parameter in every HTTP request header. For how to get the value for Authorization, see [RESTful API authentication](https://docs.agora.io/en/faq/restful_authentication).
...
You can implement different types of Media Push depending on the live streaming scenario requires transcoded or non-transcoded streaming. Generally speaking, in the case of multiple-hosts live streaming, you need to enable transcoding when using Media Push RESTful APIs; in the case of single-host live streaming, only one media stream is pushed to the CDN, so you do not need to enable transcoding. For details, see [Media Push Features](https://docs.agora.io/en/Interactive%20Broadcast/streaming_product?platform=RESTful#features).
...
| transcodeOptions.videoOptions.layout.RtcStreamView element | None                   | The video screen of each user on the canvas. See [RtcStreamView](https://docs.agora.io/en/Interactive%20Broadcast/media_push_datatype?platform=RESTful#layout) for details. |
| transcodeOptions.videoOptions.layout.ImageView element     | None                   | The video image on the canvas, which can be used as a watermark. See [ImageView](https://docs.agora.io/en/Interactive%20Broadcast/media_push_datatype?platform=RESTful#layout) for details. |
|transcodeOptions.videoOptions.vertical                  | (Optional) JSON Object      | Vertical layout. This parameter must be set when `layoutType` is 1. See [vertical](https://docs.agora.io/en/Interactive%20Broadcast/media_push_datatype?platform=RESTful#vertical) for details.|
...
| transcodeOptions.videoOptions.seiOptions                | JSON Object  | Sets the user SEI information carried in the output video stream. The default is empty. If it is not set, it means that no SEI information is output. See [seiOptions](https://docs.agora.io/en/Interactive%20Broadcast/media_push_datatype?platform=RESTful#seioptions) for details. |
...
All possible response status codes. See [Status codes](#code) for details.
...
| `state`    | String | The running status of the Converter:<li>`idle`: The Media Push has not started or has ended.</li><li>`connecting`: Connecting to the Agora streaming server and the CDN server.</li><li>`running`: The  Agora streaming server is pushing the streams.</li><li>`failed`: The Media Push fails.</li> |
...
For details about the fields, see the [Create request body](#createbody).
...
![](https://web-cdn.agora.io/docs-files/1646374583131)
...
|transcodeOptions.audioOptions|JSON Object|The audio transcoding configuration of the Converter. See [audioOptions](https://docs.agora.io/en/Interactive%20Broadcast/media_push_datatype?platform=RESTful#audiooptions) for details.|
|transcodeOptions.videoOptions|JSON Object|The video transcoding configuration of the Converter.    |
|transcodeOptions.videoOptions.canvas|JSON Object |The video canvas. See [canvas](https://docs.agora.io/en/Interactive%20Broadcast/media_push_datatype?platform=RESTful#canvas) for details. |
...
|transcodeOptions.videoOptions.layout.RtcStreamView element |NA|The video screen of each user on the canvas. See [RtcStreamView](https://docs.agora.io/en/Interactive%20Broadcast/media_push_datatype?platform=RESTful#layout) for details.      |
|transcodeOptions.videoOptions.layout.ImageView element|NA| The video image on the canvas, which can be used as a watermark. See [ImageView](https://docs.agora.io/en/Interactive%20Broadcast/media_push_datatype?platform=RESTful#layout) for details.  |
...
| transcodeOptions.videoOptions.seiOptions                | JSON Object  |The user SEI information carried in the output video stream. See [seiOptions](https://docs.agora.io/en/Interactive%20Broadcast/media_push_datatype?platform=RESTful#seioptions) for details.  |
...
|createTs|Number|The Unix timestamp(seconds) when the Converter was created. |
|state|String|The running status of the Converter:<li>`idle`: Pushing the stream has not started or has ended.</li><li>`connecting`: Connecting to the Agora streaming server and the CDN server.</li><li>`running`: Pushing the stream.</li><li>`failed`: Failed to push the stream.</li> |
...
All possible response status codes. See [Status codes](https://docs-preprod.agora.io/en/Interactive Broadcast/streaming_restful?platform=All Platforms&versionId=1d379df0-9300-11ec-814c-17df6c7c3801#code) for details.
...
| `data.members.state`         | String      | The running status of the Converter:<li>`idle`: Pushing the stream has not started or has ended.</li><li>`connecting`: Connecting to the Agora streaming server and the CDN server.</li><li>`running`: Pushing the stream.</li><li>`failed`: Failed to push the stream.</li> |
...
| `Get` | In a project, the rate limit for the rate of updating a specified Converter is 2 times per second. |
