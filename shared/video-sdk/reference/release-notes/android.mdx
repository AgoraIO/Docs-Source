...
1. **Speech driven avatar (BETA)**
   ...
   The SDK introduces a speech driven extension that converts speech information into corresponding facial expressions to animate the avatar. You can access the facial information through the newly added <Link to="{{Global.API_REF_ANDROID_ROOT}}/class_imediaengine.html#api_imediaengine_registerfaceinfoobserver">registerFaceInfoObserver</Link> method and the <Link to="{{Global.API_REF_ANDROID_ROOT}}/class_ifaceinfoobserver.html#callback_ifaceinfoobserver_onfaceinfo">onFaceInfo</Link> callback. This facial information conforms to the ARKit standard for Blend Shapes, which you can further process using third-party 3D rendering engines.
