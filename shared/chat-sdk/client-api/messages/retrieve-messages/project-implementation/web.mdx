...
Agora Chat server can store up to 3,000 conversation per end user.
...
Agora Chat servers only store up to 200 messages per Chat Group. You can store extra messages on your app server by using a webhook to sync with Agora Chat's server. End users can also store additional messages on their local database.
