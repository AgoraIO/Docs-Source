...
3. In the `video-sdk-samples-ios` reference app, open `agora-manager/config.json` and update the values of `appId`,
    `channel`, and `rtcToken` with the values for your temporary token.
</PlatformWrapper>
<PlatformWrapper platform="macos">
3. In the `video-sdk-samples-macos` reference app, open `agora-manager/config.json` and update the values of `appId`,
    `channel`, and `rtcToken` with the values for your temporary token.
</PlatformWrapper>
...
6. After joining a channel, you receive toast messages informing you of some selected call statistics, including:
