...
async function loginRTM() {
    ...
    while (started) {
        // Update the token every 30 seconds. This update frequency is for demonstration purposes only; in a production environment, best practice is to update the token every hour
        await sleep(30 * 1000);
        ...
        console.log("RTM token renewed at " + time);
    }
}
...
If the token expires, the SDK triggers the `status` callback indicating a change in connection status to `FAILED` with the reason `TOKEN_EXPIRED`.

In this scenario, log out of <Vpd k="NAME" /> using the `logout` method, retrieve a fresh token, and `login` again.

<Admonition>

An alternative approach to handling token expiration through the `tokenPrivilegeWillExpire` and `status` callbacks is to handle expiration proactively. Best practice is to update the token periodically to ensure seamless authentication and uninterrupted operation.
</Admonition>
