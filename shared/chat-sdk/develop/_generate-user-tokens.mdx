...
- App token: Admin level access to Agora Chat using the REST API. App tokens grant admin privileges for the app developer to manage the Agora Chat applications, for example, creating and deleting users. For details, see [Implement an Agora app token server for Agora Chat](../develop/generate-app-tokens).
...
- The UUID of the user to be authenticated. The user UUID is a unique internal identification that Agora Chat generates for a user through [User Registration REST APIs](../restful-api/user-system-registration). 
...
- An Agora project with Agora Chat enabled. See [Enable and Configure Chat Service](../get-started/enable) for details.
