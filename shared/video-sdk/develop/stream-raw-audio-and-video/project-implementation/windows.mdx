...
To enable or disable processing of captured raw video data, add a button to the user interface:
    ...
    1. Click **View** > **Other Windows** > **Resource View**.
    
        If **Resource View** does not have focus, select the **Resource View** tab.
...
This section describes the steps required to import libraries, declare variables, and set
  up access to UI elements.

1.  **Define a variable to manage video processing**

        In **Solution Explorer**, open `AgoraImplementationDlg.cpp` and define a global zoom variable:
...
2.    **Start and stop video processing**
        ...
        When a user presses `Zoom In`, your app captures the video frame and processes it for zooming. To implement
  the UI for this workflow, in **Dialog Editor**, double-click **Zoom In**. **Dialog Editor** automatically creates
  and opens an event listener for you. Add the following code to this event listener:
...
1.  **Set up the video and audio frame observer**
    1. Implement the `IVideoFrameObserver` and `IAudioFrameObserver` video and audio frame observer interfaces.
        ...
        Paste the following code in `AgoraImplementationDlg.h` before the `AgoraEventHandler` class declaration:
    ...
    1. Add the video and audio frame observers:
        ...
        ```
    1. Provide the member functions definitions you declared in `CustomVideoFrameObserver` and
  `CustomAudioFrameObserver`.

        In `AgoraImplementationDlg.cpp`, add the following before `CAboutDlg()`:
...
2.  **Register the video and audio frame observers**
    ...
    To receive the callbacks declared in `IVideoFrameObserver` and `IAudioFrameObserver`, you must register the video
  and audio frame observers with the <Vg k="ENGINE" /> before joining a channel. To specify the format of audio frames captured by each `IAudioFrameObserver` callback, use the `setRecordingAudioFrameParameters`, `setPlaybackAudioFrameParameters` and `setMixedAudioFrameParameters` methods. To do this:
    ...
    1. To execute this method before joining the channel, add the following to the `Join` button event listener,
  `OnBnClickedButton2()`, before `agoraEngine->enableAudio()`:
...
3. **Unregister the video and audio frame observers when you leave a channel**
    ...
    When you leave a channel, you unregister the frame observers by setting `EnableAudioVideoCapture()` to `false`.
  To do this, add the following to the`Leave` button event listener, `OnBnClickedButton1()` after
