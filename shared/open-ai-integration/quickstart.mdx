        ...
        async for message in self.client.listen():
            ...
            match message:
                case messages.ResponseAudioDelta():
                    # Handle audio response, add to the audio queue for playback
                    await self.audio_queue.put(base64.b64decode(message.delta))

                case messages.ResponseAudioTranscriptDelta():
                    # Handle partial text transcript, send to Agora chat
                ...
                case messages.ResponseAudioTranscriptDone():
                    # Handle completed transcript, send final message to Agora chat
...
Together, these methods enable smooth real-time communication, ensuring that audio data flows seamlessly between Agora and OpenAI, while also handling text transcripts and other outputs.
...
The `_process_model_messages` method uses the OpenAI Realtime API client to continuously listen for incoming messages. This is done asynchronously, allowing the agent to handle multiple types of messages concurrently with other tasks, such as streaming audio.
...
### 2. Handling Audio Data

If the incoming message contains audio data (`ResponseAudioDelta`), it is base64-encoded and needs to be decoded. Once decoded, the audio data is placed into the `audio_queue`, which the agent later processes and plays back to the user via the Agora channel.
...
With these pieces in place, the agent can effectively manage tool registration and execution, ensuring that it can handle a variety of tasks as directed by the OpenAI model.
