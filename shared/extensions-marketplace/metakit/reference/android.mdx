...
To implement the capabilities of the <Vg k="RTEE_MK"/> extension, use the <Link to="{{global.API_REF_ANDROID_ROOT}}/class_irtcengine.html#api_irtcengine_setextensionproperty">setExtensionProperty</Link> method provided by the Agora Video SDK v4.x. Pass in the specified `key` and `value` as follows:
...
Add a <Vg k="RTEE_MK"/> scene to a native `view` and render the specified scene content. This includes virtual human, Animoji, lighting effects, and 360 background.
...
After enabling scene view capture, call <Link to="{{Global.API_REF_ANDROID_ROOT}}/class_irtcengine.html#api_irtcengine_joinchannel2">joinChannel</Link> to join the channel and publish the video stream of the scene view.
...
The <Vg k="RTEE_MK"/> extension allows you to switch the image, viewpoint, face, and outfit of the avatar. To experience the avatar-related functions, set `avatarMode` to `0` when [enabling texture request](#enable-texture-request) or [adding scene view](#add-scene-view).
...
The <Vg k="RTEE_MK"/> extension provides a set of dress-up resources for avatars.
...
The <Vg k="RTEE_MK"/> extension allows you to switch the image of Animoji. To experience Animoji-related functions, set `avatarMode` to `1` when [enabling texture request](#enable-texture-request) or [adding scene view](#adding-scene-view).

<Admonition type="info" title="Note">
In addition to the existing `dog`, `girlhead` and `arkit` Animoji, the Agora <Vg k="RTEE_MK"/> extension provides an open art ecosystem and supports **one-click import** of Animoji images made according to Agora's art standards, providing users with more flexible creation and integration options. [Contact Agora technical support](mailto:extensions.marketplace@agora.io) to use this feature.
</Admonition>
...
The <Vg k="RTEE_MK"/> extension provides lighting effects such as 3D lighting, screen ripples, aurora effects, and portrait edge flames, and supports fine-tuning the color, intensity, range, and other parameters of the lighting effects. To experience the lighting effects-related functions, set `avatarMode` to `2` when [enabling texture requests](#enable-texture-request) or set `backgroundEffect` to `true` when [adding a scene view](#add-scene-view).
