...
The <Vg k="UIK" /> provides the [`insertImage`](/en/whiteboard/fastboard_api_web?platform=Web#insertimage) method for inserting and displaying an image in the main window of the whiteboard. You can call this method and pass in the URL of the image to display a specified image.
...
<div class = "alert info">The <Vg k="UIK" /> does not provide a UI for `insertImage`. You need to implement the UI yourself.</div>

## Play audio and video

The <Vg k="UIK" /> integrates the media player extension and provides the [`insertMedia`](/en/whiteboard/fastboard_api_web?platform=Web#insertmedia) method for playing audio and video files. You can simply call `insertMedia` and pass in the audio and video file name and URL to play the audio and video in a sub-window on the whiteboard.
...
<div class = "alert info">The <Vg k="UIK" /> does not provide a UI for insertMedia. You need to implement the UI yourself.</div>

## Present documents

The <Vg k="UIK" /> integrates the docs viewer and slide extensions and provides the [`insertDocs`](/en/whiteboard/fastboard_api_web?platform=Web#insertdocs-12) method for presenting documents. You can simply call `insertDocs` and pass the parameters of a document to present the document in a paginated format in a sub-window on the whiteboard. The implementation workflow is as follows:
...
2. Call the [Query file conversion progress (GET) API](../develop/generate-token-rest#query-the-progress-of-a-file-conversion-task) to get the conversion result.
...
- Ensure the format of the converted images or web pages is supported by the <Vg k="UIK" />.

    The <Vg k="UIK" /> supports inserting and displaying images or web pages in the following formats:
...
2. Call the [Query file conversion progress (GET) API](../develop/generate-token-rest#query-the-progress-of-a-file-conversion-task) to get the file conversion result.
