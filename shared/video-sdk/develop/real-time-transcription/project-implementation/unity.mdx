...
To create a simple interface for testing Real-Time Transcription functionality, you add two buttons to the UI.
...
1. **Enable a user to start, query and stop a task**
    ...
    In this example, you use buttons to manage a speech-to-text task. When a user presses the speech-to-text button, the <Vpl k="CLIENT" /> calls `startSpeechToText` and starts a speech-to-text task using the `builderToken`. If a task is already running, the <Vpl k = "CLIENT"/> stops the task. When the user presses the query button, the <Vpl k="CLIENT"/> calls `querySpeechToText` and informs you of the current status of the speech-to-text task. To implement this logic, in your script file, add the following at the end of `SetupUI`:
