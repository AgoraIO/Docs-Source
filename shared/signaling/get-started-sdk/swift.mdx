...
To implement this UI, replace the contents of `ViewController` with the following:
...
class ViewController: UIViewController {
    ...
    @objc func login(sender: UIButton!) {
        ...
        Task {
            ...
            if errorCode == .ok {
                self.text = "Login successful for user \(uid). Code: \(String(errorCode!.rawValue))"
            } else {
                self.text = "Login failed for user \(uid). Code: \(String(errorCode!.rawValue))"
            }
    ...
    @objc func logout(sender: UIButton!) {
        ...
        Task {
            ...
            if errorCode == .ok {
                self.text = "Logout successful for user \(uid). Code: \(String(errorCode!.rawValue))"
            } else {
                self.text = "Logout failed for user \(uid). Code: \(String(errorCode!.rawValue))"
            }
            addMsgToRecord(self.text)
        }
    }
    ...
    @objc func joinChannel(sender: UIButton!) {
        ...
        Task {
            ...
            if errorCode == .channelErrorOk {
                self.text = "Successfully joined channel \(channelID). Code: \(String(errorCode!.rawValue))"
            } else {
                self.text = "Failed to join channel \(channelID). Code: \(String(errorCode!.rawValue))"
            }
            addMsgToRecord(self.text)
        }
    }
    ...
    @objc func leaveChannel(sender: UIButton!) {
        Task {
            ...
            if errorCode == .ok {
                self.text = "Leave channel \(channelID) successful. Code: \(String(errorCode!.rawValue))"
            } else {
                self.text = "Failed to leave channel \(channelID). Code: \(String(errorCode!.rawValue))"
            }
            addMsgToRecord(self.text)
        }
    }
    ...
    @objc func sendGroupMsg(sender: UIButton!) {
        ...
        Task {
            ...
            if errorCode == .errorOk {
              ...
              self.text = "Message failed to send to channel (channelID):" + channelMsg + "Code:" + String(errorCode.rawValue)
            }
            addMsgToRecord(self.text)
        }
    }
    ...
    @objc func sendPeerMsg(sender: UIButton!) {
        ...
        Task {
            ...
            if errorCode == .ok {
                ...
                self.text = "Message failed to send from user: " + self.uid + " to user: " + peerId + " content: " + peerMsg + " Code: " + String(errorCode.rawValue)
            }
            addMsgToRecord(self.text)
        }
    }
...
class ViewController: NSViewController {
    ...
    // Button to log in to Signaling
    @objc func login(sender: NSButton!) {
    ...
    // Button to join the Signaling channel
    @objc func joinChannel(sender: NSButton!) {
    ...
    // Button to leave the Signaling channel
    @objc func leaveChannel(sender: NSButton!) {
        Task {
            let errorCode = await channel?.leave()
            if errorCode == .ok {
                self.text = "Leave channel \(channelID) successful. Code: \(String(errorCode!.rawValue))"
    ...
    // Button to send group message
    @objc func sendGroupMsg(sender: NSButton!) {
    ...
    // Button to send peer message
    @objc func sendPeerMsg(sender: NSButton!) {
...
4.  **Enable your <Vpl k="CLIENT" /> to display connection related changes**
    ...
    extension ViewController: AgoraRtmDelegate {
        ...
        func rtmKit(_ kit: AgoraRtmKit, connectionStateChanged state: AgoraRtmConnectionState, reason: AgoraRtmConnectionChangeReason) {
            self.text = "Connection status changed to: \(state.rawValue) Reason: \(reason.rawValue)"
            addMsgToRecord(self.text)
        }
    }
    ```
...
1. <Link to="{{Global.DEMO_TOKEN_BUILDER_URL}}">Generate a temporary RTM token</Link>.

2.  In Xcode, in `ViewController`, update `appID`, `token` with the values for your temporary token.

3.  Compile and run the project on a simulator or a physical mobile device. You can see the following interface if your project runs successfully:
...
- Sample project
    ...
    Agora  provides an open-source [sample project](https://github.com/AgoraIO/Signaling/tree/master/Agora-Signaling-Tutorial-iOS-Objective-C) on GitHub for your reference.
