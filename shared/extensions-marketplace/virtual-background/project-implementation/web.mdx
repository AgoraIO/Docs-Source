...
1. Call `videoTrack.unpipe` to remove the extension from the current video track when virtual background is no longer being used.

   ```
   localTracks.videoTrack.unpipe()
   ```

Note: If you need to open the virtual background again, you can reuse the existing `VirtualBackgroundProcessor` instance instead of recreating it.
