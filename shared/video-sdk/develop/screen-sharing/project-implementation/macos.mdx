...
- To update the <Vg k="SCR_SHR" /> parameters, such as video encoding resolution, frame rate, or bit rate, to stroke the screen or app window, or to block a specified window, call the `updateScreenCaptureParameters` method and update the `captureParams` parameter configuration.
