...
import videoURL from '@docs/assets/images/video-sdk/preload.mov';

<PlatformWrapper platform="web">
Preloading channels ensures that viewers see the content as quickly as possible when joining a channel, minimizing any visible loading delays.
...
The best practice when preloading channels is as follows:
...
1. Preload channel list
    ...
    To speed up the subsequent joining of the channel, use a [wildcard token](../get-started/authentication-workflow#generate-wildcard-tokens) in the `token` parameter of the `preload` method.
...
1. Preload individual channels
    ...
    - Before entering the channel
        ...
        To speed up joining the channel, use a [wildcard token](../get-started/authentication-workflow#generate-wildcard-tokens) in the `token` parameter of the `join` method.

    - Entering the channel

        In order to reduce the time it takes to render the first frame of the host's video, enable sending the host's media stream to Agora when a user enters the channel. To use this function, [contact technical support](mailto:support@agora.io). The following table compares the steps from when a user enters the channel to when the host's media stream is rendered, before and after enabling this function:
        ...
        Joining a channel incurs additional fees due to the transmission of media streams.
        </Admonition>
