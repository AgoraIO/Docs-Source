...
- Send a custom message
    ...
    ```
### Send chatroom global broadcast message

This RESTful API allows you to send broadcast messages to all active chatrooms (chatrooms with at least one member and at least one message sent) under the app, supporting all message types.

**Sending frequency:** You can send up to 10 times per minute and a maximum of 100 broadcast messages per day.
...
The request body for sending a text-type broadcast message is a JSON object, which contains the following parameters:

| Parameter | Type | Description | Required |
| --- | --- | --- | --- |
| `from` | String | The username of the message sender. If you do not set this field, the Chat server takes the `admin` as the sender. If you set it as the empty string "", this request fails.  | No |
| `chatroom_msg_level` | String | The chat room message priority: <ul><li>`high`</li><li>(Default) `normal`</li><li>`low`</li></ul> | No |
| `msg` | JSON | Information contained in the message body. | Yes |
| `msg.type` | String | The message type: <ul><li>`txt`: Text message</li><li>`img`: Image message</li><li>`audio`: Voice message</li><li>`video`: Video message</li><li>`file`: File message</li><li>`loc`: Location message</li><li>`cmd`: Command message</li><li>`custom`: Custom message</li></ul> | Yes |
| `msg.msg` | String | The message content. | Yes |
| `ext`  | JSON  | Broadcast messages support extended fields, allowing for custom . You cannot pass `null` to this parameter. Additionally, push notifications also support custom extended fields, see APNs Custom Display and Android Push Field Description for details.| No |

The request bodies for different types of messages only differ in the `msg` field; other parameters are the same. Besides the `type` field, parameters contained in the `msg` field have the same meanings as those in the `body` field of the request body for sending chatroom messages.For details, see [Body of different message types](#body).The other parameters and descriptions are the same with those of [Sending a one-to-one message method](#request).

    - [Send Image Message](#imageMessage)
    - [Send voice message](#voiceMessage)
    - [Send video message](#videoMessage)
    - [Send file message](#fileMessage)
    - [Send location message](#locationMessage)
    - [Send command message](#cmdMessage)
    - [Send custom message](#customMessage)

#### HTTP response

##### Response body

For all types of broadcast messages, the response contains the same fields.
...
- Send location broadcast message
...
- Send transparent (pass-through) broadcast message
