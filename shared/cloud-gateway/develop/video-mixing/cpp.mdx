...
1. Create an `IVideoMixerSource` object and call `setBackground` to set the background.
...
4. When the SDK decodes the first YUV frame, refresh the `IVideoMixerSource` object.
