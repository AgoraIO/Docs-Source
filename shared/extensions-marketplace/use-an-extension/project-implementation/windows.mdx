...
Integrating an extension into your project involves a few important steps to ensure seamless interaction between the extension and <Vg k="COMPANY" /> <Vg k="VSDK"/>. Each of these steps plays a crucial role in ensuring that the extension works correctly and efficiently within your <Vg k="COMPANY" /> project:
...
1.  **Enable extension and set extension properties**

        To use the downloaded extenstion in <Vg k="COMPANY" /> <Vg k = "VSDK"/> project, you require to:
        
            1. Load the extension. 
            2. Register the extension.
            3. Enable the extension.
            4. Set Extension property.
        ...
        To accomplish these tasks, follow these steps: 
            ...
            1. In `AgoraImplementationDlg.h`, declare `setupMarketExtension()` before `setupVideoSDKEngine();``:
                ...
                ```
            1. In `AgoraImplementationDlg.cpp`, put the following line in `setupVideoSDKEngine` function just after initializing <Vg k="ENGINE" /> `agoraEngine->initialize(context);`:
            ...
            1. Define `setupMarketExtension()`:
                ...
                ```
1.  **Disable extension before closing the application**	

    Extension is needed to unload and disable before the application close. Since you have set `unload_after_use` as `true` in `loadExtensionProvider()` call, so  extension dynamic link library unloads automatically after it has been used. Disabling the extension before closing your application to prevent any potential memory leaks or unanticipated effects on your application's performance. To disable the extension in the system put the fllowing line in `OnClose()` definition before `agoraEngine->release();`: 
