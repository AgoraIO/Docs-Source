...
- User token: User level access to Agora Chat from your app using Agora Chat SDK. User tokens are used to authenticate users when they log in to your Agora Chat application. For details, see [Implement an Agora user token server for Agora Chat](../develop/generate-user-tokens).
...
- An [Agora project](/video-calling/reference/manage-agora-account#create-an-agora-project) with the [App Certificate](/video-calling/reference/manage-agora-account#get-the-app-certificate) and Agora Chat enabled. See [Enable and Configure Chat Service](../get-started/enable) for details.

- The [App ID](/video-calling/reference/manage-agora-account#get-the-app-id) , `org_name` and `app_name` of your Agora project, see [Enable and Configure Agora Chat](../get-started/enable).
...
If you use Agora Chat together with the [Agora RTC SDK](https://docs.agora.io/en/Agora%20Platform/term_agora_rtc_sdk), Agora recommends you upgrade to [AccessToken2](../reference/access-token-2).