...
The **Concurrent users** and **Concurrent channels** line charts display data for the last 30 minutes, respectively. The two metrics are explained in [Scale metrics](#scale-metrics).
...
If you have enabled the [Alert Notification](./video-voice-sdk/alarm) service, the **Alert Notification** panel displays the latest three notifications.
...
-   Jump to Call Search: For each of the displayed alert notification, you can click **Call details** to analyze the call using the [Call Inspector](./video-voice-sdk/call-search) service.
...
<p>Audio freeze rate is not included in the quality metrics of the Agora <Vg k="VSDK" /> for Web. That is, if you select <strong>Web <Vg k="VSDK" /></strong> for product type in the global settings, the heat map chart for audio freeze rate does not display in the real-time scale panel.</p></td>
</tr>
<tr class="odd">
...
<p>Web <Vg k="VSDK" />: Only video freezing longer than 500 milliseconds is counted.</p></td>
</tr>
<tr class="even">
