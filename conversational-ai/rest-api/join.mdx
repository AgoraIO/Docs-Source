  ...
  <Parameter name="properties" type="object" required={true}>
    ...
    <Parameter name="idle_timeout" type="integer" defaultValue="30" required={false}>
      ...
      <Admonition type="caution" title="Multi-agent use cases">
        ...
        As a result, the `idle_timeout` condition, when all other users have left, might never be triggered. This can cause agents to run indefinitely and lead to significant unintended usage. See [Agent lifecycle best practices](../best-practices/agent-lifecycle) to prevent and resolve this issue.
      </Admonition>
    </Parameter>
