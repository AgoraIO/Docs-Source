...
This extension provides video <Vg k="RTEE_CLARITY" /> feature for <Vg k="VSDK" />.
...
1. Install packages the following packages.

    * `agora-rtc-sdk-ng` 
    * `agora-extension-super-clarity`
...
1. Once you have a video `track` (`ILocalVideoTrack` or `IRemoteVideoTrack`), create a processor and pipe it to `track`.
