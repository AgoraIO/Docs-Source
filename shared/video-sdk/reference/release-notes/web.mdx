...
This release allows a user whose role is set to `audience` to preload channels before joining one, which helps shorten the time of joining a channel, thus reducing the time it takes for audience members to hear the host. Contact [technical support](mailto:support@agora.io) to enable this feature.
