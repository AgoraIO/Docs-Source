...
1. **Preload sound effects**
    ...
    ```cpp
    // Preload sound effects to improve performance
...
1. **Update the UI after the effect finishes playing**
    ...
    When <Vpd k="SDK" /> has finished playing the sound effect, the `onAudioEffectFinished` event is fired. Your <Vpl k = "CLIENT"/> listens to the callback and invokes the `OnEIDStopAudioEffect` method to update the UI and stop the audio effect. To implement this logic, take the following steps:
