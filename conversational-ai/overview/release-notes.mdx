...
* **Voice isolation**
    ...
    Agora's voiceprint recognition technology filters out background voices during conversations to focus on a single speaker. This feature helps distinguish the primary speaker from background voices, improving speech recognition accuracy in noisy environments. To experience this feature, contact [technical support](mailto:support@agora.io). 

* **Image messaging**

    Send images to your <Vpd k="AGENT" /> through the toolkit. The agent can view and reference images in conversations, enabling more contextual and relevant responses based on visual content. The toolkit includes delivery confirmation through callback notifications when images are successfully sent.

    Configure image messaging by enabling the new interface in your toolkit instance. Images are sent using URLs, and the agent automatically incorporates the visual information into subsequent responses.
