...
To enable your users to start and stop relaying to another channel, add a button to the user interface. In `<View style={styles.btnContainer}>`, add the following code:
after `<Text onPress={leave} style={styles.button}>Leave</Text>`
...
1.  **Start or stop channel media relay**
    ...
    const coHost = () => {
        if (mediaRelaying) {
        engineRef.current?.stopChannelMediaRelay();
        } else {
        // Start relaying media streams across channels
        engineRef.current?.startChannelMediaRelay({
