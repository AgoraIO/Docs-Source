...
To enable the user to use audio mixing, play sound and voice effects and modify the audio route, add the following elements to the user interface:
    ...
    * A switch to turn the speakerphone on or off.
    ...
    In `App.tsx` add the following lines to the `return` statement after the `</View>` block containing the `Join` and `Leave` buttons:
...
1. **Pre-load audio effects**
    ...
    To improve performance, you can pre-load sound effects before or after you join a channel. To do this, add the following line to the `join` component after `joinChannelWithOptions`:
