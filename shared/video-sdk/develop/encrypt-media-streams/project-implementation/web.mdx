...
2.  **Convert strings from `Base64` to `Uint8Array`**
...
3.  **Convert strings from `Hex` to `ASCII`**
...
4.  **Call channel encryption methods to enable channel encryption**
...
To ensure that your users only can communicate securely in a channel:

1. **Enable end-to-end encryption when you join a channel**

    - Enable the encoded transform function by setting `ENABLE_ENCODED_TRANSFORM` to `true`.
    - Obtain an `RTCRtpTransceiver` object and establish a WebRTC encoded transform processing pipeline on the
    streaming end using `setEncryptionStream`.
...
2. **Decrypt messages you recieve from a channel**

    - Obtain an `RTCRtpTransceiver` object and establish a WebRTC encoded transform processing pipeline on the
    receiver end using `setDecryptionStream`.
    - On recieving the `user-published` callback, call `setDecryptionStream` before calling `play()`.
