...
* **Agent metrics enhancement**
    ...
    Notification event type [`111 agent_metrics`](/conversational-ai/develop/event-types#111-agent-metrics) payload adds three new fields: 

    - `aivad_delay` (optional)
    - `bhvs_delay`(optional) 
    - `total_delay` (mandatory)

* **Greeting experience optimization**

    `greeting_message` adds a peer field `greeting_mode`.

* **TTS parameter update**

    Added `wss_url` field to the TTS parameters examples for OpenAI and ElevenLabs.
