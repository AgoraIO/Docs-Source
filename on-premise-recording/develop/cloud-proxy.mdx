...
3. The Agora SDK sends data to <Vg k="CP" />, which forwards it to the <Vg k="AGORA_BACKEND" />.
4. The <Vg k="AGORA_BACKEND" /> sends the response back to <Vg k="CP" />, which then returns it to the SDK.
