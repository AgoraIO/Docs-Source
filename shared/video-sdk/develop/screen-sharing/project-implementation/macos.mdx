...
![Screen Share Process](/images/video-sdk/tmp_screen-sharing-macos-process.jpg)
...
Call the `setScreenCaptureScenario` method to set the <Vg k="SCR_SHR" /> scenario. Set the `scenarioType` to any one of the following according to the actual usage scenario:
...
Call `joinChannelByToken`[2/4] to join the channel and set the channel media options. The sample code is as follows:
    ...
    // Post screen captured video in the channel
    // To post a second screen capture video in the channel, replace the next line of code with the following: mediaOptions.publishSecondaryScreenTrack = true
