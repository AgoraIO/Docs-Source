...
4. Register the AI Denoiser extension: Call the [`AgoraRTC.registerExtensions`](./API%20Reference/web_ng/interfaces/iagorartc.html#registerextensions) method, and pass in the created `AIDenoiserExtension` instance. Optionally, listen for the callback reporting that the Wasm and JS files fail to load.
...
5. Create an `IAIDenoiserProcessor` instance: Call the `createProcessor` method to create a `processor`, and set whether to enable the extension by default. Optionally, you can listen for the callback reporting that the noise reduction process takes too long.
   ...
   ```
6. Inject the extension to the audio processing pipeline: Call the [`pipe`](./API%20Reference/web_ng/interfaces/imicrophoneaudiotrack.html#pipe) method and specify the [`processorDestination`](./API%20Reference/web_ng/interfaces/imicrophoneaudiotrack.html#processordestination) property.
