...
<Vg k="Agora" /> `CallAPI` is a scenario-based solution designed for one-on-one calls that open in seconds. The API provides efficient call connection and image transmission functions that help you quickly set up video calls and transmit images during calls.
