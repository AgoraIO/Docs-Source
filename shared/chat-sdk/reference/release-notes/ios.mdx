...
- Added the function of tagging a conversation:
  ...
  - `multiDevicesConversationEvent#AgoraChatMultiDevicesEventConversationUpdateMark`: Conversation tag update event in a multi-device login scenario. If a user adds or removes a conversation tag on one device, this event is received on other devices of the user.
