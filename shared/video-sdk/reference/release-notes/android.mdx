...
Starting from v4.5.0, both Video SDK and Signaling SDK (v2.2.0 and above) include the `libaosl.so` library. If you manually integrate Video SDK via CDN and also use the Signaling SDK, delete the earlier version of the `libaosl.so` library to avoid conflicts. The `libaosl.so` library version in Video SDK v4.5.1 is xxxx.
</Admonition>
