...
<span className="index-desc-rtcconnection">The `RtcConnection` class is used to manage the connection between the SDK and the RTC channel. Once the connection is established, you can use the `LocalUser` object to send and receive media streams in the RTC channel. Connecting to the channel is an asynchronous operation, and you can monitor connection state changes and events through the [`RtcConnectionObserver`](./irtcconnectionobserver) object. `RtcConnection` can also monitor whether a remote user joins or leaves the channel.</span>


<h2 className="anchor index-group-item-rtcconnection index-api-newrtcconnection" id="newrtcconnection">NewRtcConnection</h2>
...
<span className="index-desc-newrtcconnection">Creates an RTC connection object.</span>

#### Parameters

| Parameter     | Description                                                                 |
|---------------|-----------------------------------------------------------------------------|
| `cfg`         | RTC connection configuration. See [`RtcConnectionConfig`](./apidatatype#rtcconnconfig) for details. |
...
<span className="index-desc-connect">Connects to the RTC channel.</span>

When the method succeeds, the connection state changes from `true` to `2`.

Depending on whether the connection is successful, the connection state will change to `3` or `5`. The SDK will trigger [`OnConnected`](./rtcconnectionobserver#onconnected) or [`on_disconnected`](./rtcconnectionobserver#ondisconnected) to notify the connection state change.
...
<Table header={TableHeaderve9qoc54a4}>
  <Tr>
    <Td>`token`</Td>
    <Td>RTC Token. See [Token Authentication](https://doc.shengwang.cn/doc/rtc/windows/basic-features/token-authentication) for details.</Td>
  </Tr>
  <Tr>
    <Td>`channel`</Td>
    <Td>
    Channel name. Must be a string with a length of no more than 64 bytes. Supported character types include:
  ...
  <Tr>
    <Td>`uid`</Td>
    <Td>
    Local user ID. No more than 64 bytes in length. If you do not set a user ID or set it to nil, the SDK will automatically return a user ID in the `OnConnected` callback. You must save and manage this ID yourself. Depending on the user ID type you choose in the [`Initialize`](./agoraservice#initialize) method, the supported character sets for this parameter are different:
    - For int type user ID, the supported character set includes:
      - 10 digits 0-9
    - For String type user ID, the supported character set includes:
...
<span className="index-desc-disconnect">Disconnects from the RTC channel. When the SDK disconnects from the channel, the connection state changes to `true`. You can also be notified through the [`OnDisconnected`](./rtcconnectionobserver#ondisconnected) callback.</span>
...
<h2 className="anchor index-group-item-rtcconnection index-api-renewtoken" id="renewtoken">RenewToken</h2>
...
<span className="index-desc-renewtoken">Renews the Token. The Token will expire after a certain period. When you receive the [`OnTokenPrivilegeWillExpire`](./rtcconnectionobserver#ontokenprivilegewillexpire) callback, you must generate a new token from your server and use this method to update the token. Otherwise, the SDK will disconnect from the channel.</span>
...
<h2 className="anchor index-group-item-rtcconnection index-api-registerobserver" id="registerobserver">RegisterObserver</h2>
...
<span className="index-desc-registerobserver">Registers an RTC connection observer.</span>
...
<h2 className="anchor index-group-item-rtcconnection index-api-unregisterobserver" id="unregisterobserver">UnregisterObserver</h2>
...
<span className="index-desc-unregisterobserver">Unregisters the RTC connection observer.</span>
...
<h2 className="anchor index-group-item-rtcconnection index-api-createdatastream" id="createdatastream">CreateDataStream</h2>
...
<Table header={TableHeadervctg30ud8n}>
  <Tr>
    <Td>`reliable`</Td>
    <Td>
    Indicates whether the data stream is reliable:
    - `true`: The receiver will receive the data sent by the sender within 5 seconds.
    - `false`: The receiver is not guaranteed to receive the data, and no error will be reported if data is lost.
   </Td>
  </Tr>
  <Tr>
    <Td>`ordered`</Td>
    <Td>
    Indicates whether the data stream is ordered:
    - `true`: The receiver will receive data packets in the order sent by the sender.
...
<h2 className="anchor index-group-item-rtcconnection index-api-sendstreammessage" id="sendstreammessage">SendStreamMessage</h2>
...
Upon successfully calling this method, the remote end will trigger the [`on_stream_message`](./localuserobserver#onstreammessage) callback, where the remote user can receive the stream message.

<Admonition type="caution" title="Note">
- Ensure that you have called [`create_data_stream`](#createdatastream) to create a data channel before using this method.
- In a live broadcast scenario, this method is only applicable to host users.
</Admonition>
...
<h2 className="anchor index-group-item-rtcconnection index-api-getagoraparameter" id="getagoraparameter">GetAgoraParameter</h2>
...
<span className="index-desc-getagoraparameter">Get `AgoraParameter` Object.</span>
...
<h2 className="anchor index-group-item-rtcconnection index-api-getlocaluser" id="getlocaluser">GetLocalUser</h2>
...
<span className="index-desc-getlocaluser">Get `LocalUser` Object. Each connection corresponds to only one local user.</span>
