...
3. In Android Studio, open `app/java/com.example.<projectname>/MainActivity` and update `appId`, `channelName` and `token` with the values from <Vg k="CONSOLE" />.
...
9. Speak into the microphone. You see words converted to text displayed in your <Vpl k="CLIENT" />.

10. Press **Query speech to text**. You see a message informing you of the current status of the task.

11. Press **Stop speech to text**. The speech-to-text task is stopped. 
