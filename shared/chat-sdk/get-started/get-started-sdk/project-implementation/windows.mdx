...
1. **Set up <Vpd k="NAME" /> when the <Vpl k="CLIENT"/> starts**
    ...
    When the <Vpl k="CLIENT"/> starts, you create an instance of the `ChatClient` and set up callbacks to handle <Vpd k="NAME" /> events.
    ...
    public MainWindow()
    {
        ...
        AddChatDelegate(); // Add a chat delegate to handle chat callbacks
...
1. **Add & remove chat manager listener and closing window functionality**
...
1. **Implement the `IChatManagerDelegate` Interface**
    ...
    To set up callbacks for handling <Vpd k="NAME" /> events, you implement the `IChatManagerDelegate` interface in the `MainWindow` class.
