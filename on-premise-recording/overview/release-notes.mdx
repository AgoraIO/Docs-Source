...
The Agora <Vpd k="SDK" /> records communication and interactive live streaming
...
| SDK | Description |
| --- | --- |
| The Agora Native SDK | The Agora <Vpd k="SDK" /> is compatible with the Agora Native SDK v1.7.0 or later. If, for example, any user in the channel uses the Agora Native SDK v1.6, the whole channel cannot record. |
| The Agora Web SDK | The <Vpd k="SDK" /> is compatible with the Agora Web SDK v1.12.0 or later. |


<Admonition type="info">
The Agora <Vpd k="NAME" /> SDK supports both Java and C++ from v2.2.0.
</Admonition>

## v4.x

<Admonition type="info" title="Info">
With this release, the On Premise Recording SDK version 3.x and earlier have entered the retirement phase. For detailed retirement schedule and migration plan, refer to [Service Sunset Plans](/on-premise-recording/reference/sunset).
</Admonition>
...
This is the first official release of the SDK 4.x series. Built on the Agora Real-Time Interaction (RTC) SDK, this version provides improved reliability, expanded use cases, and broader compatibility.
...
- Record audio and video from all participants in a single channel.
- Record audio and video from all participants in multiple channels simultaneously.
- Record audio-only content from one or more channels simultaneously.
- Record encrypted channels using the built-in encryption of the Agora SDK.
- Obtain raw video frames or capture snapshots by accessing video data in RTC channels.

To learn more about the product’s core capabilities, see the [Product Overview](/on-premise-recording/overview/product-overview).
...
### v2.3.4
<Admonition type="info">As of December 8, 2021, Agora begins implementing the retirement process for the <Vpd k="SDK" /> earlier than v3.0.0. See [Service Sunset Plans for <Vpd k="NAME" />](/on-premise-recording/reference/sunset).</Admonition>
...
> The calculation of the usage of the <Vpd k="SDK" /> v2.3.0 or later is separated from that of the Agora Voice Call/Interactive Audio Streaming SDK and the Agora Video Call/Interactive Video Streaming SDK. Contact [support@agora.io](mailto:support@agora.io) for details.
...
v2.3.0 provides two pre-defined layout templates for composite recording to make the <Vpd k="SDK" /> sample code easier to use.
...
This function is disabled by default and can be enabled by setting the `audioIndicationInterval` parameter in `RecordingConfig`. The <Vpd k="SDK" /> returns the `onActiveSpeaker` callback at set intervals.
...
This is the first release of the <Vpd k="SDK" /> with the following functions:
