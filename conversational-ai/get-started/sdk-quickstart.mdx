---
title: 'Server SDK quickstart'
sidebar_position: 2
type: docs
platform_selector: false
description: Build a voice AI agent server using the Agora Conversational AI SDKs.
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

This page shows you how to build a server that starts and stops a voice AI agent using the Agora Conversational AI SDKs. The SDKs wrap the Conversational AI REST API with typed vendor configuration, automatic token generation, and a session lifecycle API.

## Prerequisites

Before you begin, make sure you have:

- [Enabled Agora Conversational AI](../get-started/manage-agora-account) for your project.
- Completed the [Voice Calling](/voice-calling/get-started/get-started-sdk) or [Video Calling](/video-calling/get-started/get-started-sdk) quickstart. 
- Your **App ID** and **App Certificate** from [Agora Console](https://console.agora.io). 
- API keys for your chosen STT, LLM, and TTS providers. This guide uses Deepgram, OpenAI, and ElevenLabs as examples.

<Tabs groupId="language">
<TabItem value="typescript" label="Node">
- Node.js 18 or later.

</TabItem>
<TabItem value="python" label="Python">
- Python 3.8 or later.

</TabItem>
<TabItem value="go" label="Go">

- Go 1.21 or later.
</TabItem>
</Tabs>

## Install the SDK

<Tabs groupId="language">
<TabItem value="typescript" label="Node">

```bash
npm install agora-agent-server-sdk
```

The package name on npm is `agora-agent-server-sdk`. Imports use `agora-agent-sdk`.
</TabItem>
<TabItem value="python" label="Python">

```bash
pip install agora-agent-sdk
```
</TabItem>
<TabItem value="go" label="Go">

```bash
go get github.com/AgoraIO-Conversational-AI/agent-server-sdk-go
```
</TabItem>
</Tabs>

## Set up environment variables

Store credentials in a `.env` file. Never commit this file or expose its contents client-side; your App Certificate must stay on the server.


```bash
AGORA_APP_ID=your_app_id
AGORA_APP_CERT=your_app_certificate
AGORA_REST_AUTH_TOKEN=your_rest_auth_token
AGORA_RTC_JOIN_TOKEN=your_rtc_join_token

# BYOK vendor keys (not needed if using presets)
DEEPGRAM_API_KEY=your_deepgram_key
OPENAI_API_KEY=your_openai_key
ELEVENLABS_API_KEY=your_elevenlabs_key
ELEVENLABS_VOICE_ID=your_voice_id
```

{/*
<EnvFile 
  fileName=".env"
  credentials={[
    // Agora credentials (from console)
    { key: 'appId', envVar: 'AGORA_APP_ID' },
    { key: 'appCertificate', envVar: 'AGORA_APP_CERT' },
  ]}
  customVariables={[
    // User-provided values (placeholders)
    { blank: true },
    { envVar: 'DEEPGRAM_API_KEY', placeholder: '<your_deepgram_key>', comment: 'ASR,LLM and TTS vendor keys' },
    { envVar: 'OPENAI_API_KEY', placeholder: '<your_openai_key>'},
    { envVar: 'ELEVENLABS_API_KEY', placeholder: '<your_elevenlabs_key>' },
    { envVar: 'ELEVENLABS_VOICE_ID', placeholder: '<your_voice_id>' },
    
    // Fixed values (pre-filled)
    // { envVar: 'MICROSOFT_TTS_REGION', value: 'eastus', comment: 'Azure region for TTS service' },
  ]}
/>
*/}

## Implementation

The following steps set up a client, configure an agent, and manage session lifecycle.

### Create a client

<Tabs groupId="language">
<TabItem value="typescript" label="Node">

`AgoraClient` holds your credentials and handles authentication. Pass your App ID and App Certificate to use app-credentials mode. The SDK auto-generates a temporary token scoped to each channel before every request.

```typescript
import { AgoraClient, Area } from 'agora-agent-sdk';

const client = new AgoraClient({
  area: Area.US, // Sets the regional API endpoint
  appId: process.env.AGORA_APP_ID!,
  appCertificate: process.env.AGORA_APP_CERT!,
});
```

Use the region closest to your users: `Area.US`, `Area.EU`, `Area.AP`, or `Area.CN`.
</TabItem>
<TabItem value="python" label="Python">

The Python SDK supports both synchronous and asynchronous usage. 
See [Sync vs. Async](./concepts/architecture) to choose the right approach for your application.

`Agora` (sync) and `AsyncAgora` (async) hold your credentials and handle authentication. Pass your App ID and App Certificate to use app-credentials mode. The SDK auto-generates a temporary token scoped to each channel before every request.

- **Sync**
  ```python
  import os
  from agora_agent import Agora, Area

  client = Agora(
      area=Area.US,
      app_id=os.environ['AGORA_APP_ID'],
      app_certificate=os.environ['AGORA_APP_CERT'],
  )
  ```

- **Async**
  ```python
  import os
  from agora_agent import AsyncAgora, Area

  client = AsyncAgora(
      area=Area.US,
      app_id=os.environ['AGORA_APP_ID'],
      app_certificate=os.environ['AGORA_APP_CERT'],
  )
  ```

Use the region closest to your users: `Area.US`, `Area.EU`, `Area.AP`, or `Area.CN`.
</TabItem>
<TabItem value="go" label="Go">

In Go, the client handles authentication for REST API calls, while your App ID and App Certificate are passed separately when creating a session. The session uses them to auto-generate a temporary token for the Agora channel.

```go
import (
    "os"

    "github.com/AgoraIO-Conversational-AI/agent-server-sdk-go/client"
    "github.com/AgoraIO-Conversational-AI/agent-server-sdk-go/option"
)

c := client.NewClient(
    option.WithArea(option.AreaUS),
)
```

Use the region closest to your users: `option.AreaUS`, `option.AreaEU`, `option.AreaAP`, or `option.AreaCN`.

</TabItem>
</Tabs>


### Define an agent


`Agent` is a reusable configuration object. Define it once at startup and call `createSession()` on it for each user conversation.

The SDK supports two approaches for configuring ASR, LLM, and TTS:

- **Presets** — Agora manages the vendor credentials and endpoints. No vendor API keys required in your application code.
- **BYOK** — You supply your own vendor API keys and configure vendors directly using typed classes.



<Tabs groupId="language">
<TabItem value="typescript" label="Node">

* **Presets**

  With presets, `Agent` holds only the agent behavior. Vendor selection is handled at session creation time via `AgentPresets`.

  ```typescript
  import { Agent } from 'agora-agent-sdk';

  const agent = new Agent({
    name: 'support-assistant',
    instructions: 'You are a helpful voice assistant. Keep responses concise.', // LLM system prompt
    greeting: 'Hello! How can I help you today?', // first words spoken on session start
    maxHistory: 20,
  });
  ```

* **BYOK**

  With BYOK, attach vendor classes using builder methods. Each vendor class takes your API keys and configuration directly.

  ```typescript
  import { Agent, OpenAI, ElevenLabsTTS, DeepgramSTT } from 'agora-agent-sdk';

  const agent = new Agent({
    name: 'support-assistant',
    instructions: 'You are a helpful voice assistant. Keep responses concise.', // LLM system prompt
    greeting: 'Hello! How can I help you today?', // the first thing the agent says
    maxHistory: 20,
  })
    .withLlm(new OpenAI({
      apiKey: process.env.OPENAI_API_KEY!,
      model: 'gpt-4o-mini',
    }))
    .withTts(new ElevenLabsTTS({
      key: process.env.ELEVENLABS_API_KEY!,
      modelId: 'eleven_flash_v2_5',
      voiceId: process.env.ELEVENLABS_VOICE_ID!,
      sampleRate: 24000,
    }))
    .withStt(new DeepgramSTT({
      apiKey: process.env.DEEPGRAM_API_KEY!,
      model: 'nova-2',
      language: 'en-US',
    }));
  ```

</TabItem>
<TabItem value="python" label="Python">

Vendor classes are imported from `agora_agent.agentkit.vendors`. Builder methods use snake_case.

```python
import os
from agora_agent.agentkit import Agent
from agora_agent.agentkit.vendors import OpenAI, ElevenLabsTTS, DeepgramSTT

agent = (
    Agent(
      name='support-assistant', 
      instructions='You are a helpful voice assistant.', # LLM system prompt
      greeting='Hello! How can I help you today?', # first words spoken on session start
    )
    .with_llm(OpenAI(api_key=os.environ['OPENAI_API_KEY'], model='gpt-4o-mini'))
    .with_tts(ElevenLabsTTS(key=os.environ['ELEVENLABS_API_KEY'], model_id='eleven_flash_v2_5', voice_id=os.environ['ELEVENLABS_VOICE_ID']))
    .with_stt(DeepgramSTT(api_key=os.environ['DEEPGRAM_API_KEY'], language='en-US'))
)
```
</TabItem>

<TabItem value="go" label="Go">

Go uses functional options to configure the agent. Vendor classes are imported from `agentkit/vendors` and constructed with options structs.
```go
import (
    "github.com/AgoraIO-Conversational-AI/agent-server-sdk-go/agentkit"
    "github.com/AgoraIO-Conversational-AI/agent-server-sdk-go/agentkit/vendors"
)

agent := agentkit.NewAgent(
    agentkit.WithName("support-assistant"),
    agentkit.WithInstructions("You are a helpful voice assistant. Keep responses concise."), // LLM system prompt
    agentkit.WithGreeting("Hello! How can I help you today?"), // the first thing the agent says
    agentkit.WithMaxHistory(10),
).WithLlm(
    vendors.NewOpenAI(vendors.OpenAIOptions{
        APIKey: os.Getenv("OPENAI_API_KEY"),
        Model:  "gpt-4o-mini",
    }),
).WithTts(
    vendors.NewElevenLabsTTS(vendors.ElevenLabsTTSOptions{
        Key:     os.Getenv("ELEVENLABS_API_KEY"),
        ModelID: "eleven_flash_v2_5",
        VoiceID: os.Getenv("ELEVENLABS_VOICE_ID"),
    }),
).WithStt(
    vendors.NewDeepgramSTT(vendors.DeepgramSTTOptions{
        APIKey: os.Getenv("DEEPGRAM_API_KEY"),
    }),
)
```

</TabItem>
</Tabs>

For the full list of `Agent` options and vendor classes, see the SDK reference.

### Create and start a session

Bind the agent to a specific RTC channel, register event listeners, then start the session.

Register listeners before starting the session. Otherwise you may miss the `started` event.

<Tabs groupId="language">
<TabItem value="typescript" label="Node">

* **Presets**
  
  Pass `AgentPresets` values to the `preset` option to select Agora-managed ASR, LLM, and TTS. Pass your RTC join token to the `token` option.

  ```typescript
  import { AgentPresets } from 'agora-agent-sdk';

  const session = agent.createSession(client, {
    channel: 'your-channel-name',
    agentUid: '1',
    remoteUids: ['100'],
    idleTimeout: 120,
    token: process.env.AGORA_RTC_JOIN_TOKEN!,
    preset: [
      AgentPresets.asr.deepgramNova3,
      AgentPresets.llm.openaiGpt5Mini,
      AgentPresets.tts.openaiTts1,
    ],
  });

  // Register before start()
  session.on('started', ({ agentId }) => {
    console.log('Agent is live:', agentId);
  });

  session.on('stopped', ({ agentId }) => {
    console.log('Agent has left:', agentId);
  });

  session.on('error', (err) => {
    console.error('Session error:', err);
  });

  const agentId = await session.start();
  ```

* **BYOK**

  ```typescript
  import { ExpiresIn } from 'agora-agent-sdk';

  const session = agent.createSession(client, {
    channel: 'your-channel-name',
    agentUid: '1',
    remoteUids: ['100'],
    idleTimeout: 120,
    expiresIn: ExpiresIn.hours(1),
  });

  // Register before start()
  session.on('started', ({ agentId }) => {
    console.log('Agent is live:', agentId);
  });

  session.on('stopped', ({ agentId }) => {
    console.log('Agent has left:', agentId);
  });

  session.on('error', (err) => {
    console.error('Session error:', err);
  });

  const agentId = await session.start();
  ```

`start()` transitions the session through `idle → starting → running` states and returns the `agentId`. The `agentId` is the same value as the `agent_id` returned by the REST API.

You can check the session state at any time with `session.status`.

</TabItem>
<TabItem value="python" label="Python">

Event handlers receive a dict payload. Register them before calling `start()`.

- **Sync**

  ```python
  from agora_agent.agentkit import expires_in_hours

  session = agent.create_session(
      client,
      channel='your-channel-name',
      agent_uid='1',
      remote_uids=['100'],
      idle_timeout=120,
      expires_in=expires_in_hours(1),
  )

  # Register before start()
  session.on('started', lambda data: print('Agent is live:', data['agent_id']))
  session.on('stopped', lambda data: print('Agent has left:', data['agent_id']))
  session.on('error', lambda err: print('Session error:', err))

  agent_id = session.start()
  ```

- **Async**

  ```python
  agent_id = await session.start()
  ```

`start()` transitions the session through `idle → starting → running` states and returns the `agentId`. The `agentId` is the same value as the `agent_id` returned by the REST API.

You can check the session state at any time with `session.status`.

</TabItem>
<TabItem value="go" label="Go">

```go
import (
    "context"
    "fmt"
    "log"
    "os"

    "github.com/AgoraIO-Conversational-AI/agent-server-sdk-go/agentkit"
)

session := agentkit.NewAgentSession(agentkit.AgentSessionOptions{
    Client:         c.Agents,
    Agent:          agent,
    AppID:          os.Getenv("AGORA_APP_ID"),
    AppCertificate: os.Getenv("AGORA_APP_CERT"),
    Channel:        "your-channel-name",
    AgentUID:       "1",
    RemoteUIDs:     []string{"100"},
})

// Register before Start()
session.On("started", func(data interface{}) {
    info := data.(map[string]string)
    fmt.Println("Agent is live:", info["agent_id"])
})

session.On("stopped", func(data interface{}) {
    fmt.Println("Agent has left")
})

session.On("error", func(data interface{}) {
    log.Println("Session error:", data)
})

ctx := context.Background()
agentId, err := session.Start(ctx)
if err != nil {
    log.Fatalf("Failed to start session: %v", err)
}
```

`Start()` transitions the session through `idle → starting → running` states and returns the agent ID. The agent ID is the same value as the `agent_id` returned by the REST API.

You can check the current session state at any time with `session.Status()`.

</TabItem>
</Tabs>

### Interact with a running session

Once a session is in the `running` state, you can instruct the agent or update its configuration mid-session.

<Tabs groupId="language">
<TabItem value="typescript" label="Node">

```typescript
// Make the agent speak a specific message
await session.say('One moment while I look that up for you.');

// Interrupt the agent mid-speech
await session.interrupt();

// Update agent configuration without restarting the session.
// Accepts a partial properties object in REST API format.
await session.update({
  llm: {
    system_messages: [{ role: 'system', content: 'You are now in concise mode.' }],
  },
});
```

</TabItem>
<TabItem value="python" label="Python">

- **Sync**

  ```python
  # Make the agent speak a specific message
  session.say('One moment while I look that up for you.')

  # Interrupt the agent mid-speech
  session.interrupt()

  # Update agent configuration without restarting the session.
  # Accepts a partial properties object in REST API format.
  session.update({
      'llm': {
          'system_messages': [{'role': 'system', 'content': 'You are now in concise mode.'}],
      },
  })
  ```

- **Async**

  ```python
  await session.say('One moment while I look that up for you.')
  await session.interrupt()
  await session.update({
      'llm': {
          'system_messages': [{'role': 'system', 'content': 'You are now in concise mode.'}],
      },
  })
  ```

</TabItem>
<TabItem value="go" label="Go">

```go
// Make the agent speak a specific message
err := session.Say(ctx, "One moment while I look that up for you.", nil, nil)
if err != nil {
    log.Printf("Say failed: %v", err)
}

// Interrupt the agent mid-speech
err = session.Interrupt(ctx)
if err != nil {
    log.Printf("Interrupt failed: %v", err)
}

// Update agent configuration without restarting the session.
// Accepts a partial UpdateAgentsRequestProperties struct in REST API format.
err = session.Update(ctx, &Agora.UpdateAgentsRequestProperties{
    Llm: &Agora.UpdateAgentsRequestPropertiesLlm{
        SystemMessages: []map[string]any{
            {"role": "system", "content": "You are now in concise mode."},
        },
    },
})
if err != nil {
    log.Printf("Update failed: %v", err)
}
```

</TabItem>
</Tabs>

### Stop the session

Call the session stop method when the user ends the conversation. This transitions the session through `running → stopping → stopped` states and removes the agent from the channel.

<Tabs groupId="language">
<TabItem value="typescript" label="Node">

```typescript
await session.stop();
```

</TabItem>
<TabItem value="python" label="Python">

- **Sync**

  ```python
  session.stop()
  ```

- **Async**

  ```python
  await session.stop()
  ```

</TabItem>
<TabItem value="go" label="Go">

```go
err := session.Stop(ctx)
if err != nil {
    log.Fatalf("Failed to stop session: %v", err)
}
```
</TabItem>
</Tabs>

## Complete example

The following is a minimal server your client app can call to manage agent sessions.

<Tabs groupId="language">
<TabItem value="typescript" label="Node">

<details>
<summary>Presets example</summary>

```typescript
import 'dotenv/config';
import express from 'express';
import {
  AgoraClient,
  Area,
  Agent,
  AgentPresets,
  AgoraError,
} from 'agora-agent-sdk';

const app = express();
app.use(express.json());

// Initialized once at startup; shared across all requests
const client = new AgoraClient({
  area: Area.US,
  appId: process.env.AGORA_APP_ID!,
  appCertificate: process.env.AGORA_APP_CERT!,
  authToken: process.env.AGORA_REST_AUTH_TOKEN!,
});

// No vendor classes needed — presets supply ASR, LLM, and TTS
const agent = new Agent({
  name: 'support-assistant',
  instructions: 'You are a helpful voice assistant. Keep responses concise.',
  greeting: 'Hello! How can I help you today?',
  maxHistory: 20,
});

const sessions = new Map<string, ReturnType<typeof agent.createSession>>();

app.post('/sessions/start', async (req, res) => {
  const { channel, userId } = req.body;

  const session = agent.createSession(client, {
    channel,
    agentUid: '1',
    remoteUids: [userId],
    idleTimeout: 120,
    token: process.env.AGORA_RTC_JOIN_TOKEN!,
    preset: [
      AgentPresets.asr.deepgramNova3,
      AgentPresets.llm.openaiGpt5Mini,
      AgentPresets.tts.openaiTts1,
    ],
  });

  session.on('error', (err) => console.error(`Session error [${channel}]:`, err));

  try {
    const agentId = await session.start();
    sessions.set(agentId, session);
    res.json({ agentId });
  } catch (err) {
    if (err instanceof AgoraError) {
      res.status(err.statusCode).json({ error: err.message });
    } else {
      res.status(500).json({ error: 'Failed to start agent' });
    }
  }
});

app.post('/sessions/stop', async (req, res) => {
  const { agentId } = req.body;
  const session = sessions.get(agentId);

  if (!session) {
    return res.status(404).json({ error: 'Session not found' });
  }

  try {
    await session.stop();
    sessions.delete(agentId);
    res.json({ status: 'stopped' });
  } catch (err) {
    if (err instanceof AgoraError) {
      res.status(err.statusCode).json({ error: err.message });
    } else {
      res.status(500).json({ error: 'Failed to stop agent' });
    }
  }
});

app.listen(3000, () => console.log('Server listening on port 3000'));
```
</details>

<details>
<summary>BYOK example</summary>

```typescript
import 'dotenv/config';
import express from 'express';
import {
  AgoraClient,
  Area,
  Agent,
  ExpiresIn,
  OpenAI,
  ElevenLabsTTS,
  DeepgramSTT,
  AgoraError,
} from 'agora-agent-sdk';

const app = express();
app.use(express.json());

// Initialized once at startup; shared across all requests
const client = new AgoraClient({
  area: Area.US,
  appId: process.env.AGORA_APP_ID!,
  appCertificate: process.env.AGORA_APP_CERT!,
});

const agent = new Agent({
  name: 'support-assistant',
  instructions: 'You are a helpful voice assistant. Keep responses concise.',
  greeting: 'Hello! How can I help you today?',
  maxHistory: 20,
})
  .withLlm(new OpenAI({ apiKey: process.env.OPENAI_API_KEY!, model: 'gpt-4o-mini' }))
  .withTts(new ElevenLabsTTS({
    key: process.env.ELEVENLABS_API_KEY!,
    modelId: 'eleven_flash_v2_5',
    voiceId: process.env.ELEVENLABS_VOICE_ID!,
    sampleRate: 24000,
  }))
  .withStt(new DeepgramSTT({ apiKey: process.env.DEEPGRAM_API_KEY!, model: 'nova-2', language: 'en-US' }));

// Maps agentId → session object.
// In production, use a database or Redis so any server process can stop any session.
const sessions = new Map<string, ReturnType<typeof agent.createSession>>();

// POST /sessions/start
// Called by your client app after the user joins the Agora channel.
// Body: { channel: string, userId: string }
// Returns: { agentId: string }
app.post('/sessions/start', async (req, res) => {
  const { channel, userId } = req.body;

  const session = agent.createSession(client, {
    channel,
    agentUid: '1',
    remoteUids: [userId],
    idleTimeout: 120,
    expiresIn: ExpiresIn.hours(1),
  });

  session.on('error', (err) => console.error(`Session error [${channel}]:`, err));

  try {
    const agentId = await session.start();
    sessions.set(agentId, session);
    res.json({ agentId });
  } catch (err) {
    if (err instanceof AgoraError) {
      res.status(err.statusCode).json({ error: err.message });
    } else {
      res.status(500).json({ error: 'Failed to start agent' });
    }
  }
});

// POST /sessions/stop
// Called by your client app when the user ends the conversation.
// Body: { agentId: string }
app.post('/sessions/stop', async (req, res) => {
  const { agentId } = req.body;
  const session = sessions.get(agentId);

  if (!session) {
    return res.status(404).json({ error: 'Session not found' });
  }

  try {
    await session.stop();
    sessions.delete(agentId);
    res.json({ status: 'stopped' });
  } catch (err) {
    if (err instanceof AgoraError) {
      res.status(err.statusCode).json({ error: err.message });
    } else {
      res.status(500).json({ error: 'Failed to stop agent' });
    }
  }
});

app.listen(3000, () => console.log('Server listening on port 3000'));
```
</details>

</TabItem>

<TabItem value="python" label="Python">

<details>
<summary>Python example</summary>

- **Sync**

  ```python
  import os
  from agora_agent import Agora, Area
  from agora_agent.agentkit import Agent
  from agora_agent.agentkit.vendors import OpenAI, ElevenLabsTTS, DeepgramSTT

  client = Agora(
      area=Area.US,
      app_id=os.environ['AGORA_APP_ID'],
      app_certificate=os.environ['AGORA_APP_CERT'],
  )

  agent = (
      Agent(
          name='support-assistant',
          instructions='You are a helpful voice assistant.',  # LLM system prompt
          greeting='Hello! How can I help you today?',       # first words spoken on session start
      )
      .with_llm(OpenAI(api_key=os.environ['OPENAI_API_KEY'], model='gpt-4o-mini'))
      .with_tts(ElevenLabsTTS(
          key=os.environ['ELEVENLABS_API_KEY'],
          model_id='eleven_flash_v2_5',
          voice_id=os.environ['ELEVENLABS_VOICE_ID'],
      ))
      .with_stt(DeepgramSTT(api_key=os.environ['DEEPGRAM_API_KEY'], language='en-US'))
  )

  session = agent.create_session(
      client,
      channel='support-room-123',
      agent_uid='1',
      remote_uids=['100'],
  )
  agent_id = session.start()

  session.say('Hello! How can I help you today?')
  session.stop()
  ```

- **Async**

  ```python
  import asyncio
  import os
  from agora_agent import AsyncAgora, Area
  from agora_agent.agentkit import Agent
  from agora_agent.agentkit.vendors import OpenAI, ElevenLabsTTS, DeepgramSTT

  async def main():
      client = AsyncAgora(
          area=Area.US,
          app_id=os.environ['AGORA_APP_ID'],
          app_certificate=os.environ['AGORA_APP_CERT'],
      )

      agent = (
          Agent(
              name='support-assistant',
              instructions='You are a helpful voice assistant.',  # LLM system prompt
              greeting='Hello! How can I help you today?',       # first words spoken on session start
          )
          .with_llm(OpenAI(api_key=os.environ['OPENAI_API_KEY'], model='gpt-4o-mini'))
          .with_tts(ElevenLabsTTS(
              key=os.environ['ELEVENLABS_API_KEY'],
              model_id='eleven_flash_v2_5',
              voice_id=os.environ['ELEVENLABS_VOICE_ID'],
          ))
          .with_stt(DeepgramSTT(api_key=os.environ['DEEPGRAM_API_KEY'], language='en-US'))
      )

      session = agent.create_session(
          client,
          channel='support-room-123',
          agent_uid='1',
          remote_uids=['100'],
      )
      agent_id = await session.start()

      await session.say('Hello! How can I help you today?')
      await session.stop()

  asyncio.run(main())
  ```
</details>

</TabItem>

<TabItem value="go" label="Go">

<details>
<summary>Go example</summary>

```go
package main

import (
    "context"
    "fmt"
    "log"
    "os"

    // uncomment for typed structs and pointer helpers
    // Agora "github.com/AgoraIO-Conversational-AI/agent-server-sdk-go"
    "github.com/AgoraIO-Conversational-AI/agent-server-sdk-go/agentkit"
    "github.com/AgoraIO-Conversational-AI/agent-server-sdk-go/agentkit/vendors"
    "github.com/AgoraIO-Conversational-AI/agent-server-sdk-go/client"
    "github.com/AgoraIO-Conversational-AI/agent-server-sdk-go/option"
)

func main() {
    // Initialized once at startup; shared across all requests
    c := client.NewClient(
        option.WithArea(option.AreaUS),
    )

    agent := agentkit.NewAgent(
        agentkit.WithName("support-assistant"),
        agentkit.WithInstructions("You are a helpful voice assistant. Keep responses concise."), // LLM system prompt
        agentkit.WithGreeting("Hello! How can I help you today?"),                               // first words spoken on session start
        agentkit.WithMaxHistory(10),
    ).WithLlm(
        vendors.NewOpenAI(vendors.OpenAIOptions{
            APIKey: os.Getenv("OPENAI_API_KEY"),
            Model:  "gpt-4o-mini",
        }),
    ).WithTts(
        vendors.NewElevenLabsTTS(vendors.ElevenLabsTTSOptions{
            Key:     os.Getenv("ELEVENLABS_API_KEY"),
            ModelID: "eleven_flash_v2_5",
            VoiceID: os.Getenv("ELEVENLABS_VOICE_ID"),
        }),
    ).WithStt(
        vendors.NewDeepgramSTT(vendors.DeepgramSTTOptions{
            APIKey: os.Getenv("DEEPGRAM_API_KEY"),
        }),
    )

    session := agentkit.NewAgentSession(agentkit.AgentSessionOptions{
        Client:         c.Agents,
        Agent:          agent,
        AppID:          os.Getenv("AGORA_APP_ID"),
        AppCertificate: os.Getenv("AGORA_APP_CERT"),
        Channel:        "support-room-123",
        AgentUID:       "1",
        RemoteUIDs:     []string{"100"},
    })

    session.On("started", func(data interface{}) {
        info := data.(map[string]string)
        fmt.Println("Agent is live:", info["agent_id"])
    })
    session.On("stopped", func(data interface{}) {
        fmt.Println("Agent has left")
    })
    session.On("error", func(data interface{}) {
        log.Println("Session error:", data)
    })

    ctx := context.Background()
    agentId, err := session.Start(ctx)
    if err != nil {
        log.Fatalf("Failed to start session: %v", err)
    }
    fmt.Println("Agent running with ID:", agentId)

    err = session.Say(ctx, "Hello! How can I help you today?", nil, nil)
    if err != nil {
        log.Printf("Say failed: %v", err)
    }

    err = session.Stop(ctx)
    if err != nil {
        log.Fatalf("Failed to stop session: %v", err)
    }
}
```
</details>

</TabItem>
</Tabs>
