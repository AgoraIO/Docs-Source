...
<CodeItem language="bash" name="curl">{`curl --request post \\
  --url https://api.agora.io/api/conversational-ai-agent/v2/projects/:appid/agents/:agentId/update`}</CodeItem>
...
print(response.text)`}</CodeItem>
  ...
  <CodeItem language="js" name="Node.js">{`const url = 'const url = 'https://api.agora.io/api/conversational-ai-agent/v2/projects/:appid/agents/:agentId/update';
