...
![token_generate_user_token](/images/chat/user_token_generation.png)
...
```java
// Generates a token with app privileges. The parameters `appid`, `appcert`, `expire` and  `chatUserId` refer to App ID, App certificate, token validity period and UID of the Agora Chat user respectively.
ChatTokenBuilder2 builder = new ChatTokenBuilder2();
return builder.buildUserToken(appid, appcert, chatUserId, expirePeriod);
```
