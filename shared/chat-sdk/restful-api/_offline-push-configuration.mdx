...
This page shows how to call <Vg k="CHAT"/> RESTful APIs to set the display name, display style, push notification mode, and do-not-disturb (DND) mode.
...
|  Parameter  |  Type  |  Description  |
| :-------- | :------- | :------- |
|  `uuid` | String | A unique internal identifier generated by the <Vg k="CHAT"/> service for the user in this request. |
| `type`  |  String  | The type of the chat. "user" indicates a <Vg k="CHAT_ONE"/>. |
...
| :----- | :----- | :------- | -------- |
| `userId` | String | The user ID of the current user.  | Yes |
| `chattype` | String | The type of the chat:<li>`user`: <Vg k="CHAT_ONE"/>s.</li><li>`chatgroup`: <Vg k="CHAT_GROUP"/>s.</li>  | Yes |
| `key` | String | The identifier of the chat:<li>If `type` is set to `user`, `key` indicates the user ID of the peer user.</li><li>If `type` is set to `chatgroup`, `key` indicates the ID of the <Vg k="CHAT_GROUP_LOWER"/>.</li> | Yes |
