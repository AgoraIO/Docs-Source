...
The Web SDK supports on Chrome 58 and higher with or without using a <Vg k="SCR_SHR" /> extension.

The <Vg k="COMPANY" /> Web SDK supports <Vg k="SCR_SHR" /> on Chrome version 58 and above. You can follow these steps to share your screen.

Call `createScreenVideoTrack` to create a <Vg k="SCR_SHR" />  track.

This method requires Chrome 74 or above. If you are using an older version, please use the screen sharing plug-in to share the screen on Chrome.
