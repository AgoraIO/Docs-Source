...
| `state`    | String | The running status of the Converter:<li>`idle`: The <Vg k="MPUSH" /> has not started or has ended.</li><li>`connecting`: Connecting to the Agora streaming server and the CDN server.</li><li>`running`: The  Agora streaming server is pushing the streams.</li><li>`failed`: The <Vg k="MPUSH" /> fails.</li> |
