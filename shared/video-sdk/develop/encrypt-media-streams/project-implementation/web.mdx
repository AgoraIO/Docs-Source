...
<Admonition type="caution" title="note">All users in the same channel must use the same encryption mode, key, and salt.</Admonition>

### Generate and set the key on your server.
...
### Generate and set the salt on your server.

    1. Execute the following command on your server to randomly generate a Base64 encoded, 32-byte salt using OpenSSL.You can also refer to the [C++ example code](https://github.com/AgoraIO/Tools/blob/master/DynamicKey/AgoraDynamicKey/cpp/sample/RtcChannelEncryptionSaltSample.cpp) provided by <Vg k="COMPANY" /> on GitHub.
    ```shell
    # Randomly generate a Base64 encoded, 32-byte salt
    openssl rand -base64 32
    X5w9T+50kzxVOnkJKiY/lUk82/bES2kATOt3vBuGEDw=
    ```
    1. The Web client obtains the Hex encoded key from the server.
