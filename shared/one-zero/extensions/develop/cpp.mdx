...
Introduction to developing a <Vg k="AI_FRAMEWORK" /> extension using the GO language, as well as debugging and deploying it to run in an app. 

This page shows you how to:

- Create a GO extension development project using arpm.
- Use TEN API to implement the functionality of the extension, such as sending and receiving messages.
- Configure cgo if needed.
- Write unit tests and debug code.
- Deploy the extension locally in an app for integration testing.
- Debug extension code in an app.
...
* Download the latest version of `arpm` and configure the `PATH`. You can check if it is configured correctly by running the following command:
    ...
    If the configuration is correct, you see help information for `arpm`.

* Install GO 1.20 or above, preferably the latest version.

    <Vg k="AI_FRAMEWORK" /> GO API uses `cgo`, so make sure `cgo` is enabled by default. You can check this by running the following command:

    ```bash
    go env CGO_ENABLED
    ```

    If it returns 1, it means `cgo` is enabled by default. To enable cgo, run the following command:

    ```bash
    go env -w CGO_ENABLED=1
    ```

## Create a GO extension project
...
`default_extension.go` contains a simple extension implementation that includes calls to the TEN GO API. The usage of the TEN API will be explained in the next section.
...
The `property.json` file is initially empty and The `manifest.json` file includes a dependency on `rte_runtime_go` by default:
...
- The api section is used to declare the schema of the extension. For the usage of schema, refer to: rte-schema
...
At this point, you have created a TEN GO extension project.
...
- `OnStart`: Used to start the extension instance, such as creating connections to external services.   
    The extension will not receive messages until it is started. In `OnStart`, you can use the `rte.GetProperty` related APIs to get the extension's configuration.
...
`OnCmd`, `OnData`, `OnImageFrame`, `OnPcmFrame`: Callback functions for receiving four types of messages. The message types in TEN can be referred to as [message-type-and-name](../framework/api).
...
- `RegisterAddonAsExtension` is the method to register an addon object as an TEN extension addon. The addon types in TEN also include extension_group and protocol, which will be covered in the subsequent integration testing section.
...
- Download the `default_app_go` from the central repository as the integration testing environment, along with any other dependencies needed.

- Download the first_go_extension from the local registry.

- Configure the graph in `default_app_go` to specify the receiver of the message as `first_go_extension` and send test messages.

### Uploading the extension to the Local registry

Before uploading, restore the dependency path for TEN GO binding in `first_go_extension/go.mod` because it needs to be installed under the `app`. For example:
...
Then, in the `first_go_extension` directory, execute the following command to upload the extension to the local registry:

```bash
arpm --config-file /tmp/code/config.json publish
```

After the command completes, the uploaded extension can be found in the directory `/tmp/code/repository/extension/first_go_extension/0.1.0`.
...
1. Install `default_app_go` as the test app in an empty directory.
    ...
    - When installing the app, its dependencies are automatically installed.
    </Admonition>

1. Install the extension_group in the app directory.
...
1. Install the first_go_extension that we want to test in the app directory.
...
1. Add an extension as a message producer.
    ...
    The `first_go_extension` is expected to receive a hello cmd, so we need a message producer. One way to achieve this is by adding an extension as a message producer. To conveniently generate test messages, we can integrate an HTTP server into the producer's extension.

    First, we can create an HTTP server extension based on `default_extension_go`. Execute the following command in the app directory:

    ```bash
    arpm install extension default_extension_go --template-mode --template-data package
    ```

    Modify the module name in `addon/extension/http_server/go.mod` to `http_server`.
