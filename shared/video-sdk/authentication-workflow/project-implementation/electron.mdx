...
6.  **Fetch a token before you join a channel**
    ...
    Before you join a channel, you need to call `FetchToken` to get a fresh token. To do this, in `preload.js`, add the following code before `agoraEngine.joinChannel(token, channel, null, 123456);`:
