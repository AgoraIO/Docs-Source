...
| `chatroom` | Number | The remaining time that this user is muted in chat rooms, in seconds. The maximum value is 2,147,483,647.<ul><li>&gt; 0: The remaining duration for muting this user in <Vg k="CHAT_ROOM_LOWER"/>s.</li><li>0: This user is unmuted in <Vg k="CHAT_ROOM_LOWER"/>s.</li><li>-1: This user is permanently muted in <Vg k="CHAT_ROOM_LOWER"/>s.</li></ul> |
| `unixtime` | Number | The Unix timestamp of the current operation. |
