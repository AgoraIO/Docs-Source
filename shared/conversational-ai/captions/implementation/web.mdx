...
1. **Implement caption UI rendering logic**
    ...
    The caption UI module you implement processes the `MessageEngine` caption message list and includes a simple component to display these messages:
    ...
    The sample code uses `window.addEventListener("message")` to listen for caption data sent by `MessageEngine` using `window.postMessage`. For complex applications, Agora recommends using [Redux](https://redux.js.org/introduction/getting-started) or other state management tools to manage these messages more efficiently. 
    </Admonition>
