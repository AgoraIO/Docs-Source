...
4.  **Enable your <Vpl k="CLIENT" /> to display a local video stream**
    ...
    <PlatformWrapper platform="ios">
    <div class="note alert">You can enable both cameras using <Link to="{{Global.API_REF_IOS_ROOT_RTC_ENGINE_KIT}}/enablemulticamera(_:config:)">enableMultiCamera</Link>.</div>
    </PlatformWrapper>
    <PlatformWrapper platform="macos">
    <div class="note alert">You can enable both cameras using <Link to="{{Global.API_REF_MACOS_ROOT_RTC_ENGINE_KIT}}/enablemulticamera(_:config:)">enableMultiCamera</Link>.</div>
    </PlatformWrapper>
