...
1. **Optimization of video pre-processing methods**
   ...
   This release adds overloaded methods with the `souкceType` parameter for the following 5 video preprocessing methods, which support specifying the media source type for applying video preprocessing effects by passing in `sourceType` (for example, applying on a custom video capture media source):
