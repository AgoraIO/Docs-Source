...
To create a simple interface for testing speech to text functionality, you add two buttons to the UI. Open `index.html` and add the following lines before `</RelativeLayout>`:
...
1. **Import the `https` module into your project**
    ...
    To call `HTTP` methods that start, stop, and query a speech-to-text task, you add a `https` module to your project. Open `main.js` and add the following before `let agoraEngine;`:
...
1. **Query task status**
    ...
    You [query the status](#query-the-status-of-speech-to-text) of a speech-to-text task using a `GET` request containing the `taskId` and `builderToken` to notify the user of any change. To do this, in `main.js`, add the following method before `document.getElementById("join").onclick = async function ()`:
...
1. **Stop the task**
    ...
    To [stop speech to text](#stop-speech-to-text), you send a `DELETE` request with the, `appId`, `taskId` and `builderToken` in the URL. In the `main.js`, add the following method before `window.onload = () =>`:
...
1. **Receive and display the text stream**
    ...
    When you set one of the `destinations` in the starting configuration as `AgoraRTCDataStream`, the speech-to-text conversion output stream is pushed to your RTC channel. To receive this stream and display the text in your <Vpl k="CLIENT" />, in `main.js`, add the following code after the declarations:
