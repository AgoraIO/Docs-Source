...
| 206 | `WEBIM_CONNCTION_USER_LOGIN_ANOTHER_DEVICE` | The user has logged in on another device. For a single device login scenario, the user is forced to log out of the current device upon successful login on another device with the same account. For a multi-device login, if you have configured the number of allowed web devices, the policy for forced logout between devices is related to the `ConnectionParameters#isFixedDeviceId` parameter. For details, see the [multi-device login documentation](../develop/multiple-device-login). |
