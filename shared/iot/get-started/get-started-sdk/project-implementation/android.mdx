...
At startup, you initialize the RTC service. When a user taps a button, the <Vpl k="CLIENT" /> joins a channel and sends audio and video data to remote users. 

### Implement the user interface

In the simple interface, you create two buttons to join and leave a channel. In `/app/res/layout/activity_main.xml`, replace the contents of the file with the following:
...
2.  **Handle Android permissions**
    ...
    Ensure that the permissions necessary to use <Vpd k="PRODUCT" /> features are granted. If the permissions are not granted, use the built-in Android feature to request them; if they are granted, return `true`.
...
To start the RTC service and join a channel, take the following steps:
...
1.  **Setup <Vg k="COMPANY" /> RTC Service**
    ...
    To use <Vpd k="PRODUCT" /> features, you use the <Vpd k="SDK" /> to create an `AgoraRtcService` instance. You then use the service instance to open a connection. In `/app/java/com.example.<projectname>/MainActivity`, add the following code before the `onCreate` method:

    ``` java
    private void setupAgoraRtcService(){
        agoraRtcService= new AgoraRtcService();
        showMessage("RTC SDK version " + agoraRtcService.getVersion());
        ...
        options.licenseValue = rtcLicense; // for license verification
        int ret = agoraRtcService.init(appId, agoraRtcEvents, options);
        if (ret != AgoraRtcService.ErrorCode.ERR_OKAY) {
            showMessage("Fail to initialize RTC sdk " + ret);
            agoraRtcService = null;
            return;
        } else {
            showMessage("Service initialized");
        }
...
1.  **At startup, ensure necessary permissions and start the service**
    
    In order to send video and audio streams to <Vg k="COMPANY" />, you need to ensure that the local user gives permission to access the camera and microphone at startup. After the permissions are granted, you setup the <Vpd k="NAME"/> service.
    ...
    In `/app/java/com.example.<projectname>/MainActivity`, replace `onCreate` with the following code:
        ...
        protected void onCreate(Bundle savedInstanceState) {
            ...
            // If all the permissions are granted, setup the RTC Service.
...
1.  **Receive and handle service events**

    The `AgoraRtcService` provides a number of callbacks that enable you to respond to important events. To add a service event handler to your <Vpl k="CLIENT" />, in `/app/java/com.example.<projectname>/MainActivity`, add the following lines before `setupAgoraRtcService`:
...
1.  **Join a channel**
    ...
    When a local user initiates a connection, call `joinChannel`. This method securely connects the local user to a channel using the authentication token. In `/app/java/com.example.<projectname>/MainActivity`, add the following code after `setupAgoraRtcService`:
        ...
        public void joinChannel(View view) {
            ...
            // Join a channel
            int ret = agoraRtcService.joinChannel(connectionId, channelName,
...
In this implementation, you initiate and destroy the `AgoraRtcService` when the app opens and closes. The local user joins and leaves a channel using the same service instance. To elegantly exit your app:
