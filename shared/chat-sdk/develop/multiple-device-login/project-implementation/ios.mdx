...
Call `kickDeviceWithUserId` to pass in the user ID and user token to kick the specified account off from a single logged-in device. Before calling this method, obtain the device ID through the `AgoraChatClient#getLoggedInDevicesFromServer` and `AgoraChatDeviceInfo#resource` methods.
...
Call `kickAllDevicesWithUserId` to kick the specified account off from all logged-in devices by passing in the user ID and user token.
...
extension AppDelegate: AgoraChatMultiDevicesDelegate {
    func multiDevicesContactEventDidReceive(_ aEvent: AgoraChatMultiDevicesEvent, username aUsername: String, ext aExt: String?) {
        ...
        case .contactDecline:
            break
            //The current user adds a user to the blacklist on other devices.
        case .contactBan:
            break
    ...
    func multiDevicesGroupEventDidReceive(_ aEvent: AgoraChatMultiDevicesEvent, groupId aGroupId: String, ext aExt: Any?) {
        ...
        case .groupKick:
            break
        //The current user has added members to the group blacklist on other devices
        case .groupBan:
            break
        //The current user has removed members from the group blacklist on other devices
        case .groupAllow:
            break
    ...
    func multiDevicesChatThreadEventDidReceive(_ aEvent: AgoraChatMultiDevicesEvent, threadId aThreadId: String, ext aExt: Any?) {
        ...
        case .chatThreadCreate:
            //The current user creates a sub-area on other devices.
            break
        case .chatThreadDestroy:
            //The current user destroys the sub-area on other devices.
            break
        case .chatThreadJoin:
            //The current user joins the sub-area on other devices.
            break
        case .chatThreadLeave:
            //The current user leaves the sub-area on other devices.
            break
        case .chatThreadUpdate:
            //The current user updates the sub-area on other devices.
            break
        case .chatThreadKick:
            //The current user kicks a member out of the sub-area on other devices.
            break
        default:
            break
        }
    ...
    func multiDevicesMessageBeRemoved(_ conversationId: String, deviceId: String) {
        //The current user deletes the historical message on the server side of a one-way conversation on other devices.
    }
}
