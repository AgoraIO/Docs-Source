...
Before you begin, make sure you have downloaded the latest <Vpd k="NAME" />. See [Integrate the C++ SDK](../get-started/integrate-c-sdk).
...
Refer to [<Vpd k="NAME" /> C++ API Reference](https://docs.agora.io/en/server_gateway/API%20Reference/linux_server_cpp/index.html) to find detailed API and parameter descriptions.
...
- When you use the [Agora RTC SDK](../reference/glossary#agora-video-sdk) to communicate with the <Vpd k="NAME" />, make sure to set the channel scenario of the RTC SDK to `LIVE_BROADCASTING`.
