...
Import the necessary Android classes and and add a method to show status updates to the user.
...
When a user opens the app, you instantiate and initialize the `ChatClient`. When the user taps a button, the app logs in or out of the chat server. When a user presses Send, the typed message is sent to the chat server. When the <Vpl k="CLIENT"/> receives a message from another user, the message is displayed in the message list. This simple workflow enables you to rapidly build a functional chat client. 
...
1. **Declare variables**
   ...
   ```java
    private String username = "<Username of the local user>";
    ...
    private void setupListeners() {
        ...
        agoraChatClient.addConnectionListener(new ConnectionListener() {
            ...
            public void onLogout(int errorCode) {
                showLog("User needs to log out: " + errorCode);
                agoraChatClient.logout(false, null);
            }
...
1. **Log in to the chat server**  
    ...
    When a user presses **Join**, your <Vpl k="CLIENT"/> logs in to the chat server. When a user presses **Leave**, the <Vpl k="CLIENT"/> log out of the chat server.
    ...
    public void joinLeave(View view) {
        ...
        if (isJoined) {
            ...
            agoraChatClient.loginWithAgoraToken(username, token, new CallBack() {
                @Override
                public void onSuccess() {
                    showLog("Signed in");
                    isJoined = true;
...
1. **Send a message**
    ...
    To send a message to the chat server when a user presses the send button, add the following method to the `MainActivity` class, before `onCreate`:
...
7. **Display sent and received messages**
    
    To display sent and received messages in a chat format, add the following method to the `MainActivity` class:
