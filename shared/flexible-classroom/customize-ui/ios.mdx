...
| `/Views` | The basic UI components used in Flexible Classroom, such as the video rendering window and the pop-up window. |
...
- `UIComponent`
   ...
   - `UIComponent` is in the `/Components` folder of the `AgoraEduUI` library, and is divided into the following two types:
      ...
      - `SuspendComponents`:  UI component of popup type.
