...
1. Generate two [temporary tokens](../reference/manage-agora-account#generate-a-temporary-token) in <Vg k="CONSOLE" />.

    1. Generate `token` using `appId` and `channelName`.

    1. Generate `token2` using `appId` and `channelName2`.
...
1. Click **Join** to join the channel over each connection. 
