...
Performance profiling helps identify bottlenecks and optimize resource usage in TEN applications. This guide covers how to profile CPU usage and memory consumption using language-specific tools and analyze the resulting performance data.

TEN supports profiling for different programming languages using their respective profiling tools:

- **Go projects**: Use `pprof`, Go's built-in profiler, for CPU and memory analysis
- **C++ projects**: Use `gperftools` (Google Performance Tools) for detailed CPU and heap memory profiling

Both approaches provide detailed performance insights through data collection, analysis tools, and visualization options to help you optimize your TEN applications effectively.


## TEN framework GO project

Use `gperftools` to profile your program’s CPU usage and heap memory usage. The following steps describe how to prepare the environment, profile your program using gperftools, and analyze the performance data.
