...
-   **Automatic** (Default mode)
    ...
    In the Automatic mode of <Vg k="CP"/>, <Vpd k="SDK" /> first attempts a direct connection to <Vg k="AGORA_BACKEND" />; if the attempt fails, <Vpd k="SDK" /> automatically falls back and sends media securely on TLS 443. This is best practice when you are not sure if your end users are behind a firewall. Sending media over TLS 443 may not have as high quality UDP. However, a connection on TLS 443 works through most firewalls.
