...
3. To launch a classroom, call [FcrUIScene.launch](/flexible-classroom/client-api/ui-scene#launch).
...
2. To launch a classroom, call [FcrUIScene.launch](/flexible-classroom/client-api/ui-scene#launch).
...
The sample code requires passing in `rtmToken`. You can refer to [Secure authentication with tokens](/flexible-classroom/develop/integrate/authentication-workflow) to learn what a <Vg k="MESS" /> token is, how to get a temporary <Vg k="MESS" /> token for testing purposes, and how to generate a <Vg k="MESS" /> token from the server. The generated token passed in `userId` must be consistent with the `userUuid` parameters passed in the `launch` method; otherwise, the generated token will be invalid.
...
<div class="alert info">Make sure you have <a href="/flexible-classroom/get-started/enable-flexible-classroom">set up a development environment</a>.</div>
...
1. Modify the code according to your needs. See [FcrUIScene SDK customization guide](/flexible-classroom/develop/integrate/customize-ui/customize-ui-scene-sdk) for details.
