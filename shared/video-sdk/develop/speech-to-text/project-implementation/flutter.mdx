...
To create a simple interface for testing speech to text functionality, you add two buttons to the UI. Open `/lib/main.dart` and add the following code to the `build` method after `ListView(...children: [`:
...
1. **Receive and display the text stream**
    ...
    When you set one of the `destinations` in the starting configuration as `AgoraRTCDataStream`, the speech-to-text conversion output stream is pushed to your RTC channel. To receive this stream in a callback and display the text in your <Vpl k="CLIENT" />, add the following code after `agoraEngine.registerEventHandler(RtcEngineEventHandler(` in `setupVideoSDKEngine()`:
