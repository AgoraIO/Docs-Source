...
If you call `logoutWithCompletion` to log out of <Vg k="SIG" />, the connection state changes to `AgoraRtmConnectionStateDisconnected`. The <Vg k="MESS" /> system removes the user from the online user list and channel, and users in the same channel receive the `memberLeft` callback.
