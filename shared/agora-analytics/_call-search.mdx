...
3.  Choose to search by **Channel Name** or **User** ([user ID](../../reference/glossary#user-id)) in the drop-down menu.

4.  Filter the calls by the call states, and you can choose to search for calls that are ongoing or ended.

5.  Enter a channel name or [user ID](../../reference/glossary#user-id) and click **Search**.
...
The user list shows the information of the users in the call, such as the [user ID](../../reference/glossary#user-id) (`uid` or `userId`), the location, when the user joins/leaves the channel and the actual time in the channel (In-call Periods).
...
The user’s basic information is shown above the QoE diagram, including the [user ID](../../reference/glossary#user-id), platform, and SDK version.
...
Select the spikes on the timeline to see the [user events](#_user_events). Pay attention to the red spikes, which represent important events, such as failure to join the channel.
...
Each user in the call has a unique [user ID](../../reference/glossary#user-id) (`uid` or `userId`). <Vpd k="NAME"/> displays and analyzes the users' QoE from the perspective of sending and receiving data.
