...
1. **Set up the music content center**
    ...
    1. To configure your `IAgoraMusicContentCenter` you define `MusicContentCenterConfiguration` and pass it to the `initialize` method. For receiving preloading and search results you define an `IMusicContentCenterEventHandler` and attach it to the music content center during configuration. Add the following `setupMusicContentCenter` method to the `MainActivity` class:
