...
## Integrated education scenarios
...
1. After version 2.8.70, the following code needs to be initialized in the Application method:

    ```kotlin
    AgoraSDKInitUtils.initSDK(this);
    ```

1. To launch a classroom, call [AgoraClassroomSDK.launch](../reference/classroom-sdk#launch). Sample code:
...
1. Call [AgoraOnlineClassroomSDK.launch](/api-ref/flexible-classroom/android/classroom-sdk#launch) to start the class. The parameters of `AgoraOnlineClassroomSDK` and `AgoraClassroomSDK` are the same. Refer to the following sample code:
