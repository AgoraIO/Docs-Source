...
The `uid` parameter is removed from the `EncodedVideoFrameInfo` struct. You can retrieve the `remoteUid` directly from the `onEncodedVideoFrameReceived` callback. If you have integrated related code previously, please update your integration logic accordingly.
...
**Seamless switching for `playEffect`**
...
**Improved accuracy of `onNetworkQuality`**

This version improves the accuracy of network quality evaluation in the `onNetworkQuality` callback, making the reported data better reflect the user's perceived experience.

**Support for 24kHz audio frames in `onPlaybackAudioFrame`**

This version adds support for audio data with a sample rate of 24000 Hz in `onPlaybackAudioFrame`. When calling `setPlaybackAudioFrameParameters` to set the playback audio format, you can set `sampleRate` to `24000`.

**`onEncodedVideoFrameReceived` supports `channelId`**

This version adds the `channelId` parameter to the `onEncodedVideoFrameReceived` callback, helping you identify which channel the encoded video data belongs to.

**Additional improvements**

- Added error codes `ERR_PCMSEND_FORMAT (200)` and `ERR_PCMSEND_BUFFEROVERFLOW (201)` to report PCM data transmission errors.
- Improved video capture performance in low-light or dark environments on some devices, resulting in brighter images.
...
- On certain devices (e.g., Redmi 14C), video from the sender occasionally displays black borders on the receiver side during audio and video engagements.
- The `bitrate` setting in the `startLocalTranscoder` API does not take effect.
- On some emulators (e.g., LDPlayer 9), electrical noise may occasionally be heard on the receiver side during audio and video engagements.
- In live PK scenarios, when the audience enables independent rendering of composite video streams, the rendered user video may display abnormally.
- During audio and video engagements, after calling `playEffect` locally, if a remote user switches to broadcaster and starts speaking, electrical noise may occasionally be heard locally.
- On some vivo devices running Android 15, the app may crash when the camera is turned off.
- When a user joins a channel as a broadcaster with the audio scenario set to `AUDIO_SCENARIO_CHATROOM` and does not publish local audio, then plays a music file using `startAudioMixing`, and switches the audio scenario to `AUDIO_SCENARIO_GAMESTREAMING`, noise may occasionally be heard.
- The app may occasionally become unresponsive (ANR) during audio and video engagements.
- Fixed occasional crashes during audio and video engagements.
- Beauty effects do not work in some scenarios.
...
- **Deprecation of direct CDN streaming APIs**  
   ...
   Deprecates the APIs related to direct CDN streaming, which will be removed in a future release. Agora recommends using [Media Push](https://docs.agora.io/en/media-push/get-started/enable-media-push) instead.  
