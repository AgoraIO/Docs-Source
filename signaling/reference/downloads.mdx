...
1. In `package.json` for your project, add agora-rtm and its version number to the dependencies field:
    ```json
      "dependencies": {
        "agora-rtm": "latest",
        "cors-anywhere": "^0.4.4",
...
1. In your code, import `AgoraRTM` from 'agora-rtm'
    ```javascript
        rtmConfig = rtmConfig || {
          token: "your Signaling token for User ID",
          logLevel: "debug",
          useStringUserId: false,
        };
        AgoraRTM.setArea({ areaCodes: ["ASIA"] });
        signalingEngine = new AgoraRTM.RTM("Your APP ID", "User ID", rtmConfig);
    ```
