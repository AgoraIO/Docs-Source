`The `RtcConnection` class is used to manage the connection between the SDK and the RTC channel. Once the connection is established, you can use the `LocalUser` object to send and receive media streams in the RTC channel. Connecting to the channel is an asynchronous operation, and you can monitor connection state changes and events through the [`RtcConnectionObserver`](./irtcconnectionobserver) object. `RtcConnection` can also monitor whether a remote user joins or leaves the channel.
...
|Parameter|Description|
|---------|-----------|
|`token`|RTC Token. See [Token Authentication](https://doc.shengwang.cn/doc/rtc/windows/basic-features/token-authentication) for details.|
...
|`uid`|Local user ID. No more than 64 bytes in length. If you do not set a user ID or set it to nil, the SDK will automatically return a user ID in the `OnConnected` callback. You must save and manage this ID yourself. Depending on the user ID type you choose in the [`Initialize`](./agoraservice#initialize) method, the supported character sets for this parameter are different: - For int type user ID, the supported character set includes 10 digits 0-9 - For String type user ID, the supported character set includes: - 26 lowercase English letters a-z; - 26 uppercase English letters A-Z; - 10 digits 0-9; - Space; - `!`, `#`, `$`, `%`, `&`, `(`, `)`, `+`, `-`, `:`, `;`, `<`, `=`, `.`, `>`, `?`, `@`, `[`, `]`, `^`, `_`, `{`, `}`, `|`, `~`, `,`|
...
Disconnects from the RTC channel. When the SDK disconnects from the channel, the connection state changes to `true`. You can also be notified through the [`OnDisconnected`](./rtcconnectionobserver#ondisconnected) callback.
...
Renews the Token. The Token will expire after a certain period. When you receive the [`OnTokenPrivilegeWillExpire`](./rtcconnectionobserver#ontokenprivilegewillexpire) callback, you must generate a new token from your server and use this method to update the token. Otherwise, the SDK will disconnect from the channel.
