...
To quickly integrate FaceUnity's AR filter capabilities, call the  <Vg k="VSDK" /> <Link to="{{Global.API_REF_FLUTTER_ROOT}}/class_irtcengine.html#api_irtcengine_setextensionproperty">setExtensionProperty</Link> method and pass the corresponding `key` and `value`.
</PlatformWrapper>
