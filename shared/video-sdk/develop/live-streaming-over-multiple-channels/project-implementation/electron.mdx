...
1.  **Add the required libraries**
    ...
    following beofore `createAgoraRtcEngine` at the top of the file:
...
1.  **Use the multi-channel enabled `IRtcEngineEx` interface**
    ...
    In the Get Started code, the <Vg k="ENGINE" /> instance was declared as `RtcEngine`. To implement multi-channel functionality, you use the multi-channel interface of the <Vg k="ENGINE" /> `IRtcEngineEx`. In order to
...
2.  **Join a second channel**
    ...
    When a user presses the button, the <Vpl k="CLIENT" /> joins a second channel. If the <Vpl k="CLIENT" /> is already connected to a second channel, it leaves the channel. To do this, add the following code before `document.getElementById("join").onclick = async function ()`:
