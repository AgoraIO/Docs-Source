...
### Document conversion
Agora interactive whiteboard provides document conversion services, which support converting `PPT`, `PPTX`, `DOC`, `DOCX` and `PDF` files into static images, and converting `PPT` and `PPTX` files into dynamic `HTML` web pages. The converted images or web pages can be displayed as presentation materials in the interactive whiteboard. For details, see [File conversion overview](/interactive-whiteboard/whiteboard-sdk/develop/file-conversion-overview).
