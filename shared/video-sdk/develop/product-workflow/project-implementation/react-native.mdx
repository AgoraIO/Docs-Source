...
* A separate `SurfaceView` to render the screensharing. This will enable you to share your screen while also keeping your local video published.
...
1. **Add the required libraries**
    ...
    An `IRtcEngine` object can publish only one video track at a time. If you want to enable both screen sharing and video on one host, you need to create an IRtcEngineEx object: which enables publishing the video track from the camera, and one for publishing the video track for screen sharing simultaneously. To publish screen and camera streams:
