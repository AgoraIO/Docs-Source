...
A Topic is a dynamic resource that users can use at any time. You do not need to define or create a Topic in advance, the SDK automatically creates it when the first user joins a Topic. You also do not need to specifically destroy a Topic, the SDK automatically destroys it when the last user leaves.
