...
* **Web SDK**: 
Requires version `4.22.1` or later. Use the latest version from [official website](https://docs.agora.io/en/sdks). To use the latest Agora Web FLS Player, contact [technical support](mailto:support@agora.io).

* **Native SDK**: 
Requires version `4.3.2.2` or above; it is recommended to use the latest version from the [official website](https://docs.agora.io/en/sdks).
...
```ts
client.setStreamFallbackOption(uid, 1); // Enables automatic fallback to LOW_STREAM
client.setRemoteVideoStreamType(uid, 0); // Enables automatic fallback to HIGH_STREAM
````
