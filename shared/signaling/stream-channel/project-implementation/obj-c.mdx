...
To use the stream channel functionality, call `createStreamChannel` to create a `AgoraRtmStreamChannel` object instance.
...
Call the `joinWithOption` method on the `AgoraRtmStreamChannel` instance with the appropriate options as follows:
...
When joining a channel, fill the `token` parameter in `AgoraRtmJoinChannelOption` with a temporary token from <Vg k="CONSOLE" />.
