...
1. **Enable the user to start and stop audio mixing**
    ...
    When the user presses **Mix Audio**, the <Vpd k = "SDK"/> fetches an audio file using `audioFilePath` and mixes it with the local audio stream. The remote and local users hear the audio file playing with the audio stream. To enable audio mixing in your <Vpl k = "CLIENT"/>, in `MyUserWidget.cpp`, add the following before `setupVoiceSDKEngine`:
