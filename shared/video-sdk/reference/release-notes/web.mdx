...
This release adds the `ILocalVideoTrack.replaceTrack` method to increase the flexibility to switch local video tracks after publishing local video streams.
Take switching from camera capture to screen sharing as an example: Before v4.17.0, you need to call `IAgoraRTCClient.unpublish` to unpublish the current track and then call `IAgoraRTCClient.publish` to publish a new track; as of v4.17.0, you can directly call ` ILocalVideoTrack.replaceTrack`.

## v4.16.0

v4.16.0 was released on January 5, 2023.
