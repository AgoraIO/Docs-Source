...
- Apps that integrate the SDK may experience UI lag caused by main thread blocking during audio and video interactions.
...
- After you call `enableAudioVolumeIndication:smooth:reportVad:` to enable volume indication, the `rtcEngine:reportAudioVolumeIndicationOfSpeakers:totalVolume:` callback may return a local user volume of 0 for both local and remote users.
- When you call `enableVideoImageSource:options:` to enable the video image source feature, the sender occasionally streams successfully but the `rtcEngine:didVideoPublishStateChange:sourceType:oldState:newState:elapseSinceLastState:` callback does not return the expected result.
- In multi-channel scenarios, if you call `setupRemoteVideoEx:connection:` to initialize the remote user's view before successfully calling `joinChannelExByToken:connection:delegate:mediaOptions:`, the view may fail to display correctly.
