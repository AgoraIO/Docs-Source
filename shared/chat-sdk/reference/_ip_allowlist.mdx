...
For the communication between end users behind enterprise firewalls, <Vg k="COMPANY"/> <Vg k="CHAT"/> allows all communications to always use TCP/TLS 443 with TLS headers. <Vg k="COMPANY"/> <Vg k="CHAT"/> supports the IP allowlist function. If you have a strict security policy for end users and allow only certain absolute domain names or specified IP addresses/port ranges to communicate through TCP/TLS 443, you can add the allowed IP addresses or domain names to the IP allowlist. To ensure smooth communication via <Vg k="COMPANY"/> <Vg k="CHAT"/>, you must add the IP addresses and ports of data centers of <Vg k="COMPANY"/> <Vg k="CHAT"/> to the firewall allowlist.

### Add IP addresses and ports of data centers of <Vg k="COMPANY"/> <Vg k="CHAT"/> to the firewall allowlist

1. Prepare the development environment. See [SDK quickstart](../get-started/get-started-sdk) for your platform.

1. Add the IP addresses and ports of <Vg k="COMPANY"/> <Vg k="CHAT"/> access points to the firewall allowlist.

1. Test message sending and receiving in the development environment.

### List of domain names of <Vg k="COMPANY"/> <Vg k="CHAT"/> data centers

For each <Vg k="COMPANY"/> <Vg k="CHAT"/> data center, for example, Singapore, virginia US, or Frankfurt Germany, <Vg k="COMPANY"/> <Vg k="CHAT"/> provides fully qualified domain names for the access points.
