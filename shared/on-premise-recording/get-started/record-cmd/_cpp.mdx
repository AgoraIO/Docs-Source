...
- `channelKey`: The [token](/realtime-communication/channel-management-api/endpoint/ban-user-privileges/create-rulesreference/glossary#token) used in the channel to be recorded. Ensure that you set this parameter if the recording channel uses a token.
