...
1. **Add the required library**
    ...
    To import the required Unity UI library, in `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelAudio.cs`, add the following to the namespace declarations:
...
1. **Declare the variables you need**
    ...
    To read, store and apply workflow settings, in `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelAudio.cs`, add the following declaration to the `JoinChannelAudio` class:
...
1. **Adjust the recorded audio volume**
   ...
   When you drag the slider left or right, your <Vpl k = "CLIENT"/> calls the `changeVolume` method. The method reads the slider value and calls `AdjustRecordingSignalVolume` to adjust the recording signal volume. To implement this logic, add the following method to `JoinChannelAudio` class.
...
2. **Get the `uid` of the remote user**
   ...
   When a remote user joins the channel, <Vpd k="SDK" /> triggers the `OnUserJoined` callback. You use this callback to save the `uid` of remote user. To implement this callback, in `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelAudio.cs`, add the following callback to `UserEventHandler` class:
...
3. **Implement the logic to mute and unmute the remote user**
   ...
   When the user taps the mute toggle button, your <Vpl k="CLIENT" /> calls `muteRemoteAudioStream` and mutes the remote user. To implement this workflow, add the following to `JoinChannelAudio` class:
