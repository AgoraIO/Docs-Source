...
1.  **Add the required UI library**
    ...
    To import the required Uinty UI library, in `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelVideo.cs`, add the following to the list of `import` statements:
...
2.  **Declare the variables you need**
    ...
    To create and manage an instance of the media player and access the UI elements, add the following declarations to the `JoinChannelVideo` class:
...
1. **Access the UI elements programmatically**
   ...
   To setup the event listener for the media player button and access the slider, in `JoinChannelVideo`, add the following at the end of `SetupUI` method:

    ```csharp
    go = GameObject.Find("playMedia");
    go.GetComponent<Button>().onClick.AddListener(playMedia);
    mediaButton = go.GetComponentInChildren<TextMeshProUGUI>(true);
    mediaButton.text = "Play Audio";
    mediaButton.fontSize = 14;
...
1.  **Open, play and pause media files**
    ...
    public void playMedia() {
        ...
        if (mediaPlayer == null) {
            ...
            mediaButton.text = "Opening Media File...";
            return;
        }
        ...
        if (isMediaPlaying) { // Start or resume playing media
            ...
            mediaButton.text = "Pause Playing Media";
        } else {
            if (state == MEDIA_PLAYER_STATE.PLAYER_STATE_PLAYING) {
                // Pause media file
                mediaPlayer.Pause();
                mediaButton.text = "Resume Playing Media";
            }
        }
    }
    ```
...
2.  **Manage media player callbacks**
    ...
    internal class MediaPlayerObserver : IMediaPlayerSourceObserver
    {
        ...
        internal MediaPlayerObserver(JoinChannelVideo videoSample)
        {
        ...
        public override void OnPlayerSourceStateChanged(MEDIA_PLAYER_STATE state, MEDIA_PLAYER_ERROR error) 
        {
            ...
            if (state == MEDIA_PLAYER_STATE.PLAYER_STATE_OPEN_COMPLETED) 
            {
                ...
                // Update the UI
                _videoSample.mediaButton.text = "Play Media File";
            ...
            else if (state == MEDIA_PLAYER_STATE.PLAYER_STATE_PLAYBACK_ALL_LOOPS_COMPLETED) 
            {
                ...
                _videoSample.mediaButton.text = "Load Media File";
...
3.  **Configure <Vg k="ENGINE" /> to publish the media player stream**
    ...
    You use `ChannelMediaOptions` and the `UpdateChannelMediaOptions` method to specify the type of stream to publish. To switch between publishing media player and camera and microphone streams, add the following method to the `JoinChannelVideo` class:
...
4.  **Display media player output locally**
    ...
    Access the `LocalView` game object from the UI and update its video surface each time when you play and pasue the media player to switch between displaying media player output and the camera stream. To implement this logic, add the following method in the `JoinChannelVideo` class:
...
6.  **Clean up when you close the <Vpl k="CLIENT" />**
    ...
    To free up resources when you exit the <Vpl k="CLIENT" />, in the `JoinChannelVideo` class, add the following lines after `RtcEngine = null;`:
