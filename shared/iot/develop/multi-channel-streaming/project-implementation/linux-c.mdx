...
To send multiple audio and video streams to a single channel, is already implemented in sample project `<extracted-folder>/agora_rtsa_sdk/example/hello_rtsa/hello_rtsa_multi.c`.
You can [test](#test-pushing-multiple-streams-to-a-single-channel) the same.

Now this section presents code snippets from the sample project that you can reuse in your own <Vpl k="CLIENT"/> to integrate <Vpd k="SDK"/>.

Open the file `<extracted-folder>/agora_rtsa_sdk/example/hello_rtsa/hello_rtsa_multi.c` in a code editor and examine the code. Reuse the following code blocks from this project to implement the desired functionality in your own <Vpl k="CLIENT"/>:
...
1. **Create multiple connections**
    ...
    When the <Vpl k="CLIENT"/> starts, you create multiple connections that you use to send and receive data:
...
1. **Stream audio and video data**
    ...
    You create tasks (threads) for sending video and audio frames.
...
1. **Send a video frame to through muliple connections**
    ...
    ```
1. **Leave and destroy all connections to a channel**
...
3. **Join multiple channels**
    ...
    To send audio and video streams to multiple channels, join these channels. Replace `agora_rtc_join_channel` api in the loop of joining channel:
