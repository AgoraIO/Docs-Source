...
| `-10019` | <code>{code.invalidchanneltype[props.ag_platform]}</code> | Invalid channel type. The SDK only supports the following channel types. Please use the correct value: <br/> - <code>{enumv.channeltypemessage[props.ag_platform]}</code>: Message Channel <br/> - <code>{enumv.channeltypestream[props.ag_platform]}</code>: Stream Channel |
