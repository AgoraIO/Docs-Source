...
1. **Set up access to the UI elements**
    ...
    In `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelVideo.cs`, add the following at the end of `SetupUI();`
...
1. **Update the UI after the effect finishes playing**
    ...
    When <Vg k="VSDK" /> has finished playing the sound effect, the `OnAudioEffectFinished` event is fired. To handle this event, in `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelVideo.cs`, add the followig to the `UserEventHandler` class:
