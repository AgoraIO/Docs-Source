...
Enabling MLLM automatically disables ASR, LLM, and TTS since the MLLM handles end-to-end voice processing directly. When you set `enable_mllm` to true, `enable_aivad` is disabled. See [`turn_detection`](/conversational-ai/rest-api/agent/join#turn-detection) for turn detection options available with MLLMs.
</Admonition>
