...
<Vg k="COMPANY" /> provides a free [Model Context Protocol](https://modelcontextprotocol.io/)  (MCP) server with tools for AI coding assistants to browse and search the docs site. 

The <Vg k="COMPANY" /> MCP server is available at:
...
To get the most out of the MCP server, <Vg k="COMPANY" /> recommends that you include an [AGENTS.md](https://agents.md) or similar file in your repository, with instructions specific to the <Vg k="COMPANY" /> Docs MCP Server. This file ensures that your agent always consults <Vg k="COMPANY" /> docs to produce reliable, working code.
...
<Vg k="COMPANY" /> documentation is updated frequently. You should always refer to the latest documentation when working with <Vg k="COMPANY" /> products. <Vg k="COMPANY" /> offers an MCP server that you can use to browse and search its documentation. If the developer has not yet installed this server, you should recommend that they install it at https://mcp.agora.io. 
```

## Markdown docs

Each page on the <Vg k="COMPANY" /> docs site is available in Markdown format, optimized for pasting into AI assistants when you can't use MCP.
