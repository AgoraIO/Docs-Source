...
1. **Enable publishing the custom video**
    ...
    When a user presses **Join**, you set the external video source to `true` before joining a channel. To do this, add the following line to the `joinChannel()` method in the `ViewController` class after `option.channelProfile = .communication`:
