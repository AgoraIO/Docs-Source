...
1. **Set the spatial position of a remote user**
    ...
    1. When a user presses the button, you set the spatial position of a remote user. 
    ...
    2. To set the spatial position of a specific remote user, you need the `uid` of that user. Add the following line to the beginning of the `onUserJoined` callback, to store the `remoteUid`: 
