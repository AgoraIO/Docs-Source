...
2. **Import the <Vpl k="NAME" /> libraries**
    ...
    2. Add the following namespace after `using UnityEngine;`:
    ``` csharp
    using UnityEngine.UI;
    ```
3. **Manage Android permissions**

    1. Add the Unity Android library. In `Assets/Agora-RTC-Plugin/Agora-Unity-RTC-SDK/Code/JoinChannelAudio.cs`, add the following to the list of namespace declarations:
    ...
    2. Create a permissions list. In the `JoinChannelAudio` class, add the following lines before `Start`:
    ...
    2. To check if permissions are granted, in the `JoinChannelAudio` class, add the following code after `Update`:
    ``` csharp
...
4. **Reference the UI elements from SampleScene**
    ...
    In the `JoinChannelAudio` class, add the reference code before `SetupVoiceSDKEngine` method:
    ``` csharp
...
5. **Setup <Vg k="ENGINE"/>**
    ...
    To call a method to initialize <Vg k="ENGINE"/> when the <Vpl k="CLIENT" /> is launched, in `JoinChannelAudio`, add the following before `CheckPermissions` method:
    ``` csharp
...
6. **Handle and respond to <Vg k="ENGINE"/> events**

    To register the callbacks, add the following lines after `SetupVoiceSDKEngine` method:
    ``` csharp
...
7. **Join a channel to start <Vpl k="ENGINE" />**

    When the local user clicks the `Join` button, you call `Join()`. This method securely connects the local user to a channel using the authentication token. In the `JoinChannelAudio` class, add the following lines before `InitEventHandler` mtehod:
    ``` csharp
    public void Join()
    {
...
8. **Leave the channel when a user ends the call**

    When a user clicks **Leave**, you call `Leave()` to exit the channel. In the `JoinChannelAudio` class, add the following lines after `Join` method:
    ``` csharp
    public void Leave()
    {
        // Leaves the channel.
...
1. **Check that the <Vpl k="CLIENT" /> has the correct permissions to start**
    ...
    For Android, call `CheckPermissions` and check that the permissions are granted. In the `JoinChannelAudio` class, replace `Update` method with the following code:
    ``` csharp
    void Update()
    {
        CheckPermissions();
...
2. **Start <Vpd k="PRODUCT" />**
    ...
    In the `JoinChannelAudio` class, replace `Start` method with the following code:
    ``` csharp
    void Start()
    {
        SetupVoiceSDKEngine();
...
3. **Clean up the resources used by your <Vpl k="CLIENT" />**
    ...
    When a user closes the <Vpl k="CLIENT" />, you call `OnApplicationQuit` to clean up the resources you created in `SetupVoiceSDKEngine`. In the `JoinChannelAudio` class, add the following code after `Update`:
    ``` csharp
