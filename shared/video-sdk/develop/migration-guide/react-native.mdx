...
<Vpd k="NAME"/> v<Vg k="VSDK_RELEASE" /> is a new version of the SDK that you can use to embed real-time video and audio into your app. It supports large-scale real-time interactive activities and provides better real-time interactive effects. For details, see [Product overview](../overview/product-overview).

This page introduces the main steps to upgrade the SDK from <Vpd k="NAME"/> v<Vg k="VSDK_PREVIOUS_RELEASE" /> to v<Vg k="VSDK_RELEASE" />, as well as the related changes.

## Migration steps

This section introduces the main steps to upgrade the SDK from <Vpd k="NAME"/> v<Vg k="VSDK_PREVIOUS_RELEASE" /> to v<Vg k="VSDK_RELEASE" />.
...
The v<Vg k="VSDK_RELEASE" /> SDK has optimized or modified the implementation of some functions, resulting in incompatibility with the v<Vg k="VSDK_PREVIOUS_RELEASE" /> SDK. In order to retain Agora functionality in your app, update the code in your app according to [What's changed](./migration-guide#what-has-changed).
...
- [Function gaps](#function-gaps): Introduces functions that were supported in v<Vg k="VSDK_PREVIOUS_RELEASE" /> but are not supported in v<Vg k="VSDK_RELEASE" />. However, these functions are intended to be added in a future release.

- [Removed APIs](#removed-apis): Introduces APIs that were supported in v<Vg k="VSDK_PREVIOUS_RELEASE" /> but removed in v<Vg k="VSDK_RELEASE" />. Most of these APIs have alternatives in v<Vg k="VSDK_RELEASE" />. Modifying the related implementation should require less time.

- [Naming and data type changes](#naming-changes): Introduces the naming and data type changes of the main APIs. You can update the relevant implementation according to the error message in the IDE, which is expected to take less time.
...
In v<Vg k="VSDK_PREVIOUS_RELEASE" />, the SDK provides the `create` and `createWithContext` method to create and initialize the `RtcEngine` instance. 
...
In v<Vg k="VSDK_PREVIOUS_RELEASE" />, the SDK provides the `addListener` method to register event listener. 
...
In v<Vg k="VSDK_PREVIOUS_RELEASE" />, the SDK provides the `RtcChannel` and `RtcChannelEvents` classes to implement multi-channel control. The v<Vg k="VSDK_PREVIOUS_RELEASE" /> SDK supports subscribing to the audio and video streams of multiple channels, but only supports publishing one group of audio and video streams in one channel.
...
- Mix multiple video streams and publish them to the remote user through one `localUid`.
  ...
  `RtcChannel` and `IRtcEngine` of v<Vg k="VSDK_PREVIOUS_RELEASE" /> are partially duplicated and overlap in their functionality, so v<Vg k="VSDK_RELEASE" /> hides the `RtcChannel` and `RtcChannelEvents` classes. See the [JoinMultiChannel](https://github.com/AgoraIO-Extensions/react-native-agora-rtc-ng/blob/main/example/src/examples/advanced/JoinMultipleChannel/JoinMultipleChannel.tsx) sample project for more details on how to replace `RtcChannel` with `joinChannel` and `ChannelMediaOptions`. The expected migration cost is one day or less.
...
In addition to the breaking changes listed here relative to v<Vg k="VSDK_PREVIOUS_RELEASE" />, v<Vg k="VSDK_RELEASE" /> has a small number of breaking changes relative to the v<Vg k="VSDK_RELEASE" /> Beta release. For example:

- In v<Vg k="VSDK_RELEASE" />, replace `publishAudioTrack` in `ChannelMediaOptions` with `publishMicrophoneTrack`.

If you used this feature in v<Vg k="VSDK_RELEASE" /> Beta and wish to upgrade to v<Vg k="VSDK_RELEASE" />, modify the implementation code of the feature after upgrading the SDK.
...
| <ul><li>setBeautyEffectOption</li><li>ssetVideoDenoiserOptions</li><li>setLowlightEnhanceOptions</li><li>setColorEnhanceOptions</li></ul> | Video enhancement extension  |
...
In v<Vg k="VSDK_PREVIOUS_RELEASE" />, if you want to enable local audio and video recording, you need to call the `startRecording` method.
...
**Device permissions**
In v<Vg k="VSDK_PREVIOUS_RELEASE" />, `AudioLocalError.DeviceNoPermission` in `onLocalAudioStateChanged` reports that there is no permission to start the capture device, and `LocalVideoStreamError` in `onLocalVideoStateChanged` reports that there is no permission to start the video capture device.
...
- The following enumerations in `AudioMixingReason`: Rarely used in v<Vg k="VSDK_PREVIOUS_RELEASE" />.
