...
To create a simple interface for testing speech-to-text functionality, add buttons to start and query speech-to-text. To implement this UI, do the following:

    1. Click **View** > **ToolBox**. The **Toolbox** opens.

    1. From **ToolBox**, drag **Button** to the surface of the **Dialog Editor**.

    1. In **Properties**, update the **Caption** field to `Start speech-to-text`.

    1. Repeat the same procedure and add another button where **Caption** says `Query speech-to-text`.
...
1. **Add cJSON library in your project**
    ...
    In **Visual Studio**, go to **Tools** > **Options**, and then click **NuGet Package Manager**. The **NuGet: AgoraImplementation** window opens. Go to the **Browse** tab and type `CppRequests` and install the package in your project.
...
1. **Define variables to manage a speech-to-text task**
	...
	CString apiKey = L"<Your API key>";
	CString apiSecret = L"<Your secret key>";
	std::string instanceID = "Test-1";
	CString authorizationHeader, builderToken;
	CString speechToTextTaskId; // Returned by the start method
...
1. **Get a `builderToken`**
    ...
    To [acquire a `builderToken`](#acquire-a-buildertoken), do the following:
    ...
    1. Convert the secret key and API key to Base64 format. 
        ...
        1. In `AgoraImplementationDlg.h`, add the following declaration to `CAgoraImplementationDlg`:
            ...
            ```cpp
       	    CString ConvertToBase64(CString str);
            ```

        1. In `AgoraImplementationDlg.cpp`, add the following before `setupVideoSDKEngine`:

            ```cpp
            CString CAgoraImplementationDlg::ConvertToBase64(CString str)
            {
                CT2AEX<1024> ansi = str;
                int len = strlen(ansi);
                int encodedLen = Base64EncodeGetRequiredLength(len);
                CStringA base64;
                Base64Encode((BYTE*)(LPSTR)ansi, len, base64.GetBuffer(encodedLen), &encodedLen);
                base64.ReleaseBuffer(encodedLen);
                CString CBase64(base64);
                return CBase64;
            }
            ```
    ...
    1. Retrieve a token from the server. 
        ...
        1. In `AgoraImplementationDlg.cpp`, add the following before `setupVideoSDKEngine`:
            ...
            void CAgoraImplementationDlg::fetchBuilderToken()
            {
                ...
                CString plainCredentials = apiKey + ":" + apiSecret;
                ...
                try
                {
                    ...
                    if (pConnection)
                    {
                        ...
                        authorizationHeader += ConvertToBase64(plainCredentials);
...
1. **Start a speech-to-text task**
    ...
    To start a task, in **Dialog Editor**, double-click **Query speech-to-text**. **Dialog Editor** automatically creates and opens an event listener for you. Add the following code to the event listener you just created:
		...
		if (pConnection)
		{
			...
			if (pHTTPFile)
			{
				...
				if (dwRet == HTTP_STATUS_OK)
				{
					...
					speechToTextTaskId = success->valuestring;
				}
				else
				{
					CString errorCode;
...
1. **Query task status**
    ...
    To notify the user of any change to the speech-to=test task you [Query the task status](#query-the-status-of-speech-to-text) using a `GET` request containing the `taskId` and `builderToken`. To implement this workflow, in **Dialog Editor**, double-click **Query speech-to-text**. **Dialog Editor** automatically creates and opens an event listener for you. Add the following code to the event listener you just created:

    ```cpp
    std::string temp = "/v1/projects/" + appId + +"/rtsc/speech-to-text/tasks/";
	CString path(temp.c_str());
	path += speechToTextTaskId + "builderToken=" + builderToken;
	CInternetSession session;
	CHttpConnection* pConnection = NULL;
	try
	{
		...
		if (pConnection)
		{
			...
			if (pHTTPFile)
			{
				...
				if (dwRet == HTTP_STATUS_OK)
				{
					...
					AfxMessageBox((LPCTSTR)status->valuestring);
				}
				else
				{
					CString errorCode;
...
1. **Stop the task**
    ...
    To [stop speech-to-text](#stop-speech-to-text), you send a `DELETE` request with the, `appId`, `taskId` and `builderToken` in the URL. To implement this logic, do the following:
    ...
    2. In `AgoraImplementationDlg.cpp`, add the following method before `setupVideoSDKEngine`:
        ...
        ```cpp
        void CAgoraImplementationDlg::stopSpeechToText()
        {
            std::string temp = "/v1/projects/" + appId + +"/rtsc/speech-to-text/tasks/";
            CString path(temp.c_str());
            path += speechToTextTaskId + "builderToken=" + builderToken;
            ...
            try
            {
                ...
                if (pConnection)
                {
                    ...
                    if (pHTTPFile)
                    {
                        ...
                        if (dwRet == HTTP_STATUS_OK)
                        {
                            AfxMessageBox(L"Speech-to-text task stopped");
