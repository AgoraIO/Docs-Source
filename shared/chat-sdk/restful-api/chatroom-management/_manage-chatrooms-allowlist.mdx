...
A <Vg k="CHAT_ROOM_LOWER"/> allow list refers to a list of <Vg k="CHAT_ROOM_LOWER"/> members that can send messages after the <Vg k="CHAT_ROOM_LOWER"/> owner or admins have muted all the <Vg k="CHAT_ROOM_LOWER"/> members using the global mute method. <Vg k="CHAT"/> provides a complete set of allow list management methods, including adding users to the allow list and removing them from it, as well as retrieving the members on the allow list.

This page shows how to manage a <Vg k="CHAT_ROOM_LOWER"/> allow list using the <Vg k="CHAT"/> RESTful APIs. Before calling the following methods, ensure that you understand the call frequency limit described in [Limitations](/agora-chat/reference/limitations).
