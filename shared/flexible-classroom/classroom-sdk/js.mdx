...
| `web`   | Web configuration for browser codec format and channel mode. <ul><li>`codec`: Browser codec format. Available values are as follows: <ul> <li>`"vp8"`: VP8</li><li>`"h264"`: H.264</li></ul></li><li>`mode`: Channel mode. Available values are as follows: <ul><li>`"rtc"`: Communication mode, commonly used for one-to-one or one-to-many classrooms.</li><li>`"live"`: Live-streaming mode. It costs less and has a higher latency than the communication mode.</li></ul></li><ul></ul></ul>|
