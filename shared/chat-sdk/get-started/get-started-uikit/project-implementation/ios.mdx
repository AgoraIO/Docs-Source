...
This section shows how to use Chat UI sample to rapidly implement peer-to-peer messaging in your app.

### Initialize UI sample

Follow the steps to initialize the UI sample.
...
2. Initialize the UI sample by calling the `initWithAgoraChatOptions` method. In `SceneDelegate.m`, replace the `willConnectToSession` method with the following code:
