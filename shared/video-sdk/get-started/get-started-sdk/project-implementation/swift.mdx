...
class ViewController: NSViewController {
    ...
    override func viewDidLayout() {
        ...
        // Set the remote view's frame so that it fills the entire CustomView
        remoteView.frame = view.bounds
    }
...
After joining the channel, call the `setClientRole` [2/2] method and set the `role` parameter to `AgoraClientRoleAudience` and the `options` parameter to low latency. Refer to the following sample code:
...
// Set the channel scene to live broadcast
options.channelProfile = .liveBroadcasting
// Set the user role to anchor
// If you want to set the user role to audience, modify the next line of code to options.clientRoleType = .audience
options.clientRoleType = .broadcaster
// Publish the audio collected by the microphone
options.publishMicrophoneTrack = true
// Publish the video captured by the camera
...
Define necessary callbacks based on the use-case. The following sample code demonstrates how to implement the `didJoinChannel` and `didOfflineOfUid` callbacks:
...
override func viewDidLoad() {
    super.viewDidLoad()
    // After loading the view, you can do other settings
...
options.channelProfile = .liveBroadcasting
// Set the user role to anchor
// If you want to set the user role to audience, modify the next line of code to options.clientRoleType = .audienceaudience
options.clientRoleType = .broadcaster
// Publish the audio collected by the microphone
options.publishMicrophoneTrack = true
// Automatically subscribe to all audio streams
...
1. Call `joinChannel`, when the view is loaded:
...
1. To release all resources used by Agora SDK, call `leaveChannel` and `destroy` when the app is closed.
