...
When a token is about to expire, the SDK triggers the `token-privilege-will-expire` callback. When you receive this callback, obtain a fresh token from the token server, and call `renewToken` to pass the newly generated token to the SDK.
</PlatformWrapper>
...
| Characters and use-cases  | Authentication steps  |
|:--------------------------|:---------------------------------------------|
| Host joins the channel to send streams | <ol><li>Request a token with the `role` set to `kRolePublisher`. This token gives the host permission to send streams. </li><li>Call `setClientRole` to set the role as the host.</li><li>Call `join` to pass in a token with streaming privileges to join the channel.</li></ol>           |
| Audience members join the channel	  | <ol><li>Request a token with the `role` set to `kRoleSubscriber`. This token does not permit the user to send streams</li><li>Since a user's role when joining a channel is by default audience, it is sufficient to call `join` directly and pass in a token to join the channel.</li></ol>         |
| Audience gets the mic after joining the channel | <ol><li>Request a token with the `role` set to `kRolePublisher` to give the user permission to send streams.</li><li>Call `renewToken` to update the new token on the <Vg k="COMPANY" /> server.</li><li>Call `setClientRole` to switch the role to host.</li></ol> |
...
Regularly call the <Vg k="COMPANY" /> [RESTful API](../channel-management-api/endpoint/query-channel-information/query-user-list) `http://api.sd-rtn.com/dev/v1/channel/user/{appid}/{channelName}/{hosts_only}` on the app server to query the <Vg k="COMPANY" /> server's online channel user list, and compare it with the list of users maintained on the app server to find illegal users. The API call frequency is capped at 20 times per second.
...
<PlatformWrapper platform="web">
- The app server sends a message to the user's client asking the client to call `setMuted(true)` on the local audio track to stop publishing the audio stream.
