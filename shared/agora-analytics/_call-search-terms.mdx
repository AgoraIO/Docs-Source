...
Each user in a call has a unique [user ID](./glossary#user-iduiduid) (either `uid` or `userId`).
...
- For a user, the total audio duration is the sum of the duration of all subscribed audio streams, that is, audio duration [calculated by stream](./billing-policies#how-does-agora-calculate-service-minutes).
...
- For a user, the total video duration is the sum of the duration of all subscribed video streams, that is, video duration [calculated by stream](./billing-policies#how-does-agora-calculate-service-minutes).
