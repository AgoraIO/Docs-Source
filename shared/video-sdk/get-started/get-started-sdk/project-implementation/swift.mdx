...
extension ViewController: AgoraRtcEngineDelegate {
    ...
    // Called while remote user or broadcaster leaving the current channel
...
// Callback for remote user or broadcaster leaving the current channel
func rtcEngine(_ engine: AgoraRtcEngineKit, didOfflineOfUid uid: UInt, reason: AgoraUserOfflineReason) {
    let videoCanvas = AgoraRtcVideoCanvas()
    videoCanvas.uid = uid
    videoCanvas.view = nil
...
override func viewDidLoad() {
    super.viewDidLoad()
    // After the view is loaded, you can perform other setups
    // Initialize the video window
    initView()
    // The following functions will be called when using the Agora API
    initializeAgoraEngine()
    setupLocalVideo()
    joinChannel()
}
