...
After a failure is confirmed, the <Vg k="MPULL" /> task is migrated within 120 seconds. During this period, the task may be interrupted.
...
When you create multiple <Vg k="MPULL" /> tasks, you are charged separately for each of them. For details, see [Media Pull pricing](../reference/pricing).
...
2. Setup your retry strategy in the following order:
    1. **Primary domain retry**: If the primary domain name fails to initiate a RESTful API request, retry using the primary domain name.
...
response to querying the push status is `failed`, then the <Vg k="MPULL" /> task has failed. Perform the following steps to rectify the fault:
