...
| Message                       | Send a broadcast message to all chat rooms under an app | POST      | `/{org_name}/{app_name}/messages/chatrooms/broadcast`        | 10 times per minute and 100 times per day                    | N                        | <ul><li>$10/million downlink messages. Each downlink message is defined as a message delivered to one end user.</li><li>Each API call can send 1 message to up to 20K end users. This feature has a unit price of $0.2/100 broadcast messages (each broadcast message is received by 1 end user and not shared by multiple users)</li></ul> |
