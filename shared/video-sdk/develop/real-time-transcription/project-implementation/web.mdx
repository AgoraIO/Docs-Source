...
1. **Stop the task**
    ...
    function stopRealTimeTranscription()
    {
      ...
      .catch(error => {
        console.error("Error while stopping the real-time transcriptiontask: " + error)
      });  
    }
    ```
...
1. **Receive and display the text stream**
    ...
    When you set one of the `destinations` in the starting configuration as `AgoraRTCDataStream`, the real-time transcriptionconversion output stream is pushed to your channel.
