...
- App token: Admin level access to Chat using the REST API. App tokens grant admin privileges for the app developer to manage the Chat applications, for example, creating and deleting users. For details, see [Implement an Agora app token server for Chat](../develop/generate-app-tokens).

This page shows you how to create an Chat user token server and an Chat client app. The client app retrieves a user token from the token server. This token enables the current user to access Chat.
...
1. In `<Project name>/src/main/resource`, create a `application.properties` file to hold the information for generating tokens and update it with your project information.
   ...
   For details on how to get the app key and REST API domain, see [Enable and Configure Chat](../get-started/enable).
