...
**The RESTful API for Media Inject** enables you to inject an online media stream into the Agora channel as a live video source. After the injection succeeds, the online media stream automatically plays in the Agora channel, and remote users can see the content of the media stream.

## Working principle

Injecting an online media stream into the Agora channel creates a Media Inject task (shown as "cloud player" in the rest of the article) in the Agora channel. You can control the cloud player through the RESTful API and accomplish the following functions for Media Inject:
...
The Media Inject service supports the following formats:
...
- `token`: (Optional) String. The authentication token used by the cloud player in the channel. If you do not enable the App Certificate in Agora Console, ignore this field. If you enable the App Certificate in Agora Console, you must pass a value in this field:
  - For low-security requirements: Use the temporary token generated in Agora Console. For details, see [Generate an RTC temporary token](manage-your-agora-account#generate-a-temporary-token).
  - For high-security requirements: Use the token generated at your server. For details, see [Implement authentication workflow](@Docs/interactive-live-streaming/develop/authentication-workflow).
...
- `encryptKdfSalt`: (Optional) String. The salt with the length of 32 bytes. Agora recommends using OpenSSL to generate the salt on your server. For details, see [Media Stream Encryption](@Docs/interactive-live-streaming/develop/media-stream-encryption).
...
**Media inject without transcoding**
...
For details, see [Message Notification Service](message-notification-service).

## Considerations
This section summarizes some important considerations when using the Media Inject RESTful API.
...
| The cloud player may be deleted automatically due to server errors. Agora recommends that you enable the Message Notification Service to listen for events of the Media Inject service. For details, see the [Media Inject Message Notification Server](message-notification-service). | ★★★★★  |
...
| The default maximum number of concurrent tasks is 50, which means that a maximum of 50 media inject tasks are allowed to run simultaneously for each project. You can contact [sales@agora.io](mailto:sales@agora.io) to get a higher quota. | ★★★★☆  |