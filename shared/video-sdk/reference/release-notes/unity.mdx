...
3. **Dual-stream mode optimization**
   ...
   This release optimizes the video dual-stream mode. In the low-quality video stream adaptive mode (`AUTO_SIMULCAST_STREAM`), when the receiver requests a low-quality video stream by calling <Link to="{{Global.API_REF_UNITY_ROOT}}/class_irtcengine.html#api_irtcengine_setremotevideostreamtype">SetRemoteVideoStreamType</Link> or <Link to="{{Global.API_REF_UNITY_ROOT}}/class_irtcengineex.html#class_irtcengineex">SetRemoteVideoStreamTypeEx</Link>, the sender now processes requests from all users, expanding beyond the previous restriction to broadcasters only. This change ensures that the audience requests for low-quality video streams are implemented, thus improving the flexibility and adaptability of the dual-stream mode.
