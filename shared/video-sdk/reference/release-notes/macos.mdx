...
- **Adaptive video publishing (Beta)**  
  ...
  Added support for sending multiple video streams with different resolutions from the same video source, including multi-channel scenarios. Use `setSimulcastConfig:` with `AgoraSimulcastConfig` to configure up to four layers (one high-resolution main stream and up to three lower-resolution streams) and control publishing options such as resolution and bitrate. When fallback is enabled, the SDK can switch to lower-resolution streams under poor uplink or device conditions. Subscribers can select the desired stream via `setRemoteVideoStream:type:`. This feature improves viewing experiences in conferences, large classes, and interactive live streaming, especially under poor network conditions.

- **Multipath network transmission**  

  Introduced multipath transmission for devices with multiple network interfaces (for example, 5G, Wi-Fi, and LAN). Enable it by setting `enableMultipath` in `AgoraRtcChannelMediaOptions` to `YES`.
  ...
  Configure uplink and downlink modes separately with `uplinkMultipathMode` and `downlinkMultipathMode`. Monitor performance with the `rtcEngine:didMultipathStatsChanged:` callback. Contact support@agora.io to enable duplicate mode.
...
- **Token renewal result callback**  
  ...
  Added the `rtcEngine:didRenewTokenResult:` callback and `RENEW_TOKEN_ERROR_CODE` to report the result of `renewToken:`.
...
- **Other new features**  
  ...
  - Added `preloadEffectEx:soundId:` to preload audio effects into a specific channel.  
  - Added `playEffectEx:soundId:filePath:loopCount:pitch:pan:gain:publish:startPos:` for advanced audio effect playback in a specific channel.  
  - Added the `position` member in `AgoraContentInspectModule` to choose the capture point for screenshot uploads.  
  - Added the `screenColorType` member to `AgoraSegmentationProperty` for specifying or auto-detecting background screen color.

#### Improvements

- Added support for G.711 and G.722 audio codecs when interoperating with the Web SDK for improved cross-platform audio compatibility.  
...
- Occasional echoes in media volume mode when publishing a microphone stream while simultaneously playing effects and music.  
...
- Occasional crashes.  
- Media relay occasionally reported `AgoraChannelMediaRelayStateFailure` and `AgoraChannelMediaRelayErrorServerErrorResponse` after certain join/relay sequences.
