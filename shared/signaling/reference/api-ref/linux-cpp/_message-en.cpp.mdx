...
| :--------------: | :------------------------------------------------------------: | :------: | :----: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `channelName` | const char* | Required | - | Channel name. You can only send messages to one channel at a time. |
...
int ret = rtm_client->publish("channel_name", msg.c_str(), msg.size(), pub_opt, req_id);
if (ret != RTM_ERROR_OK) {
