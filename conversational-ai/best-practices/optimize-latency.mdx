...
Understanding latency composition helps you identify optimization opportunities. The following sections explain how latency differs between cascaded and MLLM architectures.
...
| Component | Latency metric | Description | Typical latency range |
|-----------|----------------|-------------|----------------------|
| RTC | Audio and video latency | Includes audio capture, encoding, network transmission, decoding, and playback | 100-300 ms |
| Algorithm preprocessing | Preprocessing latency | Includes VAD (Voice Activity Detection), graceful interruption (AIVAD), and other algorithm processing time | 80-120 ms |
| ASR | `asr_ttlw` | Time To Last Word. The latency from when the user stops speaking to when ASR outputs the last word. | 100-150 ms |
| LLM | `llm_ttfb` / `llm_ttfs` | TTFB: Time To First Byte, the first byte latency.<br/>TTFS: Time To First Sentence, the first sentence latency. | 270-950 ms |
| TTS | `tts_ttfb` | Time To First Byte. The response latency from when the TTS request starts to when the first byte is received. | 60-110 ms |
| Avatar rendering | Rendering latency | Avatar video rendering and synchronization latency (if enabled) | 50-200 ms |
...
* **LLM**
   * **Choose smaller models**: Models such as `gpt-4o-mini` and `claude-3-haiku` typically respond faster than larger models.
...
* **Choose low-latency LLM models**: Refer to [Artificial Analysis](https://artificialanalysis.ai/) to select models with excellent TTFT and throughput performance.
* **Enable streaming response**: Ensure `stream: true`.
* **Choose low-latency ASR and TTS vendors**: Use modes such as turbo or low-latency.
...
* **Consider using MLLM**: If latency is a critical factor, evaluate the feasibility of MLLM architecture.
...
* [Restrict agent zones](/conversational-ai/best-practices/regional-restrictions)
