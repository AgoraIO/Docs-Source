...
1. Ensure `cmake` is installed.
    ...
    The sample project uses `cmake` to control the compilation process. Execute `cmake --version` to check that the version number is greater than `3.6.0`. If `cmake` is not installed, install it using the following command:
