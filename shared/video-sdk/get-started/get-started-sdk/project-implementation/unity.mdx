...
1. **Setup <Vg k="ENGINE"/>**
    ...
    To call a method to initialize <Vg k="ENGINE"/> when the <Vpl k="CLIENT" /> is launched, in your script file, add the following after `CheckPermissions`:
    <ProductWrapper product="video-calling">
    ``` csharp
...
1. **Join a channel to start <Vpd k="PRODUCT" />**
    ...
    When the local user clicks the `Join` button, you call `Join()`. This method securely connects the local user to a channel using the authentication token. In your script file, add the following lines after `InitEventHandler`:
    <ProductWrapper product="video-calling">
    ``` csharp
    public void Join()
    {
...
8. **Manage the user role**
    ...
    When toggles are switched on, to switch client role and turn off the toggles, in your script file, add the following code to the `NewBehaviourScript` class:
...
1. **Check that the <Vpl k="CLIENT" /> has the correct permissions to start**
    ...
    For Android, call `CheckPermissions` and check that the permissions are granted. In your script file, replace `Update` with the following code:
    ``` csharp
    void Update()
    {
        CheckPermissions();
...
2. **Start <Vpd k="PRODUCT" />**
    ...
    In your script file, replace `Start` with the following code:
    ``` csharp
    void Start()
    {
        SetupVideoSDKEngine();
...
3. **Clean up the resources used by your <Vpl k="CLIENT" />**
    ...
    When a user closes the <Vpl k="CLIENT" />, use `OnApplicationQuit` to clean up the resources you created in `SetupVideoSDKEngine` method. To implement this logic, in your script file, add the following code at the end of `NewBehaviourScript` class:
    ``` csharp
