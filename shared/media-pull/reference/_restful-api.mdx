...
| The cloud player may be deleted automatically due to server errors. Agora recommends that you enable the Message Notification Service to listen for events of <Vg k="MPULL" />. For details, see the [<Vg k="MPULL" /> Message Notification Server](message-notification-service). | ★★★★★  |
