...
- Combined message
    ...
    | `secret`       | String | The key for accessing the attachment of the combined message. br>This field exists if you set the access restriction when calling the [Upload a file](#upload) method. |
...
| `restrict-access` | Bool | Whether to restrict access to this file.<ul><li>`true`: Restrict access to the file. The user needs to provide a file access key (`share-secret`) to download the file. You can obtain the access key from the response body.</li><li>`false`: The access is not restricted. Users can download the file directly.</li></ul> To restrict the access to uploaded files, contact [support@agora.io](mailto:support@agora.io) to enable this function.  | No |
...
| `sync_device` | Bool| No | Whether to synchronize the recall of a one-to-one message to all online devices of the message sender. <ul><li>(Default) `true`: Yes</li><li>`false`: No</li></ul><Admonition type="info">When `force` is set to `true`, to recall a message that expires, you need to set `from` to the sender of the message.</Admonition>                  |
...
| `is_ack_read` | Bool | Specify whether to mark the conversation as read: <ul><li>`true`: Marks the conversation as read.</li><li>`false`: Leaves the conversation unread.</li></ul> When a message is imported, the corresponding conversation is automatically created. | No |
