...
This page shows how to create, retrieve, modify, and delete a group by calling Agora Chat RESTful APIs. Before calling the following methods, ensure that you understand the call frequency limit described in <Link to="/agora-chat/reference/limitations?platform={{platform.PATH}}#call-limit-of-server-sides">Limitations</Link>.
...
| `maxusers` | String | The maximum number of chat group members (including the group owner). The default value is 200 and the maximum value is 2000.  The upper limit varies with your price plans. For details, see [Pricing Plan Details](/agora-chat/reference/pricing-plan-details#group). | No |
