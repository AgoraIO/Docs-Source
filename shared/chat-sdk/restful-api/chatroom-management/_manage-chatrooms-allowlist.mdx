...
A <Vg k="CHAT_ROOM_LOWER"/> allow list refers to a list of <Vg k="CHAT_ROOM_LOWER"/> members that can send messages after the <Vg k="CHAT_ROOM_LOWER"/> owner or admins have muted all the <Vg k="CHAT_ROOM_LOWER"/> members using the global mute method. Chat provides a complete set of allow list management methods, including adding users to the allow list and removing them from it, as well as retrieving the members on the allow list.

This page shows how to manage a <Vg k="CHAT_ROOM_LOWER"/> allow list using the Chat RESTful APIs. Before calling the following methods, ensure that you understand the call frequency limit described in [Limitations](/agora-chat/reference/limitations).
...
## Retrieving the <Vg k="CHAT_ROOM_LOWER"/> allow list

Retrieves the list of all the members on the <Vg k="CHAT_GROUP_LOWER"/> allow list.
...
If the returned HTTP status code is `200`, the request succeeds, and the `data` field in the response body contains the usernames in the <Vg k="CHAT_ROOM_LOWER"/> allow list.
...
## Adding a user to the <Vg k="CHAT_ROOM_LOWER"/> allow list

Adds the specified user to the <Vg k="CHAT_ROOM_LOWER"/> allow list. Members in the <Vg k="CHAT_ROOM_LOWER"/> allow list can still send messages after the <Vg k="CHAT_ROOM_LOWER"/> owner or an admin has muted all the <Vg k="CHAT_ROOM_LOWER"/> members using the global mute method.
...
| Parameter      | Type | Description                                                         |
| :-------- | :----- | :----------------------------------------------------------- |
| `result`  | Boolean | Whether the user is successfully added to the <Vg k="CHAT_ROOM_LOWER"/> allow list. <li>`true`: Yes. </li><li>`false`: No.</li> |
...
## Adding multiple members to the <Vg k="CHAT_ROOM_LOWER"/> allow list

Adds multiple users to the <Vg k="CHAT_ROOM_LOWER"/> allow list. You can add a maximum of 60 users to the <Vg k="CHAT_ROOM_LOWER"/> allow list each time.
...
| Parameter        | Type  | Description              |
| :---------- | :---- | :---------------- |
| `usernames` | Array | The array of usernames to be added to the <Vg k="CHAT_ROOM_LOWER"/> allow list. You can specify a maximum of 60 usernames, separated by commas (,). |
...
| Parameter      | Type    | Description                                                         |
| :-------- | :------ | :----------------------------------------------------------- |
| `result`  | Boolean | Whether the users are successfully added to the <Vg k="CHAT_ROOM_LOWER"/> allow list.<li>`true`: Yes. </li><li>`false`: No.</li> |
| `reason`  | String  | The reason why the users fail to be added to the <Vg k="CHAT_ROOM_LOWER"/> allow list.                                       |
| `user`    | String  | The usernames added to the <Vg k="CHAT_ROOM_LOWER"/> allow list.                                     |
...
## Removing users from the <Vg k="CHAT_ROOM_LOWER"/> allow list

Removes the specified users from the <Vg k="CHAT_ROOM_LOWER"/> allow list. You can remove a maximum of 60 group members from the <Vg k="CHAT_ROOM_LOWER"/> allow list each time.
...
| Parameter      | Type    | Description                                                         |
| :-------- | :------ | :----------------------------------------------------------- |
| `result`  | Boolean | Whether the users are successfully removed from the <Vg k="CHAT_ROOM_LOWER"/> allow list.<li>`true`: Yes.</li><li>`false`: No.</li> |
| `user`    | String  | The username removed from the <Vg k="CHAT_ROOM_LOWER"/> allow list.                              |
