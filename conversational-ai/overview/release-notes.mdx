...
- **Stop agent API is now asynchronous**
    ...
    The [Stop a conversational AI agent](/conversational-ai/rest-api/agent/leave) API now responds immediately after request parameters are validated. The agent exits the channel asynchronously after the API returns.
