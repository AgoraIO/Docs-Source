...
Tokens are generated on your app server using the token generator provided by <Vg k="COMPANY"/>. For how to generate a token on the server and retrieve and renew the token on the client, see <Link to="/realtime-communication/token-authentication/authentication-workflow">Authenticate users with tokens</Link>.
