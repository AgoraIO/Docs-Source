...
This guide assumes you understand the fundamentals of how <Vpd k="NAME" /> works. If you're new to <Vg k="COMPANY" /> or need a refresher on Agora channels, auth tokens, and agent architecture, start with [Core concepts](core-concepts).
...
- **Understanding of core concepts**: Review [Core concepts](core-concepts) to understand channels, RTC tokens, and agent architecture
- **Development environment**: Node.js (16+), Python (3.8+), or Go (1.19+) installed
- **<Vg k="COMPANY" /> project configured**:
    - [Enabled <Vg k="COMPANY" /> conversational AI](../get-started/manage-agora-account#enable-conversational-ai) for your project—**this is critical**. Without it, your API calls will fail with cryptic errors.
    - [App ID](../get-started/manage-agora-account#get-the-app-id): Identifies your <Vg k="COMPANY" /> project
    - **App Certificate**: Used to generate RTC tokens (found in <Vg k="CONSOLE" /> alongside your App ID). Keep this secret—never expose it to clients.
...
- **AI provider API keys**: API keys from your chosen providers (OpenAI, Azure, etc.). Don't have these yet? That's fine—we'll show you exactly which ones you need and where to get them in the AI models section.

### Client applications

To interact with your AI agents, use one of these demo client applications:

- [Web Demo (TypeScript)](https://github.com/AgoraIO-Community/Conversational-AI-Web-Demo)
- [Android Demo](https://github.com/AgoraIO-Community/Conversational-AI-Android-Demo)
- [iOS Demo](https://github.com/AgoraIO-Community/Conversational-AI-iOS-Demo)
...
![IMAGE: Side-by-side comparison diagram showing STT-LLM-TTS pipeline workflow (left) with three separate components flowing linearly, and MLLM workflow (right) with a single multimodal model handling voice-to-voice directly](/images/conversational-ai/convo-ai-cascade.svg)
...
This approach gives you flexibility to mix and match providers based on your needs—whether you want to optimize for cost, speed, language support, or voice quality. For example, you might use <Vg k="COMPANY" />'s ARES for fast speech recognition, OpenAI's GPT-4o for smarter responses, and ElevenLabs for natural-sounding voices.

#### Recommended starter configuration

This configuration gives you excellent quality right out of the box—no juggling multiple API keys for the ASR component since it uses <Vg k="COMPANY" />'s built-in ARES engine:
...
- Ability to switch providers independently (swap out TTS without touching your LLM)
- Cost optimization by mixing free/paid services
- Support for specific languages or voices not available in MLLM models
- Integration with custom LLM endpoints or RAG systems—you get complete visibility into the text transcript
...
Here's what makes MLLMs powerful: they process your user's voice directly, picking up on tone and emotion that gets lost when you convert speech to text first. This means faster responses that sound more natural.
...
- The fastest response times—cut latency by 30-50% compared to the pipeline
...
- Test with a [demo client application](#client-applications) (Web, Android, or iOS)
