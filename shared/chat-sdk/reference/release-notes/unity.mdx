...
- Fixed the memory leak in the convert coding API.
- Changed the `messageReactionDidChange` callback to Unity suspend.
- `startMessageId` no longer takes effect in the `LoadMessages` API.
- Fixed the crash when you cannot find `callbackid` in the dictionary.
- Adjusted the code converting c++ to c#.
