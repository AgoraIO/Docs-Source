...
1. Best practice is to switch to the latest release. To switch the branch version, run the following command. Remember to replace `{VERSION}` with a specific version number:
   ...
   <Vg k="COMPANY" /> recommends that you switch to the latest release branch. The following image shows how to find the latest release branch in the GitHub repository:
