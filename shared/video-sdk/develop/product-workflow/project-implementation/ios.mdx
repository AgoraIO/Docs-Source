...
Enable your <Vpl k="CLIENT" /> to run in the background while it streams, and capture the screen for streaming.
...
1. **Declare the variables you need**
    ...
    To read, store, and apply workflow settings, add the following variables to the declarations to the top of the `ViewController` class:
...
1. **Setup volume control and mute**
    ...
    To change the volume or mute for local or remote users:

    a. Store the `uid` of the remote user
...
1. **Create an extension for screen sharing**
    ...
    a. In the `screenSharer` target, create a new swift file called `screenSharingAgoraEngine`, then paste the following into it:
        ...
        class screenSharingAgoraEngine {
            ...
            static func startScreenSharing(to channel: String) {
                ...
                channelMediaOptions.publishAudioTrack = .of(false)
                channelMediaOptions.publishCameraTrack = .of(false)
                channelMediaOptions.publishCustomVideoTrack = .of(true)
                channelMediaOptions.publishCustomAudioTrack = .of(true)
                channelMediaOptions.autoSubscribeAudio = .of(false)
                channelMediaOptions.autoSubscribeVideo = .of(false)
    ...
    b. Update the `SampleHandler` in the `screenSharer` target to stream the screen capture video frames to the channel. In `screenSharer` > `SampleHandler` replace the contents with:
    ...
    c. Create an iOS `RPSystemBroadcastPickerView` and enable it to connect to the `screenSharer` extension.
    ...
    d. Initialize the screenSharing extension when you initialize your <Vpl k="CLIENT" />.
