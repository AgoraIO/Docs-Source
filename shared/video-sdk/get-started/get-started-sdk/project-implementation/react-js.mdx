...
1. Use `<RemoteUser />` a component to render audio and video for each remote user.
...
### Leave the channel​
When the user clicks the leave channel button, closes the browser window, or refreshes the browser tab, useJointhe operation of leaving the channel will be performed because the destruction of the React component is triggered. You don't need to make additional API calls.
