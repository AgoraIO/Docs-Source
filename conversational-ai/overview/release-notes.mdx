...
- Added `greeting_configs.delay_ms`: Specifies the delay in milliseconds before the agent plays the greeting message after the user joins the channel. 

- Added `llm.headers`: Allows you to pass custom headers in LLM requests, such as business-specific fields or tenant identifiers.
