...
In a real-word application, you report your local spatial position to a server in your environment and retrieve positions of remote users in the channel from your server. In this simple example, you use a [`Slider`](https://docs.unity3d.com/2018.3/Documentation/ScriptReference/UI.Slider.html) to set the spatial position of a remote user. 
...
To add a button to the UI, take the following steps:
    ...
    1. In **Inspector**, rename **Slider** to `distanceSeekBar`, and then change the following coordinates:
...
1. **Declare the variables you need**
    ...
    private Slider distanceSeekBar;
    ```
...
1. **Set the spatial position of a remote user**
    ...
    2. To call `updateSpatialAudioPosition` when a user moves the slider, in your script file, add the following at the end of `SetupUI`:
        ...
        go = GameObject.Find("distanceSeekBar");
        distanceSeekBar = go.GetComponent<Slider>();
        // Specify a minimum and maximum value for slider.
        distanceSeekBar.maxValue = 10;
        distanceSeekBar.minValue = 0;
        // Add a listener to the slider and which invokes distanceSeekBar when the slider is dragged left or right.
        distanceSeekBar.onValueChanged.AddListener(delegate {updateSpatialAudioPosition((int)distanceSeekBar.value);});
        ```
