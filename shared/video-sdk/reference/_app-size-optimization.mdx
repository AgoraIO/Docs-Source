...
In versions before v4.19.0, the `AgoraRTC` module is the entrance to all callable methods in the Web <Vg k="VSDK" />. Even if you actually only use part of the <Vg k="VSDK" />'s functionality, you still need to load the complete SDK, which may result in the <Vpl k="CLIENT" /> containing a lot of unused code.
The Web <Vg k="VSDK" /> has integrated [tree shaking](https://webpack.js.org/guides/tree-shaking/) support since v4.19.0. Using tree shaking has the following advantages:
...
To use tree shaking, import all used methods, enumerations, constants, types, etc. by adding `/esm` at the end of the package name `agora-rtc-sdk-ng`:
