...
[Agora Analytics](/agora-analytics/overview/product-overview) provides end-to-end quality data for [Broadcast Streaming](/broadcast-streaming/overview/product-overview) that helps you locate quality issues.
...
- Values and labels for five quality metrics. For definitions of the metrics and labels, see [Reference](#terms).
...
The **Quality details** tab displays the line charts for five quality metrics. For definitions of the metrics, see [Reference](#terms). On each line chart, you can do the following:
...
| Video freeze rate    | The duration of video freezing divided by the total video duration. Video freezing occurs if the video pauses for over 600 ms. The duration is [calculated by the number of streams](/interactive-live-streaming/reference/billing-policies#how-does-agora-calculate-service-minutes). | <li>Low: The freeze rate is no higher than 5%.</li><li>High: The freeze rate is higher than 5%.</li> |
