...
In `ViewController`, add the <Vg k="UIK" /> libraries after `import Cocoa`:
...
```swift ViewController focus=2:17
...
In `ViewController`, replace the `viewDidLoad` function with the following code:
