...
* Two image widgets: One to display local video, the other to display remote video.

* Two button widgets: One to join the channel, the other to leave the channel.

<ProductWrapper product="interactive-live-streaming">
* One selector to choose to join as the host or the audience
</ProductWrapper>

To implement this user interface, you use a `UserWidget` instance to create the blueprint you add user widgets to.
...
3. **Add the join and leave buttons**
    ...
    1. Add a label to `JoinBtn`.
        ...
        1. Drag **Text** from the **Common** section of **Palette Panel** to **Hierarchy** and drop it over `JoinBtn`.
        1. In **Details**, change the **Text** field to `Join`.

    1. Use the same procedure and add a text widget for the leave button where the **Text** field says `Leave`. 
...
3. **Enable the user to join a channel as the host or the audience**
    ...
    1. Add a text widget for `hostToggle`. Drag **Text** from **Palette Panel** to **Hierarchy** and drop it over `hostToggle`, then change the **Text** field to `Host` in **Details**.


    1. Use the same procedure to add a text widget for `audienceToggle` where the **Text** field says `Audience`.
...
1. **Add the widget blueprint to viewport**
    ...
    1. Use the same procedure to add the **Add to Viewport** node to **Level Blueprint**.
...
1. **Import the <Vg k="COMPANY"/> library**
    ...
    1. Import the  <Vg k="COMPANY"/> namespaces. Add the following before `UCLASS()`:
...
1. **Setup <Vg k="ENGINE"/>**
    ...
    To setup an <Vg k="ENGINE"/> instances, take the following steps:

    1. Declare the function that creates an <Vg k="ENGINE" /> instance. In `MyUserWidget.h`, add the following method declaration to `UMyUserWidget`:

        ```cpp
       	void setupVideoSDKEngine();
        ```

    2. Setup the <Vg k="ENGINE" /> instance. Add the following method to `MyUserWidget.cpp`:
        ...
        void UMyUserWidget::setupVideoSDKEngine() 
        {
            ...
            // Initialize the engine instance with a context.
...
1. **Handle and respond to <Vg k="ENGINE"/> events**
    ...
    1. In `MyUserWidget.h`, add the following after `class AGORAIMPLEMENTATION_API UMyUserWidget : public UUserWidget`:
...
1. **Check that the <Vpl k="CLIENT" /> has the correct permissions to start**
    ...
    2. Call `CheckAndroidPermission` in the constructor.
        ...
        In `MyUserWidget.cpp`, add the following code before `setupVideoSDKEngine`:
    ...
    3. After you check permissions, you call `setupVideoSDKEngine` to create an engine instance.
