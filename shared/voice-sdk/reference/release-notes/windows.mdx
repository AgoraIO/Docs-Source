...
1. **Audio loopback capturing**
   ...
   - As of v4.3.0, if you call the <Link to="{{Global.API_REF_CPP_ROOT_VOICE_SDK}}/class_irtcengine.html#api_irtcengine_disableaudio">disableAudio</Link> method to disable the audio module, audio loopback capturing will be disabled as well. If you need to enable audio loopback capturing, you need to enable the audio module by calling the [enableAudio](API/api_irtcengine_enableaudio.html) method and then call [enableLoopbackRecording](API/api_irtcengine_enableloopbackrecording.html).
