...
an authentication token that is valid for a specific user.

When a user attempts to connect to <Vg k="MESS" />, your <Vpl k="CLIENT" />  retrieves a token from the
...
When the token is about to expire, <Vg k="AGORA_BACKEND" /> sends an event to your <Vpl k="CLIENT" /> and you renew
the session with a new token.
