...
1. **Listen to <Vg k="ENGINE" /> events to receive state change notifications and quality statistics.**
    ...
    * `onLocalAudioStats`: Receives stats about the local audio.
    ...
    func rtcEngine(_ engine: AgoraRtcEngineKit, reportRtcStats: AgoraChannelStats) {
        ...
        if (msg.count > 0) { showMessage(title: "Voice SDK Stats", text: msg) }
    }

    func rtcEngine(_ engine: AgoraRtcEngineKit, remoteAudioStateChangedOfUid: UInt, state: AgoraAudioRemoteState, reason: AgoraAudioRemoteReason, elapsed: Int) {
        let stateChangeReport = ["Uid = \(remoteAudioStateChangedOfUid)", "NewState = \(state):", "Reason = \(reason):",
                                    "Elapsed = \(elapsed)"].joined(separator: "\n")

        showMessage(title: "Remote audio state changed:", text: stateChangeReport, delay: 8)
    }

    func rtcEngine(_ engine: AgoraRtcEngineKit, localAudioStats: AgoraRtcLocalAudioStats, sourceType: AgoraVideoSourceType) {
        counter2 += 1

        if counter2 == 5 {
            let localAudioStatsReport = ["SentBitrate = \(localAudioStats.sentBitrate)", "codecType = \(localAudioStats.internalCodec)"].joined(separator: "\n")
            counter2 = 0;
