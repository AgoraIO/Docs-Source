...
| ⇒`enable_string_uid` | boolean | No | Whether to enable String uid: <ul><li>`true`: Both agent and subscriber user IDs use strings.</li><li>`false`: (Default) Both agent and subscriber user IDs must be integers.</li></ul> |
| ⇒`idle_timeout` | integer | Yes   | Sets the timeout after all the users specified in `remote_rtc_uids` are detected to have left the channel. When the timeout value is exceeded, the agent automatically stops and exits the channel. A value of `0` means that the agent does not not exit until it is stopped manually. Default value `30` |
| ⇒`advanced_features` | object | | Advanced features configuration. |
| ⇒⇒`enable_aivadmd` | boolean | Optional    | Whether to enable the graceful interruption function (AIVAD). This feature is a value-added service, please refer to [pricing](../overview/pricing) for details. Default value `false`|
| ⇒⇒`enable_bhvs` | boolean | Optional     | Whether to enable background noise reduction. This feature is a value-added service, please refer to [pricing](../overview/pricing) for details. Default value `true`  |
| ⇒`asr` | object | Optional | Automatic Speech Recognition (ASR) configuration |
| ⇒⇒`language` | string | Optional | The language used by users to interact with the agent. Currently <Vpd k="NAME" /> only supports English `en-US` (Default).  |
| ⇒`tts` | object | Optional | Text-to-speech (TTS) module configuration |
| ⇒⇒`vendor` | string | Optional | TTS provider. Supports the following values: <ul><li>`microsoft`: Microsoft Azure</li><li>`elevenlabs`: ElevenLabs</li></ul> See [TTS vendor configuration](#tts-vendor-configuration) for details.| 
| ⇒⇒`params` | object | Optional | The configuration parameters for the TTS provider. |
| ⇒`llm` | object  | Optional| Large language model (LLM) configuration. |
...
| ⇒⇒`headers`  | object | Optional | Other headers for custom LLM. |
| ⇒⇒`params` | object | optional | Additional LLM information transmitted in the message body, such as the `model` used, and the maximum token limit. Refer to the [OpenAI official documentation](https://platform.openai.com/docs/api-reference/chat/create) for parameter details. |
| ⇒⇒`max_history` | integer | Optional | The number of short-term memory entries cached in the custom LLM. `0` means no short-term memory is cached. Users and agents log entries separately. Default value `10`  |
| ⇒⇒`input_modalities` | array[string] | No | LLM input modalities. Supports ["text"], ["text", "image"]. Default ["text"] |
| ⇒⇒`output_modalities` | array[string] | No | LLM output modalities. Support ["audio"],["text"], ["text", "audio"]. Default ["text"] |
...
| ⇒⇒`style` | string | Optional | The request style for chat completion. Supports: <ul><li>`openai` (Default)</li><li>`anthropic`</li><li>`gemini`</li></ul> |
| ⇒`vad` | object   | Optional | Voice Activity Detection (VAD) configuration |
| ⇒⇒`interrupt_duration_ms` | number  | Optional | The amount of time in milliseconds that the agent waits after detecting that the voice input has been interrupted. This parameter ensures that the agent does not immediately process the input until the user stops speaking. Default value `160` |
| ⇒⇒`prefix_padding_ms` | integer | Optional | The extra forward padding time in milliseconds before the processing system starts to process the speech input. This padding helps capture the beginning of the speech. Default value `300` |
| ⇒⇒`silence_duration_ms` | integer | Optional | The duration of audio silence in milliseconds. If no voice activity is detected during this period, the agent assumes that the user has stopped speaking. Default value `500` |
| ⇒⇒`threshold` | number  | Optional | Identification sensitivity determines the level of sound in the audio signal that is considered voice activity. The value range is (0.0, 1.0). Lower values ​​make it easier for the agent to detect speech, and higher values ignore weak sounds. Default value `0.5` |
| ⇒`parameters` | object   | Optional | Other configuration parameters.    |
| ⇒⇒`aivad_force_threshold` | number | No | The maximum waiting time in milliseconds for graceful interruption (AIVAD). This parameter only takes effect when `advanced_features.enable_aiavid` is set to `true`. Default value: `3000` |
| ⇒⇒`output_audio_codec` | string | No | RTC output codec. Use `PCMU`, `PCMA`, `G722`, `OPUS`, or `OPUSFB` |
| ⇒⇒`audio_scenario` | string | No | RTC audio scenario. Use `default`, `chorus`, or `aiserver`. Default value `aiserver` |
