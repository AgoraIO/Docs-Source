...
2. [Generate a temporary token](../reference/manage-agora-account#generate-a-temporary-token) in <Vpl k="CONSOLE" /> using `_appID` and `_channelName`. Use it to update `_token` in `NewBehaviourScript`. Use the same values to generate another token and update `sourceChannelToken`.

3. Generate a third token in <Vpl k="CONSOLE" /> using `_appID` and `destChannelName`. Use it to update `destChannelToken` in `NewBehaviourScript`.
...
2. [Generate a temporary token](../reference/manage-agora-account#generate-a-temporary-token) in <Vpl k="CONSOLE" /> using `_appID` and `_channelName`. Use it to update `_token` in `NewBehaviourScript`.

3. Generate a second token in <Vpl k="CONSOLE" /> using `_appID` and `secondChannelName`. Use it to update `secondChannelToken` in `NewBehaviourScript`.
