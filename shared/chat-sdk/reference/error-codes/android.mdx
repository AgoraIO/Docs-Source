...
| 215        | `USER_MUTED`                     | The user is muted and not allowed to send messages in a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ONE_LOWER"/>. |
...
| 507        | `MESSAGE_ILLEGAL_WHITELIST`  | The user is not on the allow list of a group or <Vg k="CHAT_ONE_LOWER"/>. This error occurs when a user that is not on the allow list attempts to send a message in a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ONE_LOWER"/> if all members of the group or <Vg k="CHAT_ONE_LOWER"/> are muted. |
...
| 602        | `GROUP_NOT_JOINED`                    | The user hasn't joined this <Vg k="CHAT_GROUP_LOWER"/>. This error occurs when a user attempts to send a message or perform operations in a <Vg k="CHAT_GROUP_LOWER"/> that he or she is not in. |
...
| 612    | `GROUP_MEMBER_ATTRIBUTES_VALUE_REACH_LIMIT`   | The value of a custom attribute for a <Vg k="CHAT_GROUP_LOWER"/> member exceeds the limit of 512 bytes.  |
| 700        | `CHATROOM_INVALID_ID` | Invalid <Vg k="CHAT_ONE_LOWER"/> ID. This error occurs when the <Vg k="CHAT_ONE_LOWER"/> ID is an empty string for the <Vg k="CHAT_ONE_LOWER"/>-related method that is used. |
| 701        | `CHATROOM_ALREADY_JOINED`            | The user is already in the <Vg k="CHAT_ONE_LOWER"/>. This error occurs when a user attempts to join a <Vg k="CHAT_ONE_LOWER"/> that he or she is already in. |
| 702        | `CHATROOM_NOT_JOINED`                | The user hasn't joined this <Vg k="CHAT_ONE_LOWER"/>. This error occurs when a user attempts to send a message or perform operations in a <Vg k="CHAT_ONE_LOWER"/> that he or she is not in. |
| 703        | `CHATROOM_PERMISSION_DENIED`  | The user does not have the permission to perform the operation to a <Vg k="CHAT_ONE_LOWER"/>. For example, a <Vg k="CHAT_ONE_LOWER"/> member does not have permission to add or remove a <Vg k="CHAT_ONE_LOWER"/> admin. |
| 704        | `CHATROOM_MEMBERS_FULL`    | The number of members in the <Vg k="CHAT_ONE_LOWER"/> has reached the upper limit specified during <Vg k="CHAT_ONE_LOWER"/> creation. |
| 705        | `CHATROOM_NOT_EXIST`  | The <Vg k="CHAT_ONE_LOWER"/> to which the user is requesting to perform operations does not exist.  |
...
| 1302   | `REACTION_OPERATION_IS_ILLEGAL`  | The user does not have permission to operate on this Reaction. For example, a user that does not add this Reaction attempts to delete it or a user that is neither the message sender nor recipient tries to add a Reaction in a <Vg k="CHAT_ONE_LOWER"/>. |
