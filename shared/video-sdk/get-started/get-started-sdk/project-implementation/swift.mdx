...
To ensure that you receive all events from the moment the engine starts, set the delegate when [initializing the engine](#initialize-the-engine).
</Admonition>
...
<ProductWrapper notAllowed="voice-calling">
The following example creates two `UIView`s to display local and remote videos. To use this interface, replace the contents of the `ViewController.swift` file with the following code:
</ProductWrapper>
<ProductWrapper product="voice-calling">
