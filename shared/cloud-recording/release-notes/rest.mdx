...
- Improves player compatibility for MPD files generated in individual recording `standard` mode, see [Recording configuration](../rest-api/acquire#clientrequest-startparameter-recordingconfig).
...
For voice chat use-cases, this release adds the `appsCollection` and `transcodeOptions` parameters in the `start` method to support [Postpone Audio Mixing](../develop/individual-nontranscoding#implement-an-postpone-audio-mixing). If you enable this function, the recording service merges and transcodes the recorded files of all user IDs in the specified channel within 24 hours after the recording ends to generate an MP3/M4A/AAC file, and then uploads the recording file to your specified third-party cloud storage.

**A Combined audio and video file after individual recording**

When calling `start`, set the `streamMode` field of the [`recordingConfig`](../rest-api/acquire#clientrequest-startparameter-recordingconfig) parameter to `standard`, you can get a M3U8 audio index file, a M3U8 video index file and a combined M3U8 audio and video index file. If VP9 encoding is used on the web side, a combined MPD audio and video index file are generated.
...
This release adds the [`extensionParams`](/cloud-recording/rest-api/acquire#clientrequest-startparameter-storageconfig-extensionparams) field to support the third-party cloud storage service to encrypt and tag uploaded recording files. This feature is applicable to Amazon S3 and Alibaba Cloud OSS.
### 2021.12.09
...
Adds support for converting the content and audio of a web page into a media stream and pushing it to the CDN during the web page recording. See the [`start`](/cloud-recording/rest-api/start), [`update`](/cloud-recording/rest-api/update) and [`query`](/cloud-recording/rest-api/query) methods of the Agora Cloud Recording RESTful API for details.
...
This release adds the `onhold` parameter to the [`start`](/cloud-recording/rest-api/start) and [`update`](/cloud-recording/rest-api/update) methods, which is used to pause the recording when the web page recording starts or is in progress.
...
- The [`update`](/cloud-recording/rest-api/update) method adds the `webRecordingConfig` parameter, which is used to update related parameters during a web page recording.
...
This release adds `maxVideoDuration` to the [`start`](/cloud-recording/rest-api/start) method to set the maximum duration of the MP4 slice file generated by the web page recording.
...
This release adds the `region` parameter to the [`acquire`](/cloud-recording/rest-api/acquire) method so you can geofence your cloud recording service. If you specify a region, the cloud recording server connects only to the Agora servers within that region. The following regions are supported:
...
- Adds the `region` parameter in the [`acquire`](/cloud-recording/rest-api/acquire) method.
...
- Adds enumerators `5` and `6` for `decryptionMode` in the [`start`](/cloud-recording/rest-api/start) method.
...
- Adds the `backgroundImage`, `defaultUserBackgroundImage` and `backgroundConfig` fields in the `transcodingConfig` parameter of the [`start`](/cloud-recording/rest-api/start) method.
- Adds the `backgroundImage`, `defaultUserBackgroundImage` and `backgroundConfig` fields in the `clientRequest` parameter of the [`updateLayout`](/cloud-recording/rest-api/update-layout) method.
...
- Adds `"web_recorder_service"` as an extension service in the `extensionServiceConfig` parameter of the [`start`](/cloud-recording/rest-api/start) method.
...
**API changes**
- [`start`](/cloud-recording/rest-api/start) adds the `extensionServiceConfig` parameter for specifying the configurations of ApsaraVideo for VoD.
...
- In [`start`](/cloud-recording/rest-api/start) adds the `unSubscribeVideoUids` and `unSubscribeAudioUids` parameters for setting up subscription blacklists.
- Adds [`update`](/cloud-recording/rest-api/update) for updating the subscription lists during the cloud recording.
...
In the [`acquire`](/cloud-recording/rest-api/acquire) method, adds the `resourceExpiredHour` parameter for setting the time limit for all cloud recording method calls.
...
- [RESTful API Reference](/cloud-recording/rest-api/overview): Details of the RESTful API methods.
