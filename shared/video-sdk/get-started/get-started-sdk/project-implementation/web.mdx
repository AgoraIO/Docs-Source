...
Get your App ID, and token from <Vg k="CONSOLE"/>. For more information, see [Generate temporary token](./get-started/manage-agora-account?#generate-temporary-tokens).
...
Use event listeners to subscribe or unsubscribe to remote tracks. The SDK triggers the `user-published` callback when a user publishes an audio track in the channel. Similarly, it triggers the `user-unpublished` callback when a user leaves the channel, goes offline, or unpublishes their media track. 
...
To display the remote video, call the `play` method of remote user's `videoTrack` and pass in the HTML element where you want to render the video.
...
A complete code sample demonstrating the basic process of real-time interaction is provided here for your reference. To use the sample code in your app, create a file named `agoraLogic.js` in the `src` folder. This file contains `AgoraRTCClient` code that implements specific application logic.
