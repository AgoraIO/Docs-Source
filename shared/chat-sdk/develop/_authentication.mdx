...
- An [Agora project](../reference/manage-agora-account#create-and-manage-projects) with the [App Certificate](../reference/manage-agora-account#manage-app-certificates) and [Agora Chat](enable) enabled.
- App ID, OrgName, and AppName of your Agora project. See [Enable and Configure Agora Chat Service](enable).
...
4. In `<Project name>/src/main/resource`, create an `application.properties` file to store the information for generating tokens and update it with your project information and token validity period. For example, set `expire.second` as `6000`, which means the token is valid for 6000 seconds.
    ...
    <div class="alert info">To get the app key and the RESTful API domain, refer to <a href="../get-started/enable#get-the-information-of-the-chat-project">Get the information of the Agora Chat project</a>.</div>
