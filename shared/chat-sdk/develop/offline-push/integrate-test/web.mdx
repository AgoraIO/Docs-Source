...
|  ` AT `  | Only receive push notifications for messages mentioned. < br />This parameter is recommended for group chats. If you want to mention one or more users, you need to pass "em_at_list": [ "user1", "user2" ... ] to the ` ext ` field when creating a message ; if you want to mention everyone, pass "em_at_list": "all" to this field. | ✓ | ✓ |
...
For example, suppose the push method for the app is set to ` AT` , and the push method for a given conversation is set to ` ALL` . You will receive all push notifications from that conversation, but for all other conversations, you will only receive push notifications for messages that mention you.
...
If you need to push messages to specific users during the do not disturb period or the effective time, you need to set [ forced push ] ( ./7_push_extension#forced ) .
...
You can call the ` setSilentModeForAll` method to set app-level push notifications, and set the push notification method and do not disturb mode by specifying the ` paramType` field, as shown in the following code example:
...
You can call ` getSilentModeForAll` to get the push notification settings of the app. The sample code is as follows :
...
You can call ` setSilentModeForConversation` to set the push notification settings for a specified conversation. The sample code is as follows :
...
1. You can get push notification settings for up to 20 sessions at a time.
...
You can call ` getSilentModeForConversations` to get the push notification settings for multiple conversations. The sample code is as follows :
...
You can call the ` clearRemindTypeForConversation` method to clear the push notification type settings for a specified conversation. After clearing, this conversation will use the app's settings by default. The sample code is as follows:
...
Push notifications work with the translation feature. If a user enables the [ automatic translation ] ( ./agora_chat_translation_web ) feature and sends a message, the SDK will send both the original message and the translated message.

As a recipient, you can set the preferred language for push notifications you wish to receive when offline. If the language of the translated message matches your preferred language, the translated message is displayed in the push notification; otherwise, the original message is displayed.

You can call the ` setPushPerformLanguage` method to set the preferred language for push notifications. The sample code is as follows:
...
The default push template is mainly used when the default configuration provided by the server does not meet your needs. It allows you to set the global push title and push content. For example, the default settings provided by the server are push titles and content in Chinese and English. If you need to use push titles and content in Korean or Japanese, you can set the push template in the corresponding language.

#### Use the default push template

To use the default template, you need to create a default push template in the Agora console or [ call the RESTful interface ] ( ./agora_chat_restful_push#set-up-push-templates ). The template name is ` default ` . After setting, the default template is automatically used when pushing messages. You do not need to enter the template name when creating messages.
