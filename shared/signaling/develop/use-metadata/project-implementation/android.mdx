...
1. **Clear metadata on logout**
    ...
    When the local user logs out of the <Vpd k="PRODUCT" /> server, you clear the local user metadata. To do this, in `onClickLogout`, add the following lines to the `onSuccess` callback of `mRtmClient.logout`:
