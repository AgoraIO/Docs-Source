...
- Audio transcoding duration: The duration that Media Inject service transcodes an audio stream.
- Video transcoding duration: The duration that Media Inject service transcodes a video stream or an audio and video stream.

<div class="alert note">When using the Agora Media Inject service, if you set the value of <code>playTs</code> greater than <code>createTs</code>, the duration between these two timestamps is billed as audio transcoding charges.</div>
