...
{data.AGORA_BACKEND} uses integer user Ids for token generation. To ensure smooth communication, all the users in a channel must use the integer `uid`. You generate a token using `buildTokenWithUid` for an RTC token based on `uid`
