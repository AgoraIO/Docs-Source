...
| `completion`   | `@escaping (ConversationalAIAPIError?) -> Void`   | Callback function invoked when the operation completes. <li>Returns nil on success.</li> <li>Returns error information on failure, see ConversationalAIAPIError for details.</li> |
...
- `base64`: Base64-encoded image data.
    ...
    <Admonition type="info">When using `base64`, ensure the entire message size (including the JSON structure) is less than 32KB to comply with the RTM message channel limit. For larger images, use a URL.</Admonition>
...
| `url`         | `String?`         | HTTP/HTTPS link to the image file. The agent will download and process the image from this link. <br/><Admonition type="info"> Mutually exclusive with `base64`.</Admonition> |
| `base64`      | `String?`         | Base64-encoded image data. When using this parameter, the total message size is limited to 32KB. <br/><Admonition type="info"> Mutually exclusive with `url`; total message size must not exceed 32KB.</Admonition> |
...
| `renderMode` | `TranscriptRenderMode`| Translate the rendering mode. See [`TranscriptRenderMode`](#transcriptrendermode). |
