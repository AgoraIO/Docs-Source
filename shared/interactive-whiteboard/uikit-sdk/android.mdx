...
After successfully [launching a file conversion task](../reference/whiteboard-api/file-conversion#start-file-conversion), you can call this method and pass in the parameters of the converted file. After a successful call, the SDK automatically creates a sub-window to insert and display the converted file per page.
...
- `taskUUID`: String. The UUID of the file conversion task. You can get `uuid` from the response body when the [Start file conversion (POST) API](../reference/whiteboard-api/file-conversion#start-file-conversion) call succeeds.
