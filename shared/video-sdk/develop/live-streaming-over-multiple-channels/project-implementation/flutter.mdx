...
``` dart
ElevatedButton(
child: const Text("Join second channel"),
onPressed: () => _isJoined ? {joinSecondChannel()} : null,
),
```
...
1.  **Declare the variables you need**
    ...
    ``` dart
    late RtcConnection rtcSecondConnection;
...
1.  **Update the `setupRemoteVideo` method**
