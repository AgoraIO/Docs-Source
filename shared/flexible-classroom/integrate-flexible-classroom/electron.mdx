...
4. Import the Agora Classroom SDK in the HTML file and launch a classroom.
    ...
    The sample code requires passing in `rtmToken`. You can refer to [Generate a <Vg k="MESS" /> token](/flexible-classroom/get-started/manage-agora-account#generate-temporary-tokens) to learn what is a <Vg k="MESS" /> token, how to get a temporary <Vg k="MESS" /> token for testing purpose, and how to generate a <Vg k="MESS" /> token from the server. The generated token passed in `userId` must be consistent with the `userUuid` parameters passed in the `launch` method; otherwise, the generated token will be invalid.
...
<div class="alert info">Make sure you have <a href="/flexible-classroom/get-started/demo-quickstart#prerequisites">set up the development environment</a>.</div>
...
1. Modify the code according to your needs. See [Classroom and Proctor SDK UI customization guide](/flexible-classroom/develop/integrate/customize-ui/customize-classroom).
