...
- Before joining a topic, a user need to create an <code>{channel.istreamchannel[props.ag_platform]}</code> instance and call the <code>{channel.join[props.ag_platform]}</code> method to join the stream channel. 
</Admonition>
