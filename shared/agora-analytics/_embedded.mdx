...
You can customize the content in the embedded <Vpd k="NAME"/> page using the **Page Attributes** panel. Settings in the panel are transformed into parameters in the code snippet in real time, and these parameters are appended to the URL you get from [Request the URL for Embed](#request-the-url-for-embed).
...
-   `token`: The `token` parameter returned in [1. Request the URL for Embed](#request-the-url-for-embed) when you set `feature` as `callSearch`.
