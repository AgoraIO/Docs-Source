...
3. Call the [Query file conversion progress (GET) API](https://docs.agora.io/en/whiteboard/whiteboard_file_conversion?platform=RESTful#query-file-conversion-progress-get), and pass in the UUID and token of the conversion task to get the information on the converted files, that is, the value of the `convertedFileList` parameter in the response body.
...
1. Successfully launch a dynamic-document conversion task, and call the [Query file conversion progress (GET) API](https://docs.agora.io/en/whiteboard/whiteboard_file_conversion?platform=RESTful#query-file-conversion-progress-get) to get the list of converted files.
