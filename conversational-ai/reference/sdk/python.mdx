...
Sets the MLLM vendor for multimodal mode. Pass [`OpenAIRealtime`](#openairealtime-1), [`GeminiLive`](#geminilive) or [`VertexAI`](#vertexai-1). Requires `advanced_features={'enable_mllm': True}` in the constructor. Calling `with_mllm()` automatically sets `mllm.enable = True`.
...
```python
from agora_agent.agentkit.vendors import GroqTTS
```
...
| `predefined_tools` | `List[str]` | No | `None` | Predefined tools, for example `['_publish_message']` |
