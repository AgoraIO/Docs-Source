...
<h4 className="index-group-proxy" id={config.proxy[props.ag_platform]}>{config.proxy[props.ag_platform]}</h4>
...
The <code>{config.release[props.ag_platform]}</code> method returns an `AgoraRtmErrorCode` data structure. See <a href="#error-codes-table">Error codes</a >.