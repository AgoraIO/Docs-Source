...
When a user sends a message in a <Vg k="CHAT_ONE_LOWER"/>, <Vg k="CHAT_GROUP_LOWER"/>, or <Vg k="CHAT_ROOM_LOWER"/> of the Chat app, the Chat server sends a callback to your app server. The sample code is as follows:
...
| `chat_type` | String | The type of chat. <ul><li>`chat`: One-to-one chats.</li><li>`groupchat`: Chat groups and <Vg k="CHAT_ROOM_LOWER"/>s.</li></ul> |
...
When a user sends a text message in a <Vg k="CHAT_ONE_LOWER"/>, <Vg k="CHAT_GROUP_LOWER"/>, or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When a user sends an image message in a <Vg k="CHAT_ONE_LOWER"/>, <Vg k="CHAT_GROUP_LOWER"/>, or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When a user sends an audio message in a <Vg k="CHAT_ONE_LOWER"/>, <Vg k="CHAT_GROUP_LOWER"/>, or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When a user sends a video message in a <Vg k="CHAT_ONE_LOWER"/>, <Vg k="CHAT_GROUP_LOWER"/>, or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When a user sends a location message in a <Vg k="CHAT_ONE_LOWER"/>, <Vg k="CHAT_GROUP_LOWER"/>, or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When a user sends a command message in a <Vg k="CHAT_ONE_LOWER"/>, <Vg k="CHAT_GROUP_LOWER"/>, or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When a user sends a custom message in a <Vg k="CHAT_ONE_LOWER"/>, <Vg k="CHAT_GROUP_LOWER"/>, or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When a user recalls a message in a <Vg k="CHAT_ONE_LOWER"/>, <Vg k="CHAT_GROUP_LOWER"/>, or <Vg k="CHAT_ROOM_LOWER"/> of the Chat app, the Chat server sends a callback to your app server. The sample code is as follows:
...
## Chat group and <Vg k="CHAT_ROOM_LOWER"/> events

When a user performs operations on a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/> in the Chat app, the Chat server sends a callback to your app server. The sample code is as follows:
...
<div class="alert note">This callback is triggered only if the multi-device service is enabled. Once a user creates a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/> on one device, the Chat server sends callbacks to the other devices, notifying about the creation of the <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/>. </div>

When a user creates a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When the owner destroys a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When a user sends a join request to a <Vg k="CHAT_GROUP_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When the owner or an admin accepts a join request to the <Vg k="CHAT_GROUP_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When a user invites another user to a <Vg k="CHAT_GROUP_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When a user joins a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When a member voluntarily leaves a group or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When a member voluntarily or passively leaves a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends the leave result to your app server. The sample code of the `payload` field is as follows:
...
When the owner or an admin removes a member from a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When the owner or an admin adds a member to the block list of a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When the owner or an admin removes a member from the block list of a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When the owner or an admin adds a member to the allow list of a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When the owner or an admin removes a member from the allow list of a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When the owner or an admin mutes a member in a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When the owner or an admin unmutes a member in a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When a member mutes a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When a member unmutes a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/> is globally muted, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/> is globally unmuted, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When the owner transfers the ownership of a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When the owner promotes a member to an admin in a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When the owner demotes an admin to a member in a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When the owner or an admin updates the information of a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When the owner or an admin updates the announcements of a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When the owner or an admin deletes the announcements of a <Vg k="CHAT_GROUP_LOWER"/> or <Vg k="CHAT_ROOM_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When a user uploads a shared file to a <Vg k="CHAT_GROUP_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
...
When a user deletes a shared file in a <Vg k="CHAT_GROUP_LOWER"/>, the Chat server sends a callback to your app server. The sample code of the `payload` field is as follows:
