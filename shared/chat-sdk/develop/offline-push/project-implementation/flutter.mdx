...
3. Listen for device token generation.
   ...
   Rewrite the `onTokenRefresh` callback. Once a device token is generated, this callback passes the new device token to the <Vg k="CHAT_SDK"/> at the earliest opportunity.
