...
This sections describes the steps required to use the relevant libraries, declare the necessary variables, and setup access to the UI elements.
...
1.  **Setup the audio frame observer**
    ...
    The `IAudioFrameObserver` gives you access to each audio frame after it is captured or before it is played back. To setup the `IAudioFrameObserver`, add the following lines to the `MainActivity`
    class after variable declarations:
...
2.  **Setup the video frame observer**
    ...
    The `IVideoFrameObserver` gives you access to each local video frame after it is captured, and to each remote video frame before it is played back. In this example, your modify the captured video frame buffer to crop and scale the frame and play a zoomed-in version of
    the video. To setup the `IVideoFrameObserver`, add the following lines to the `MainActivity` class after the variable declarations:
    ...
    private final IVideoFrameObserver iVideoFrameObserver = new IVideoFrameObserver() {
        ...
        public int getVideoFrameProcessMode() {
            // The process mode of the video frame. 0 -> Read only, 1 -> Read and Write
            return 1;
        }
...
3.  **Register the video and audio frame observers**
    ...
    To receive callbacks declared in `IVideoFrameObserver` and `IAudioFrameObserver`, you must register the video and audio frame observers with the <Vg k="ENGINE" />, before joining a channel .
    To specify the format of audio frames captured by each `IAudioFrameObserver` callback, use the `setRecordingAudioFrameParameters`, `setMixedAudioFrameParameters` and `setPlaybackAudioFrameParameters` methods. To do this, add the following lines after `if (checkSelfPermission()) {` in the
    `joinChannel` method:
...
4.  **Unregister the video and audio observers when you leave a channel**
    ...
    When you leave a channel, you unregister the frame observers by calling the register frame observer method again with a `null` argument. To do this, add the following lines to the
    'leaveChannel(View view)' method before `agoraEngine.leaveChannel();`:
