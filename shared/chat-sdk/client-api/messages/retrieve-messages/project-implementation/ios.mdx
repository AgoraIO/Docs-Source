...
<Admonition type="info" title="Hint">Adding tags to a conversation, such as stars, does not affect other logic of the conversation, such as the number of unread messages in the conversation.</Admonition>
...
You can use the `getConversationsFromServer` method to retrieve a list of conversations from the server based on the tag. The SDK returns the conversation list in reverse order of the conversation tag's timestamp. Each conversation object includes the following information:
...
For local conversations, you can use the `getAllConversations` method to retrieve all local conversations and then perform session filtering as needed. The following example demonstrates how to query all local conversations marked with `EMMarkType0:`
```objective-c
...
### Get all tags of a local single conversation

To retrieve all tags associated with a local single conversation, first use the `getConversationWithConvId` method to obtain the local single conversation. Then, access the `marks` property of the conversation object to view all its tags. Below is a sample code snippet demonstrating this process:
