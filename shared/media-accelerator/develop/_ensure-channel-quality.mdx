...
- **Choose an audio codec to optimize bit rate and quality**
  ...
  In audio and video streaming, the choice of an encoding algorithm affects both quality and bit rate. Your goal is to lower the required bit rate while maintaining quality at the desired level. <Vpd k="SDK" /> offers the following built-in audio codecs to choose from:
  ...
  You specify an audio codec when you join a channel. You also set the sampling rate and the number of channels at this time, depending on your audio quality requirements. If your <Vpl k="CLIENT"/> needs a custom encoding algorithm, you disable use of a built-in codec and implement your own encoder. 
