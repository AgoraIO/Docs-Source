...
- On Mac OS Ventura, when sharing the screen using Safari, the video resolution set by the SDK may be different from the actual captured resolution.
  If the browser does not support the resolution set by the user, the billing system will use the actual captured resolution. As a result, the captured resolution may be larger or smaller than the set resolution due to browser restriction.
