...
1. **Import the required dart libraries**
    ...
    To convert an image to a video frame, import the necessary Electron libraries. Add the following modules after the last `createAgoraRtcEngine,`:
...
1. **Enable publishing the custom video track**
    ...
    To allow publishing of video from a custom source, you call the `setExternalVideoSource` method of the `MediaEngine`. To implement thid workflow, in `preload.js`, add the following lines after `agoraEngine.initialize({appId: appID});`:
...
1. **Load image data and push it in a video frame**
    ...
    Your <Vpl k="CLIENT" /> loads an image file and pushes it as a video frame. To read an image file and convert it to a video frame, add the following method before `window.onload = () =>`:
        ...
        ```
1. **Implement the logic to transform the selected image to `Uint8Array`**
    ...
    Add the following code to `preload.js` before `window.onload = () =>`
        ...
        ```
        To push video frame when user presses **Push Video Frame**, in `preload.js`, add the following code before `document.getElementById("join").onclick = async function ()`:
...
1.  **Publish the custom video track**
    ...
    Customize the channel media option to publish the custom video track. In `preload.js`, replace `agoraEngine.joinChannel(token, channel, null, Uid);` with the following code:
...
1. **Update the user interface**
    ...
    ```html
    <button type="button" id="customVideoSource">Push Audio Frame</button>
    ```
