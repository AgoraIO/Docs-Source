...
```dart
ElevatedButton(
    child: const Text("Update remote position"),
    onPressed: () => {updateRemotePosition()},
),
```
...
1. **Declare the variables you need**
    ...
    You create an instance of `LocalSpatialAudioEngine` to configure spatial audio parameters and set self and remote user positions. In `/lib/main.dart`, add the following declaration to the `_MyAppState` class:
...
1. **Create and configure the spatial audio engine**
    ...
    1. When a user launches the <Vpl k="CLIENT" />, you create an instance of `LocalSpatialAudioEngine`, configure it and update the user's self position. To do this, add the following method to the `MainActivity` class:
...
1. **Set the spatial position of a remote user**
    ...
    To update the spatial position of a remote user, define the `RemoteVoicePositionInfo` and call `updateRemotePosition`. Add the following method to the `_MyAppState` class:
        ...
        void updateRemotePosition() {
            ...
            RemoteVoicePositionInfo positionInfo = const RemoteVoicePositionInfo(
            ...
            showMessage("Remote user's spatial position updated.");
        }
        ```
