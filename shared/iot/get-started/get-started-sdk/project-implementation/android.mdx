...
At startup, you initialize the RTC engine. When a user taps a button, the <Vpl k="CLIENT" /> joins a channel and sends audio and video data to remote users. 
...
1.  **At startup, ensure necessary permissions and start the engine**
    
    In order to send video and audio streams to <Vg k="COMPANY" />, you need to ensure that the local user gives permission to access the camera and microphone at startup. After the permissions are granted, you setup the <Vpd k="NAME"/> engine.
