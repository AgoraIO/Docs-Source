...
1. **Stop the task**
    ...
    To [stop real-time transcription](#stop-real-time-transcription), you send an `HTTP DELETE` request with the `appId`, `taskId`, and `builderToken` in the URL. In the `_MyAppState` class, add the following method:
