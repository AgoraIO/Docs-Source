...
1. In your code, replace the values for `appID` and `token` with your App ID and token from <Vg k="CONSOLE" />.
    ...
    Replace `your_appid` and `your_token` values in the code with your project's app ID and token. To quickly verify the basic functionality during the testing phase, set the **Authentication mechanism** to **Debugging mode** when creating the Agora project, and then replace both `your_appid` and `your_token` values with your project's app ID.
    </Admonition>
