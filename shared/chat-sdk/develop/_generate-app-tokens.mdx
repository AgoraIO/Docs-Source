...
- User token: User level access to Chat from your app using Chat SDK. User tokens are used to authenticate users when they log in to your Chat application. For details, see [Implement an Agora user token server for Agora Chat](../develop/generate-user-tokens).
...
You use Chat app tokens to make authenticated REST calls to Agora Chat. To obtain an Chat app token, you retrieve an Agora app token from your app token server and convert it using the Agora app token obtained to an Chat app token.
...
An Chat app token is valid for a maximum of **24 hours**. When you call Chat REST APIs, the Chat server reads the information stored in the token and uses it for authentication. An Chat app token contains the following information:
...
- The [App ID](/agora-chat/reference/manage-agora-account#get-the-app-id) , `org_name` and `app_name` of your Agora project, see [Enable and Configure Agora Chat](../get-started/enable).
...
This section shows how to implement authentication with an Chat app token step by step.
...
4. In `<Project name>/src/main/resource`, create a `application.properties` file. You need to update the properties with the value for your Chat project and set the validity period of the Agora app token you request, for example, `expire.second=6000`. 
   ...
   > Note: The validity period of an Chat app token is subject to that of the Agora app token generated by your token server. 
   >
   > For example, if you set the validity period of the Agora app token as 10,000 seconds, and you use this token to convert and obtain an Chat app token after 100 seconds, the Chat app token is to be expired after 9,900 seconds.
   ...
   For details on how to get the REST API domain, see [Enable and Configure Agora Chat](../get-started/enable).
...
### Call the Chat REST APIs with an Chat app token

This section shows how to create a new user in your Chat using an authenticated call to Chat REST API. To do this you:

1. Retrieve an Agora app token and convert it to an Chat app token. For example:
...
2. Add the Agora app token returned previously as the `Authorization: Bear` parameter and retrieve an Chat app token:
...
The following parameters are required when you send a request for an Chat app token:

| Parameter  | Data type | Required/Optional | Description                                                  |
| :--------- | :-------- | :---------------- | :----------------------------------------------------------- |
| `org_name` | String    | Required          | The unique identification that the Chat assigns to each customer developing with Agora Chat. |
| `app_name` | String    | Required          | The name that the Chat assigns to each app.            |
| `token`    | String    | Required          | The Agora app token.                                    |

### Request headers

To convert an Agora app token to an Chat app token, you need to pass in the following parameters in the request header:
...
| `5xx`                | The request fails probably because the API to convert an Agora app token to an Chat app token might be rate-limited, or an error occur during the process. |
