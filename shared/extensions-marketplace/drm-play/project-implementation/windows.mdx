...
In this example, you add two simple buttons to search and play DRM music. To create these buttons, take the following steps:
    ...
    1. To view the dialog resources, in **Resource View**, go to **AgoraImplementation.rc** > **Dialog** and double-click `IDD_AGORAIMPLEMENTATION_DIALOG`.
        The resource opens inside the **Dialog Editor**. 
    
    1. To open **Toolbox**, go to menu **View** > **ToolBox**.
    ...
    1. To change the button label, in **Properties**, update the **Caption** field with `Search Music`.

    1. Repeat the step four and create another button where its **Caption** says **Play Music**.
...
1. **Import the required libraries**
    ...
    To access and use the `IAgoraMusicContentCenter` library, in `AgoraImplementation.h`, add the following statements to the list of header files:
...
1. **Declare the variables you need**
    ...
    To instantiate `IAgoraMusicContentCenter`, play music, and handle its events, in `AgoraImplementation.cpp`, add the following declarations after the list of headers:
...
1. **Set up the music content center**
    ...
    To configure your `IAgoraMusicContentCenter` you define a `MusicContentCenterConfiguration` instance and pass it to the `initialize` method. To receive preloading and search results, define an `IMusicContentCenterEventHandler` instance and attach it to the music content center. To implement this workflow, take the following steps:
        ...
        1. Add the following to `AgoraImplementation.h` after `virtual BOOL OnInitDialog();`:

            ```cpp
            void setupMusicContentCenter();
            ```

        1. Add the following to `AgoraImplementation.cpp` after the list of headers: 
           
            ```cpp
            void CAgoraImplementationDlg::setupMusicContentCenter()
            {
                ...
                contentCenterConfiguration.appId = APP_ID;
