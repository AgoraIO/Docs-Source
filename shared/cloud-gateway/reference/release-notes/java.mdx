...
- **Send and receive audio metadata**

   Send auxiliary audio information like playback progress to remote users. Call the `sendAudioMetadata` method to transmit metadata. Remote users receive this information through the `onAudioMetadataReceived` callback.
