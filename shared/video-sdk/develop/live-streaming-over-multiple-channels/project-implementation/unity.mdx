...
1.  **Declare the variables you need**
    ...
    To store source and destination channel settings and manage channel relay, in `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelVideo.cs`, add the following declarations to the `JoinChannelVideo` class:
    ``` csharp
...
2.  **Add the required UI namespace**
    ...
    To import the required <Vpl k="NAME" /> UI namespace, in `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelVideo.cs`, add the following to the list of namespace declarations:
...
2.  **Monitor the channel media relay state**
   ...
   To receive the state change notifications sent during media relay, you add a method to the `UserEventHandler` class. Your <Vpl k="CLIENT" /> responds to connection and failure events in the `onChannelMediaRelayStateChanged` event handler. In `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelVideo.cs`, add the following method after the `OnUserOffline` callback:
...
To receive notifications of important channel relay events such as network disconnection, reconnection, and users joining channels, you add the `onChannelMediaRelayEvent` method to the `UserEventHandler` class. To implement this callback in your <Vpl k ="CLIENT"/>, add the following method after `OnChannelMediaRelayStateChanged` callback:
...
1.  **Declare the variables you need**
    ...
    To join and manage a second channel, in `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelVideo.cs`, add the following declarations to the `JoinChannelVideo` class:
...
1.  **Add the required UI namespace**
    ...
    To import the required Uinty UI namespace, in `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelVideo.cs`, add the following to the list of namespace declarations:
...
1.  **Use the multi-channel enabled `RtcEngineEx` interface**
    ...
    1.  In the `JoinChannelVideo` class, locate the `SetupVideoSDKEngine` method, replace the line `RtcEngine = Agora.Rtc.RtcEngine.CreateAgoraRtcEngine();` with the following:
