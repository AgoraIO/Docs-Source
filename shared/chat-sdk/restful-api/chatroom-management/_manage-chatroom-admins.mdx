...
In an app integrated with Chat, only the super admin of <Vg k="CHAT_ROOM_LOWER"/>s has the permission to create <Vg k="CHAT_ROOM_LOWER"/>s on the client.

This page shows how to manage super administrators of <Vg k="CHAT_ROOM_LOWER"/>s by calling the Chat RESTful APIs, including adding and retrieving super administrators and revoking the <Vg k="CHAT_ROOM_LOWER"/> creation privilege of a super administrator. Before calling the following methods, ensure that you understand the call frequency limit of the Chat RESTful APIs described in <Link to="/agora-chat/reference/limitations#call-limit-of-server-sides">Limitations</Link>.
