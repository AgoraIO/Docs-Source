...
1. **Add an image file**
    ...
    Your <Vpl k="CLIENT" /> loads this image file and pushes it as a video frame. Create a folder named `assets` in the root folder of your project. Add an image file to this folder. In `pubspec.yaml` add the following lines:
...
1. **Load the buffer and push the audio frame**
    ...
    Future<void> _pushAudioFrame() async {
        var buffer;
        // Add code to read the input stream into a buffer
