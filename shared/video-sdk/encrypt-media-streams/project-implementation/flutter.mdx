...
In a production environment, you retrieve the key and salt from an authentication server. For this simple example, you generate them locally using `OpenSSL` and insert in your code. In `/lib/main.dart`, add the following declarations to the `_MyAppState` class:
