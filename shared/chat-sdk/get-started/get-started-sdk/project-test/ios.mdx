...
1. Create an <Vpl k="CLIENT" /> instance for the first user:
    ...
    1. In the `ViewController` class, update `userId`, `token`, and `appKey` with values from <Vg k="CONSOLE" />.  To get your app key, see [Get <Vg k="CHAT"/> project information](enable#get-chat-project-information).
...
1. On each device, click **Join** to log in to the <Vg k="CHAT_SERVER" />.
