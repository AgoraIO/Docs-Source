...
- A valid Agora account. If you do not have an Agora account, see [Sign up for an Agora account](https://docs.agora.io/en/Agora Platform/sign_in_and_sign_up?platform=AllPlatforms).
- An active Agora project. If you do not have an Agora project, see [Create a project in Agora Console](https://docs.agora.io/en/Agora Platform/get_appid_token#create-an-agora-project).
...
4. On the **Notification Center Service Configuration** page, fill in the following information, and click **Save**:
   ...
   - **Event**: Select the [channel events](https://confluence.agoralab.co/display/TEKP/2.0+Channel+events) that you want to subscribe to. 
   ...
   - **Secret**: The secret used for signature verification. For instructions on how to use the secret for signature verification, see [Signature verification](https://confluence.agoralab.co/cn/Voice/rtc_channel_event#验证签名).

   - **Allowed IP List**: If your server is behind a firewall, ensure that you have called the [IP address query API](https://confluence.agoralab.co/display/TEKP/1.0+Enable+Agora+Notification+Center+Service#id-1.0EnableAgoraNotificationCenterService-ipquery) to get the IP addresses of the Agora NCS server and added them to the firewall's allowed IP list, and check the box here.
...
If your server that receives notification callbacks is behind a firewall, you need to call the [IP address query API](https://confluence.agoralab.co/display/TEKP/1.0+Enable+Agora+Notification+Center+Service#id-1.0EnableAgoraNotificationCenterService-ipqueryapi) to get the IP addresses of the Agora NCS server and configure your firewall to trust all these IP addresses.

Agora occasionally adjusts the IP addresses of the Agora NCS server. Therefore, Agora highly recommends that you call the [IP address query API](https://confluence.agoralab.co/display/TEKP/1.0+Enable+Agora+Notification+Center+Service#id-1.0EnableAgoraNotificationCenterService-ipqueryapi) at least every 24 hours and automatically update the firewall configuration; otherwise, your server might not receive the notification callbacks.
...
`Authorization`: You must generate a Base64-encoded credential with the **Customer ID** and **Customer Secret** provided by Agora, and then pass the credential to the `Authorization` field in the HTTP request header. See [HTTP authentication](https://docs.agora.io/en/Video/faq/restful_authentication) for details.
