...
When the connection state is `CONNECTED`, if the connection with the Agora RTM system is interrupted and cannot recover in four seconds due to network reasons, the connection state changes to `RECONNECTING`, and the cause for the state change is `INTERRUPTED`. When in the `RECONNECTING` state, the <Vg k="MESS_SDK" /> continues to automatically reconnect to <Vg k="SIG" /> until the login is successful, so you do not need to perform any login operations. After successful reconnection, the connection state changes to `CONNECTED`.
