...
<Admonition type="info" title="Hint">Adding tag to a conversation, such as stars, does not affect other logic of the conversation, such as the number of unread messages in the conversation.</Admonition>
...
You can call `EMConversation#marks` the method to get all the tags of a local conversation. The sample code is as follows:
