...
the following will be printed for v2.80.2:
...
1. To launch the classroom in the education scenario, call [AgoraClassroomSDK.launch](../reference/classroom-sdk#launch). Sample code:
    ...
    The sample code requires passing in `rtmToken`. You can refer to [Generate a <Vg k="MESS" /> token](../reference/manage-agora-account#generate-an--token) to learn what <Vg k="MESS" /> token is, how to get a temporary <Vg k="MESS" /> token for testing purpose, and how to generate an <Vg k="MESS" /> token from the server. In a production environment, to ensure security, you need to deploy and generate a Token on the server. The `userId` passed in the generated token must be consistent with the `userUuid` parameter of the `launch` method. Otherwise, the generated token will be invalid.
