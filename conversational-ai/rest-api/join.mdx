...
<ParameterList title="BODY" required={true}>
  ...
  <Parameter name="properties" type="object" required={true}>
    ...
    <Parameter name="avatar" type="object" required={false}>  
      ...
      <Parameter name="params" type="object" required={false}>  
        Avatar configuration parameters. The parameter structure is different for different vendors.

        Agora <Vpd k="NAME" /> does not guarantee full support for parameters not mentioned in the sample code of each vendor.

        For the specific protocol of SenseTime avatar configuration parameters, refer to the following example or the official SenseTime documentation. 
        ...
        The following SenseTime avatar configuration fields are not passed through to the vendor in `params`: `tts_config`, `tts_query`, `audios`, `quick_response`, `quick_response_preload_list`, `opening_words`.
