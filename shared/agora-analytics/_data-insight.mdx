...
1.  Subscribe to a [pricing plan](../understand/pricing) to enable the **Data Insights** feature for your project.
...
The usage metrics are described in [Usage metrics](#_usage_metrics).
...
The six dimensions are described in [Dimensions](#_dimensions).
...
The quality metrics are described in [Quality metrics](#_quality_metrics).
...
-   Channel data sampling: When data granularity is set to **By day** or **By minute**, click a data point and select **Channel data sampling** to view its scatter chart. The x coordinates are described in [Quality metrics](#_quality_metrics).

-   User data sampling: On the scatter chart, click a data point and select **User data sampling** to view the top six users who experience a high freeze rate. To view call quality details about a user, click **Call Inspector** to use the [Call Inspector](./call-inspector/call-search) function.
...
In the country and region bubble charts, the Y coordinate is the value of the selected metric, and the corresponding X coordinate is listed in [Quality metrics](#_quality_metrics). The bubble color represents whether the metric value is normal. The bubble turns red if one of the following occurs:
...
This section describes the key terms used in **Data Insights**. See [Agora Key Terms](../reference/glossary) for more key terms.
...
Each user in a channel is identified by a unique [user ID](../reference/glossary#user-iduid). There can never be two identical user IDs in a channel.
