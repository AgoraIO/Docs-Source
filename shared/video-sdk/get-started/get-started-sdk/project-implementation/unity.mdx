...
### Create and set up a script

This section shows you how to create a script required to implement <Vpd k="NAME" /> and bind the script to the canvas.
...
Add device permissions to your project according to your target platform.
...
Before calling other Agora APIs, create and initialize an `IRtcEngine` instance. Call `CreateAgoraRtcEngine` to create the instance. 
...
<ProductWrapper notAllowed="voice-calling">
Create an instance of the `UserEventHandler` class and set it as the engine event handler.Define the necessary callbacks based on your use-case. 
...
internal class UserEventHandler : IRtcEngineEventHandler
{
    ...
    // Triggered when the SDK receives and successfully decodes the first frame of remote video
    ...
    // Triggered when the remote user leaves the current channel
...
Call `JoinChannel` to join a channel. Configure the following in `ChannelMediaOptions`:
...
When the local user clicks **Leave**, call `LeaveChannel` to leave the current channel. 
...
<ProductWrapper product="voice-calling">
When a user clicks **Leave**, call `LeaveChannel` to leave the current channel and `DisableAudio` to turn off the audio module. 
...
1. To clean up all session-related resources when a user exits the <Vpl k="CLIENT" />, call the `Dispose` method of the `IRtcEngine`.
    ...
    After calling `Dispose`, you can no longer use any methods or callbacks of the SDK. To use <Vpd k="PRODUCT"/> features again, you must create a new engine instance. 
    </Admonition>
