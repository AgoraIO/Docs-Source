...
2.  **Implement the logic to enable the extension**

    To load extension in your <Vpl k= "CLIENT">, call `loadExtensionProvider` and pass the extension path. To enable the extension, call `enableExtension` and pass the provider name and extension name. In `preload.js`, add the following method before `window.onload = () =>`:
...
2. **Implement the logic to disable the extension**
    ...
    To disable the extension, call `enableExtension` and pass the `provider`, `extension`, and `enableExtension` variables to the method. In `preload.js`, add the following method before `window.onload = () =>`:
...
3. **Setup an event listener to start and stop the extension**

   When the user presses **Enable Extension**, your <Vpl k= "CLIENT"> loads and starts the extension by a call to the `enableExtension` method. When the user presses the button again, your <Vpl k= "CLIENT"> disables the extension by a call to the `disableExtension` method. To implement this workflow, in `preload.js`, add the following method before `document.getElementById("leave").onclick = async function ()`:
...
4. **Implement the required callbacks to manage the extension**
    ...
    <Vg k = "VSDK"/> provides the following callbacks to manage the extension:
