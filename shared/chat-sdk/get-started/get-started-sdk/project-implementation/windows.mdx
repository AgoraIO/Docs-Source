...
* A `Button` to **Join** or **Leave** Agora <Vg k="CHAT" />.
...
To add the UI framework to your <Vpl k="NAME"/> project, open the `<project-name>` > `MainWindow.xaml` from the Solution Explorer, and replace the file contents with the following:
...
When a user opens the <Vpl k="CLIENT"/>, you instantiate and initialize a `SDKClient`. When the user clicks the **Join** button, the <Vpl k="CLIENT"/> logs in to the <Vg k="CHAT_SERVER" />. When a user types a message in the text box and then presses **Send**, the typed message is sent to another registered user. When the <Vpl k="CLIENT"/> receives a message from the server, the message is displayed in the message list. This simple workflow enables you to rapidly build a <Vpd k="NAME" /> client with basic functionality. 
...
1. **Set up <Vpd k="NAME" /> when the <Vpl k="CLIENT"/> starts**
    ...
    When the <Vpl k="CLIENT"/> starts, you create an instance of `SDKClient` and set up callbacks to handle <Vpd k="NAME" /> events.
    ...
    public MainWindow()
    {
        ...
        SDKClient.Instance.ChatManager.AddChatManagerDelegate(this); // Add a chat delegate to handle chat callbacks

        Closed += CloseWindow; // Execute the CloseWindow method when the app closes
    }
    ```
1. **Instantiate the `SDKClient`**   

    To implement peer-to-peer messaging, you use <Vpd k="SDK" /> to initialize a `SDKClient` instance. In the `MainActivity` class, add the following method:
...
1. **Log in to the <Vg k="CHAT_SERVER" />** 
    ...
    When a user clicks **Join**, your <Vpl k="CLIENT"/> logs in to the <Vg k="CHAT_SERVER" />. When a user clicks **Leave**, the <Vpl k="CLIENT"/> logs out from the <Vg k="CHAT_SERVER" />. To implement this logic, add the following method after `showLog()` in the `MainWindow` class:
...
1. **Implement the `IChatManagerDelegate` Interface**

    To set up callbacks for handling <Vpd k="NAME" /> events, you implement the `IChatManagerDelegate` interface in the `MainWindow` class. Update the `MainWindow` class definition as follows:
...
1. **Receive messages**
    ...
    To read and display received chat messages, you add code to the `OnMessagesReceived` callback of the `IChatManagerDelegate`. To implement all the `IChatManagerDelegate` callbacks, add the following code to the `MainWindow` class: 
...
1.  **Remove the chat manager delegate when the window is closed**
    ...
    ```
1. **Implement the `IConnectionDelegate` Interface**

    1.  To set up callbacks for handling connection events such as connection, disconnection, and token expiration, you implement the `IConnectionDelegate` interface in the MainWindow class. Update the `MainWindow` class definition as follows:
    
    ```c#
    public partial class MainWindow : Window, IChatManagerDelegate, IConnectionDelegate
    ```
    2. Add below line after `SDKClient.Instance.ChatManager.AddChatManagerDelegate(this);`  to add AgoraChat connection delegate:

    ```c#
    SDKClient.Instance.AddConnectionDelegate(this);
    ```
    3. To cleanup the connection listener ,add below line in `CloseWindow` after `SDKClient.Instance.ChatManager.RemoveChatManagerDelegate(this);`
    ```c#
    SDKClient.Instance.DeleteConnectionDelegate(this);
    ```
    4. To implement all the `IConnectionDelegate` callbacks, add the following code to the `MainWindow` class:
    ```c#
