...
In real-world use-cases, disruptions affect a user's network connection. These disruptions cause corresponding changes in <Vg k="SDK" />'s connection state. To maintain a seamless user experience despite these changes, the SDK ensures continuity of channel subscriptions and the temporary user status. Upon reconnecting from a disconnected state, the SDK automatically restores the previous subscriptions and the temporary user status, eliminating the need for user intervention.
