...
description: >
  Technical architecture, performance specifications, and hardware details for ConvoAI Device Kit R1.
---

The ConvoAI Device Kit solution is built on Agora's Media Stream Acceleration (RTSA) and Conversational AI Engine. RTSA is Agora's RTC client product designed specifically for the IoT industry, providing the real-time communication foundation for voice AI interactions.

### Solution architecture

The ConvoAI Device Kit solution consists of two main components connected through Agora's <Vg k="BACKEND_NAME"/>.

![Solution Architecture](\images\convo-ai-device-kit\architecture.png)

- **Conversational AI Device Kit**

  The hardware device handles local processing including power management, Bluetooth pairing, voice wake-up, OTA updates, and peripheral driver support. It captures audio and video through dedicated modules, encodes the data, and transmits it via RTSA (Real-Time Streaming Acceleration) to Agora's SD-RTN. The device also receives and decodes audio/video responses for playback.
...
For specific supported chip models and compatibility details, [contact technical support](https://ticket.shengwang.cn).
