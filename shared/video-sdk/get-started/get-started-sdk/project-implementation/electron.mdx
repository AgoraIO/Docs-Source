...
1. tart the local video preview by calling `startPreview`.
...
To display the remote video, follow these steps:

1. Call `setupRemoteVideo` to configure the remote video feed.
1. Specify the video source, user ID, and container element.
1. Define the setup mode to determine how the video should be rendered.
...
1. To start <Vpd k="PRODUCT"/>, call these methods:
    <ProductWrapper notAllowed="voice-calling">
    ```js
...
1. When <Vpd k="PRODUCT"/> ends, you leave the channel and clean up resources:
...
A complete code sample demonstrating the basic process of real-time interaction is provided for your reference. Replace the entire contents of the `main.js` and `renderer.js` files with the following code samples: 
