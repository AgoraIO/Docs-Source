...
1. **Ensure required permissions**
     ...
     * *Android*
        ...
        Add the following code to the `App.tsx` file before `export default App;` to define a helper function for microphone and camera access on an Android device:
...
Add the following elements in the layout file of your project:

- A **Join** button
- A **Leave** button
- `RtcSurfaceView` elements for local and remote users

To return these UI elements, add the following code at the bottom of the `const App` block in `App.tsx`:
...
1. **Check user permissions and instantiate Agora Engine**
    ...
    * Register an event handler to receive <Vg k="ENGINE" /> callbacks
    ...
    const setupVideoSDKEngine = async () => {
        try {
        // use helper function to get permissions on Android
        await getPermission(); 
...
1. **Join a channel to start <Vpd k="NAME" />**
    ...
    When a local user presses **Join**, you call `join`. This method securely connects the local user to the named channel using the authentication token. It sets the client role and channel profile, and starts the local preview. In `App.tsx`, add the following code to the `const App` block:
...
1. **Leave the channel**
    ...
    Call `leaveChannel` to leave the current channel according to your scenario. For example, when the <Vpd k="NAME"/> ends, when you need to close the app, or when your app runs in the background. In `App.tsx`, add the following code to the `const App` block:
