...
In `agora_rtc_engine` 5.x, if the `uid` parameter returned in `onNetworkQuality` is 0, the callback reports the network quality of the local user. In `agora_rtc_engine` 6.x, the `uid` of the local user returned in this callback is the same as the local user's actual `uid` in the channel.
