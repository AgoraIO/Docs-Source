...
Starting from <Vg k="VSDK" /> v6.6.2, <Vg k="COMPANY" /> provides the <Link to="{{global.API_REF_FLUTTER_ROOT}}/class_agorapip.html">`AgoraPipController`</Link> and related configuration options for enabling and managing PiP on Android and iOS. This section describes how to implement PiP in your Flutter project.
...
<TabItem value="ios" label="iOS">
On iOS, call `pipStop()` to stop PiP and restore the app interface. If the page is about to be destroyed or you no longer need PiP, call `pipDispose()` and `dispose()` to release resources.
