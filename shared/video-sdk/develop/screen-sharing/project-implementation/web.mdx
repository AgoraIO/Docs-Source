...
The Web SDK supports <Vg k="SCR_SHR" /> on Chrome 58 and higher with or without using a <Vg k="SCR_SHR" /> plug-in.
...
1. Install the [<Vg k="COMPANY" /> Chrome <Vg k="SCR_SHR" /> plug-in](http://download.agora.io/sdk/release/chrome-extension.zip).
1. Get the `extensionId` of the plug-in.
