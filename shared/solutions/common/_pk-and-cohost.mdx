...
| :---------------- | :------------------- |
| Relay a live stream to one destination channel. | The Media Relay server pulls the live stream of UID 01 from Channel A, and pushes it to Channel B (assume UID as 01 in Channel B). UID 02 in Channel B, subscribes to UID 01's live stream. |
| Relay a live stream to multiple destination Channels. | The Media Relay server pulls the live stream of UID 01 from Channel A, and pushes it to Channels B, C, and D (assume UID as 01 in Channel B,C, and D). UID 02 in Channel B, UID 03 in Channel C, and UID 04 in Channel D subscribe to UID 01's live stream. |
...
- Device performance issues: Some audience members have devices with performance issues that cannot handle the encoding, decoding, or rendering of multiple high-definition audio and video streams simultaneously. Therefore, it is necessary to merge the streams in the cloud and then push them back to the channel to reduce the device performance requirements.

- Network issues: Some audience members have poor downstream networks. When subscribing to video streams from multiple hosts or a single high-definition video stream, they may experience stuttering or other network issues. By merging the streams in the cloud or converting them to lower resolution before delivery, the bitrate requirement is reduced, thus improving the viewer experience.
