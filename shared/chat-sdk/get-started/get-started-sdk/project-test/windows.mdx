...
1. Create an <Vpl k="CLIENT" /> instance for the second user:
   ...
   1. Register a second user in <Vg k="CONSOLE" /> and generate a user token.
    ...
    1. Build and launch a second instance of your <Vpl k="CLIENT" /> after updating `userId` and `token` with values for the second user. Make sure you use the same `appKey` as for the first user.

    1. Click the **Start** button in the second instance. 

1. On each instance, click **Join** to log in to <Vg k="COMPANY" /> <Vg k="CHAT" />. 

1. Edit the recipient name in each instance to show the user ID of the user logged in to the other instance.
