...
This page shows how to manage thread members by calling the Agora Chat RESTful APIs. Before calling the following methods, ensure that you understand the call frequency limit described in <Link to="/agora-chat/reference/limitations?platform={{platform.PATH}}#call-limit-of-server-sides">Limitations</Link>.
...
| Parameter      | Type           | Description |
| :------- |:-------------|:-------------|
| `result` | Bool | Whether the specified thread member is removed from the thread:<li>`true`: Yes.</li><li>`false`: No.</li>| 
