...
### Implement the user interface
 ...
 1. In `/app/res/layout/activity_main.xml`, add the following code before `</RelativeLayout>`:
    ...
    You see an error in your IDE. This is because this layout refers to methods that you create later.
...
1. **Leave all connections to a channel**
    ...
    To leave the channel on the second connection when a user taps **Leave**, add the following lines to `leaveChannel(View view)` after `int ret = agoraRtcService.leaveChannel(connectionId);`:
