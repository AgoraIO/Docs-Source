...
1. **Listen to <Vg k="ENGINE" /> events to receive state change notifications and quality statistics**
    ...
    2. **Configure the <Vpd k="SDK" /> log file**
        ...
        To customize the location, content and size of log files, in `App.tsx`, add the following code to `const setupVideoSDKEngine = async () => {` before `agoraEngine.initialize({})`:
