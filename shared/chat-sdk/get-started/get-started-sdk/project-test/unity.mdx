...
1. Create an <Vpl k="CLIENT" /> instance for the second user:
    ...
    1. In the `NewBehaviourScript` class, update `userId` and `token` with values for the second user. Make sure you use the same `appKey` as for the first user.

    1. Run the modified <Vpl k = "CLIENT"/> on a second device.
