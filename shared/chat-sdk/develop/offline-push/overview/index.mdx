...
<Admonition type="info" title="Note">When the app is running in the background or the phone is locked and the client has not disconnected from the Agora server, Chat will not receive offline push notifications. When multiple devices are logged in, if a device is force offline, even if the offline push is configured, the notifications will not be received.</Admonition>

<PlatformWrapper platform="android">
To use FCM offline push, configure the FCM push certificate information, such as **Private Key** and **Certificate Name** in the <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link>, and call the API provided by the client SDK to upload the device token to the Agora server.
...
To use FCM offline push, configure the FCM push certificate information, such as **Private Key** and **Certificate Name** in the <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link>, and call `updateFCMPushToken` provided by the client SDK to upload the device token to the Agora server.
</PlatformWrapper>

<PlatformWrapper platform="react-native">
To use FCM offline push, configure the FCM push certificate information, such as **Private Key** and **Certificate Name** in the <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link>, and call `updatePushConfig` provided by the client SDK to upload the device token to the Agora server.
...
<PlatformWrapper platform="ios">
To use APNs offline push, configure the push certificate information in the <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link> and call `bindDeviceToken` provided by the client SDK to upload the device token to the Agora server.

To test the offline push function, download our [demo](https://testflight.apple.com/join/MO0UuYJH).
</PlatformWrapper>

<PlatformWrapper platform="web">
To use third-party offline push, configure the push certificate information in the <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link>. For example, for FCM push configure the FCM push certificate information, such as **Private Key** and **Certificate Name** in the <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link>, and call `updatePushConfig` provided by the client SDK to upload the device token to the Agora server.
