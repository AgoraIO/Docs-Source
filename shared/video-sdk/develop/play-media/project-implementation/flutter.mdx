...
The `_mediaPLayerButton` widget displays a suitable caption on the button depending on the state of the media player. To define this widget, add the following code to the `_MyAppState` class:
...
**Declare the variables you need**
    ...
    To create and manage an instance of the `MediaPlayerController` and configure the UI elements, add the following declarations to the `_MyAppState` class:
    ...
    int _seekPos = 0; // Current position
    int _duration = 0; // Duration of the loaded media file
    ```
...
1.  **Load, play and pause media files**
...
1.  **Initialize the media player and manage callbacks**
    ...
    You create an instance of `MediaPlayerController` and register a `MediaPlayerSourceObserver` with the media player instance. When the player state changes, you take appropriate actions to update the UI in `onPlayerStateChanged`. You use the `onPositionChanged` callback to update the progress on the Slider. To create an instance named `_mediaPlayerController`, add the following code to the `_MyAppState` class:
...
1.  **Display media player output locally**
    ...
    Create a `AgoraVideoView` and set its `controller` to `_mediaPlayerController`  to show the media player output locally. To switch between displaying media player output and the camera stream, **replace** the `_localPreview()` method in the `_MyAppState` class with the following:
