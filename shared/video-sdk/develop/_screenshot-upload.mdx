...
* Take the following steps to enable and configure the <Vg k="COMPANY" /> video screenshot upload service:
    ...
    * In [<Vg k="CONSOLE" />](https://console.agora.io/), go to the **Project Management** tab and click **Configure** against the project for which you want to enable Video screenshot upload.
...
The video screenshot upload service is priced as follows:

#### Cloud-based option 

* Thee is no additional charge for this feature when bundled with [Cloud Recording](../../cloud-recording/overview/product-overview). ​
* [Pricing](../../cloud-recording/reference/pricing) is the same as Cloud Recording when used standalone.

#### Client-side option

The following prices apply when used as a feature under Video calling and Interactive live streaming products:​
...
*The fee will is waived for customers using the [Active Fence](../../extensions-marketplace/develop/integrate/activefence?platform=web) content moderation extension.​

*Additional charge for monthly peak QPS above 500 apply on a case-by-case basis.
