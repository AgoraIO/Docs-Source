...
An authentication token is a dynamic key that is valid for a maximum of 24 hours. On request, a token server returns an authentication token that is valid to join a specific channel or a set of wildcard channels.
...
* Deployed a token server using either of the following guides:
    ...
    - [Deploy a token server](/video-calling/token-authentication/deploy-token-server)
    - [Deploy the middleware server](/video-calling/token-authentication/middleware-token-server)
    </ProductWrapper>    
