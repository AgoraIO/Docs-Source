...
* **Managed mode**
    ...
    Managed mode replaces presets as the recommended way to use Agora-managed provider credentials. Set `credential_mode` to `"managed"` within the `asr`, `llm`, or `tts` block to use a supported provider without supplying your own API key. See [Use managed mode](/conversational-ai/develop/managed-mode).
...
* Changes to [**Start a conversational AI agent**](/conversational-ai/rest-api/agent/join)
    ...
    * **New parameters**
        ...
        - [`properties.asr.credential_mode`](/conversational-ai/rest-api/agent/join#properties-asr-credential-mode): Set to `"managed"` to use Agora-managed credentials for ASR. The API key is not required when using managed mode.
        - [`properties.llm.credential_mode`](/conversational-ai/rest-api/agent/join#properties-llm-credential-mode): Set to `"managed"` to use Agora-managed credentials for LLM. The API key is not required when using managed mode.
        - [`properties.tts.credential_mode`](/conversational-ai/rest-api/agent/join#properties-tts-credential-mode): Set to `"managed"` to use Agora-managed credentials for TTS. The API key is not required when using managed mode.
    ...
    * **Deprecated parameters**
        ...
        - [`preset`](/conversational-ai/rest-api/agent/join#preset): Deprecated. Use `credential_mode: "managed"` within the relevant provider block instead. The `preset` field remains valid for existing integrations.
