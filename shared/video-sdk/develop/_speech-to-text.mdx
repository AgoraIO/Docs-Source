...
<Vg k="COMPANY" /> provides a speech-to-text service which takes the audio content of a host's media stream and transcribes it into written words in real time. This page explains how to start and stop speech recognition in an app, then download and present the transcript.

## Understand the tech

To start speech to text conversion in real time, you send an `HTTP` request to the Agora server through your business server. The speech to text service supports two modes:

* Convert speech to text in real time and stream data to the RTC channel.
...
1. Call the `start` method within five minutes of receiving the `builderToken` to start the speech to text task.
1. Call the `query` method to check task status.
...
This section shows how to implement speech to text into your <Vpl k="CLIENT" />, step-by-step.
