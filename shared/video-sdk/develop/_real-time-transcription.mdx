...
This page shows you how to start and stop <Vg k="RTT" /> in your <Vpl k="CLIENT" />, through a business server and then display the text in your <Vpl k="CLIENT" />.
...
A business server (API Server) acts as a bridge between your client and the <Vg k="COMPANY"/> <Vg k="RTT" /> API Server. Implementing a business server for managing your <Vg k="RTT" /> tasks provides the following benefits:
...
Google Protocol buffers serve as an extensible and language-neutral mechanism for serializing transcription data. Protobuffer enables you to generate source code in multiple languages, based on a specified structure. For more information about Google protocol buffers, see [protobuf.dev](https://protobuf.dev/). <Vg k="COMPANY"/> provides the following Protobuffer template file `Audio2TextProtoBuffer.proto` for parsing <Vg k="RTT" /> data:
...
1. Edit the following properties in the `.proto` file to match your project:
...
2. Generate a Protobuffer class using shell script. 

    To generate code for your platform, you invoke the Protocol Compiler `protoc` on the `.proto` file. For details, see [Generating Your Classes](https://protobuf.dev/programming-guides/proto3/#generating)
