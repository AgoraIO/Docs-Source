...
1.  **Set up the audio frame observer**
    ...
    `IAudioFrameObserver` gives you access to each audio frame after it is captured or access to each audio frame before it is played back. To setup the `IAudioFrameObserver`, add the following code to the `JoinChannelAudio` class:
    ...
    internal class AudioFrameObserver : IAudioFrameObserver
    {
        ...
        internal AudioFrameObserver(JoinChannelAudio audioSample)
        {
...
4.  **Unregister the audio observer when you leave a channel**
    ...
    When you leave a channel, you unregister the frame observer by calling the unregister frame observer method. To do this, add the following line to the `Leave` method before `RtcEngine.LeaveChannel();`:
