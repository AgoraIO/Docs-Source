...
<Admonition type="caution">
`NewHeyGenAvatar` and `HeyGenAvatarOptions` are deprecated. Use [`NewLiveAvatarAvatar`](#newliveavataravatar) instead. `HeyGenAvatarOptions` is an alias of `LiveAvatarAvatarOptions` — the fields are identical — and the emitted `vendor` is `"heygen"`.
</Admonition>
