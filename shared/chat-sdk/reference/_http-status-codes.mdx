...
| Status code | Error code | Error message | Description | Fix |
| :---------- | :--------- | :------------ | :---------- | :--- |
| `404` | <code>organization_<wbr/>application_<wbr/>not_<wbr/>found</code> | "Could not find application for `{org_url}` from URI: `{app_url}`/token." | The specified organization or application does not exist. | Pass the correct `app_id` when calling the RESTful API. |
