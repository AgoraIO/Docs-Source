...
The Real-time Messaging RESTful API only supports HTTPS.
...
For basic HTTP authentication, you need to put `api_key:api_secret` in the `Authorization` field of the HTTP header. For more information on how to generate the `Authorization` filed, see [RESTful API authentication](/interactive-live-streaming/reference/restful-authentication).
...
For more information on how to generate the `x-agora-token` field and the `x-agora-uid` field, see [RESTful API authentication](/interactive-live-streaming/reference/restful-authentication).
...
> Note: <code>&lt;appid&gt;</code> is the [App ID](/video-calling/reference/manage-agora-account) used by your project. All the request URLs and request bodies are case-sensitive.
