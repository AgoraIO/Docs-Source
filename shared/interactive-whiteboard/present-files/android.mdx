...
The Agora Fastboard SDK provides the [`insertImage`]/en/whiteboard/fastboard_api_android?platform=Android#insertimage) method for inserting and displaying an image in the main window of the whiteboard. You can call this method and pass in the URL, width, and height of the image to display a specified image.

### Prerequisites

Before you start, make sure you meet the following requirements:

- Integrate the Agora Fastboard SDK in your project, and join the room. See [Join a Whiteboard Room](./join_whiteboard_room_android_fastboard?platform=Android).
...
The following steps show how to implement inserting an image in the [Join a Whiteboard Room](./join_whiteboard_room_android_fastboard?platform=Android) project:
...
<div class = "alert note">The Fastboard SDK does not provides a UI for <code>insertImage</code>. You need to implement the UI yourself.</div>

## Play audio and video

The Agora Fastboard SDK integrates the media player extension and provides the [`insertVideo`](/en/whiteboard/fastboard_api_android?platform=Android#insertvideo) method for playing audio and video files. You can simply call `insertVideo` and pass in the URL of the audio or video file and the title of the sub-window play the audio and video in a sub-window on the whiteboard.
...
The following steps show how to implement playing a video in the [Join a Whiteboard Room](./join_whiteboard_room_android_fastboard?platform=Android) project:
...
3. After joining the whiteboard room, call the `insertDocs` method, and pass the following information in the params parameter of the method:
    ...
    - `fileType`: String. The file type `.pdf` represents a static document; pptx represents dynamic document.
...
- Understand and enable the [file-conversion service](https://docs.agora.io/en/whiteboard/file_conversion_overview?platform=RESTful).

### Implementations

The following steps show how to implement presenting an animated PPTX document in the [Join a Whiteboard Room](./join_whiteboard_room_android_fastboard?platform=Android) project:
