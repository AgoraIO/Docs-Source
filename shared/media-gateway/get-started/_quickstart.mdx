...
channel. With <Vpd k="NAME" />, you can employ advanced transcoding capabilities on media streams for enhanced distribution. To enable and configure transcoding options, you use the [<Vpd k="NAME" /> RESTful API](https://documenter.getpostman.com/view/6319646/SVSLr9AM#6aed9690-285e-45f0-a329-c995adbd0956). When transcoding is enabled, you can set parameters such as resolution, frame rate, bitrate, and whether to enable video streams with high and low resolutions.

To push online media streams as live video source streams into <Vg k="COMPANY" /> channels, you need to obtain a server domain name and streaming key. Taking the OBS streaming software as an example, you configure the server's domain name and streaming key in the following way:
...
Create and publish the streaming by calling the following endpoint:
...
* The app ID of the <Vg k="COMPANY" /> project from [<Vg k="CONSOLE" />](<Vg k="AGORA_CONSOLE_URL" />).
* The app certificate corresponding to your app ID.
* A channel name (`channelName).
...
appcert = "" // Your App certificate from <Vg k="COMPANY" />
...
const rtcInfo = {
   ...
   U: uid,
   E: expiresAt,
   // If you are not sure whether to use the flow configuration template, keep the following commented
   // T: templateId,
};
