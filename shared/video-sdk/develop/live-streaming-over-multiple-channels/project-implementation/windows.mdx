...
To enable your users to start and stop relaying to another channel, add a `Button` to the user interface. To add a button, take the following steps:
    ...
    4. From **ToolBox**, drag **Button** and drop it onto the surface of the **Dialog Editor**

    5. To change the button label, in **Properties**, update the **Caption** field with `Start Channel Media Relay`.
...
3.  **Access the channel relay button**
    ...
    To access the channel relay button, in `AgoraImplementationDlg.cpp`, add the following to `OnInitDialog()` before `SetIcon(m_hIcon, TRUE);`:  
...
1.  **Start or stop channel media relay**
    ...
    else
    {
        ...
        m_rtcEngine->startChannelMediaRelay(mediaRelayConfiguration);
        mediaRelaying = true;
    }
    ```
...
In this example, you use a single `Button` to join and leave a second channel. To add the `Button`, take the following steps:
    ...
    5. To change the button label, in **Properties**, update the **Caption** field with `Join Second Channel`.
