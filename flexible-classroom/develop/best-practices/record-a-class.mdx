...
If you want the recording to start automatically, you can listen for the event of class starting on the server side, and call [Set the recording state](/flexible-classroom/restful-api/classroom-api#set-the-recording-state) to start automatic recording.
...
Whether you initiate the recording on the client or server, call [Set the recording state](/flexible-classroom/restful-api/classroom-api#set-the-recording-state). When calling this method, pay attention to the following parameters:
...
After starting the recording, the Flexible Classroom cloud service generates an event to indicate the [recording state change](/flexible-classroom/restful-api/classroom-api#the-recording-state-changes). You can get the recording state by calling [Query a specified event](/flexible-classroom/restful-api/classroom-api#query-a-specified-event) or [Get classroom events](/flexible-classroom/restful-api/classroom-api#get-classroom-events). Pay attention to the `reason` field in the recording state change event:
...
The clients also receive callbacks that indicate the recording state change in [room properties](/flexible-classroom/restful-api/classroom-api#the-recording-state-changes). You can further implement your own logic based on the recording state change.
...
2. At least 60 seconds later, call [Update the recording configurations](/flexible-classroom/restful-api/classroom-api#update-the-recording-configurations) and set the `onhold` parameter as `false` to start the recording and file slicing. The following sample code shows this logic:
...
In use-cases where the recorded content is a shared screen or whiteboard, if you have high requirements for video clarity, you can set the following parameters when calling [Set the recording state](/flexible-classroom/restful-api/classroom-api#set-the-recording-state):
