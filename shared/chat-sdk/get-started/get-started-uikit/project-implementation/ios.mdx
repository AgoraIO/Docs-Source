...
This section shows how to use Agora Chat Starter Kit to rapidly implement peer-to-peer messaging in your app.

### Initialize Starter Kit

Follow the steps to initialize the Starter Kit.
...
2. Initialize the Starter Kit by calling the `initWithAgoraChatOptions` method. In `SceneDelegate.m`, replace the `willConnectToSession` method with the following code:
