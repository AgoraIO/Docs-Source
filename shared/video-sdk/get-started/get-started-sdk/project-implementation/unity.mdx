...
1. **Create a new script to hold the system logic**
    ...
    1. In **Project**, open **Assets** > **Agora-<Vg k="VSDK" />-Plugin** > **Agora-Unity-<Vg k="VSDK" />-SDK** > **Code**.
...
3. **Manage Android permissions**
    ...
    1. Add the Unity Android libraries. In `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelVideo.cs`, add the following to the list of namespace declarations:
    ...
    2. Create a permissions list. Add the following lines before `Start`:
    ...
    3. Check permissions are granted. Add the following code after `Update`:
...
4. **Bind your script to the canvas**
    ...
    1. In `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code`, drag and drop `JoinChannelVideo.cs` to **Canvas** on the **Hierarchy**.
...
1. **Import the <Vg k="COMPANY"/> library**
    ...
    In `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelVideo.cs`, add the following library after `using UnityEngine.UI;`:
    ``` csharp
    using Agora.Rtc;
    ```
...
1. **View the remote user when they join a channel**
    ...
    When a remote user joins the channel, <Vg k="COMPANY"/> fires the `OnUserJoined` callback. To catch this callback and start remote video, add the following code after the `OnJoinChannelSuccess` callback:
    <ProductWrapper product="video-calling">
    ``` csharp
...
3. **Clean up the resources used by your <Vpl k="CLIENT" />**
    ...
    When a user closes the <Vpl k="CLIENT" />, use `OnApplicationQuit` to clean up the resources you created in `SetupVideoSDKEngine` method. To implement this logic, add the following code at the end of `JoinChannelVideo` class:
    ``` csharp
