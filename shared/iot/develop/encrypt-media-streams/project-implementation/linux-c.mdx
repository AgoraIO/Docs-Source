...
1.  **Add variables to hold the encryption mode, encryption key and salt**
    ...
    Open `<extracted-folder>/agora_rtsa_sdk/example/hello_rtsa/hello_rtsa.c` in your development environment and add the following declation in `main()` after `char params[512];`:
...
1. **Set encryption parameters**
    ...
    You specify the media stream encryption method and the related parameters in JSON format by calling
  `agora_rtc_set_params`. Depending on your choice of an encryption method, set any of the following parameters:

    - **SM4-128-ECB** encryption

        1. Set the encryption parameter:
        ...
        2. Set  the `encryptionMode` variables to match your choice of encryption. Paste the following code
        before `agora_rtc_join_channel` function in `main()`:
    ...
    -  **AES-128-GCM2** encryption
        ...
        1. Set the `encryptionMode` variables as **"AES-128-GCM2"** and paste below code before calling of
        `agora_rtc_join_channel` function in `main()`:
