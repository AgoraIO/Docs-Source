...
1. **Enable the user to test the network**
    ...
    In `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelVideo.cs`, add the following method to the `JoinChannelVideo` class:
...
1. **Implement best practice for app initiation**
    ...
    To implement these features, in `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelVideo.cs`, add the following code to `setupVideoSDKEngine` after `RtcEngine.SetLogFileSize(2048);`:
...
1. **Listen to <Vg k="ENGINE" /> events to receive state change notifications and quality statistics**
    ...
    In `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelVideo.cs`, add the following callback methods to the `UserEventHandler` class:
