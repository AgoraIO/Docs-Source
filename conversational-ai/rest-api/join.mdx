...
- Method: `POST`
- Endpoint: `https://api.agora.io/api/conversational-ai-agent/v1/projects/{appid}/join`
...
| ⇒⇒`language` | string | Default `en-US` | Currently <Vpd k="NAME" /> only supports English `en-US`.  |
...
print(response.text)
  ...
  <CodeItem language="js" name="Node.js">{`const url = 'https://api.agora.io/api/conversational-ai-agent/v1/projects/:appid/join';
const options = {
  method: 'post',
  body: JSON.stringify({
    name: 'unique_name',
