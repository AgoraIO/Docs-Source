...
1. Enable MLLM: Set `"enable": true` in the `mllm` object
...
Enabling MLLM automatically disables ASR, LLM, and TTS since the MLLM handles end-to-end voice processing directly. See [`mllm.turn_detection`](/conversational-ai/rest-api/agent/join#properties-mllm-turn-detection) for turn detection options available with MLLMs.
</Admonition>
...
- [Google Gemini Live (Vertex AI)](google-vertex-ai)
