...
- **Adaptive video publishing (Beta)**  
  ...
  Added support for sending multiple video streams with different resolutions from the same video source, including in multi-channel scenarios. Use `setSimulcastConfig` to configure up to four layers (one high-resolution stream and up to three lower-resolution streams) and control stream publishing with `SimulcastConfig` options such as resolution, bitrate, and whether to automatically disable multiple streams when uplink network or device performance is poor. Subscribers can select the desired stream by calling `setRemoteVideoStreamType [1/2]`. This feature is ideal for scenarios with multiple terminals and varying network conditions, such as conferences, large classes, and interactive live streaming, and significantly improves the viewing experience under poor network conditions.
...
- Fixed an issue where calling `seek` during playback of an online audio effect caused it to restart from the beginning.
- Fixed occasional echoes in media volume mode when a broadcaster published a microphone audio stream while playing an audio effect with `playEffect [2/2]` and a music file with `startAudioMixing [2/2]`.
- Fixed an issue where the media metadata observer callback was not triggered if `registerMediaMetadataObserver` was called after `setExternalVideoSource` (Android, iOS).
- Fixed occasional crashes (Android, iOS).
- Fixed an issue where in certain `joinChannelEx` and media relay scenarios, the callback occasionally reported `RELAY_STATE_FAILURE` and `RELAY_ERROR_SERVER_ERROR_RESPONSE`.
- Fixed occasional audio noise when using Bluetooth or wired headphones after setting the audio scenario to `AUDIO_SCENARIO_CHATROOM` (Android).
