...
|---------|-----------|
|`role`|User role. See [`ClientRole`](./apidatatype#clientrole) for details.|
...
Stops subscribing to the audio streams sent by all remote users in the channel. This method will automatically unsubscribe from the audio streams of users who join the channel later. You must call [`subscribe_audio`](#subscribeaudio) or [`subscribe_all_audio`](#subscribeallaudio) to resume subscribing to audio.
...
Sets the playback audio format. This method sets the format of the data for the [`OnPlaybackAudioFrameBeforeMixing`](./audioframeobserver#onplaybackaudioframebeforemixing) callback.
