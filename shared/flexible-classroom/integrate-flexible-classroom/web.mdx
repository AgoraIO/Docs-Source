...
- `agora-chat-widget`: The chat widget that the <Vg k="EASEMOB_SDK"/> implements.
- `agora-plugin-gallery`: This package contains the code for plugins in <Vg k="FC"/>, including the screen sharing plugin, Pop-up Quiz, Poll, and Countdown Timer.
- `agora-scenario-ui-kit`: This package contains all the UI function components.
- `agora-widget-gallery`: This package contains the chat widgets that the <Vg k="EASEMOB_SDK"/> and the Agora <Vg k="MESS_SDK" /> implement.
...
- If you want to customize the classroom UI based on the default UI of <Vg k="FC"/>, you need to integrate <Vg k="FC"/> by downloading the [source code](https://github.com/AgoraIO-Community/CloudClass-Desktop) on GitHub.
...
The sample code requires passing in `rtmToken`. You can refer to [Generate an <Vg k="MESS" /> token](../reference/manage-agora-account#generate-an-rtm-token) to learn what a <Vg k="MESS" /> token is, how to get a temporary <Vg k="MESS" /> token for testing purposes, and how to generate a <Vg k="MESS" /> token from the server.
...
<div class="alert info">Make sure you have <a href="../get-started">set up in the development environment</a>.</div>
...
2. To change the directory to `CloudClass-Desktop` and switch the branch, run the following commands. Remember to replace `{VERSION}` with a specific version number. For example, if you want to switch to branch v2.6.2, run the command `git checkout release/2.6.2`.
