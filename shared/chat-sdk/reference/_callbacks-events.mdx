...
When a user sends a read receipt for a message in a one-on-one chat, the <Vg k="COMPANY" /> chat server triggers a callback to your app server. 
...
When the message recipient sends a conversation read receipt in a one-to-one chat, the Agora Chat server sends a callback to your app server. The following is a sample code snippet:
