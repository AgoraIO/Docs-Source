...
From version 4.2.0 onwards, users have the ability to determine the number of groups they have joined by utilizing the `fetchJoinedGroupCount` method provided by the `ChatGroupManager`. This functionality enables users to retrieve this count directly from the server. The limit on the number of groups a user can join is determined by their subscribed instant messaging package. For more details, please refer to the [product pricing](/agora-chat/reference/pricing-plan-details#group).
