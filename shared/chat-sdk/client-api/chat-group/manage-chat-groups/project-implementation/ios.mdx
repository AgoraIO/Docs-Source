...
1. Call `getJoinedGroups` to retrieve the list of the groups that the user is already in from the local database.
...
1. Call `requestToJoinPublicGroup` to send a join request to the chat group:
