...
To implement <Vpd k="PRODUCT" /> logic, enable `nodeIntegration` and disable `contextIsolation` features. To set this settings, in `main.js`, add the following code after `preload: path.join(__dirname, 'preload.js')`:
...
1. Call <Link target="_blank" to="{{global.API_REF_ELECTRON_ROOT}}/class_irtcengine.html#api_createagorartcengine">createAgoraRtcEngine</Link> and create an instance of the <Vg k="ENGINE" />.
2. Initialize the created instance with your `appID`. Call <Link target="_blank" to="{{global.API_REF_ELECTRON_ROOT}}/class_irtcengine.html#api_create2">initialize</Link> and pass in your `appID`.
3. To receive callbacks during <Vpd k="PRODUCT" />, call <Link target="_blank" to="{{global.API_REF_ELECTRON_ROOT}}/class_irtcengine.html#api_addhandler">registerEventHandler</Link> and register the required callbacks.
<ProductWrapper product="video-calling">
4. In <Vg k= "VIDEO"/>, you use communcation profile which allows only two users in the channel. Call <Link target="_blank" to="{{global.API_REF_ELECTRON_ROOT}}/class_irtcengine.html#api_setchannelprofile">setChannelProfile</Link> and set the channel profile to `ChannelProfileCommunication`.
</ProductWrapper>
<ProductWrapper product="interactive-live-streaming">
4. For <Vg k= "ILS"/>, you use live streaming profile which allows multiple users in the channel. Call <Link target="_blank" to="{{global.API_REF_ELECTRON_ROOT}}/class_irtcengine.html#api_setchannelprofile">setChannelProfile</Link> and set the channel profile to `ChannelProfileLiveBroadcasting`.
</ProductWrapper>
5. To enable the local video capturer, call <Link target="_blank" to="{{global.API_REF_ELECTRON_ROOT}}/class_irtcengine.html#api_enablelocalvideo"> enableLocalVideo</Link>.
6. To setup local view, call <Link target="_blank" to="{{global.API_REF_ELECTRON_ROOT}}/class_irtcengine.html#api_setuplocalvideo"> setupLocalVideo</Link> and pass your local video conatiner.
7. To preview the local video, call <Link target="_blank" to="{{global.API_REF_ELECTRON_ROOT}}/class_irtcengine.html#api_startpreview2"> startPreview</Link>.
...
When the remote user joins the channel, this <Vpl k="CLIENT" />:
    1. Listens for the <Link to = "{{global.API_REF_ELECTRON_ROOT}}/class_irtcengineeventhandler.html#callback_onuserjoined">onUserJoined</Link> event which returns an instance of <Link target="_blank" to="{{global.API_REF_ELECTRON_ROOT}}/rtc_api_data_type.html#class_rtcconnection">RtcConnection</Link> and the remote user ID.
    2. Calls  the <Link to = "{{global.API_REF_ELECTRON_ROOT}}/class_irtcengine.html#api_setupremotevideo">setupRemoteVideo</Link> method and plays the remote video in the remote video container.
<ProductWrapper product="interactive-live-streaming">
When you select **Host**, this <Vpl k="CLIENT" />:
    1. Sets the user role as Host.
    2. Starts the local video preview.
    3. Unsubscribes to the remote video stream with a call to the <Link to = "{{global.API_REF_ELECTRON_ROOT}}/class_irtcengine.html#api_muteremotevideostream">muteRemoteVideoStream</Link> method.

When you select **Audience**, this <Vpl k="CLIENT"/>:
    
    1. Sets the user role as Audience.
    2. Stops the local video video preview.
    3. Subscribes to the remote video stream <Link to = "{{global.API_REF_ELECTRON_ROOT}}/class_irtcengine.html#api_muteremotevideostream">muteRemoteVideoStream</Link> method.
</ProductWrapper>
When you press **Leave**, this <Vpl k="CLIENT" />:
    1. Stops the local preview with a call to the <Link to = "{{global.API_REF_ELECTRON_ROOT}}/class_irtcengine.html#api_stoppreview2">stopPreview</Link> method.
...
<ProductWrapper product="video-calling">
    ...
    const token= "";
    // A variable to hold the ID of the remote user.
    let remoteUID;
    ...
    window.onload = () => 
    {
        ...
        // Programmatically access the local video conatiner.
        ...
        // Programmatically access the remote video conatiner.
...
<ProductWrapper product="interactive-live-streaming">
   ...
   // Pass your App ID here.
    ...
    window.onload = () => 
    {
        ...
        // Listen to the host radio butto nclick event.
