...
You can get push notification settings for up to 20 conversations in each call. If a session uses app settings or its push notification settings have expired, the returned dictionary does not contain that conversation. Call  `getSilentModeForConversations` to get the push notification settings for multiple conversations, as shown in the following  example:
