...
1.  In <Vg k="CONSOLE" />, subscribe to a [pricing plan](https://console.agora.io/aa/pricing).
...
3. Press **Apply**. This initiates Datadog enablement.
    ...
    [<Vg k="COMPANY" /> support](support@agora.io) reach out to you once the feature is enabled. You can only use Datadog in <Vg k="AA" /> when the Datadog Integration is enabled in your Datadog package.

4. Update the Datadog configuration:

    ![](/images/analytics/datadog-configuration.png)

    **Note**: It can take up to three hours before data push is resumed or stopped after you change status.
