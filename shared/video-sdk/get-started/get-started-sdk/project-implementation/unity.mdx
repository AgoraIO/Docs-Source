...
1. **Create a new script and import the required Unity libraray**
    ...
    1. In `NewBehaviourScript.cs`, add the following code after `using UnityEngine;`:
...
3. **Manage Android permissions**
    ...
    2. Create a permissions list. In your script file, add the following lines before the `Start` method:
    ...
    3. Check permissions are granted. In your script file, add the following code after the `Update` method:
...
1. **Setup <Vg k="ENGINE"/>**
    ...
    To call a method to initialize <Vg k="ENGINE"/> when the <Vpl k="CLIENT" /> is launched, in your script file, add the following after the `CheckPermissions` method:
    <ProductWrapper product="video-calling">
    ``` csharp
...
1. **Handle and respond to <Vg k="ENGINE"/> events**
    ...
    To register the callbacks, in your script file, add the following at the end of `NewBehaviourScript` class:
    ``` csharp
...
1. **Reference the UI elements from SampleScene**
    ...
    In your script file, add the reference code before the `SetupVideoSDKEngine` method:
    <ProductWrapper product="video-calling">
    ``` csharp
...
1. **Join a channel to start <Vpd k="PRODUCT" />**
    ...
    When the local user clicks the `Join` button, you call `Join()`. This method securely connects the local user to a channel using the authentication token. In your script file, add the following lines after the `InitEventHandler` method:
    <ProductWrapper product="video-calling">
    ``` csharp
    public void Join()
    {
...
1. **View the remote user when they join a channel**
    ...
    When a remote user joins the channel, <Vg k="COMPANY"/> fires the `OnUserJoined` callback. To catch this callback and start remote video, in your script file, add the following code after the `OnJoinChannelSuccess` callback:
    <ProductWrapper product="video-calling">
    ``` csharp
...
8. **Manage the user role**
    ...
    When the user role switches, <Vg k="COMPANY"/> fires the `OnClientRoleChanged` callback. To catch this callback and enable or disable `LocalView`, `RemoteView` accordingly, in your script file, add the following code after the `OnUserJoined` callback:
        ``` csharp
    ...
    When toggles are switched on, to switch client role and turn off the toggles, in your script file, add the following methods to the `NewBehaviourScript` class:
...
8. **Stop remote video when a remote user leaves the channel**
    ...
    When a remote user leaves the channel or drops offline, <Vg k="COMPANY"/>  fires the `OnUserOffline` callback. To catch this callback and stop remote video, in your script file, add the following callback method to `UserEventHandler` class:
    ``` csharp
...
1. **Leave the channel when a user ends the call**
    ...
    When a user clicks **Leave**, you call `Leave()` to exit the channel. In your script file, add the following to the `NewBehaviourScript` class:
    ``` csharp
    public void Leave()
    {
        // Leaves the channel.
...
1. **Check that the <Vpl k="CLIENT" /> has the correct permissions to start**
    ...
    For Android, call `CheckPermissions` and check that the permissions are granted. In your script file, replace the `Update` method with the following:
    ``` csharp
    void Update()
    {
        CheckPermissions();
...
2. **Start <Vpd k="PRODUCT" />**
    ...
    In your script file, replace the `Start` method with the following:
    ``` csharp
    void Start()
    {
        SetupVideoSDKEngine();
...
3. **Clean up the resources used by your <Vpl k="CLIENT" />**
    ...
    When a user closes the <Vpl k="CLIENT" />, use `OnApplicationQuit` to clean up the resources you created in `SetupVideoSDKEngine` method. To implement this logic, in your script file, add the following at the end of `NewBehaviourScript` class:
    ``` csharp
