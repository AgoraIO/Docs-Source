...
- `encryptKdfSalt`: (Optional) String. The salt with the length of 32 bytes. Agora recommends using OpenSSL to generate the salt on your server. For details, see [Secure channel encryption](/interactive-live-streaming/advanced-features/media-stream-encryption).
