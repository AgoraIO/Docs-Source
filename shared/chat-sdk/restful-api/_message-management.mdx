...
| `thumbnail` | Bool | Whether to download the thumnail of the image or video file.<ul><li>`true`: Yes.</li><li>`false`: No.</li></ul> | No. |
...
- The default storage time of historical messages differs by plan version. For details, see [package details](/agora-chat/reference/pricing-plan-details) for details.
