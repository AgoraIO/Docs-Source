...
1. Log in to [<Vg k="CONSOLE" />](https://console.agora.io/projects). On the **Project Management** tab, locate the project for which you want to enable <Vg k="NCS" /> and click **Config**.
...
1. In **Notification Center Service Configuration**, fill in the following information:
    ...
    * **Allowed IP List**: If your server is behind a firewall, check the box here, and ensure that you call the [IP address query API](#ip-address-query-api) to get the IP addresses of the Agora <Vg k="NCS" /> server and add them to the firewall's allowed IP list.
...
1. Press **Save**. Agora performs a health test for your configuration as follows:
    ...
    1. When the <Vg k="NCS" /> health test succeeds, read the prompt and press **Save <Vg k="NCS" /> Configuration**. After your configuration is approved, the Status of Notification Center Service shows **Enabled**.
        ...
        If the <Vg k="NCS" /> health test fails, follow the prompt on the <Vg k="CONSOLE" /> to troubleshoot the error. Common errors include the following:
