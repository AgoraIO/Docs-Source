...
Follow the implementation steps to understand the core API calls used in the sample code or use the snippets in your own code.
...
When a user no longer needs to use <Vg k="SIG" />, call `logout`. Logging out means closing the connection between the client and <Vg k="SIG" />. The use is automatically logged out or unsubscribed from all Message and Stream channels. Other users in the channel receive an `onPresenceEvent` notifications of the user leaving the channel.
