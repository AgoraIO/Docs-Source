...
1. <Link to="{{Global.AGORA_CONSOLE_URL}}/marketplace/extension/introduce?serviceName=faceunity-ar-en">Download</Link> the <Vg k="RTEE_FU" /> AR Filter package for your platform from the Extensions Marketplace.
1. Download the [FaceUnity resource package](https://download.agora.io/marketplace/release/FaceUnity_v8.4.1_Resources.zip).
1. [Contact <Vg k="COMPANY" />](support@agora.io) for activation and provide your package name, for example, `yourCompany.yourProject.faceunity`. You then receive the `authpack` certificate file that corresponds to your package name for subsequent integrations.
