...
Successfully [Initiate document conversion task](whiteboard-api/file-conversion) and call [Query document conversion task progress API](whiteboard-api/file-conversion#query-the-progress-of-a-file-conversion-task) After obtaining the document conversion result, you can call this method and pass in the obtained document conversion result. The SDK will automatically create a sub-window, insert and display the conversion in pages. subsequent documentation.
...
Static documents refer to files converted from PPT, PPTX, DOC, DOCX, and PDF formats through [Angora.io/cn/whiteboard/file_conversion_overview?platform=RESTful] Still images in PNG, JPG/JPEG format.

Successfully [initiate document conversion task](whiteboard-api/file-conversion) and call [Query document conversion task progress API](whiteboard-api/file-conversion#query-the-progress-of-a-file-conversion-task). After obtaining the document conversion result, you can call this method and pass in the obtained document conversion result, and the SDK will automatically create a child window, insert and display the converted document in pages.

**Parameters**

- `pages`: Converted file list. For details, see [Query Document Conversion Task Progress API](whiteboard-api/file-conversion#query-the-progress-of-a-file-conversion-task) The `convertedFileList` parameter in the response package body returned after the request is successful value.
- `title`: window title.
- `completionHandler`: method call result callback. Passing in `nil` means not to register a callback.
