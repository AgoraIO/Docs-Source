...
<Vg k="MESS_SDK"/> uses `RtmListener` to process messages and event notifications. Each message and event notification has a corresponding event handler, where you can implement your own processing logic:

<Vg k="SIG"/> uses `RtmListener` to process messages and event notifications. Each message and event notification has a corresponding event handler, where you implement your own processing logic. Refer to the following code to create and use an instance of `RtmEventListener`:
