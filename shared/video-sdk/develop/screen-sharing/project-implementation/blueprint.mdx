...
This subsection describes how to start and stop screen capture on Android and iOS mobile platforms.

#### Create UMG

Create two Button controls named `Btn_StartScreenCapture` and `Btn_StopScreenCapture`. Users can **start** or **stop** <Vg k="SCR_SHR" /> with these two buttons as shown in the figure below:

![Create UMG](/images/video-sdk/screen-sharing-blueprint-android-ios-mobile-umg.png)

#### Join a channel and publish the screen share stream

Refer to the following steps to join a channel and publish a screen share stream:

    1. In **Bind UIEvent**, bind the UI event of clicking the **Start Screen Share** button. Create a **Bind Event to On Clicked** node that connects the `Btn_StartScreenShare` button control and the `OnStartScreenShareClicked` callback function respectively. This callback is triggered when the user clicks the **Start Screen Share** button as shown in the following figure:

        ![Bind UI Start](/images/video-sdk/screen-sharing-blueprint-android-ios-bind-ui-start.png)

    1. Create and implement the `OnStartScreenShareClicked` callback. When a button click triggers this callback, the following flow is executed:
...
Refer to the following steps to create a UMG:
    ...
    1. Create two **Button** controls named `Btn_StartScreenCapture` and `Btn_StopScreenCapture`, with which the user can start or stop <Vg k="SCR_SHR" />. As shown in the following figure:

        ![Windows macOS UMG](/images/video-sdk/screen-sharing-blueprint-windows-macos-umg.png)

#### Add the basic process

In **EventGraph**, based on the basic flow of real-time audio/video interaction, add and connect the event nodes required for <Vg k="SCR_SHR" /> to increase the logic and interaction of <Vg k="SCR_SHR" />. This is shown in the following diagram:
...
| Node | Description |
|:---|:---------------|
| Get Screen Display Id | After initializing the <Vg k="ENGINE" />, get information about the screens and windows that are currently shareable, which is used to set the options and indexes in the drop-down lists. |
| Init Default Screen Capture Config | Prepare the parameter configuration for <Vg k="SCR_SHR" /> and save the configuration for use in subsequent screen captures. |
...
1. In **Get Screen Display Id**, refer to the following steps to get the currently available screen and window resources and add them to the options in the drop-down list:
    ...
    1. Call the **Get Screen Capture Sources** method to return a list of shareable screens and windows, and call **Get Count** to get the number of shareable screens and windows. This is shown in the following figure:

        ![Get Screen Capture Sources](/images/video-sdk/screen-sharing-blueprint-windows-macos-get-screen-capture-sources.png)

    1. Traverse the list, call **Get Source Info** to get information about the specified screen or window, and add the screen or window's name (**Source Name**) to each of the options in the drop-down list, so that users can specify which screens or windows to share before they start sharing. When you are done, call **Release** to release the list of shareable screens and windows. This is shown in the following figure:
...
In **Init Default Screen Capture Config**, configure <Vg k="SCR_SHR" /> related parameters such as **Dimensions**, **Frame Rate**, **Bitrate**, etc. You can make advanced configurations as follows:

    - **Screen Stroke**: Check **Enable High Light** and set **High Light Width** and **High Light Color** to specify the stroke width and color. The SDK will stroke the shared screen or window according to your settings.
    - **Shield Window**: Set **Exclude Window List** to the list of windows you want to shield. When you subsequently call **Start Screen Capture by Display Id**, the specified window will be shielded through this parameter.

Save this configuration for subsequent use when starting screen capture. This is shown in the figure below:

![Save Screen Share Config](/images/video-sdk/screen-sharing-blueprint-windows-macos-save-config.png)

#### Join a channel and publish a screen share stream
...
1. Create and implement the `OnStartScreenShareClicked` callback. When the callback is triggered by clicking the button, the following process is executed:
    1. Call the **Stop Screen Capture** method to pause the ongoing <Vg k="SCR_SHR" /> process.
    1. Based on the list of available screens and windows saved in the previous steps, find the element that corresponds to the option and determine whether the option is a `ScreenCaptureSourceType_Screen` (screen) or a `ScreenCaptureSourceType_Window` (window) by using the **Type** (type of shared target) in the element.
    ...
    1. If the shared target is a screen, call **Start Screen Capture by Display Id** to pass in the screen ID and the parameter configuration saved in the previous steps to start capturing the video stream of the specified screen. If the shared target is a window, call **Start Screen Capture by Window Id** to pass in the window ID and the parameter configuration to start capturing the video stream of the specified window.  To share a specific region of a screen or window, you can set the **Region Rect** parameter to the position of the region you want to share relative to the entire screen or window.
    1. Call **Join Channel** to join the channel and configure the following parameters in **Make ChannelMediaOptions** to publish the captured screen share stream:
        ...
        - Check **Channel Profile Set Value** and set **Channel Profile** to **CHANNEL_PROFILE_LIVE_BROADCASTING** to set the channel scene as a live broadcast scene.
