...
To create a simple interface for testing speech-to-text functionality, you add the following buttons to the UI. To do this:
...
To start, stop and query a text-to-speech task in your <Vpl k="CLIENT" />, do the following.
...
1. **Query task status**
    ...
    To notify the user of any change to the speech-to=test task you [Query the task status](#query-the-status-of-speech-to-text) .
