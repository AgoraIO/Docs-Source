...
Starting from v4.5.0, both Video SDK and Signaling SDK (v2.2.0 and above) include the `libaosl.so` library. If you manually integrate Video SDK via CDN and also use the Signaling SDK, delete the earlier version of the `libaosl.so` library to avoid conflicts. The `libaosl.so` library version in Video SDK v4.5.1 is 1.2.13.
</Admonition>
...
**12. Shim feature for video stream replacement**

This version introduces the shim feature, that enables you to replace the currently published video stream with a local PNG image. You can enable this feature by calling  `enableVideoImageSource` and specifying a custom image. When the shim feature is disabled, remote users resume viewing the live video stream.
