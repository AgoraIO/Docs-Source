...
In the `CONNECTION_STATE_RECONNECTING` state, the SDK keeps reconnecting to <Vg k="SIG" />. If the token expires, the SDK returns the `onTokenExpired` callback, which does not affect the connection state.
...
-   <Link to="{{Global.SIGNALING_API_REF_LINUX_JAVA_ROOT}}/interfaceio_1_1agora_1_1rtm_1_1_rtm_status_code_1_1_connection_change_reason.html">ConnectionChangeReason</Link>
