...
1. **Receive and display the text stream**
    ...
    When you set one of the `destinations` in the starting configuration as `AgoraRTCDataStream`, the speech-to-text conversion output stream is pushed to your RTC channel and you receive it via `onStreamMessage`. To receive this stream and display the text in your <Vpl k="CLIENT" />, in `preload.js`, add the following code after `const EventHandles = {`:
