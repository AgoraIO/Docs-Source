...
The following figure shows the overall product architecture of <Vpd k="NAME" />:
...
| Custom transcoding parameters              | Supports setting whether to enable transcoding for media streams input into the channel. If you enable transcoding, you can set parameters such as resolution, frame rate, bitrate, and whether to enable large and small streams. |
| Restricted access area                     | When activating <Vpd k="NAME" />, you need to specify the access area to ensure the transmission quality of the media stream. |
...
In ultra-high-definition scenarios, the main signal is generally processed by the content provider and then pushed to Agora through the RTMP protocol (applicable to low bitrates) or the SRT protocol (applicable to high bit rates). <Vpd k="NAME" /> allows source streams to enter SD-RTN™ with ultra-low time consumption, achieving low latency while ensuring high stability. Therefore, <Vpd k="NAME" /> can be widely used in the following scenarios:
