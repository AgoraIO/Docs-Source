...
1. **Add the required library to acces the UI elements**
    ...
    In `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelAudio.cs`, add the following after the last import statement:
...
1. **Define variables to manage audio effects and access the UI elements**
    ...
    In `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelAudio.cs, add the following declarations to the `JoinChannelAudio` class:
...
1. **Set up access to the UI elements**
    ...
    In `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelAudio.cs`, add the following at the end of `SetupUI();`
...
1. **Enable the user to start and stop audio mixing**
    ...
    Add the following method to the `JoinChannelAudio` class:
...
1. **Pre-load sound effects**
    ...
    To set up playing voice effects, pre-load the sound effects. To add this, in `JoinChannelAudio`, add the following lines at the end of the `setupVoiceSDKEngine` method:
...
1. **Play, pause, or resume playing the sound effect**
    ...
    When a user presses the button, the sound effect is played. When they press the button again, the effect is paused and resumed alternately. To implement this workflow, add the following method to the `JoinChannelAudio` class:
...
1. **Update the UI after the effect finishes playing**
    ...
    When <Vg k="SDK" /> has finished playing the sound effect, the `OnAudioEffectFinished` event is fired. To handle this event, in `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelAudio.cs`, add the following to the `UserEventHandler` class:
    ...
    public override void OnAudioEffectFinished(int soundId) 
    {
        ...
        _audioSample.RtcEngine.StopEffect(soundId);
        _audioSample.soundEffectStatus = 0; // Stopped
        _audioSample.playEffectButton.text = "Play audio effect";
    }
    ```
1. **Set an audio profile**

    For applications where special audio performance is required, you should set a suitable audio profile and audio scenario. In `JoinChannelAudio` class, add the following code at the end of `setupVoiceSDKEngine` method:
...
1. **Apply various voice and audio effects**
    ...
    When a user presses the button, apply a new voice effect and change the text on the button to describe the effect. To implement this workflow, add the following method to the `JoinChannelAudio` class:
...
1. **Set the audio route**
    ...
    When the user toggles the switch, enable or disable the speakerphone. To implement this workflow, add the following method to the `JoinChannelAudio` class:
