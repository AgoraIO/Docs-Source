...
<Admonition type="info">If `Options.IsAutoDownload` is set as `true` on the recipient's client, the SDK automatically downloads the thumbnail after receiving the message. If not, you need to call `SDKClient.Instance.ChatManager.DownloadThumbnail` to download the thumbnail and get the path from the `ThumbnailLocalPath` member in `msg.Body`.</Admonition>
...
To send a GIF image message, set the `isGif` property of the `ImageBody` to `true`:

```csharp
Message msg = Message.CreateImageSendMessage(toChatUsername, localPath);
// Set the isGif property of the image body to true.
...
1. Call `ModifyMessage` with the message ID, the new message body, and the new extension attributes:
    ...
    // The body and attributes cannot both be null.
    ...
    // File, video, voice, image, location, or combined message: you can modify the extension attributes only. Pass null as the message body.
