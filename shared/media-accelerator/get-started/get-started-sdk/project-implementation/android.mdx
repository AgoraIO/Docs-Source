...
1.  **Declare the variables that you use to create and join a channel**
    ...
    In `/app/java/com.example.<projectname>/MainActivity`, add the following lines to `MainActivity` class:
        ...
        private  String deviceId = "<Your IoT device Id>";
...
1.  **Setup <Vg k="COMPANY" /> RTC Service**
    ...
    To implement <Vpd k="PRODUCT" />, you use <Vpd k="SDK" /> to create a RTC service instance. In `/app/java/com.example.<projectname>/MainActivity`, add the following code before `onCreate`.

    ``` java
    private void setupAgoraRtcService(){
        agoraRtcService= new AgoraRtcService();

        // This method is used for the Agora License verification mechanism. 
...
1.  **Handle and respond to service events**
    ...
    In `/app/java/com.example.<projectname>/MainActivity`, add the following lines after `setupAgoraRtcService`:
        ...
        private final AgoraRtcEvents agoraRtcEvents = new AgoraRtcEvents() {
            ...
            public void onVideoData(int connId, int uid, int sent_ts, byte[] data, AgoraRtcService.VideoFrameInfo info) {
                // Receive the video frame callback of the remote user in the channel.
            ...
            public void onTargetBitrateChanged(int connId, int targetBps) {
                ...
                // Use this callback to remind the app to update the encoder bit rate.
            }

            @Override
            public void onTokenPrivilegeWillExpire(int connId, String token) {
                // Authentication token will expire in 30 seconds
            }
...
1.  **Join a channel**
    ...
    When the local user initiates a connection, call `joinChannel`. This method securely connects the local user to a channel using the authentication token. In `/app/java/com.example.<projectname>/MainActivity`, add the following code after `setupLocalVideo`:
        ...
        public void joinChannel(View view) {
            ...
            // Set parameters to join a channel
            AgoraRtcService.ChannelOptions channelOptions;
            channelOptions = new AgoraRtcService.ChannelOptions();
...
1. **Send and receive audio and video data**
    ...
    When sending audio and video data, you need to set the sending interval such that it is consistent with the video frame rate and audio frame length. 
    
    Use the following code to create and push a video frame:

        ```java
            byte[] videoBuffer = new byte[frameSize];
            // Add code here to get video frame data
            ...
            int ret = agoraRtcService.sendVideoData(connectionId, videoBuffer, videoFrameInfo);
        ```
        
    Use the following code to create and push an audio frame:

        ```java
            byte[] audioBuffer = new byte[frameSize];
            // Add code here to get audio frame data
            ...
            int ret = agoraRtcService.sendAudioData(connectionId, audioBuffer, audioFrameInfo);
        ```
...
2.  **Clean up the resources used by your app**
    ...
    protected void onDestroy() {
        ...
        int ret = agoraRtcService.leaveChannel(connectionId);
        if (ret != AgoraRtcService.ErrorCode.ERR_OKAY) {
            showMessage("Failed to leave channel");
        }
