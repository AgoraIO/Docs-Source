...
1. **Specify the audio codec, sampling rate, and the number of channels**
    ...
    You set the audio codec, sampling rate, and the number of channels in `channel_options` that you pass to `agora_rtc_join_channel`. To set these parameters, modify the following lines in `main()` according to your requirements:
...
1. **Manage audio and video streaming status**
    ...
    To pause or resume sending local audio and video streams. To do this, add the following code to the `onCreate` method in the `MainActivity` class:
    ...
    To pause and resume playing remote audio or video streams call `agora_rtc_mute_local_audio (connection_id_t conn_id, bool mute);`  `agora_rtc_mute_local_video (connection_id_t conn_id, bool mute);`.
