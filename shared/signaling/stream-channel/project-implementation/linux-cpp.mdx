...
To use the stream channel functionality, call `createStreamChannel` to create a `IStreamChannel` object instance.
...
Call the `join` method on the `IStreamChannel` instance with the appropriate options as follows:
...
When joining a channel, fill the `token` parameter in `JoinChannelOptions` with a temporary token from <Vg k="CONSOLE" />. When you call this method, the SDK triggers the `onJoinResult` callback and returns the call result.
