...
The Agora UI Kit provides the [`insertImg`]/en/whiteboard/fastboard_api_ios?platform=iOS#insertimg) method for inserting and displaying an image in the main window of the whiteboard. You can call this method and pass in the URL, width, and height of the image to display a specified image.

### Prerequisites

Before you start, make sure you meet the following requirements:

- Integrate the Agora UI Kit in your project, and join the room. See [Join a Whiteboard Room](./join_whiteboard_room_ios_fastboard?platform=iOS).
...
The following steps show how to implement playing a video in the [Join a Whiteboard Room](./join_whiteboard_room_ios_fastboard?platform=iOS) project:
...
The Agora UI Kit integrates the docs viewer and slide extensions and provides the [`insertStaticDocument`](/en/whiteboard/fastboard_api_ios?platform=iOS#insertstaticdocument) and [`insertPptx`](/en/whiteboard/fastboard_api_ios?platform=iOS#insertpptx) methods for presenting documents. You can simply call `insertStaticDocument` or `insertPptx` and pass in the information on the converted files to present a document in a paginated format in a sub-window on the whiteboard. The implementation workflow is as follows:

1. Upload the source document to a third-party cloud storage service, and call the Start file conversion API to create a file conversion task. When the call succeeds, the Agora server returns uuid in the response body, which is the unique identifier (UUID) of the file conversion task.

2. Call the [Generate a task token (POST) API](https://docs.agora.io/en/whiteboard/generate_whiteboard_token?platform=RESTful#generate-a-task-token-post), and pass the uuid in the request path to generate a task token for the file conversion task.

3. Call the [Query file conversion progress API](/interactive-whiteboard/reference/whiteboard-api/file-conversion#query-the-progress-of-a-file-conversion-task), and pass in the UUID and token of the conversion task to get the information on the converted files, that is, the value of the `convertedFileList` parameter in the response body.
...
The following steps show how to implement presenting an animated PPTX document in the [Join a Whiteboard Room](./join_whiteboard_room_ios_fastboard?platform=iOS) project:

1. Successfully launch a dynamic-document conversion task, and call the [Query file conversion progress API](/interactive-whiteboard/reference/whiteboard-api/file-conversion#query-the-progress-of-a-file-conversion-task) to get the list of converted files.
