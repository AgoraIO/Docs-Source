...
**Attention:** Starting from v4.4.0, the SDK provides an API sunset notice, which includes information about deprecated and removed APIs in each version. See <Link to="{{Global.API_REF_CPP_ROOT}}/rtc_api_sunset.html">API Sunset Notice</Link>.
...
1. Before v4.4.0, when a device was plugged in or in an idle state, the SDK only reported `MEDIA_DEVICE_STATE_IDLE` (0). As of v4.4.0, a new status code `MEDIA_DEVICE_STATE_PLUGGED_IN` (3) is added to report that a device is plugged in.
