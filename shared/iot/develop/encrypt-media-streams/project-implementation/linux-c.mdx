...
1.  **Add variables to hold the encryption mode, encryption key, and salt**
    ...
    Open `<extracted-folder>/agora_rtsa_sdk/example/hello_rtsa/hello_rtsa.c` in your development environment and add the following declarations to `main()` after `char params[512];`:
