...
Starting from v2.2.1, Signaling `2.x` supports User Channels for peer-to-peer messaging. Use the `publish()` method with `channelType` set to `RtmChannelType.USER` to send messages to a specific user, and handle incoming messages in the `onMessageEvent` callback. For more information, see [User channels](/signaling/core-functionality/user-channel).
