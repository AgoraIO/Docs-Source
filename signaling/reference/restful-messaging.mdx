...
The Messaging REST API enables you to send messages to a <Vg k="MESS" /> user or a channel.
...
You can send peer-to-peer or channel messages with any user ID when using basic HTTP authentication. When you use token authentication, you nay send peer-to-peer or channel messages with only the user ID you used to generate the Signaling token. For more information about how to generate a Signaling token, see [Secure authentication with tokens](/signaling/get-started/authentication-workflow).
