...
1. **Add variables for your connection to the token server**
    ...
    Declare the variables you need to specify the user id, token-server URL and the token expire time. Add the following declarations to `AgoraImplementation.cs` before `internal IRtcEngine RtcEngine;`:
...
1. **Handle the event triggered by <Vg k="AGORA_BACKEND" /> when the token is about to expire**
    ...
    ii. In `SetupVoiceSDKEngine`, add the following code before `private void InitEventHandler()`:
...
1. **Update the `Join` method to fetch a token**
    ...
    Use `FetchToken` to retrieve a fresh token from server before joining a channel. Add the following code in the top of `Join()` method:
