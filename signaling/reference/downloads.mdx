...
1. Download the latest [RTM Linux Java SDK](https://docs.agora.io/cn/AgoraPlatform/downloads) and decompress the file. Copy the `*.so` file and the `*.jar` file to the `lib` folder.
...
1. Download the latest version of [Real time Messaging SDK (Linux C++)](https://download.agora.io/rtmsdk/release/Agora_RTM_SDK_for_Linux_v1_5_0.zip) and extract the files. Copy the `*.so` file in the SDK folder into your `lib` folder, and copy the `*.h` file in the SDK folder into your `include` folder.
