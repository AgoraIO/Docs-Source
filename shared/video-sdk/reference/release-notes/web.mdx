...
As of v4.15.0, the `mode` parameter in `startProxyServer` defaults to `3`, which represents the Force UDP cloud proxy mode. In this mode, the SDK always transmits data over UDP. For details, see [Use Cloud Proxy](./cloud_proxy_web_ng?platform=Web).
...
<div class="alert info">① To ensure the best user experience, Agora recommends using the latest version of the browsers listed above.</div>

To start a call using the VP9 codec with an app that integrates the Agora Native SDK (including the SDK for Android, iOS, Windows, and macOS), ensure that the Native SDK version is v4.0.0 or later.

**CSP violation callback**

v4.15.0 adds the `AgoraRTC.onSecurityPolicyViolation` callback. When Agora fails to load a resource or send a request due to CSP (Content Security Policy) violations, the SDK triggers this callback. After receiving this callback, modify your CSP configuration to ensure that you can access Agora-related services.

#### Improvements

The SDK has the following improvements:

- Improves the accuracy of network quality reported by `AgoraRTCClient.on("network-quality")` in certain scenarios.
- Speeds up the resumption of audio or video streams when the user switches between Wi-Fi networks.
