...
5. **Local audio state changed callback optimization**
   ...
   This release introduces the following enumerations in `AgoraAudioLocalReason`, enabling users to obtain more details about local audio errors through the <Link to="{{Global.API_REF_MACOS_ROOT}}/agorartckit/agorartcenginedelegate/rtcengine(_:localaudiostatechanged:error:)">localAudioStateChanged</Link> callback:
...
- The following enumerations in `AgoraAudioLocalReason`:
