...
To do this add the provided code sample code to `<app-name>/src/App.js` after the `rtcProps` declaration:
...
If `videoCall` is `true`, show the `AgoraUIKit` interface. If it is `false` show the _Join_ button. To do this, add the provided code sample code after the `callbacks` declaration:
