...
- **User ID**: A 32-bit signed integer that identifies a user in the channel. You can specify a unique user ID for each user. If you set the user ID to `0` when joining a channel, the SDK generates a random number for the user ID and returns the value in the `onJoinChannelSuccess` callback. 
...
private void joinChannel() {
    ...
    // Set the channel profile to LIVE_BROADCASTING for interactive live streaming 
...
![UI design](/images/rtc-sdk/quickstart-ui-android-design.png)
