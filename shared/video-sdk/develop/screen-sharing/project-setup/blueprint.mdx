...
<Vg k="COMPANY" /> currently supports the following two <Vg k="SCR_SHR" /> solutions on macOS/Windows platforms:

    - Share a specified screen or part of a specified screen via `displayId`.
    - Share a specified window or part of a specified window via `windowId`.
