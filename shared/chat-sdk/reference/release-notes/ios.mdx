...
- Adds the function of managing custom chat room attributes to implement functions like seat control and synchronization in voice chat rooms.
- Adds the `AgoraChatMessage#priority` attribute to implement the chat room message priority function to ensure that high-priority messages are dealt with first.
- Adds the pagination parameters `pageNum` and `pageSize` to the `IAgoraChatManager#getConversationsFromServer` method to allow users to get the conversation list from the server with pagination.
- Adds the support for push notifications on the server side to allow you to send push notifications to all users, individual users specified by IDs, or groups of users by labels. For how to configure and send push notifications, see the following RESTful API documents:
  - [Send push notifications](../restful-api/message-management/send-push-notifications)
  - [Configure push notifications](../restful-api/message-management/configure-push-notifications)
  - [Set push labels](../restful-api/message-management/set-push-labels)
