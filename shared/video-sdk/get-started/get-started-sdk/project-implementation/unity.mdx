    ...
    When toggles are switched on, to switch client role and turn off the toggles, in your script file, add the following methods to `NewBehaviourScript`:
        ...
        void Func2(bool value)
        {
            if (value==true)
            {
                ...
                clientRoleOptions.audienceLatencyLevel = AUDIENCE_LATENCY_LEVEL_TYPE.AUDIENCE_LATENCY_LEVEL_LOW_LATENCY; // Use low level latency.
                RtcEngine.SetClientRole(CLIENT_ROLE_TYPE.CLIENT_ROLE_AUDIENCE, clientRoleOptions);
                clientRole = "Audience";
            }
        }
        ```
        For <Vg k="BS" />, use `AUDIENCE_LATENCY_LEVEL_LOW_LATENCY` for audience roles. This ensures low latency, which is a feature of <Vg k="BS" /> and its use is subject to special [pricing](../reference/pricing#unit-pricing).
</ProductWrapper>
