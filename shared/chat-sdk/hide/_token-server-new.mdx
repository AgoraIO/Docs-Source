...
Authentication is the process of validating identities. <Vg k="COMPANY"/> uses digital tokens to authenticate users and their privileges before they access an <Vg k="COMPANY"/> service, such as joining an <Vg k="COMPANY"/> call, or logging in to Chat.

To ensure security in real-time communication, <Vg k="COMPANY"/> provide tokens for you to authenticate your users. Tokens, generated in your app server, can be used in the following scenarios:
...
To ensure communication security, <Vg k="COMPANY"/> recommends using tokens to authenticate users who log in to the <Vg k="COMPANY"/> Chat system.

For testing purposes, <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link> supports generating temporary tokens for <Vg k="COMPANY"/> Chat. To generate a user token, do the following:
...
- An [<Vg k="COMPANY"/> project](/agora-chat/reference/manage-agora-account#create-and-manage-projects) with the [App Certificate](/agora-chat/reference/manage-agora-account#manage-app-certificates) and [Chat](/agora-chat/get-started/enable) enabled.
- App ID, OrgName, and AppName of your <Vg k="COMPANY"/> project. See [Enable and Configure Chat Service](/agora-chat/get-started/enable).
- [Node.js and npm](https://docs.npmjs.com/downloading-and-installing-node-js-and-npm).

If you have a firewall implemented in your network environment, <Vg k="COMPANY"/> provides a firewall whitelist solution for you to access Chat in environments with restricted network access. If you want to use the firewall whitelist solution, [submit a ticket](https://agora-ticket.agora.io/) and our technical support will provide the target domain and corresponding IP.
...
If you use Chat together with the [<Vg k="COMPANY"/> <Vg k="VSDK" />](/agora-chat/reference/glossary#agora-video-sdk), <Vg k="COMPANY"/> recommends you upgrade to [AccessToken2](../reference/access-token-2).
