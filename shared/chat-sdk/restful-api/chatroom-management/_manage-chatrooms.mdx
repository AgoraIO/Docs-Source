...
| `cursor` | String | No   | The starting position of the data query. This parameter is only required when fetching pages. | no  |

<div class="alert info"> If neither is set in the request `limit` and `cursor`, before the server returns the first page of the chat room list 10 chat room.</div>
...
<div class="alert info">If neither query parameter is specified, the server returns the 500 chat rooms that the user joined most recently.</div>
...
| data.result | Boolean | Whether the chat room announcement is successfully modified: <br/> - `true`: Success<br/> - `false`: Failure |
