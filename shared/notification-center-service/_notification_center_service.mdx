...
import CloudRecordingEvents from './channel-recording-events.mdx'
import MaintainStatus from '@docs/shared/notification-center-service/maintain-user-online-status.mdx';
import SignatureVerification from '@docs/shared/notification-center-service/signature-verification.mdx'; import {
  ProductWrapper
} from '../../../src/mdx-components/ProductWrapper';


A webhook is a user-defined callback over HTTP. You use webhooks to notify your <Vpl k="CLIENT" /> or back-end system when certain <Vpd k="PRODUCT" /> events occur. For example, when a channel is created, a user joins and leaves a channel, or changes role from host to audience. <Vg k="COMPANY" /> <Vg k="NCS_LONG" /> enables you to subscribe to events and receive notifications about them across multiple product lines.
...
The following figure illustrates the workflow when <Vg k="NCS_LONG" /> is enabled for the specific events you subscribe to:
...
- [Enable <Vg k="NCS_LONG" />](#enable-ncs)
...
1. **Setup a Java project for your server**
    ...
    Your server receives and handles <Vg k="NCS_LONG" /> notifications. The `RootHandler` handles requests sent to the root address of your server and the `NcsHandler` handles the <Vg k="NCS_LONG" /> notifications sent to `<your server address>/ncsNotify`. To Set up your server, create a new Java project and add the following code to a file named `Main.java`:
...
* The Agora <Vg k="NCS_LONG" /> does not guarantee that notification callbacks arrive at your server in the same order as events occur.
