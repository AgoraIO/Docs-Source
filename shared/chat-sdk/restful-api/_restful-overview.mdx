...
APIs serving the same function on client platforms correspond to a RESTful API. Typically, when you call the client APIs, you actually call the matching RESTful API. The call frequency of the RESTful API is the sum of the call frequencies of both the RESTful API and its corresponding client APIs. If the call frequency reaches the upper limit, it is possible that client APIs are called frequently.

Note that the RESTful APIs marked with an asterisk in the tables below correspond to no client APIs. The call frequency refers only to that of the RESTful APIs, without involving any client APIs. If the call frequency exceeds the upper limit, contact support@agoro.io to increase the limit.
...
| Name | Method | Request | Description | Rate Limits |
| :--------------- | :--- | :------------------------------------------ | :------------------------------------------------- |  :----- |
| Send a one-to-one message* | POST | `/{org_name}/{app_name}/messages/users` | Send messages to individual users. | 100/second |
