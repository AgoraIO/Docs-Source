...
v1.4.0 was released on XX, 2026.

#### New features

- Added support for sending and receiving GIF image messages. A GIF image message is a subtype of image message that is not compressed when sent. For details, see [Send and receive a GIF image message](/agora-chat/client-api/messages/send-receive-messages?platform=android#send-and-receive-a-gif-image-message).
- Added support for setting, updating, and retrieving chat group avatars. For details, see [Manage the chat group avatar](/agora-chat/client-api/chat-group/manage-group-attributes?platform=android#manage-the-chat-group-avatar).
- Added support for retrieving the following information when a user joins a chat room:
  - The current member count, using `ChatRoom#getMemberCount`. This count updates when members join or leave the chat room.
  - The chat room-wide mute status, using `ChatRoom#isAllMemberMuted`. This status updates when a mute or unmute event is received.
  ...
  - The current user's mute expiration time, using `ChatRoom#getMuteExpireTimestamp`.
- Added the `ChatRoomChangeListener#onMuteListAdded(String, Map<String, Long>)` event, which provides the muted members' user IDs and their mute expiration timestamps through the `muteInfo` parameter. This event deprecates `ChatRoomChangeListener#onMuteListAdded(String, List<String>, long)`. For details, see [Listen for chat room events](/agora-chat/client-api/chat-room/manage-chatrooms?platform=android#listen-for-chat-room-events).
- Added support for retrieving roaming messages sent by specific group members from the server. For details, see [Retrieve message history of the specified conversation](/agora-chat/client-api/messages/retrieve-messages?platform=android#retrieve-message-history-of-the-specified-conversation).
- Added support for retrieving messages sent by specific members in a local group conversation. For details, see [Search local messages sent by specific members](/agora-chat/client-api/messages/retrieve-messages?platform=android#search-local-messages-sent-by-specific-members).
- Added support for searching all local conversations by keyword. The SDK returns the matching conversation IDs and their message IDs. For details, see [Search local conversations by keyword](/agora-chat/client-api/messages/retrieve-messages?platform=android#search-local-conversations-by-keyword).
- Added support for retrieving one or more local messages by message ID. For details, see [Retrieve local messages by message ID](/agora-chat/client-api/messages/retrieve-messages?platform=android#retrieve-local-messages-by-message-id).
- Added the `GroupChangeListener#onMembersJoined` and `GroupChangeListener#onMembersExited` events, which notify other members in batches when one or more members join or leave a group. Previously, the SDK triggered a separate event for each member. These events deprecate `GroupChangeListener#onMemberJoined` and `GroupChangeListener#onMemberExited`. For details, see [Listen for chat group events](/agora-chat/client-api/chat-group/manage-chat-groups?platform=android#listen-for-chat-group-events).

#### Improvements

- Changed the unique app identifier from the app key to the app ID.
- Added support for modifying more message types with `ChatManager#asyncModifyMessage`:
  ...
  For details, see [Modify sent messages](/agora-chat/client-api/messages/send-receive-messages?platform=android#modify-sent-messages).
- Allowed group owners, chat room owners, and admins to recall messages sent by other members. For details, see [Recall a message](/agora-chat/client-api/messages/send-receive-messages?platform=android#recall-a-message).
- Changed the trigger threshold for the `ConnectionListener#onTokenWillExpire` event: the SDK now triggers the event when 80% of the token's validity period has elapsed, instead of 50%. For details, see [Authentication](/agora-chat/develop/authentication?platform=android).
- Added support for retrieving a group member list that includes each member's role and join time, using `GroupManager#asyncFetchGroupMembersInfo`. For details, see [Retrieve the member list of a chat group](/agora-chat/client-api/chat-group/manage-chat-groups?platform=android#retrieve-the-member-list-of-a-chat-group).
- Changed `CursorResult#cursor` from `undefined` to an empty string (`""`) when `ChatManager#asyncFetchHistoryMessages` reaches the last page of roaming messages.
- Removed APIs that were deprecated before Android SDK v1.1.0.
- Optimized certain database operations.
- Added exception clearing and null pointer protection at the JNI layer.
...
- `MessageListener#onMessageContentChanged` did not return modification details when editing message types other than text and custom messages.
...
- The latest message in a conversation retrieved from the server with `ChatManager#asyncFetchConversationsFromServer` did not contain translations or message reactions.
- An exception occurred during logout when SDK API calls were nested within SDK events.
- A crash occurred under extreme network conditions.

<Admonition type="info">
A compilation error may occur when you use Agora Chat SDK with Signaling SDK 2.2.0 or later, or Video SDK 4.3.0 or later. For the fix, see [Integration issues](/agora-chat/get-started/get-started-sdk?platform=android#integration-issues).
</Admonition>
