...
- If you want to customize the classroom UI based on the default UI of <Vg k="FC"/>, you need to integrate <Vg k="FC"/> by downloading the [source code](https://github.com/AgoraIO-Community/flexible-classroom-desktop) on GitHub.
...
1. To clone the Flexible-Classroom-Desktop repository and check out the latest release branch, run the following commands:
   ...
   ```bash
   https://github.com/AgoraIO-Community/flexible-classroom-desktop
   ```

2. To change directory to `Flexible-Classroom-Desktop` and switch the branch, run the following commands. Remember to replace `{VERSION}` with a specific version number. For example, if you want to switch to branch v2.6.2, run the command `git checkout release/2.6.2`.
    ...
    Agora recommends switching the branch to the latest release. The following image shows how to see the latest release branch in the [GitHub repository](https://github.com/AgoraIO-Community/flexible-classroom-desktop):

    ![](/images/flexible-classroom/flexible-classroom-latest-branch.png)
