...
The Web SDK supports <Vg k="SCR_SHR" /> on Chrome 58 and higher with or without using a <Vg k="SCR_SHR" /> plugin.
...
This method requires Chrome 74 or above. If the version you are targeting does not meet this requirement, use the <Vg k="SCR_SHR" /> plug-in to share the screen.
...
1. Create the track by using the `extensionId`.
...
<Vpd k="SDK" /> Web supports <Vg k="SCR_SHR" /> on Edge 80 and higher on the Windows 10+ platform.
...
<Vpd k="SDK" /> Web supports <Vg k="SCR_SHR" /> on Firefox 56 and higher. For Firefox, you specify the type of <Vg k="SCR_SHR" /> by setting `screenSourceType`.
...
<Vpd k="SDK" /> Web supports <Vg k="SCR_SHR" /> on Safari 13 and higher on the macOS platform.
...
If you choose to use the default interface, there is no difference between using <Vg k="SCR_SHR" /> under Electron and web. Just call `createScreenVideoTrack` directly:
...
Subscribing to your own track incurs additional charges, as illustrated in the figure:
