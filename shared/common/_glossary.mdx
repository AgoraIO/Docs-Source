...
You use the <Vg k="SIG_SDK"/> to implement real-time messaging scenarios that require low latency and high concurrency for a global audience.
...
A caller is a <Vg k="SIG"/> user who sends a [call invitation](#call-invitation).

###  Call invitation

Call invitation is a communication protocol based on the peer-to-peer messaging functionality of the Agora <Vg k="SIG_SDK"/>. Call invitation supports starting, ending, accepting, and refusing calls.
...
Offline describes the status of a <Vg k="SIG"/> user who has successfully logged out of Agora <Vg k="SIG"/>.
...
Online describes the status of a user who has successfully logged in to <Vg k="SIG"/> or stays disconnected from <Vg k="SIG"/> for more than 30 seconds.
...
In the Agora Video SDK, subscribing is the action of receiving media streams published to the channel. In the Agora <Vg k="SIG_SDK"/>, subscribing is the action of monitoring the online status of one or multiple <Vg k="SIG"/> users.
...
A token, also known as a dynamic key, is used for authentication when an app user joins a channel or logs into <Vg k="NAME"/>.
...
In the Agora Video SDK, a user ID identifies a user in the channel. In the Agora <Vg k="SIG_SDK"/>, a user ID identifiers a user in <Vg k="SIG"/>. The user ID in the Agora Video SDK and the Agora <Vg k="SIG_SDK"/> are independent from each other.
