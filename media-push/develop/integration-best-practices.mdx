...
- To monitor Media Push events, enable [Notification Center Service](../../video-calling/develop/receive-notifications) (optional).
...
* If resources are insufficient, then tasks started later will fail.
...
- Create the converter by region parameter:
    - The region you set must be in the same area as your CDN origin station. For example, if your CDN origin is located in US, set region to `na`.
...
-   Set an appropriate value for `idleTimeout`. The default value of 300 seconds is recommended. It means that the converter will automatically destroy 300 seconds after all the subscribed leave the channel.
