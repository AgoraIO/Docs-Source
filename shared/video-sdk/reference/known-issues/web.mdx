...
- After creating a screen sharing video track using `createScreenVideoTrack` on Safari 16.2 and 16.3 on macOS Ventura, there may be an issue of a green screen when calling `setEncoderConfiguration` to modify the video resolution of that track. This issue is caused by a functional regression in WebKit. See [Bug 254652](https://bugs.webkit.org/show_bug.cgi?id=254652) for details. The solution is to upgrade to Ventura 13.3 (Safari 16.4).
