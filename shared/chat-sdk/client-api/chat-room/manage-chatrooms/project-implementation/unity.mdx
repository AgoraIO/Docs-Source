...
If a chat room member is offline for more than two minutes, they are automatically logged out due to network conditions or other factors. To adjust this timeout, contact [support@agora.io](mailto:support@agora.io).
    ...
    - **Users added via the RESTful API**: If a user is added when [creating a chat room](#create-a-chat-room) using the RESTful API and has never logged in, they are not removed.
