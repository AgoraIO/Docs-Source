In social entertainment and online live streaming scenarios, reviewing video streams published by hosts can identify any illegal content and mitigate compliance risks. This section describes and compares various content moderation solutions offered by Agora to enable you to choose the most appropriate one for your business scenario.
...
| [Raw video data](#raw-video-data) | Scenarios with strong development capabilities and the ability to obtain raw data. | - | This method is suitable for scenarios where you have strong development capabilities and can obtain raw video data directly. However, it requires more technical expertise and may not be suitable for all businesses. |

#### Client-Side Screenshots

<Vg k="COMPANY" /> provides the `enableContentInspect` method in native and web SDKs to capture and upload screenshots of video streams in RTC channels. Refer to the [Local screenshot upload](/video-calling/advanced-features/screenshot-upload) page for implementation details.

After uploading client-side screenshots, you can choose to integrate Agora's video moderation functionality provided by its cloud marketplace partners such as [ActiveFence (Beta)](https://www.agora.io/en/agora-extensions-marketplace/?extension=activefence-video-moderation), or select a third-party video moderation service provider of your own choice.
...
Cloud-based screenshots refer to the use of Agora's cloud recording screenshot function to capture images from video streams within a channel and upload the images to a third-party cloud storage. Refer to the following documents for further details:
...
Currently, cloud-based screenshots only support integration with third-party moderation service providers for video moderation. Select a third-party service provider and implement the moderation logic yourself. 
