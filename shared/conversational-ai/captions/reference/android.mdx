...
- `callback`: The callback interface for receiving caption content updates, see [`IConversationSubtitleCallback`](#iconversationsubtitlecallback) for details.
...
The callback interface for caption content update events.
...
- `onSubtitleUpdated`: Caption update callback.
    - `subtitle`: Updated caption message, see for details [`SubtitleMessage`](#subtitlemessage).
...
Use `SubtitleStatus` for special UI processing based on the status, such as displaying an interruption mark at the end of the caption.
...
- `Progress`: The captions are still being generated; the user or agent has not finished speaking.
