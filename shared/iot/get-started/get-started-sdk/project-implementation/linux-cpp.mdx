...
1. **Initialize the SDK, verify license and create a connection**
    ...
    The following code initializes <Vg k="COMPANY"/> service event handler, configures service options, and creates a connection that you use to send and receive data:
...
1. **Handle Agora RTC service events**

    <Vpd k="SDK"/> notifies you of important service events, such as users joining or leaving a channel and receipt of audio or video data, through callbacks in the event handler. The following code adds methods to handle <Vpd k="SDK"/> events and initializes the event handler.
