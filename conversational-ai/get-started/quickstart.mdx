...
To interact with your AI agents, use one of the following demo client applications:
...
The MLLM workflow uses a single, powerful AI model that processes voice input directly and generates voice output without the need for separate STT or TTS. This creates faster, more natural conversations with better context understanding.
...
Your server authenticates with <Vg k="COMPANY" />'s APIs using HTTP Basic Authentication. You also need to generate RTC tokens for agents to join channels.
