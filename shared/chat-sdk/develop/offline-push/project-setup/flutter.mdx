...
To optimize user experience when dealing with an influx of push notifications, <Vg k="COMPANY"/> <Vg k="CHAT"/> provides fine-grained options for the push notification and do-not-disturb (DND) modes at both the app and conversation levels.
...
<Vg k="CHAT"/> allows users to use ready-made templates for push notifications.
...
2. On the **Project Management** page, locate the project that has <Vg k="CHAT"/> enable and click **Config**.
