...
1. Ensure `cmake` is installed.
    ...
    The sample project uses `cmake` to control the compilation process. Execute `cmake --version` to check that the version number is `3.6.0` or above. if it is of lower version , then remove it.
    ...
    If `cmake` is not installed on the system or removed because of low version, install it using the following command:
