...
2. **Get the `uid` of the remote user**
   ...
   When a remote user joins the channel, <Vg k ="VSDK"/> triggers the `OnUserJoined` callback. You use this callback to save the `uid` of remote user. To implement this callback, in `Assets/Agora-RTC-Plugin/Agora-Unity-RTC-SDK/Code/JoinChannelAudio.cs`, add the following callback to `UserEventHandler` class:
