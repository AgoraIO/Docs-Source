...
| 706        | `CHATROOM_OWNER_NOT_ALLOW_LEAVE`  | The chat room owner is not allowed to leave the chat room. The configure parameter `EMOptions#allowChatroomOwnerLeave` is set to `false` by default during initialization. An error will be prompted when the chat room owner tries to leave the chat room by calling the `leaveChatRoom` method. |
