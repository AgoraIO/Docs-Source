...
|**Connected** |Occurs after the <Vpl k="CLIENT" /> has successfully joined the channel. The SDK also triggers the `FOnJoinChannelSuccess` callback to report that the local client has joined the channel. At this point the user can publish or subscribe to the audio and video in the channel. |
|**Reconnecting** |Occurs when the connection is interrupted. The SDK automatically tries to reconnect after an interruption. <ul><li>If the rejoin channel succeeds, the SDK triggers the `FOnRejoinChannelSuccess` callback.</li><li>If the channel is not rejoined within 10 seconds, the SDK triggers `FOnConnectionStateChanged(RECONNECTING, LOST)`, while still trying to rejoin the channel.</li></ul> |
