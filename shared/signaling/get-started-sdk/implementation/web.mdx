...
To continuously monitor the network connection state of the client, best practice is to continue to listen for `status` event notifications throughout the life cycle of the application. For further details, see [Event Listeners](../reference/api?platform=android#event-listeners).
</Admonition>
