...
1. In **Notification Center Service Configuration**, fill in the following information, and click **Save**:
    ...
    * **Allowed IP List**: If your server is behind a firewall, ensure that you have called the [IP address query API](#ip-address-query-api) to get the IP addresses of the Agora <Vg k="NCS" /> server and added them to the firewall's allowed IP list, and check the box here.
...
1. Agora performs a health test for your configuration as follows:
    ...
    1. After receiving each test event callback, your server needs to respond to the Agora <Vg k="NCS" /> server within 10 seconds with a status code of 200. The response body must be in JSON format.
