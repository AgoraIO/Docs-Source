...
3. Check if the calculated value equals to the secret parameter in the request body. If yes, the callback is sent by <Vg k="COMPANY"/> Chat.

## Prerequisites

To use the HTTP callbacks, you must meet the following requirements:

- You have an <Vg k="COMPANY"/> project with Chat [enabled](../get-started/enable).
