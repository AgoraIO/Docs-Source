...
2.  **Set the token URL for the AgoraVideoViewer**
    ...
    To set the token URL, you create an `AgoraSettings` object, set its `TokenURL` property and pass this object to the constructor when initializing the `AgoraVideoViewer`. To do this, replace the code in the `` try {…​} block of the `initializeAndJoinChannel() `` method with the following:
