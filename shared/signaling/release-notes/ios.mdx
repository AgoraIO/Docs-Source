...
This release optimizes the implementation of following features, which involves renaming, deleting, or modifying the behavior of some APIs. To ensure the proper functioning of your project, you need to modify your implementation after upgrading the SDK.
...
- Removes the `AgoraRtmStateItem` calss, and change the type of the following parameters from `NSArray<AgoraRtmStateItem *>` to `NSDictionary`:
  - The `items` parameter of the [`setState`](/api-ref/rtm2/ios/toc-presence/presence#setState) method.
  - The `states` parameter of the [`PresenceEvent`](/api-ref/rtm2/ios/toc-configuration/configuration#PresenceEvent) class.
  - The `states` parameter of the [`UserState`](/api-ref/rtm2/ios/toc-presence/presence#getState) class.

- Adds the the <code>{enumv.reasoncertification[props.ag_platform]}(22)</code> in the [<code>{enumv.connectionreason[props.ag_platform]}</code>](/api-ref/rtm2/ios/enumv#connectionreason). The values of the <code>{enumv.reasonstreamchannel[props.ag_platform]}</code> and <code>{enumv.reasoninconsistentappid[props.ag_platform]}</code> also changes to 23 and 24, respectively.

#### New features

1. macOS and VisionOS support

This release supports the macOS and VisionOS platforms. For the minimum supported version and target architecture, see [Platform support](./platform-support). This release also changes the package name from `AgoraRtm_iOS` to `AgoraRtm`. After upgrading the SDK, if you use Cocoapods to integrate the SDK, you need to modify the `Podfile` file as follows:
...
This release adds the `privateConfig` parameter in [<code>{config.rtm[props.ag_platform]}</code>](/api-ref/rtm2/ios/toc-configuration/configuration) to set private deployment. See [Private configuration](../user-guide/setup/private-setup).

1. Heartbeat interval configuration

This release adds the `heartbeatInterval` parameter in [<code>{config.rtm[props.ag_platform]}</code>](/api-ref/rtm2/ios/toc-configuration/configuration) to set the interval at which the SDK sends heartbeat packets to the server. See [Heartbeat interval and presence timeout](../user-guide/link/link-basic#heartbeatinterval-and-presencetimeout).

1. Dual environment configuration

This release adds the `protocolType` parameter in [<code>{config.rtm[props.ag_platform]}</code>](/api-ref/rtm2/ios/toc-configuration/configuration) to set the network transport protocol. See [Protocol configuration](../user-guide/setup/application-setup#protocol).

1. User channel

This release adds the <code>{enumv.channeltypeuser[props.ag_platform]}</code> type in [<code>{enumv.channeltype[props.ag_platform]}</code>](/api-ref/rtm2/ios/enumv#channeltype) for sending messages to specific users. This feature can replace the point-to-point messaging feature in v1. See [Sending messages in user channel](../user-guide/message/send-message#userchannel).

1. Quiet mode configuration

This release adds <code>{enumv.subscribechannelfeaturebequiet[props.ag_platform]}</code> in [<code>{enumv.subscribechannelfeature[props.ag_platform]}</code>](/api-ref/rtm2/ios/enumv#subscribechannelfeature), and <code>{enumv.joinchannelfeaturebequiet[props.ag_platform]}</code> in [<code>{enumv.joinchannelfeature[props.ag_platform]}</code>](/api-ref/rtm2/ios/enumv#joinchannelfeature) to enable quiet mode when subscribing or joining a channel. Once you enable the quiet mode, other users in the channel cannot receive your presence event notifications.

#### Improvements

1. Connection state management

This release deprecates the <code>{config.onconnection[props.ag_platform]}</code> callback and adds the [<code>{config.onlinkstateevent[props.ag_platform]}</code>](/api-ref/rtm2/ios/toc-configuration/configuration#event-listening) callback instead. See [Connection management](../user-guide/link/link-basic).
...
- [<code>{config.messageevent[props.ag_platform]}</code>](/api-ref/rtm2/ios/toc-configuration/configuration#messageevent)
- [<code>{config.presenceevent[props.ag_platform]}</code>](/api-ref/rtm2/ios/toc-configuration/configuration#presenceevent)
- [<code>{config.topicevent[props.ag_platform]}</code>](/api-ref/rtm2/ios/toc-configuration/configuration#topicevent)
- [<code>{config.storageevent[props.ag_platform]}</code>](/api-ref/rtm2/ios/toc-configuration/configuration#storageevent)
- [<code>{config.lockevent[props.ag_platform]}</code>](/api-ref/rtm2/ios/toc-configuration/configuration#lockevent)

1. Optimized API behavior

This release improves the behavior of the following APIs:
- [<code>{config.login[props.ag_platform]}</code>](/api-ref/rtm2/ios/toc-configuration/configuration#login)
...
- [<code>{channel.subscribe[props.ag_platform]}</code>](/api-ref/rtm2/ios/toc-channel/channel#subscribe)
...
- [<code>{channel.join[props.ag_platform]}</code>](/api-ref/rtm2/ios/toc-channel/channel#join)
  - Earlier than v2.2.1: the SDK does not support multiple consecutive calls to this method, or passing an empty string in the `token` parameter.
...
This release changes the range of the `presenceTimeout` parameter in the [<code>{config.rtm[props.ag_platform]}</code>](/api-ref/rtm2/ios/toc-configuration/configuration) from [10, 300] to [5, 300].

1. Others

This release also enhances underlying algorithm capability to improve data synchronization speed.
