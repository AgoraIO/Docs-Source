...
- `X-Request-ID`: UUID (Universally Unique Identifier) to identify this request. After passing in the field, the <Vg k="AGORA_BACKEND"/> returns it in the response header.
    Best practice is to use this field. If you do not pass in any value, the <Vg k="AGORA_BACKEND"/> automatically generates a UUID to pass in.
...
- `token`: (Optional) String. The authentication token used by the cloud player in the channel. If you do not enable the App Certificate in Agora Console, ignore this field. If you enable the App Certificate in Agora Console, you must pass a value in this field:
  ...
  - For high-security requirements: Use the token generated at your server. For details, see [Secure authentication with tokens](/realtime-communication/token-authentication/authentication-workflow).
...
- `encryptKdfSalt`: (Optional) String. A 32-byte salt. Agora recommends using OpenSSL to generate this salt on your server. For details, see [Secure channel encryption](/realtime-communication/advanced-features/media-stream-encryption).
  <Admonition type="info">
