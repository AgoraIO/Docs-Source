...
- An [<Vg k="COMPANY"/> project](../reference/manage-agora-account#create-and-manage-projects) for which you have [enabled the Chat service](enable#enable-).
...
In a production environment, best practice is to deploy your own token server. Users retrieve a token from the token server to log in to the chat server. To see how to implement a server that generates and serves tokens on request, see [Secure authentication with tokens](../develop/authentication)
