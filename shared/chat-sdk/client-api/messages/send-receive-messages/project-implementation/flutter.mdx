...
You can use the `Message#createSendMessage` and `Message#attribute` methods to create a new message identical to the original one, including its body and extended fields. After creating the message, you can forward it using the `ChatManager#sendMessage` method.
