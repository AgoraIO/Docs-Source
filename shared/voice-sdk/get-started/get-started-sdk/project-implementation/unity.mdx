...
1. **Create a new script to hold the system logic**
    ...
    1. Open **Assets** > **Agora-Unity-<Vg k="VSDK" />-SDK** > **Code**.
...
1. Add the Unity Android libraries. In `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelAudio.cs`, add the following to the list of namespace declarations:
...
4. **Bind your script to the Canvas**
    ...
    In `Assets/Agora-Unity-<Vg k="VSDK" />-SDK/Code, drag and drop `JoinChannelAudio.cs` on **Canvas** to the **Hierarchy**.
...
1. **Import the <Vg k="VSDK"/> library**
    ...
    In `Assets/Agora-<Vg k="VSDK" />-Plugin/Agora-Unity-<Vg k="VSDK" />-SDK/Code/JoinChannelAudio.cs`, add the following library after `using UnityEngine.UI;`:
    ``` csharp
    using Agora.Rtc;
    ```
