...
In scenarios involving screen sharing, the unit price of the screen-sharing stream is calculated on the basis of the video dimension that you set in <Vg k="API_REF_ANDROID" />/rtc\_api\_data\_type.html#class\_screencaptureparameters[ScreenCaptureParameters,role=external,window=\_blank]. For details, see descriptions in the following classes:
