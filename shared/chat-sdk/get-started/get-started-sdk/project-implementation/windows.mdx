...
To add the UI framework to your <Vpl k="NAME"/> project, open the `<project-name>` > `MainWindow.xaml` file from the Solution Explorer, and replace the file contents with the following:
...
1. **Set up <Vpd k="NAME" /> when the <Vpl k="CLIENT"/> starts**
    ...
    public MainWindow()
    {
        ...
        // Setup Agora Chat SDK client.
        setupChatClient();  
        // Add a chat delegate to handle chat callbacks
...
1. **Log in to <Vg k="COMPANY" /> <Vg k="CHAT" />** 
    ...
    When a user clicks **Join**, your <Vpl k="CLIENT"/> logs in to <Vg k="COMPANY" /> <Vg k="CHAT" />. When they click **Leave**, the <Vpl k="CLIENT"/> logs out from <Vg k="COMPANY" /> <Vg k="CHAT" />. To implement this logic, add the following method after `showLog()` in the `MainWindow` class:
...
1. **Implement the `IConnectionDelegate` Interface**
    ...
    1. Add below line after `SDKClient.Instance.ChatManager.AddChatManagerDelegate(this);`  to add AgoraChat connection delegate:

        ```c#
        SDKClient.Instance.AddConnectionDelegate(this);
        ```

    1. To cleanup the connection listener, add below line in `CloseWindow()` after `SDKClient.Instance.ChatManager.RemoveChatManagerDelegate(this);`
    
        ```c#
        SDKClient.Instance.DeleteConnectionDelegate(this);
        ```

    1. To implement all the `IConnectionDelegate` callbacks, add the following code to the `MainWindow` class:
