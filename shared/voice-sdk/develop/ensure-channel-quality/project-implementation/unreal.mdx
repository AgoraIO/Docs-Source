...
1. Add a text widget for the echo test button. Drag **Text** from the **Common** section of the **Palette** to the canvas panel. In **Details**, change the **Text** field to `Echo Test`, then change the following properties:
...
1.  **Setup an event listener for the echo test button**
    ...
    1. In `MyUserWidget.h`, add the following method declaration after `UTextBlock* networkStatus = nullptr;`:
