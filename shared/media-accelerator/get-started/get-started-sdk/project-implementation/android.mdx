...
At startup, you initialize the RTC service. When a user taps a button, the <Vpl k="CLIENT" /> joins a channel. The <Vpl k="CLIENT" /> sends audio and video data to remote users. 
...
1.  **Setup <Vg k="COMPANY" /> RTC Service**
    ...
    To use <Vpd k="PRODUCT" /> features, you use the <Vpd k="SDK" /> to create a RTC service instance. You then use the service instance to open a connection. In `/app/java/com.example.<projectname>/MainActivity`, add the following code before the `onCreate` method:
...
1. **Send and receive audio and video data**
    ...
    After joining an RTC channel, you can:
     ...
     When sending audio and video data, you set the sending interval such that it is consistent with the video frame rate and audio frame length.

    In Android Studio, select **File > New > Java Class**. Name the class `StreamFile` and replace the contents of the file with the following code:
