...
1. **Unregister the audio observer when you leave a channel**
    ...
    When you leave a channel, you unregister the frame observer by calling the register frame observer method again with a `null` argument. To do this, add the following lines to the 'leaveChannel(View view)' method before `agoraEngine.leaveChannel();`:
