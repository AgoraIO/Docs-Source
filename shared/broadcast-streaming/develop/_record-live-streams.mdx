...
### Standard recording
Call the [Set standard recording configuration](./rest-api-standard-recording-fls?platform=RESTful) API. If the call is successful, standard recording is automatically enabled for all live streams under the entry point.

### Custom recording
Call the [Set custom recording configuration](./rest-api-recording-fls?platform=RESTful) API. If the call is successful, custom recording is automatically enabled for all live streams under the entry point.
...
1. Call the [Set standard recording configuration](./rest-api-standard-recording-fls?platform=RESTful) API and set the format of the recording file.
2. Call the [Enable/Disable standard recording for an entry point](./rest-api-start-standard-recording-fls?platform=RESTful) API and set `enabled` to `false`, which disables the current standard recording for the entry point.
4. Call the [Enable standard recording for a live stream](./rest-api-start-standard-recording-stream-fls?platform=RESTful) API and set the required parameters such as the live stream name.

### Custom recording
Follow these steps to enable custom recording for a live stream:
1. Call the [Set custom recording configuration](./rest-api-recording-fls?platform=RESTful) API, and set the format of the recording file and third-party cloud storage.
2. Call the [Enable/Disable custom recording for an entry point](./rest-api-start-recording-fls?platform=RESTful) API and set `enabled` to `false`, which disables the current custom recording for the entry point.
4. Call the [Enable custom recording for a live stream](./rest-api-start-recording-stream-fls?platform=RESTful) API and set the required parameters such as the live stream name.