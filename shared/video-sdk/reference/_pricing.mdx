...
[AI Noise Suppression](../advanced/ai-noise-suppression) helps reduce noise and voice distortion in virtual communication.
...
[3D Spatial Audio](../advanced/spatial-audio) brings theater-like effects to virtual communication, making it seem as if the sound originates from all around the user.
...
Media relay allows streaming from one source channel to multiple target channels at the same time. For details, see [Streaming over multiple channels](../develop/live-streaming-over-multiple-channels).
...
For example, imagine a 1280 × 720 video is streamed over 3 channels for 10 minutes. The total duration is 10 minutes x 3 channels = 30 minutes. The usage type based on the resolution is HD. The total cost is 30 minutes x 3.99 USD = 119.7 USD.

For an additional example, imagine an audio streamed over 3 target channels for 10 minutes. The total duration is 10 minutes x 3 channels = 30 minutes. The total cost is 30 minutes x 0.99 USD = 29.7 USD.
