...
* **Channel metadata**: You use channel metadata to store and distribute contextual channel data in your <Vpl k="CLIENT" />, such as props, announcements, member lists, and relationship chains. 
...
In this example, after the local user logs in to <Vpd k="NAME" />, you set their metadata in the form of key-value pairs. When the user changes a value stored as metadata, you call the update method to save the new value. You show a list of all users currently in the channel. When the local user selects another user from the list, you retrieve and display their metadata. You subscribe to the metadata change event of remote users to be notified of updates.
