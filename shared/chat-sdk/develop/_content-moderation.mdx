...
Delivering a safe and appropriate chat environment to your users is essential. <Vg k="CHAT"/> gives you multiple options to intelligently moderate the chat content and handle inappropriate user behavior.
...
- You have created a valid [<Vg k="COMPANY"/> developer account](../reference/manage-agora-account#create-an-agora-account).
- Moderation is not enabled by default. To use this feature, you need to subscribe to the **Pro** or **Enterprise** [pricing plan](../reference/pricing-plan-details) and enable it in <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link>.
<div class="alert note">Add-on fees are incurred if you use this feature. See <a href="../overview/pricing#optional-add-on-fee">Pricing</a> for details.</div>

## Enable the moderation feature

1. Log in to <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link>.
2. In the left navigation menu, click **Project Management** > **Config** for the project for which you want to enable the moderation feature > **Config** of <Vg k="CHAT"/> > **Features** > **Overview**.
...
The following table summarizes the message management tools provided by <Vg k="CHAT"/>:

| Function | Description |
|--------|-----|
| Message reporting | The reporting API allows end-users to report objectionable messages directly from their applications. Moderators can view the report items on <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link> and process the messages and message senders. |
...
The <Vg k="CHAT_SDK"/> provides a message reporting API, which allows end-users to report objectionable messages directly from their applications. After the <Vg k="CHAT"/> server receives the report, it stores the report and displays it on <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link>. Moderators can view the report items on the <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link> and process the messages and message senders according to their content policy.

To use the reporting feature, refer to the following code sample to call the reporting API:

<ProjectImplement />

After a user reports a message from the application, moderators can check and deal with the report on <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link>:

1. To enter the **Message Report** page, in the left navigation menu, click **Project Management** > **Config** for the project that you want > **Config** of <Vg k="CHAT"/> > **Message Report** > **History**, as shown in the following figure:
	 
	 ![report_en](https://web-cdn.agora.io/docs-files/1656312948783)

2. On the **Message Report** page, moderators can filter the message report items by time period, session type, or handling method. They can also search for a specific report item by user ID, group ID, or chat room ID. For the reports, <Vg k="CHAT"/> supports two handling methods: withdrawing the message or asking the sender to process the message.

### Text and image moderation

#### Introduction

Powered by [Microsoft Azure Moderator](https://azure.microsoft.com/en-us/services/cognitive-services/content-moderator/), <Vg k="COMPANY"/> <Vg k="CHAT"/>'s text and image moderation can scan messages for illegal and inappropriate text and image content and mark the content for moderation. Microsoft Azure Moderator uses the following four categories to moderate the message:
...
After enabling the text and image moderation feature on <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link>, you can set a sensitivity threshold for each moderation category. If the sensitivity threshold is exceeded, <Vg k="COMPANY"/> <Vg k="CHAT"/> will handle the message as specified in the moderation rule. A greater sensitivity indicates more strict review and that <Vg k="COMPANY"/> <Vg k="CHAT"/> will block more inappropriate content. You are advised to test the moderation rule to check whether the sensitivity threshold you set meets your requirements.
...
- Penalties at the app level or in one-to-one chat:
  - **Ban**: If the user is banned, he will be forced to go offline immediately and cannot log in to <Vg k="COMPANY"/> <Vg k="CHAT"/>. The banned user cannot log in again until he or she is unbanned. For example, if a user sends inappropriate content 5 times within 1 minute, he or she will be banned. Once a user is banned, the user state is changed to `Blocked` on the **Users** page under **Operation Management**. You can click **Unblock** in the **Action** column or call the [RESTful API](../restful-api/user-system-registration#unbanning-a-user) to unblock the user.
  - **Force Offline**: The user is forced to go offline and needs to log in again to use <Vg k="COMPANY"/> <Vg k="CHAT"/> normally.
...
To see how moderation works and determine which moderation settings suit your needs, you can test different text strings and images on <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link>.

#### Create a moderation rule

Taking a one-to-one chat text as an example, follow these steps to create a text moderation rule:

1. To enter the **Rule Config** page, in the left navigation menu, click **Project Management** > **Config** for the project for which you want > **Config** of <Vg k="CHAT"/> > **Text Moderation** or **Image Moderation** > **Rule Config**, as shown in the following figure:
...
1. To enter the **Rule Test** page, in the left navigation menu, click **Project Management** > **Config** for the project that you want > **Config** of <Vg k="CHAT"/> > **Text Moderation** or **Image Moderation** > **Rule Test**, as shown in the following figure:
...
1. To enter the **Rule Config** page, in the left navigation menu, click **Project Management** > **Config** for the project that you want > **Config** of <Vg k="CHAT"/> > **Word List** > **Rule Config**, as shown in the following figure:

	 ![keyword_en](https://web-cdn.agora.io/docs-files/1656313419195)

2. On the **Rule Config** page, you can add or delete words and determine which filtering method to apply to messages that contain the specified keywords. You can replace the word with \*\*\* or simply not send the word. You can add up to 10,000 words to the list. Contact [support@agora.io](mailto:support@agora.io) if you need to extend this limit to 100,000 words.
...
1. To enter the **Rule Config** page, in the left navigation menu, click **Project Management** > **Config** for the project that you want > **Config** of <Vg k="CHAT"/> > **Domain Filter** > **Rule Config**, as shown in the following figure:
...
You can check the history of the **text moderation**, **image moderation**, **profanity filter**, and **domain filter** on <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link>. You can filter the moderation items by the time period, session type, or moderation result. You can also search for a specific item by the sender ID or receiver ID.

![history](https://web-cdn.agora.io/docs-files/1657017751717)

## User management

You can impose a penalty on users for repeated violations. The penalties can be applied as a global application setting, or only to a specific chat group or room. The following table lists all the user moderation options that <Vg k="CHAT"/> supports:
...
<table>
   ...
   <tr>
      ...
      <td>Users who are forced to go offline need to log in again to use the <Vg k="CHAT"/> service.</td>
   </tr>
   <tr>
    ...
    <tr>
      <td rowspan="4"><Vg k="CHAT"/> group management</td>
   ...
   <tr>
    ...
    <tr>
      <td rowspan="4"><Vg k="CHAT"/> room management</td>
...
1. To enter the **User Management** page, in the left navigation menu, click **Project Management** > **Config** for the project that you want > **Config** of <Vg k="CHAT"/> > **Operation Management** > **User**, as shown in the following figure:
...
1. To enter the <Vg k="CHAT"/> Group Management page, in the left navigation menu, click **Project Management** > **Config** for the project that you want > **Config** of <Vg k="CHAT"/> > **Operation Management** > **Group**, as shown in the following figure:
...
1. To enter the Chat Room Management page, in the left navigation menu, click **Project Management** > **Config** for the project that you want > **Config** of <Vg k="CHAT"/> > **Chat Room Management**, as shown in the following figure:
