...
Agora charges for the time each user spends in a channel with 3D Spatial Audio activated. For example, imagine there are 3 users in a channel, each one of them subscribing to remote audio. User A enables 3D Spatial Audio for 10 minutes. User B enables 3D Spatial Audio and has it on until leaving the channel. User C does not enable it. In this case, Agora charges for the combined time that users A and B had 3D Spatial Audio enabled, regardless of whether the remote audio was on mute at the time.
