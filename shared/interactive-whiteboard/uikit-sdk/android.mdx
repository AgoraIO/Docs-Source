...
Fastboard SDK does not support direct initialization of `Fastboard` objects. You need to add `FastboardView` to the layout of the app first, and then call the `getFastboard` method to obtain the `Fastboard` object.

**Notice**

Please make sure to call this method to obtain the `Fastboard` object before calling other APIs.
...
This method needs to be called after obtaining the `Fastboard` object.
...
- `US_SV`: Silicon Valley, USA, service area covers North America and South America.

- `SG`: Singapore, the service area covers Singapore, East Asia, and Southeast Asia.

- `IN_MUM`: Mumbai, India, service area covers India.

- `GB_LON`: London, UK, service area covers Europe.
