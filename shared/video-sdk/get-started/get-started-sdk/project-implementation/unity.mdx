...
1. **Create a new script to hold the system logic**
    ...
    1. In **Project**, open **Assets** > **Agora-RTC-Plugin** > **Agora-Unity-RTC-SDK** > **Code**.
...
3. **Manage Android permissions**
    ...
    1. Add the Unity Android library. In `Assets/Agora-RTC-Plugin/Agora-Unity-RTC-SDK/Code/JoinChannelVideo.cs`, add the following to the list of namespace declarations:
    ...
    2. Create a permissions list. In the `JoinChannelVideo` class, add the following lines before `Start`:
    ...
    3. Check permissions are granted. In the `JoinChannelVideo` class, add the following code after `Update`:
...
4. **Bind your script to the canvas**
    ...
    1. In `Assets/Agora-RTC-Plugin/Agora-Unity-RTC-SDK/Code`, drag and drop `JoinChannelVideo.cs` to **Canvas** on the **Hierarchy**.
...
1. **Declare the variables that you use to create and join a channel**
    ...
    Add the following declarations to the `JoinChannelVideo` class:
    <ProductWrapper product="video-calling">
    ``` csharp
    // Fill in your app ID.
...
1. **Reference the UI elements from SampleScene**
    ...
    In the `JoinChannelVideo` class, add the reference code before `SetupVideoSDKEngine`:
    <ProductWrapper product="video-calling">
    ``` csharp
...
1. **Setup <Vg k="ENGINE"/>**
    ...
    To call a method to initialize <Vg k="ENGINE"/> when the <Vpl k="CLIENT" /> is launched, in the `JoinChannelVideo` class, add the following after `CheckPermissions` method:
    <ProductWrapper product="video-calling">
    ``` csharp
    ...
    private void SetupVideoSDKEngine()
    {
        ...
        RtcEngine = Agora.Rtc.RtcEngine.CreateAgoraRtcEngine();
        // Specify the context configuration to initialize the created instance.
        RtcEngineContext context = new RtcEngineContext(_appID, 0, true, CHANNEL_PROFILE_TYPE.CHANNEL_PROFILE_LIVE_BROADCASTING,
        AUDIO_SCENARIO_TYPE.AUDIO_SCENARIO_DEFAULT);
...
1. **Handle and respond to <Vg k="ENGINE"/> events**
    ...
    To register the callbacks, add the following at the end of `JoinChannelVideo` class:
    ``` csharp
...
1. **Join a channel to start <Vpl k="NAME" />**
    ...
    When the local user clicks the `Join` button, you call `Join()`. This method securely connects the local user to a channel using the authentication token. In the `JoinChannelVideo` class, add the following lines before `InitEventHandler`:
    <ProductWrapper product="video-calling">
    ``` csharp
    public void Join()
    {
...
1. **View the remote user when they join a channel**
    ...
    When a remote user joins the channel, <Vg k="COMPANY"/> fires the `OnUserJoined` callback. To catch this callback and start remote video, in the `UserEventHandler` class, add the following code after the `OnJoinChannelSuccess` callback:
    <ProductWrapper product="video-calling">
    ``` csharp
...
8. **Manage the user role**
    ...
    When the user role switches, <Vg k="COMPANY"/> fires the `OnClientRoleChanged` callback. To catch this callback and enable or disable `LocalView`, `RemoteView` accordingly, add the following code after the `OnUserJoined` callback:
        ``` csharp
    ...
    When toggles are switched on, to switch client role and turn off the toggles, add the following code to the `JoinChannelVideo` class:
...
8. **Stop remote video when a remote user leaves the channel**
    ...
    When a remote user leaves the channel or drops offline, <Vg k="COMPANY"/>  fires the `OnUserOffline` callback. To catch this callback and stop remote video, add the following callback method to `UserEventHandler` class:
    ``` csharp
...
1. **Leave the channel when a user ends the call**
    ...
    When a user clicks **Leave**, you call `Leave()` to exit the channel. Add the following to the `JoinChannelVideo` class:
    ``` csharp
    public void Leave()
    {
        // Leaves the channel.
...
1. **Check that the <Vpl k="CLIENT" /> has the correct permissions to start**
    ...
    For Android, call `CheckPermissions` and check that the permissions are granted. To implement this worflow, in  `JoinChannelVideo` class, replace the `Update` method with the following code:
    ``` csharp
    void Update()
    {
        CheckPermissions();
...
2. **Start <Vpd k="PRODUCT" />**
    ...
    In the `JoinChannelVideo` class, replace the `Start` method with the following code:
    ``` csharp
    void Start()
    {
        SetupVideoSDKEngine();
...
3. **Clean up the resources used by your <Vpl k="CLIENT" />**
    ...
    When a user closes the <Vpl k="CLIENT" />, you call `OnApplicatiouit` to clean up the resources you created in `SetupVideoSDKEngine` method. To implement this logic, add the following code at the end of `JoinChannelVideo` class:
    ``` csharp
