...
2.  **Implement the logic to enable the extension**
    ...
    To load extension in your <Vpl k= "CLIENT"/>, call `loadExtensionProvider` and pass the extension path. To enable the extension, call `enableExtension` and pass the provider name and extension name. To implement this workflow, in `preload.js`, add the following method before `window.onload = () =>`:
...
2. **Implement the logic to disable the extension**
    ...
    To disable the extension, call `enableExtension` and pass the `provider`, `extension`, and `enableExtension` variables to the method. The `enableExtension` variable is set to `false` to disable the extension. To implment this logic, in `preload.js`, add the following method before `window.onload = () =>`:
