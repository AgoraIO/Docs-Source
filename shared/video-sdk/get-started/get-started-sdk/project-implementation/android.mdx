...
1. When a user closes the <Vpl k="CLIENT"/>, or switches the <Vpl k="CLIENT"/> to the background, call `stopPreview` to stop the video preview and then call `leaveChannel` to leave the channel and release all session-related resources.
