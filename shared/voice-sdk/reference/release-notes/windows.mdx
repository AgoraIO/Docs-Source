...
- When playing a multi-track media file, you might hear noise after calling the `setAudioPitch` method to adjust the pitch.
- After the host calls `createCustomAudioTrack` with `trackType` set to `AUDIO_TRACK_DIRECT`, pushes custom audio frames using `pushAudioFrame`, and plays audio effects with `playEffect`, audience members may hear noise.
- Apps integrated with the SDK may occasionally experience UI lag due to main thread blocking during audio and video interactions.
- The local preview of a shared screen may flicker after calling `startScreenCapture [2/2]`, enabling `enableHighLight` in `ScreenCaptureParameters`, placing the shared window on the top layer, and maximizing it.
- When using `startScreenCaptureByDisplayId` with `excludeWindowList` in `ScreenCaptureParameters`, some windows may fail to be excluded as expected.
- The app may crash after sharing video from an external screen and then disconnecting the screen.
- Calling `openWithMediaSource` and setting `isLiveSource` to `true` may fail to play the video stream.
- When sending multi-channel encoded audio, the receiver may occasionally hear noise.
- When the app integrates a media player and calls `open` twice to load different media resources in succession, the `onPlayerInfoUpdated [1/2]` callback may incorrectly return info for the first resource.
- After calling `enableAudioVolumeIndication`, the `rtcEngine:reportAudioVolumeIndicationOfSpeakers:totalVolume:` callback may return a local user volume of 0 for both local and remote users.
- In scenarios involving audio/video communication and screen sharing on a 21:9 ultra-wide display, setting a high resolution such as 3840×2160 could result in the shared screen image being cropped in both the local preview and on the receiver's display.
- When the app calls `enableVideoImageSource` to enable the video image source feature, the sender could sometimes stream successfully, but the `onVideoPublishStateChanged` callback did not return as expected.
- In multi-channel scenarios, if the app called `setupRemoteVideoEx` to initialize the remote user’s view before successfully calling `joinChannelEx`, the first frame of the remote video might be significantly delayed.
