...
1. **Pre-load sound effects**
    ...
    To setup playing sound effects, get the `agoraEngine` and pre-load the sound effects. In the `setupLocalVideo` function, add the following lines:
    ...
    @objc func playAudioEffectBtnClicked() {
        if (soundEffectStatus == 0) { // Stopped
            agoraEngine.playEffect(
                    ...
                    loopCount: 0,  // The number of sound effect loops. -1 means an infinite loop. 0 means play once.
...
1. **Update the UI after the effect finishes playing**
    ...
    When <Vg k="VSDK" /> has finished playing the sound effect, the `onAudioEffectFinished` event is fired. To handle this event, in `ViewController`, add the following function inside `extension ViewController: AgoraRtcEngineDelegate` along with the existing event handlers:
