...
```
### Plugin effect display

Start the classroom, and the effect after the plugin is mounted is as follows:

![](https://web-cdn.agora.io/docs-files/1664451212904)

### Quickly create a teaching aid plugin based on existing capabilities
...
Here is the `AgoraEduToolWidget` class definition:

```javascript
// AgoraEduToolWidget is the base class of teaching props Widget
...
The following shows how to create a teaching aid plugin for pre-class roll call based on the existing Widget capabilities of Smart Classroom. See the full source code in rollbook-widget .

### Using AgoraEduToolWidget
Create a `AgoraEduToolWidget` class the `RollbookWidget` class:
...
### Pass in a custom Widget in launch
Pass `launch` in a `widgets` custom Widget through the parameter in:
...
Start the classroom, you can find this plugin in the toolbox, as shown in the figure:
