<Vg k="COMPANY" /> utilizes a licensing system as a payment method for its IoT services. <Vg k="COMPANY" /> grants a device-specific license to bill for its services, with each device having its own unique license. During the licensed period, <Vg k="COMPANY" /> does not charge for the duration of the call. Typically, IoT device manufacturers predict the sales volume and usage of their devices, buy and activate licenses from <Vg k="COMPANY" />, and then embed the license into each device in the form of a single code.

## Understand the tech

The following figure shows the overall process of using an IoT device license:
...
Follow the steps below to add a license to your IoT devices.
...
After receiving the Agora license, refer to the information in this section to activate the license. You use the [IoT License Activation](https://documenter.getpostman.com/view/6319646/SVSLr9AM#c8160c19-a234-49fd-a333-7723b7ca53b2) RESTful API for activation.
