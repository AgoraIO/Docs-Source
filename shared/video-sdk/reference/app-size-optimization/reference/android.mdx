...
Agora provides a face recognition plugin since v4.1.1, that recognizes faces or portraits through face or virtual background segmentation algorithms, and uses higher-quality encoding for the ROI (Region of Interest) area during the encoding process to achieve face or portrait sharper effect. If you need to use this function, please contact [technical support](support@agora.io).
...
The screen sharing plugin provides the screen sharing function, and supports sharing your own screen content with the audience or other users to watch, so as to improve communication efficiency. See screen sharing for usage details .

> To use the screen plugin, you must integrate both `libagora_screen_capture_extension.so` and `AgoraScreenShareExtension.aar`.
...
Since v4.1.0, the SDK provides a video quality assessment Video Quality Assessment (VQA) plugin, which simulates the subjective feelings of real people to score video quality, and is an efficient video quality assessment tool. If you need to use this function, please contact [technical support](support@agora.io).
...
SoundNet has provided AI echo cancellation plugin since v4.1.0. This plugin can preserve the complete, clear and smooth near-end human voice under the condition of poor echo-to-signal ratio, and significantly improve the system's echo cancellation and The dual-speak performance brings users a more comfortable call and live broadcast experience, and is widely used in conferences, voice chats, karaoke and other scenarios. If you need to use this function, please contact technical support .
...
When integrating the Android SDK through Maven Central, you can modify the `dependencies` in the `/Gradle Scripts/build.gradle(Module: <projectname>.app)` file to specify the dynamic library that needs to be integrated. The relationship between each file and `implementation` is detailed in the table below:
