...
| `password` | String | The user's login password. The length cannot exceed 64 bytes. | Yes |
| `nickname` | String | The nickname of the user displayed in the notification bar of the message push when the message is pushed. The length cannot exceed 100 bytes. The default value is null. <br/>This field is used to set the user nickname displayed in the message push, not the user nickname in the user attributes. | Yes |
...
This method deletes multiple users in the app. If the deleted users include group or chat room admins, the groups and chat rooms managed by those users are also deleted.
...
| Parameter   | Description                                                       |
| :----- | :----------------------------------------------------------- |
| `msg_id` | The delivery state of an offline message, in the format of `"message id": "delivery state"`. The delivery state contains: <li> `delivered`: The offline message has been delivered to the user. </li><li> `undelivered`: The offline message is temporarily stored at the server and has not been pulled from the server and delivered to the user.</li> |
