...
This sections describes the steps required to use the relevant libraries, declare the necessary variables, and setup access to the UI
elements.
...
2.  **Setup the video frame observer**
    ...
    The `IVideoFrameObserver` gives you access to each local video frame after it is captured, and to each remote video frame before it is played back. In this example, your modify the captured video frame
    buffer to crop and scale the frame and play a zoomed-in version of the video. To setup the `IVideoFrameObserver`, add the following lines to the `MainActivity` class after the variable declarations:
...
3.  **Register the video and audio frame observers**
    ...
    To specify the format of audio frames captured by each `IAudioFrameObserver` callback, use the `setRecordingAudioFrameParameters`, `setMixedAudioFrameParameters` and `setPlaybackAudioFrameParameters` methods. To do this, add the following lines after `if (checkSelfPermission()) {` in the `joinChannel` method:
...
4.  **Unregister the video and audio observers when you leave a channel**
    ...
    'leaveChannel(View view)' method before
