...
2. **Add a Button to the UI**
    ...
    1. Add a text widget for the media button. Drag **Text** from the **Common** section of the **Palette Panel** and drop it on `mediaButton`.
...
3.  **Setup event listeners for the UI elements**
     
    To setup event listeners for the media player button and the progress bar, do the following:
    ...
    2. In `MyUserWidget.cpp`, add the following at the end of `setupVideoSDKEngine`:
        ...
        mediaProgressBar->OnValueChanged.AddDynamic(this, &UMyUserWidget::onProgressBarValueChanged);
        ```
...
2.  **Manage media player callbacks**
    ...
    1. In `MyUserWidget.h`, add the following class before `MediaPlayerEventHandler`:
    ...
    1. In `MyUserWidget.cpp`, add the following callback before `setupVideoSDKEngine`:
...
3.  **Configure <Vg k="ENGINE" /> to publish the media player stream**
    ...
    1. In `MyUserWidget.h`, add the following to  `UMyUserWidget` before `void setupVideoSDKEngine();`
...
4.  **Display media player output locally**
    ...
    To display the media file, you call `setupLocalVideo` and disable the local video capturer, setup a video canvas and set its `sourceType` property to `VIDEO_SOURCE_MEDIA_PLAYER`, then you call `setupLocalVideo` and pass the video canvas to preview the media stream. To switch between displaying media player output and the camera stream, take the following steps:
    ...
    2. In `MyUserWidget.cpp`, add the following after `setupLocalVideo`:
