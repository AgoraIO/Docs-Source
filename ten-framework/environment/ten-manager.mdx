...
description: >
  Use TEN Manager to create new TEN packages and install packages from the Cloud store.
---

In the TEN ecosystem, almost everything is a component. For example, a TEN app is a type of component, a TEN extension is another, and a TEN protocol is yet another. To help developers manage operations related to these components, the ecosystem provides a tool called TEN Manager.

TEN Manager includes a TEN package manager, which simplifies the management of TEN packages. It handles tasks such as determining required dependency packages, fetching dependencies, and installing them. In short, TEN Manager streamlines the process of creating new TEN packages from scratch and simplifies the installation of packages from the TEN Cloud Store.
...
`tman` provides the `check graph` command to validate predefined graphs or the configuration of a `start_graph` command for correctness. To see usage details, use the following command:
