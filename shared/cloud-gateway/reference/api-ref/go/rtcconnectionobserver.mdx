...
|`conInfo`|Connection information. See [`RtcConnectionInfo`](./rtcconnection#RtcConnectionInfo) for details.|
...
Triggered when an error occurs in receiving stream messages. This callback indicates that the local user did not receive the stream message sent by the remote user using the [`send_stream_message`](./rtcconnection#sendstreammessage) method.
