...
This article shows how to create a simple Maven project, integrate the Cloud Gateway Java SDK, and run the app.
...
2. Navigate to `my-app/src/main/java/com/mycompany/app/App.java` and enter the following code. Replace `Your Token` and `Your App ID` with your RTC temporary token and App ID.

    <div class="alert warning">The channel name you use to connect to the RTC channel must be the same as the one you use to generate your RTC temporary token. In the following code, the channel name is set to "test_channel". If you use a different channel name to generate your RTC temporary token, replace "test_channel" with your own channel name.</div>
...
3. Navigate to `my-app`. Create a `lib` folder, get the linux-sdk jar file from maven, and extract the files.
