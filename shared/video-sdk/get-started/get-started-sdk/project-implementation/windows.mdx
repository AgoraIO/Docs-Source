...
2. **Add windows to the UI for the local and remote view**
    ...
    1. In **Solution Explorer**, open `AgoraImplementationDlg.h` and add the following declarations to `CAgoraImplementationDlg`:
    ...
    1. In **Solution Explorer**, open `AgoraImplementationDlg.cpp`, replace `OnInitDialog()` with the following code:
