...
* Transcribe speech in real time, store the text in `WebVTT` format, and upload the file to third-party cloud storage.
...
The following figure shows the workflow to integrate Real-Time Transcription functionality into your <Vpl k="CLIENT" />:

![Speech to text](/images/video-sdk/real-time-transcription.svg)
...
* To record and store Real-Time Transcription videos and texts, activated a [supported cloud storage service](#supported-third-party-cloud-storage-services)
...
### Real-Time Transcription REST API

#### Acquire a `builderToken`

A `builderToken` secures your Real-Time Transcription tasks. Call this method to generate a `builderToken` before starting a transcription task.
...
#### Start a Real-Time Transcription task
...
`builderToken`: The dynamic key obtained through the generate a `builderToken` method. Used to ensure the security of Real-Time Transcription tasks.
...
| `audio.subscribeSource` | String | (Required) The source type of the audio input.Supports the audio stream published in the channel only. The type is `AGORARTC`. |
| `audio.agoraRtcConfig` | JSON Object | (Required) Information required to enter the Agora RTC channel. |
| `audio.agoraRtcConfig.channelName` | String | (Required) The channel name. |
| `audio.agoraRtcConfig.uid` | String | (Required) The user ID. Every user ID in the channel must be unique. |
| `audio.agoraRtcConfig.token` | String | (Optional) The token required to enter the channel. Used to ensure channel security. |
| `audio.agoraRtcConfig.channelType` | String | (Required) The channel profile. For a Real-Time Transcription task, you need to set the channel profile to live streaming (LIVE_TYPE). |
...
| `config.features` | JSON Array | (Required) The service type. Only Real-Time Transcription (RECOGNIZE) is supported. |
| `config.recognizeConfig` | JSON Object | (Required) The Real-Time Transcription conversion configuration. |

**Subscription configuration**

| Field | Type | Description |
|:------|:----|:----------------|
| `subscribeConfig.subscribeMode` | String | (Required) The subscription mode. To use Real-Time Transcription, you need to set this field to `CHANNEL_MODE`. |

**Conversion configuration**
| Field | Type | Description |
|:------|:----|:----------------|
| `recognizeConfig.language` | String | (Required) The conversion language. Because Real-Time Transcription currently only supports English as conversion language, you need to set this field to `ENG`. |
| `recognizeConfig.model` | String | (Required) The conversion mode. To use Real-Time Transcription, you need to set this field to `Model`. |
| `recognizeConfig.output` | JSON Object | (Required) The output configuration. |
| `recognizeConfig.output.destinations` | JSON Array | (Required) The target channel type for the output streams. Set this field as `AgoraRTCDataStream` to push Real-Time Transcription conversion output to RTC channel. If you need to store Real-Time Transcription output text in WebVTT format to third-party cloud storage, you also need to add `Storage` to the field. |
| `recognizeConfig.output.agoraRTCDataStream` | JSON Object | (Required) The target channel configuration. | 
| `recognizeConfig.output.cloudStorage` | JSON Array | (Optional) The third-party cloud storage configuration. This field must be set to store Real-Time Transcription output text and subtitle file. |
...
| `agoraRTCDataStream.uid` | String | (Required)The user ID in target channel. |
| `agoraRTCDataStream.token` | String | (Required) The token required to enter the channel. Used to ensure channel security. |
...
| `cloudStorage.storageConfig` | JSON Object | (Required) The third-party cloud storage information configuration, for storing Real-Time Transcription output text and subtitle file. |

**`cloudStorage.storageConfig`**

When the Real-Time Transcription process ends, an output text file (txt format) and a subtitle file (WebVTT format) are generated. This field is used to store the two files.
...
| Field | Type | Description |
|:------|:----|:----------------|
| `taskId` | String | The task ID, a UUID (Universal Unique Identifier) generated by the Agora server to identify a Real-Time Transcription task that has been created. |
| `createTs` | Number | The Unix timestamp (seconds) when the task was created. |
| `status` | String | The running status of tasks:<ul><li>`IDLE`: The task has not started or has ended.</li><li>`PREPARING`: The task has received a start request.</li><li>`STARTED`: The task has started.</li><li>`IN_PROGRESS`: The task is in progress.</li><li>`STOPPING`: The task is stopping.</li><li>`STOPPED`: The task has been stopped.</li><li>`RECONNECTING`: The task is being reestablished.</li></ul> |
...
```
#### Query the status of Real-Time Transcription
...
* `taskId`: (Required) String. The task ID generated by calling the start method. A task ID is a UUID (Universal Unique Identifier) generated by the Agora server to identify a Real-Time Transcription task that has been created.
