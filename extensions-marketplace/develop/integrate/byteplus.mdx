...
The BytePlus Effects extension encapsulates the core APIs of the BytePlus Effects SDK. By calling the <Link to="{{Global.API_REF_ANDROID_ROOT}}/class_irtcengine.html#api_irtcengine_setextensionproperty">setExtensionProperty</Link> or <Link to="{{Global.API_REF_IOS_ROOT}}/class_irtcengine.html#api_irtcengine_setextensionproperty">setExtensionPropertyWithVendor</Link> method of the <Link to="{{Global.API_REF_ANDROID_ROOT}}/rtc_api_overview.html">Agora Video SDK v4.0.0</Link> and passing in the corresponding `key` and `value`, you can quickly integrate BytePlus's capabilities.
...
The BytePlus Effects extension works together with the <Link to="{{Global.AGORA_PREPROD_DOCS_URL}}/video-call-4.x/product_video_ng?platform=Android">Agora Video SDK v4.x</Link>. Refer to the following docs to integrate the SDK and start a basic video call:

<PlatformWrapper platform="android">
- <Link to="{{Global.AGORA_PREPROD_DOCS_URL}}/video-call-4.x/start_call_android_ng?platform=Android#project-setup">Start a Video Call (Android)</Link>
</PlatformWrapper>
<PlatformWrapper platform="ios">
- <Link to="{{Global.AGORA_PREPROD_DOCS_URL}}/video-call-4.x/start_call_ios_ng?platform=iOS#project-setup">Start a Video Call (iOS)</Link>
</PlatformWrapper>
...
<PlatformWrapper platform="android">
1. On <Link to="{{Global.AGORA_CONSOLE_URL}}/marketplace/list/all">Agora Console</Link>, download the Android package of **BytePlus Effects**.
...
└── Resource
```
2. On <Link to="{{Global.AGORA_CONSOLE_URL}}/marketplace/list/all">Agora Console</Link>, download the iOS package of **BytePlus Effects**.
...
This section describes the call sequence of using the extension. For a detailed parameter description, see the <a href="byteplus#api-ref">API Reference</a>.
...
You can call the method as needed. For a full list of keys and values, see the [API Reference](byteplus#key-value).
