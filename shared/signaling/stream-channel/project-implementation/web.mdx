...
To use the stream channel functionality, call `createStreamChannel` to create a `RTMStreamChannel` object instance.
...
Call the `join` method on the `RTMStreamChannel` instance with the appropriate options as follows:
