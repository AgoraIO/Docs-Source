...
1. **Import the required Android and Agora libraries**
    ...
    To set the `TextView` color, and integrate <Vg k="VOICE_SDK" /> channel quality libraries, add the following statements after the last `import` statement in `/app/java/com.example.<projectname>/MainActivity`.
