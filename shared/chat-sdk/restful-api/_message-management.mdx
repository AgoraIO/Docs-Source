...
This page shows how to call <Vg k="CHAT"/> RESTful APIs to send different types of messages, upload and download files, and retrieve historical messages.
...
| Parameter | Type | Description | Required |
| --- | --- | --- | --- |
| `from` | String | The username of the message sender. If you do not set this field, the <Vg k="CHAT"/> server takes the `admin` as the sender. If you set it as the empty string "", this request fails.  | No |
...
| Parameter | Type | Description | Required |
| --- | --- | --- | --- |
| `to` | Array | An array of the group IDs that receives the message. Within one second, you can send a maximum of 20 messages to a <Vg k="CHAT_GROUP_LOWER"/>, and for each request, you can send messages to a maximum of 3 <Vg k="CHAT_GROUP_LOWER"/>s. | Yes |
...
| Parameter      | Type   | Description |
| :------- |:-------------|:-------------|
| `data` | JSON | The detailed content of the response. The value of this parameter includes a key-value pair where key represents the group ID that receives the message and value the message ID. For example, if the returned data is `"184524748161025": "1029544257947437432"`, it means that a message with the ID of 1029544257947437432 is sent in <Vg k="CHAT_GROUP_LOWER"/> 184524748161025.  | 
...
- Send a text message to all members in a <Vg k="CHAT_GROUP_LOWER"/> whether they are online or not
...
- Send a text message to online users in a <Vg k="CHAT_GROUP_LOWER"/> by setting `routetype` to `ROUTE_ONLINE`.
...
### Send a <Vg k="CHAT_ROOM_LOWER"/> message

This RESTful API allows you to send a maximum of 100 messages to 10 <Vg k="CHAT_ROOM_LOWER"/>s in total on each call. Assuming that you send two messages to 10 <Vg k="CHAT_ROOM_LOWER"/>s, the server counts it as 20 messages.

You can set the priority of a certain message type or <Vg k="CHAT_ROOM_LOWER"/> member to high, normal, or low. In this case, when the server is overloaded, low-priority messages
...
| Parameter | Type | Description | Required |
| --- | --- | --- | --- |
| `to` | Array | An array of the <Vg k="CHAT_ROOM_LOWER"/> IDs that receives the message. Within one second, you can send messages to a maximum of 100 <Vg k="CHAT_ROOM_LOWER"/>s, and for each request, you can send messages to a maximum of 10 <Vg k="CHAT_ROOM_LOWER"/>s. | Yes |
| `chatroom_msg_level` | String | The <Vg k="CHAT_ROOM_LOWER"/> message priority: <ul><li>`high`</li><li>(Default) `normal`</li><li>`low`</li></ul> | No       |
...
| Parameter      | Type   | Description |
| :------- |:-------------|:-------------|
| `data` | JSON | The detailed content of the response. The value of this parameter includes a key-value pair where key represents the <Vg k="CHAT_ROOM_LOWER"/> ID that receives the message and value the message ID. For example, if the returned data is `"185145305923585": "1029545553039460728"`, it means that a message with the ID of 1029545553039460728 is sent in <Vg k="CHAT_ROOM_LOWER"/> 185145305923585.  | 
...
- Send a text message to members in a <Vg k="CHAT_ROOM_LOWER"/>
...
- Images: After uploading an image, the <Vg k="COMPANY"/> server automatically generates the thumbnail of the image.
- Videos: The <Vg k="COMPANY"/> server does not generate thumbnails for videos automatically. After uploading a video, you must recall this method to upload the thumbnail for the video yourself.
...
| :---------------------- | :----- | :------------------------------------------------------------------------------------------------------------------- |
| `entities.uuid` | String | The file ID, a unique ID assigned to the file by the <Vg k="CHAT"/> service. <br/>You need to save this `uuid` yourself, and provide it when calling the [Send-file-messages](#sendmessage) method. |
...
When uploading an image or video file, the <Vg k="CHAT"/> server can create a thumbnail for the file. This method has an extra `thumbnail` field in the request header compared with [downloading a file](#download).
...
| `to` | String | The message recipient.<ul><li>For a <Vg k="CHAT_ONE_LOWER"/>, this parameter indicates the peer user that receives the message.</li><li>For a group chat, this parameter indicates the <Vg k="CHAT_GROUP_LOWER"/> ID.</li></ul>|
| `chat_type` | String | The chat type:<ul><li>`chat`: <Vg k="CHAT_ONE"/>.</li><li>`groupchat`: <Vg k="CHAT_GROUP"/>.</li><li>`chatroom`: <Vg k="CHAT_ROOM"/>.</li></ul> |
...
The default time limit for recalling a message is two minutes. You can extend this time frame to up to 7 days in <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link>. To do so, select a project that enables <Vg k="COMPANY"/> <Vg k="CHAT"/>, then click **Configure** > **Features** > **Message recall**.
...
| `to`        | String   | Yes   | The user, <Vg k="CHAT_GROUP_LOWER"/>, or <Vg k="CHAT_ROOM_LOWER"/> that receives the message to recall. You can specify a user ID, a <Vg k="CHAT_GROUP_LOWER"/> ID, or a <Vg k="CHAT_ROOM_LOWER"/> ID. **Note**: If the message to recall no longer exists on the server, only the message on the recipient client is recalled. |
| `chat_type` | String   | Yes     | The type of the chat where the message to recall is sent. <ul><li>`chat`: An one-on-one chat.</li><li>`groupchat`: A <Vg k="CHAT_GROUP_LOWER"/>.</li><li>`chatroom`: A <Vg k="CHAT_ROOM_LOWER"/>.</li></ul> |
...
| `to`      | String  | The user, <Vg k="CHAT_GROUP_LOWER"/>, or <Vg k="CHAT_ROOM_LOWER"/> that receives the recalled message.                      |
| `chattype` | String | The type of the chat where the recalled message is located. <ul><li>`chat`: An one-on-one chat.</li><li>`group_chat`: A <Vg k="CHAT_GROUP_LOWER"/>.</li><li>`chatroom`: A <Vg k="CHAT_ROOM_LOWER"/>.</li></ul> |
...
## Import a <Vg k="CHAT_ONE_LOWER"/> message
...
## Import a <Vg k="CHAT_GROUP_LOWER"/> message

This method imports a <Vg k="CHAT_GROUP_LOWER"/> message. You can import only one message each time.
...
| --- | --- | --- | --- |
| `from` | String | The username of the message sender.   | Yes |
| `target` | String | The <Vg k="CHAT_GROUP_LOWER"/> ID that receives the message. | Yes |
| `type` | String | The message type: <ul><li>`txt`: Text message</li><li>`img`: Image message</li><li>`audio`: Voice message</li><li>`video`: Video message</li><li>`file`: File message</li><li>`loc`: Location message</li><li>`cmd`: Command message</li><li>`custom`: Custom message</li></ul> | Yes |
| `body` | JSON | The message content. For different message types, this parameter contains different fields. For details, see [Body of different message types](#body). | Yes |
| `ext` | JSON | The message extension field that allows you to add custom information in the format of key-value pairs. | No |
| `is_ack_read` | Bool | Whether to set the message as read. <ul><li>`true`: Yes.</li><li>`false`: No.</li></ul> | No |
