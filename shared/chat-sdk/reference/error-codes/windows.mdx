...
| 100    | `INVALID_APP_KEY`                 | The App Key is invalid. For how to get the App Key, see [Get the information of the <Vg k="COMPANY"/> Chat project](https://docs.agora.io/en/agora-chat/get-started/enable?platform=android#get-chat-project-information).  |
...
| 217    | `USER_KICKED_BY_OTHER_DEVICE`     | In a multi-device login scenario, the user gets kicked off from a device in on the <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link> or by calling an API on another device. |
