...
1. **Add the UI elements you need**
    ...
    ```swift
    var speechToTextButton: NSButton!
    var querySpeechToTextButton: NSButton!
    ```
...
1. **Configure the UI elements in your interface**
    ...
    speechToTextButton = NSButton()
    speechToTextButton.frame = CGRect(x: 230, y: 240, width: 80, height: 20)
    speechToTextButton.title = "Start speech to text"
    speechToTextButton.target = self

    speechToTextButton.action = #selector(speechToText)
    self.view.addSubview(speechToTextButton)

    // Button to query the status of the speech to text task
    querySpeechToTextButton = NSButton()
    querySpeechToTextButton.frame = CGRect(x: 310, y: 240, width: 80, height: 20)
    querySpeechToTextButton.title = "Query speech to text"
    querySpeechToTextButton.target = self

    querySpeechToTextButton.action = #selector(query)
            ...
            if let responseDict = responseJSON as? [String: Any] {
                ...
                if let status = responseDict["status"] as? String, let taskId = responseDict["taskId"] as? String {
                    ...
                    if status == "STARTED" || status == "IN_PROGRESS" {
                        ...
                        DispatchQueue.main.async {
                            self.speechToTextButton.title = "Stop speech to text"
                        }
                    } else {
            ...
            if let responseDict = responseJSON as? [String: Any] {
                ...
                if let status = responseDict["status"] as? String, let taskId = responseDict["taskId"] as? String {
                    ...
                    if status == "IN_PROGRESS" {
                        ...
                        DispatchQueue.main.async {
                            self.speechToTextButton.title = "Start speech to text"
                        }
                    }
                } else {
                    self.showMessage(title: "Error", text: "Speech to text status and/or taskId not received: \(responseDict["message"]!)", delay: 0)
...
1. **Enable a user to start, query, and stop a task**
    ...
    In this example, you use two buttons to manage a speech-to-text task.
        ...
        @objc func speechToText(sender: NSButton!) {
