...
You provide the Signaling authentication token when calling `login`. During the testing phase, you can simplify the process by setting the authentication mode to Debug when creating the Agora project in the <Vg k="CONSOLE" />. This allows you to use the project's App ID instead of the token for logging in.
</Admonition>
