import videoURL from '@docs/assets/images/rtc-sdk/enable-notifications.mp4';
...
1. Log in to <Link to="{{Global.AGORA_CONSOLE_URL}}"><Vg k="CONSOLE" /></Link>. On the **Projects** tab, locate the project for which you want to enable <Vg k="NCS_LONG" /> and click **Edit**.
    ...
    ![Project tab](/images/rtc-sdk/enable-ncs-project-tabs.png)
...
1. In the **All Features** section, open the **Notifications** tab and click on the service for which you want to enable notifications. The section expands to show configuration options.
    ...
    ![Notification tab](/images/rtc-sdk/enable-ncs-notification-tab.png)
...
1. Fill in the following information:
    ...
    * **Whitelist**: If your server is behind a firewall, check the box here, and ensure that you call the [IP address query API](#ip-address-query-api) to get the IP addresses of the <Vg k="COMPANY" /> <Vg k="NCS_LONG" /> server and add them to the firewall's allowed IP list.
    ...
    ![Notification tab](/images/rtc-sdk/enable-ncs-configuration-tab.png)
...
1. Press **Check**. Agora performs a health test for your configuration as follows:
    ...
    1. When the <Vg k="NCS_LONG" /> health test succeeds, read the prompt and press **Apply Settings**. After your configuration is saved, the **Status** of <Vg k="NCS_LONG" /> shows **Enabled**.
        ...
        ![Apply Settings](/images/rtc-sdk/enable-ncs-apply-settings.png)
