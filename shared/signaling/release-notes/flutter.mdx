...
- **SDK connection state change reason**  
  Added the [`RtmLinkStateChangeReason`](/signaling/reference/api#enumvlinkstatereasonpropsag_platform) enumeration to the `LinkStateEvent`, reporting the reason for connection state changes.

- **IoT device domain whitelist**  
  Added the `ispPolicyEnabled` field to [`RtmConfig`](/signaling/reference/api#configrtmpropsag_platform). When set to `true`, the SDK connects only to servers in the domain or IP whitelist registered with carriers, helping IoT devices bypass ISP restrictions.
...
- Other improvements
    ...
    - Increased <Vg k="RTM " /> service recovery and reconnection speed after backgrounding or hibernation.
...
- On iOS, exiting the app sometimes caused <Vg k="RTM " /> SDK crashes
