...
If the number of online users in the channel is less than the **Max number of instant event** value, presence event notifications operate in real-time notification mode. In this mode, `remoteJoinChannel`, `remoteLeaveChannel`, `remoteTimeout`, and `remoteStateChanged` notifications are sent to the client in real-time. 
