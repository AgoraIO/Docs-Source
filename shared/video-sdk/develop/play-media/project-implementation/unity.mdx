...
4.  **Display media player output locally**
    ...
    Access the `LocalView` game object from the UI and update its video surface each time when you play and pasue the media player. To switch between displaying media player output and the camera stream, call `SetForUser` and pass the video source you want to display. To implement this logic, add the following method in the `JoinChannelVideo` class:
    ...
    private void setupLocalVideo(bool forMediaPlayer) {
        if (forMediaPlayer) {
          ...
          LocalView.SetForUser(Convert.ToUInt32(mediaPlayer.GetId()), "", VIDEO_SOURCE_TYPE.VIDEO_SOURCE_MEDIA_PLAYER);
...
6.  **Clean up when you close the <Vpl k="CLIENT" />**
    ...
    To free up resources when you exit the <Vpl k="CLIENT" />, add the following lines at the end of `OnApplicationQuit` method:
