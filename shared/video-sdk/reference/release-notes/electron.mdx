...
3. **Dual-stream mode optimization**
   ...
   This release optimizes the video dual-stream mode. In the low-quality video stream adaptive mode (`AutoSimulcastStream`), when the receiver requests a low-quality video stream by calling <Link to="{{Global.API_REF_ELECTRON_ROOT}}/class_irtcengine.html#api_irtcengine_setremotevideostreamtype">setRemoteVideoStreamType</Link> or <Link to="{{Global.API_REF_ELECTRON_ROOT}}/class_irtcengineex.html#api_irtcengineex_setremotevideostreamtypeex">setRemoteVideoStreamTypeEx</Link>, the sender now processes requests from all users, expanding beyond the previous restriction to broadcasters only. This change ensures that the audience requests for low-quality video streams are implemented, thus improving the flexibility and adaptability of the dual-stream mode.
