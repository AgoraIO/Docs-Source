...
The file-conversion feature provided by Agora Interactive Whiteboard can convert PPT, PPTX, DOC, DOCX, and PDF files into static images or dynamic HTML web pages. The generated images and web pages can be presented on the whiteboard. See [Old File Conversion Overview](/en/whiteboard/file_conversion_overview_old?platform=RESTful).

<div class="alert note">Before calling the RESTful API for file conversion, ensure that:
<ul><li>You have enabled <b>Docs to Picture</b> or <b>Docs to Web</b> and configured storage settings in <a href="https://console.agora.io/">Agora Console</a >. See <a href="https://docs.agora.io/en/whiteboard/enable_whiteboard?platform=RESTful&versionId=4cb08090-b1fd-11eb-b31a-57565fd331e4#enable-whiteboard-server-side-features">Enable whiteboard server-side features</a >.</li><li>You have generated a URL address for the file you want to convert, and the address is publicly accessible.</li></ul></div>
...
| :-------- | :-------- | :---------------- | :----------------------------------------------------------- |
| `token`   | string    | Required          | A SDK token with the `writer` or `admin` role. <br/>To get a SDK Token, you can:<li>Get a test-purpose SDK token from Agora Console. See [Get security credentials for your whiteboard project](/en/whiteboard/enable_whiteboard?platform=RESTful&versionId=4cb08090-b1fd-11eb-b31a-57565fd331e4#get-security-credentials-for-your-whiteboard-project).</li><li>Call the RESTful API. See [Generate an SDK Token](/en/whiteboard/generate_whiteboard_token?platform=RESTful#generate-an-sdk-token-post).</li><li>Use code samples. See [Generate a token from your app server](/en/whiteboard/generate_whiteboard_token_at_app_server?platform=RESTful).</li> |
...
For details about all possible response status codes, see the [status code table](/en/whiteboard/basic_info?platform=RESTful#status-codes).
...
| Parameter | Data type | Required/Optional | Description                                                  |
| :-------- | :-------- | :---------------- | :----------------------------------------------------------- |
| `token`   | string    | Required          | A Task Token. To get one, you can:<li>Call the RESTful API. See [Generate a Task Token](/en/whiteboard/generate_whiteboard_token?platform=RESTful#generate-a-task-token-post).</li><li>Use code samples. See [Generate a token from your app server](/en/whiteboard/generate_whiteboard_token_at_app_server?platform=RESTful).</li> |
| `region`  | string    | Required             | Specifies a data center to process the request: <li>`us-sv`: Silicon Valley, US, which provides services to North America and South America.</li><li>`sg`: Singapore, which provides services to Singapore, East Asia, and Southeast Asia.</li><li>`in-mum`: Mumbai, India, which provides services to India.</li><li>`gb-lon`: London, England, which provides services to Europe.</li><li>`cn-hz`: Hangzhou, China, which provides services to the areas not covered by other data centers.</li> |
