...
1.  **Declare the variables that you use to join a channel and stream <Vg k="AV"/>**
    ...
    In `/app/java/com.example.<projectname>/MainActivity`, add the following lines to `MainActivity` class:
        ...
        private int uid = 0; // An integer that identifies the local user.
        private AgoraRtcService agoraRtcService;
        private String deviceId = "MyDev01";
        private boolean isJoined = false;
        private String rtcLicense = "";
...
1.  **Setup <Vg k="COMPANY" /> RTC Service**
    ...
    To use <Vpd k="PRODUCT" /> features, you use the <Vpd k="SDK" /> to create an RTC service instance. You then use the service instance to open a connection. In `/app/java/com.example.<projectname>/MainActivity`, add the following code before the `onCreate` method:
...
1.  **At startup, ensure necessary permissions and start the service**
    ...
    In `/app/java/com.example.<projectname>/MainActivity`, replace `onCreate` with the following code:
        ...
        protected void onCreate(Bundle savedInstanceState) {
            ...
            // If all the permissions are granted, initialize the RTC Service.
...
1.  **Handle and respond to service events**
    ...
    The RTC service provides a number of callbacks that allow you to handle and respond to key events. To add a service event handler, in `/app/java/com.example.<projectname>/MainActivity`, add the following lines before `setupAgoraRtcService`:
        ...
        private final AgoraRtcEvents agoraRtcEvents = new AgoraRtcEvents() {
            ...
            public void onJoinChannelSuccess(int connId, int uid, int elapsed_ms) {
                ...
                videoThread = new VideoSendThread(getApplicationContext(), agoraRtcService, channelName, connectionId);
                // Create a thread to send audio frames
                audioThread = new AudioSendThread(getApplicationContext(), agoraRtcService, channelName, connectionId,
                        16000, 1, 2);
...
You stream audio and video data in a separate thread. The sending interval should be consistent with the video frame rate and audio frame length.

To stream audio and video data:

1. **Add a `StreamFile` class to manage media files**

    You use the `StreamFile` class to open, read, and close audio and video files. In Android Studio, select **File > New > Java Class**. Name the class `StreamFile` and replace the contents of the file with the following code:

        ```java
        package <com.example.your app name>; // Should be same as your MainActivity package name
...
1. **Add a class to run an audio thread**
