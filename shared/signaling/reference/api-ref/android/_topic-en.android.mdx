...
<Admonition type="info" title="Information">Before joining a topic, a user needs to create the <code>{channel.istreamchannel[props.ag_platform]}</code> instance and call the <code>{channel.join[props.ag_platform]}</code> method to join the channel.</Admonition>
...
| :-------------: | :------------------------------------------------------: | :------: | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `messageQos` | <code>{enumv.messageqos[props.ag_platform]}</code> | Optional | <code>{enumv.unordered[props.ag_platform]}</code> | Whether the data transmitted in the topic is ordered. See <a href="../enumv#messageqos"><code>{enumv.messageqos[props.ag_platform]}</code></a>. |
| `priority` | <code>{enumv.priority[props.ag_platform]}</code> | Optional | <code>{enumv.normal[props.ag_platform]}</code> | The priority of data transmission in the topic compared to other topics in the same channel. See <a href="../enumv#messagepriority"><code>{enumv.priority[props.ag_platform]}</code></a>. |
...
You can call the <code>{topic.publish[props.ag_platform]}</code>[1/2] and <code>{topic.publish[props.ag_platform]}</code>[2/2] method in the following way:
...
| `resultCallback` | `ResultCallback<SubscribeTopicResult>` | Required | - | Callback of invocation result:<li>Success: Executes the `onSuccess` method and return the <code>SubscribeTopicResult</code> data.</li><li>Failure: Executes the `onFailure` method.</li> |
