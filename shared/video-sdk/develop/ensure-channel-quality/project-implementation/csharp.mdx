...
1. **Implement best practice for app initiation**
     ...
     * _Set the video profile_: Setting a video profile is also optional. It is useful when you want to change one or more of `mirrorMode`, `frameRate`, `bitrate`, `dimensions`, `orientationMode`, `degradationPreference`, `degradationPrefer` or `compressionPrefer` from default setting to custom values.
     * _Start the network probe test_: A quick test at startup to gauge network quality.

    To implement these features, in your script file, add the following code to `setupVideoSDKEngine` after `RtcEngine.SetLogFileSize(256);`:
     ...
     // Use the AdvanceOptions class to set compression preference to low latency or quality
     ...
     videoConfig.advanceOptions = advanceOptions;     // Apply the configuration
