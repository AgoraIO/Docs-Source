...
**The RESTful API for <Vg k="MPUSH" />** enables you to inject an online media stream into the Agora channel as a live video source. After the injection succeeds, the online media stream automatically plays in the Agora channel, and remote users can see the content of the media stream.

## Working principle

Injecting an online media stream into the Agora channel creates a <Vg k="MPUSH" /> task (shown as "cloud player" in the rest of the article) in the Agora channel. You can control the cloud player through the RESTful API and accomplish the following functions for <Vg k="MPUSH" />:
...
<Vg k="MPUSH" /> supports the following formats:
...
## Considerations
This section summarizes some important considerations when using the <Vg k="MPUSH" /> RESTful API.
...
| The cloud player may be deleted automatically due to server errors. Agora recommends that you enable the Message Notification Service to listen for events of <Vg k="MPUSH" />. For details, see the [<Vg k="MPUSH" /> Message Notification Server](message-notification-service). | ★★★★★  |
...
| The default maximum number of concurrent tasks is 50, which means that a maximum of 50 <Vg k="MPUSH" /> tasks are allowed to run simultaneously for each project. You can contact [sales@agora.io](mailto:sales@agora.io) to get a higher quota. | ★★★★☆  |