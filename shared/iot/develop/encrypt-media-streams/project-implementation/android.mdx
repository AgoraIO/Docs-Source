...
1. **Add variables to hold encryption key and salt**
...
1. **Set encryption parameters**
    ...
    You specify the media stream encryption method and the related parameters in JSON format by calling `setParams`. Depending on your choice of an encryption method, add **one** of the following pieces of code to `joinChannel` before `agoraRtcService.joinChannel(...)`:

    * SM4-128-ECB encryption

        ```java
        // Enable SM4-128-ECB encryption
        agoraRtcService.setParams("{\"rtc.encryption\": {\"enable\": true,"
...
1. **Disable encryption**
    ...
    agoraRtcService.setParams("{\"rtc.encryption\": {\"enable\": false}}");
    ```
